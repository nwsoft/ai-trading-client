...
class StrategySourceIngestor:
    ...
    def __init__(self, ai_client: Optional[OpenAIClient] = None, logger: Optional[logging.Logger] = None):
    ...
    def _extract_youtube(self, url: str) -> ExtractedStrategySource:
        ...
        if not video_id:
            raise ValueError("YouTube 영상 ID를 확인할 수 없습니다.")
        warnings: List[str] = []
        transcript_text = ""
        screen_text = ""
        ...
        try:
            ...
            try:
                ...
                transcript_text = " ".join(str(item.text) for item in transcript)
            except AttributeError:
                transcript = YouTubeTranscriptApi.get_transcript(video_id, languages=["ko", "en"])
                transcript_text = " ".join(str(item.get("text", "")) for item in transcript)
        except Exception as exc:
            warnings.append(f"자막을 가져오지 못했습니다: {exc}")
        ...
        try:
            ...
            response = requests.get(
                ...
                params={"url": url, "format": "json"}, timeout=12,
                headers={"User-Agent": "NoahAI/3.8.9.29"},
            )
        ...
        try:
            ...
            with YoutubeDL({"quiet": True, "no_warnings": True, "skip_download": True, "format": "worst[ext=mp4]/worst"}) as ydl:
            ...
            warnings.append(f"영상 화면 OCR을 수행하지 못했습니다: {exc}")
        if not screen_text:
    ...
    def _extract_web(self, url: str, *, tradingview: bool) -> ExtractedStrategySource:
        ...
        response = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0 NoahAI/3.8.9.29"})
        response.raise_for_status()
        body = response.text
    ...
    def analyze(self, value: str, kind: str = "auto") -> Dict[str, Any]:
        ...
        if self.ai_client and self.ai_client.is_ready() and source.text.strip():
            system_prompt = (
