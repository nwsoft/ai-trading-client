...
def order_fills(adapter, symbol, order_id, epoch):
    ...
    if 'kiwoom' in broker:
        ...
        if getattr(getattr(adapter,'kiwoom',None),'tr_remained',False):
        ...
        data = adapter._get(config['path'],params={'accNo':adapter.account_no,config['date_parameter']:day})
        items = data.get(config.get('rows_key','trades'))
        parse = adapter._parse_order
        if data.get('next') or data.get('next_key'): raise RuntimeError('history_page_incomplete')
    else:
        if 'korea' not in broker and broker != 'kis':
            ...
            data = adapter._get(config['path'],params={config['date_parameter']:day,'CANO':adapter.account_no[:8]})
            items = data.get(config.get('rows_key','output1'))
        else:
            data = adapter._get('/uapi/domestic-stock/v1/trading/inquire-daily-ccld',params={
                ...
                'ODNO':str(order_id),'INQR_DVSN_3':'00','INQR_DVSN_1':'','CTX_AREA_FK100':'','CTX_AREA_NK100':''})
    ...
    for raw in items:
        ...
        if identity != str(order_id): continue
