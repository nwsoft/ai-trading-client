...
From v3.9.1.46, Coinone uses these same PAPER/LIVE controls; no separate E2E approval switch is required. Public KRW spot PAPER / learning does not require private API credentials. LIVE requires valid trading credentials and an explicit start confirmation. Updating does not start LIVE. Simulated tests do not certify real-account fills.
...
The v46 candidate accepts multiple files or a local folder: at most 40 files, 24 MiB each and 64 MiB total. CSV, TSV, XLSX and DOCX text extraction is included. Download shared Drive materials first; a folder URL does not crawl its contents. XLS/DOC are unsupported. Check Source coverage for duplicate, included and truncated text. PDF extraction is limited to the first 100 pages and the combined AI text to 60,000 characters. XLSX uses stored cell values, not formula recalculation or full chart interpretation. Missing, truncated or conflicting source rules require clarification and cannot be certified as ready. Image/video extraction may use the configured paid provider for each source; the existing budget still applies.
...
Missing exit-order ownership cannot be reconstructed merely from matching prices or timestamps. The v46 broker recovery path compares exact entry and exit order identities, filled quantities, prices, fees and taxes. KIS/Kiwoom date queries and partner-profile history paths do not establish complete historical lot recovery: absent ownership, incomplete pages or missing costs remain unresolved. Statement import, full daily-account reconciliation and conditional resumption with unresolved historical attribution are NOT implemented in this candidate. Completing this check is not an authorization to trade.'''),
...
No API key, strategy source or account number should be included in a public support message.'''),
    ('updates', 'Updates', '''v3.9.1.45 preserves RSI, MACD and trend from crypto analysis when writing learning evidence. Existing nested indicator records are read, but absent historical measurements are never inferred from prose or current prices. Not recorded is not zero. Candidate technical scores require selection-time 15-minute and 1-hour RSI evidence; overall scores are separate. Market regime mode filters and input/delivery validation are improved. Default crypto observation/confirmation/dwell remains 300 seconds / 2 observations / 600 seconds. Zero dwell is not recommended for normal operation. Public v3.9.1.44 assets remain unchanged; the v45 Windows installer and live external verification are separate gates.

Inherited from v3.9.1.44: evidence-preserving learning storage, indexed decision metadata and storage maintenance. Open Settings > Update > Maintenance · Storage to inspect log/learning/database sizes, last compression and detailed-log expiry. Optimize storage runs in the background and can resume. It preserves original JSON and trade/PAPER ledgers, so first migration requires free disk space. Detailed file logs can be enabled for 24 hours and expire automatically. The default 512 MiB diagnostic log budget excludes permanent important-event evidence. Older compressed learning records remain stored; the screen shows recent records. Storage optimization does not repair unresolved PnL, reset history or start LIVE trading. Unknown historical mode/order metadata stays unknown. Windows installer and long-running verification are separate gates.

v3.9.1.43 improved direct difficulty navigation, consistent level labels and refresh after a verified settings save. v3.9.1.42 added English beta presentation and remote controls with explicit PC permissions. The client installer is released separately from website updates.
