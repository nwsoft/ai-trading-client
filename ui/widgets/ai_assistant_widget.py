...
【증권사 연결 FAQ — 기술 지원 지식】
v3.9.1.47 공통 안내: 체결 복구는 실제 주문/체결/비용 근거가 필요합니다. 여러 진입이 동일 가격으로 완전히 청산된 경우 진입 주문별 lot 복구를 지원하지만, 모든 과거 미대조 기록의 자동 해결이나 자동 LIVE 재개를 약속하지 마세요. 전략 등록 문제 수정과 원문 전략의 실행 가능성은 별개입니다. ENTRY/EXIT 문단의 지원 비교식은 처리하며 미지원·모호한 조건은 행별 질문으로 확인합니다. 기본 AI 진입으로 전환한 버전을 원문 복원 또는 기존 PAPER 성과 인증으로 안내하지 마세요. 국내 현물 공개시세 PAPER/학습과 실제 계좌 API 인증도 구별하세요.
과거 재생은 제한된 백테스트입니다. 최근 500개가 모든 시장국면을 대표하지 않으며 기본 최대 12봉 보유 등 체결 가정이 있습니다. 표본 0건은 성과 판단 보류입니다. 실행 가능한 승인 전략은 과거 성과 합격 없이 WebUI에서 PAPER를 선택할 수 있습니다. 실행식 누락과 LIVE 계좌 위험은 별도로 검사합니다. Level 5 연구실은 같은 모의 거래의 비용 민감도 비교이며 위험 해제 권한이 아닙니다. WebUI의 모든 Level에서 날짜·종목·최대 보유 봉 가정을 선택합니다. 7코인은 워밍업 포함 최대 5,000봉, 증권은 공급 가능한 일봉에 한정되며 부족한 과거 시세를 최근 시세로 대체하지 않습니다. Google Drive 폴더 전체 수집은 제공 완료로 안내하지 마세요.
