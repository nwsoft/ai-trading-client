...
def verify_build_structure():
    ...
    if check_file_exists(spec_path):
        ...
        checks = {
            "build_safe.py 자동 생성본": "AUTO-GENERATED BY build_safe.py" in spec_content,
            "config 템플릿 포함": "('config/settings_template.json', 'config')" in spec_content,
            "금융상품 카탈로그 포함": "('data/finance_products', 'data/finance_products')" in spec_content,
            "사용자 문서 포함": "('docs', 'docs')" in spec_content,
            "증권 팩토리 hiddenimport": "'trading.exchanges.exchange_factory'" in spec_content,
            "레퍼럴 자동확인 hiddenimport": "'referral_account_proof'" in spec_content,
            "Python 소스 datas 중복 없음": all(
                item not in spec_content
                for item in (
                    "('trading', 'trading')",
                    "('api', 'api')",
            ...
            "폐기 테마 시스템 미포함": "'theme_system'" not in spec_content,
        }
    ...
    if folder_ok and file_ok:
        ...
        print("=" * 60)
        print("\n다음 명령어로 빌드 실행:")
        print("  python build_safe.py --platform windows --gate-profile release")
        return 0
    else:
