...
### 2026-09-26 설계 정합 감사 — 배포 버전 변경 없음

NoahAI의 제품 정체성은 AI 금융 동반자이며 기술 구조는 판단·설명·기록·검증·환류를 연결하는 AI 자산 의사결정 인프라다. 복구/차단은 이 목적을 지원하는 운영 기능이지 독립된 제품 목적이 아니다. 복구 건수나 차단 수가 많다는 이유로 품질이 높다고 판단하지 않는다.

- 사용자 의도 → 원문 근거 → 공통 Strategy IR → 기관별 실행 어댑터 → 확인된 체결/비용 원장 → 설명·환류의 책임을 유지한다. 새 기관의 특성은 어댑터/기관 프로필에서 보존하고, 모드·주문 소유·금융 근거·오류 의미는 공통 계약을 사용한다.
- 실행 규칙 유효성, 과거 성과 평가, PAPER/LIVE 권한을 분리한다. Level 1~5는 같은 계약을 보여주는 깊이이며 별도 엔진이나 안전 해제 권한이 아니다.
- Level 5 도입은 명시적 제품 결정이다. 향후 Level 6 이상도 추가할 수 있으며 5를 영구 상한으로 고정하지 않는다. 단계 추가 시 공통 IR·기존 프로필/버전 호환·권한 분리·UI와 시험을 함께 제공한다. 정의되지 않은 숫자 단계를 즉시 실행 권한으로 인정하지 않는다.
- `RecordRecovery`는 계정·기관별 복구 큐/백업/재개를, 제공사 복구기는 조회·귀속 근거를 담당한다. Binance 이력 경로와 OKX/Bybit/Bitget 선형 선물 경로는 API 차이를 보존한다. `recovery_statement.py`는 검토 파일의 출처/단위/완전 구간/귀속을 검증한다. UI는 근거를 확정하거나 주문을 시작하지 않는다.
- 기간 조건은 source_condition_compiler → 공통 IR temporal 노드 → temporal_strategy_conditions로 평가한다. 중첩식은 각 과거 봉의 당시 이력만 보며 평가별 메모이제이션과 합성 300봉 한도로 비용을 제한한다. 이름 붙인 latched(설정, 해제, N)는 초기 false·해제 우선·N봉 만료를 명시한 유한 이력 상태다. 신호/신뢰도 과거를 추측하지 않으며 영속 임의 코드 상태와 구분한다. OHLCV 시작 시각 대신 완료 시각으로 재생 경계를 계산한다.
- 숫자 상태는 numeric_strategy_state의 제한 AST 인터프리터를 사용한다. STATE/UPDATE는 완료 봉 기준 이전 값을 읽어 일괄 커밋하며 계정 DB 경로+기관/종목/모드/전략 key/version/규칙 해시로 격리한다. SQLite에 최신 상태 한 행만 저장하고 연결을 닫는다. 최대 32변수·식 500자·96 AST 노드/깊이 12·수치 절댓값 1e12로 제한한다. 최초 활성화는 최신 완료 봉부터, 이후 최대 400봉 확보 이력의 누락 구간만 전진한다. 변경 봉/이력 단절/저장 실패는 상태를 초기화하지 않고 불확실 상태로 반환한다. 재생은 메모리, 병행 PAPER는 별도 상태 파일/모드를 사용한다. 증권 진입·청산은 같은 완료 일봉/상태 경로를 사용한다.
- 같은 주문이 여러 원장을 소유하면 원본 ledger_allocations가 로컬 원장 ID별 수량을 증명해야 한다. 진입 주문만으로 단일 원장을 식별할 수 있으면 entry_allocations를 사용한다. 진입/청산 교차에서도 각 원장의 시간순 잔량·총수량·비용 보존을 증명한 뒤 기존 원자적 claim 트랜잭션에 저장한다. 조회된 보유량만으로 과거 청산 손익을 추정하지 않는다.
- strategy_drive_source는 Google Drive v3 읽기 목록·다운로드/내보내기와 기존 자료 묶음 분석을 연결한다. drive_authorization은 NoahAI 회원 인증과 분리된 desktop loopback/PKCE/state 승인·메모리 토큰 갱신을 담당한다. 공개 자료는 운영자 API 키로 읽고, 제한 자료만 기본 브라우저 승인을 요청한다. 토큰은 전략/로그/AI/디스크에 쓰지 않으며 회원 전환·연결 해제로 제거한다. 운영자 Google 앱 등록/검증과 실제 계정 승인은 배포 외부 게이트다.
- 복구한 거래 순손익은 계좌 전체 일별 PnL과 같지 않다. 파일의 사용자 확인과 API 인증도 구분한다. 복구 완료 후에도 실제 시작 명령과 계좌 위험 재평가가 필요하다.
- 확인된 결함 수정, 아직 없는 엔진/복구 기능, 근거 부족, 실제 설치본 시험, 제품 연구 목표를 분리한다. 최신 상태 정본은 `V39147_RUNTIME_BOUNDARY_TEST_PLAN.md`, 실행 결과는 `TEST_STATUS.md`다. CHANGELOG의 항목 수나 과거 시험 수는 배포 승인 근거가 아니다.

이 감사는 전체 설계 무결함 인증이나 남은 구현 완료 선언이 아니다. 아래 날짜/버전별 절은 해당 시점의 이력이며 상단 현행 계약과 충돌하는 과거 정책을 현재 동작에 적용하지 않는다.
...
# 현재 소스 후보 v3.9.1.46 · 공개 자산 기준 v3.9.1.45
...
2026-09-22: 학습 지표는 `indicator_evidence.indicator_snapshot`으로 생산·조회 경계에서 같은 검증을 적용한다. 유한한 실제 0은 보존하고 누락·bool·비정상 수치는 미기록으로 구분한다. 과거 기록은 구조화된 근거만 읽으며 원장을 추정 복구하지 않는다. 코인 선정 기술점수는 종합 가중치와 별도이고 후보 worker 추가 API 조회를 금지하는 기존 최적화를 유지한다.
