...
배포 대상 릴리스: `https://github.com/nwsoft/ai-trading-client/releases/tag/v3.9.0.2`

> 2026-07-26 기준 소스/문서/`deploy/version.txt`/Windows 버전 리소스는 3.9.0.2입니다. 새 Windows EXE가 생성되기 전 `deploy/release-manifest.json`은 `pending_windows_rebuild`, size 0, 빈 SHA를 유지합니다. Windows에서 현재 소스를 빌드·서명한 뒤 `generate_release_assets.py`로 실제 크기/SHA를 생성해야 합니다. 기존 EXE를 이름만 바꿔 재사용하면 파일 시각과 ProductVersion 게이트가 배포를 중단합니다.
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.2 -Branch main -PushBranch
```

옵션
- `-Version`: 필수. `3.9.0.2` 또는 `v3.9.0.2` 모두 허용
- `-Branch`: 기본 `main`
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.2
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.2 -Branch main -PushBranch -StrictBranchPush
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.2 -SkipCommit
```
...
gh release upload v3.9.0.2 deploy/AITrading.exe deploy/version.txt deploy/release_notes.md deploy/release-manifest.json --repo nwsoft/ai-trading-client --clobber
```
