## 현재 인계: 3.9.1.43 / updater 3.9.143

공개42 이후 난이도 동선·표기·즉시 반영 변경입니다. [43 진단/시험 경계](V39143_FEEDBACK_REVIEW.md)를 우선 확인하세요. 아래42 문서는 상속 기능의 설명이며, 43에서 과거 미대조 40건을 복구했다는 의미가 아닙니다. 사용자 DB는 수정하지 않았습니다. Client 설치기 빌드·배포는 사용자가 수행합니다.

## v3.9.1.43 빌드 인계 — 2026-09-20
...
제품 3.9.1.43 / updater 3.9.143. 공개 v3.9.1.42 설치기·태그·latest.yml·manifest는 보존합니다. **이번 작업에서는 Client 설치기 빌드·릴리스를 하지 않으며 사용자가 수행합니다.** [V39142_REMOTE_CONTROL_PLAN.md](V39142_REMOTE_CONTROL_PLAN.md)의 최종 소스 시험과 미완료 Windows/실기관 게이트를 확인하세요. `web_platform/remote_control.py`, 원격 monitor/gateway/UI, 승인·명령 회귀 및 재생성한 매뉴얼을 포함한 fingerprint로 새 Windows 빌드를 식별합니다. daltrading 서버 배포 완료는 설치본 제어 검증 완료가 아닙니다.

# NoahAI 빌드·릴리즈 가이드

현행 소스 후보: **3.9.1.43** / Electron updater: **3.9.143**. 공개 stable/latest는 **v3.9.1.42**이며 기존 v42 자산·태그·manifest는 불변입니다. 과거 설명은 [보존본](archive/build/BUILD_GUIDE_PRE_V39135.md)에 있으며 아래 명령만 현행입니다.

**v43 신규 빌드 필요:** [v42 검증 계획](V39142_REMOTE_CONTROL_TEST_PLAN.md)을 따릅니다. 공개 v42 자산을 이름만 바꾸거나 재사용하지 않습니다. 현재 Mac 동기화 작업본에는 Git 이력과 Windows 도구가 없으므로 검토된 소스 커밋을 정식 Windows 빌드 환경에 전달해야 합니다. 이전 PnL 미완료 검증도 완료로 승계하지 않습니다.
...
- 소스 테스트 통과, 패키지 실행, 실제 계정 연결, 장시간 PAPER 검증은 서로 다른 증거입니다. [v3.9.1.43 검증 계획](V39142_REMOTE_CONTROL_TEST_PLAN.md)의 실제 확인한 행만 `[x]`로 표시합니다.
...
- 3.9.1.43 일반 설치본은 GitHub stable/latest만 조회하고 prerelease·다운그레이드를 허용하지 않습니다. 앱의 Beta 명칭은 사전공개 채널 선택을 뜻하지 않습니다. 테스트 prerelease는 수동 설치용입니다. Windows에는 `latest.yml`, macOS에는 `latest-mac.yml`과 해당 플랫폼 설치 자산을 같은 버전·같은 Release에 함께 게시해야 합니다.
...
- v3.9.1.43은 새 x64 엔진·x86 키움 호스트·Electron 설치기로만 빌드합니다. 공개 v3.9.1.42의 설치기·blockmap·`latest.yml` 이름이나 해시를 복사·교체하지 않습니다.
...
deploy/web-release/NoahAI-3.9.1.43-Setup.exe
deploy/web-release/NoahAI-3.9.1.43-Setup.exe.blockmap
deploy/web-release/latest.yml                      updater 3.9.143
deploy/release-manifest.json
```

빌드 전 `deploy/release-manifest.json`이 v3.9.1.42을 가리키는 것은 정상입니다. 이 파일은 공개 증거이며 Windows v43 빌드가 성공해 새 해시를 계산하기 전 문서 편집만으로 v43 값이나 가짜 해시를 넣지 않습니다. 빌드 스크립트가 새 산출물에서 v43 manifest를 생성한 뒤 설치기·blockmap·`latest.yml`과 함께 검증합니다.

### v3.9.1.43 추가 확인

**현재 배포 보류:** [거래 기록 복구 계획](V39143_RECORD_RECOVERY_PLAN.md)의 미완료 항목을 먼저 확인합니다. 버튼/자동시험 통과를 과거 40건 복구 또는 전 기관 일별 계좌 대조 완료로 해석하지 않습니다. 공개 manifest는 계속42이며 설치기 생성·게시를 수행하지 않았습니다.
...
청산 ID 없는 Binance 시험: 주문·체결·Algo·income 수집 구간이 증가하고, 조회량 제한 후 화면이 열려 있으면 추가 클릭 없이 이어지는지 확인합니다. 정확한 진입 주문의 독립 포지션 구간이 복구되면 원래 거래 행·체결 원장·순손익이 일치해야 합니다. 음수 손익을 양수로 바꾸거나 미대조를 삭제하지 않아야 하며, 타임아웃/혼합 진입/포지션 변경/보존기간 초과는 구체적 사유가 남아야 합니다. 실제 사용자 API 키를 개발자에게 전달할 필요 없이 사용자의 설치된 연결로 수행합니다. 현재 소스 시험 413개 통과와 고객 DB 40/40건의 조회 경로 진입 확인은 고객 실제 API 복구 완료와 구분합니다.
...
8. 공개 v3.9.1.42 설치본에서 v3.9.1.43을 확인·다운로드하고 거래 엔진 안전 종료 뒤 설치·재시작되는지 확인합니다. 설정, 계정 자격정보, 전략 버전, 검증 결과와 PAPER/LIVE 원장을 대조합니다.
...
```text
deploy/mac-release/NoahAI-3.9.1.43-arm64.dmg
deploy/mac-release/NoahAI-3.9.1.43-arm64.zip
...
Windows와 macOS는 같은 제품 버전의 **공용 GitHub Release 태그 `v3.9.1.43`**를 사용합니다. Windows는 `latest.yml`, macOS는 `latest-mac.yml`을 읽으므로 플랫폼별 자동업데이트 자산이 충돌하지 않습니다. DMG·ZIP과 두 blockmap, `latest-mac.yml`, Mac manifest를 Windows 설치기와 같은 릴리즈에 게시합니다.

`scripts/publish_macos.py`는 공용 릴리즈가 이미 있으면 Mac 자산만 추가하고 원격 SHA-256을 검증합니다. Windows 릴리즈가 아직 없으면 같은 `v3.9.1.43` 태그의 draft를 만들며, 별도 `-macos-candidate` 태그는 만들지 않습니다. 같은 이름의 원격 자산이 다르면 덮어쓰지 않고 새 제품 버전을 요구합니다. 새 릴리즈에서 서명 없는 Mac 자산을 stable 릴리즈에 추가하는 작업은 스크립트가 차단합니다. v3.9.1.42 이하의 기존 Mac 자산은 과거 공개·수동 테스트 기록이며 v3.9.1.43 자동업데이트 증거로 승계하지 않습니다.
