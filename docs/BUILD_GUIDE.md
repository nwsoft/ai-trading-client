...
배포 대상 릴리스: `https://github.com/nwsoft/ai-trading-client/releases/tag/v3.9.0.10`

> v3.9.0.10 AI Custom Management & Runtime Integrity Update 소스는 `pending_windows_rebuild`입니다. 직전 v3.9.0.9 자산은 `deploy/previous/AITrading-v3.9.0.9-AI-Custom-Stability-Update.exe`와 SHA `857ee230a60f...`로 보존했습니다. Windows에서 새로 빌드한 뒤 설정 50회, 거래소 탭 왕복 100회·급속 전환 200회, 단일 프로세스, USER/GDI/TK_MENU, 분리 native 창 0, Binance WebSocket 장시간 PAPER와 기존 AI 커스텀·VC 런타임·Kiwoom·OCR 회귀를 함께 검증해야 합니다.
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.10 -Branch main -PushBranch
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.10
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.10 -Branch main -PushBranch -StrictBranchPush
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.10 -SkipCommit
```
...
gh release upload v3.9.0.10 deploy/AITrading.exe deploy/version.txt deploy/release_notes.md deploy/release-manifest.json --repo nwsoft/ai-trading-client --clobber
```

- 태그를 새로 만들지 않고 릴리즈 에셋만 교체
- v3.9.0.10에서는 생성 후 manifest의 `release_label=v3.9.0.10 AI Custom Management & Runtime Integrity Update`, `build_status=built`, EXE `size>0`, 실제 SHA-256 일치를 확인한 뒤 업로드한다.
