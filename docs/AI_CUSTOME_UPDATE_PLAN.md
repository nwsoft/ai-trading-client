...
## 2026-09-26 현행 해석 — v3.9.1.47 소스 후보 / 공개 v3.9.1.46

제품 비전과 원문 의미 보존 원칙은 유지한다. 현재 Level 1~5는 편집·연구 깊이이며 기존 1~4 식별자를 바꾸지 않는다. 실행 조건의 유효성, 과거 성과 평가, PAPER/LIVE 권한은 별도 계약이다. 유효한 승인 전략의 PAPER에 과거 백테스트 합격을 요구하지 않으며, 누락 실행식을 과거 성과나 사용자 동의만으로 허용하지 않는다. 기존 전략은 원문·버전·성과를 보존하고 지원 조건은 보완·재컴파일하며 미지원 조건을 다른 전략으로 몰래 대체하지 않는다.

Level 5를 도입하며 향후 Level 6 이상도 추가할 수 있다. 5단계를 영구 최대치로 제한하지 않는다. 추가 단계는 공통 IR·기존 설정/버전 호환·설명·시험을 갖춰 제공하며 자동 LIVE 권한 확대와는 별개다.

2026-09-26 구현 범위: 기간 조건 중첩과 명시 초기/해제/만료를 가진 latched 이름 상태를 같은 완료 봉 IR에서 실행한다. 정의 없는 무기한 상태 코드를 실행하는 기능은 아니다. Drive는 공개 링크의 앱 API 키와 제한 자료의 기본 브라우저 승인·메모리 토큰 갱신을 지원하며 회원 인증과 분리한다. 운영자 앱 등록·실계정 검증은 별도다. 선택형 백테스트와 PAPER 진행을 유지하고 기존 전략 자동 재작성·승인·LIVE 시작은 하지 않는다.

이 문서는 제품 목표이며 모든 목표를 47의 구현 완료나 배포 차단 목록으로 해석하지 않는다. 현재 구현/미구현/실환경 확인은 [47 실행 원장](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md), 검증 결과는 [TEST_STATUS](TEST_STATUS.md)가 정본이다. 아래 v27 기준선은 당시 이력이고 현행 공개 버전 표시가 아니다. 9/26 고정한 숫자 상태 실행기와 원본 근거 기반 공유/교차 체결 배분은 구현했다. STATE/UPDATE도 일반 전략과 같은 원문 확인·IR 검증·별도 승인 경로를 사용하며, 고객 정의를 임의 대체하지 않는다. Windows 설치본과 실제 기관/Drive 운영 확인은 별도다.

### 이전 v27 기준선

기준 제품: NoahAI Client v3.9.1.27 공개판  
기준일: 2026-09-11  
문서 상태: 전략 의미 보존 P0, Web AI 멘토, Level 1~4, 거래소별 과거검증·Validation Lab·통화별 PAPER 근거를 포함한 제품·개발 정본  
배포 경계: v3.9.1.27 Windows 설치 파일·blockmap·`latest.yml`·manifest는 공개됐다. 설치·업데이트/재시작·실제 Provider·실계정·6개 거래소·4개 증권사 장시간 E2E는 환경별 후속 증거다.  
릴리스 결정: text/Pine/PDF/이미지/영상/URL 원본, 제한 IR·XAI, 버전/diff/rollback, `.noahstrategy`, 거래소별 과거 최소검증, 암호화폐·주식 PAPER 자동 귀속과 Web Strategy Studio를 통합했다. v3.9.1.27은 결정형 컴파일과 외부 AI 보조를 분리하고, 탭별 어시스턴트 문맥·비용 가시성·역할별 모델 구성을 보강한다. 이는 모든 Pine 의미 동등성, TradingView 운영 webhook, 전체 LIVE 장시간 집계, 유료 마켓 완료를 뜻하지 않는다.

> 파일명 `AI_CUSTOME_UPDATE_PLAN.md`와 내부 `AI_CUSTOM` 식별자는 기존 경로·데이터 호환을 위해 유지한다. 사용자 제품명은 `NoahAI 전략 스튜디오`로 통일한다.
...
- 전략 저장, 사용자 승인, 권장 자동검증, PAPER 전진검증, 사용자 최종 적용, LIVE를 각각 다른 상태로 유지한다.
...
사용자 피드백을 반영해 백테스트를 제거하지 않는다. 총 PnL, 총 수익률, MDD, 승률, Profit Factor와 기간별 성과는 연구 목표/제공 범위를 구분한다. 다음은 연구 평가 순서이지 PAPER 진입의 강제 통과 순서가 아니다.

```text
선택한 과거 구간의 규칙 재생과 성과 평가
→ 미사용 구간·워크포워드
→ 비용·파라미터 민감도·몬테카를로·과최적화 경고
↔ 유효한 승인 전략의 PAPER 전진검증 (과거 합격과 분리)
→ 사용자 검토와 최종 적용
```

과거 전체를 알고 맞춘 수익률은 미래 성과 근거가 아니다. 따라서 백테스트만으로 자동 승격하지 않으며, 성과표에는 `과거 결과이며 미래를 보장하지 않음`, 사용 데이터 기간, 비용 가정과 PAPER 상태를 함께 표시한다. 음수 총 PnL·과도한 MDD는 성과 평가 미달, 표본 부족은 평가 보류로 설명한다. 이를 실행식 유효성 위반이나 PAPER 금지와 동일시하지 않는다. LIVE 권한·계좌 위험 검사는 별도로 유지한다.
