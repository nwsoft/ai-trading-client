...
## 0. v3.9.1.21 PAPER 손익·비용 정합 추가 계약

- 네 증권사는 `StockAnalysisService`의 동일한 국내 현물 LONG/보유분 청산 계약을 사용한다. PAPER에서 신규 SHORT·레버리지·외부 주문은 만들지 않는다.
- 열린 PAPER 주식·ETF는 분석 현재가마다 KRW 기준 gross PnL과 예상 수수료·세금·슬리피지를 계산하고, 화면의 미실현 PnL에는 예상 비용을 차감한 net PnL을 표시한다.
- 청산 원장은 증권사·종목·주식/ETF·진입/청산가·수량·gross/net PnL·수수료·예상 세금·슬리피지를 분리한다. 계산 필수값이 없으면 0원 정상 거래로 위장하지 않고 invalid/미확정으로 분리한다.
- 일부 청산은 청산 수량 비율만큼 기존 진입비용을 배분한다. 보유 수량 초과 매도는 PAPER에서도 차단한다.
- PAPER 성과회복·수익성 표본은 같은 증권사의 유효한 PAPER 청산만 사용한다. LIVE 증권사 체결과 다른 증권사의 표본은 섞지 않는다.
- 기본 추정 프로필은 매수/매도 수수료 각 `0.00015`, 주식 매도세 `0.002`, ETF 매도세 `0`, 매수/매도 슬리피지 각 `0.0003`이다. 이는 실제 계좌 청구액이 아니며 설정에서 편집한다. 포지션 생성 시 저장한 비용 계약은 설정 변경으로 소급 변경하지 않는다.
- 소스/Mock 회귀는 Windows 키움 OCX, 증권사 제휴 계정, 실제 세금·수수료 또는 배포 설치본을 증명하지 않는다. v3.9.1.21 Windows 후보에서 네 증권사별 PAPER 가격변동·부분/전체청산·재시작·로그 귀속을 별도 대조한다.
