## 2026-09-23 후속 수정 — 아래 과거 취소/재설정 지침보다 우선

미배포 소스 기준: Binance는 일반+Algo 주문을 모두 확인한다. OKX TP+SL은 OCO, Bybit는 확인된 positionIdx에 position trading-stop, Bitget은 양쪽 계획 주문을 사용한다. 양쪽 보호·종목·방향·수량·열린 상태 근거를 확인하며 접수 성공과 검증 완료를 구분한다.

조회 오류/불완전 목록/부분 보호/다른 주문이 있으면 보존하고 자동 재발주하지 않는다. 최소 20초 간격의 점검에서 완전히 빈 주문 목록과 NoahAI 소유 포지션·기록된 보호 가격이 확인될 때만 재접수를 시도한다. 임의 가격·최소 수량을 추정하거나 다른 주문을 취소하지 않는다. 우아한 정지는 미청산 포지션 보호주문을 일괄 취소하지 않는다.

기존 부분 보호의 자동 교체는 완료 경로가 아니다. 거래소 포지션에서 TP/SL을 확인해야 하며 실제 API/계좌 검증 전 실거래 안전을 보장했다고 표시하지 않는다. [수정·시험·제한](../reports/v39145-cross-venue-feedback-audit-20260923.md).
