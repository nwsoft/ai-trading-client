...
## 2026-07-05 운영 정합 업데이트 (거래소 검증/진단)

### 핵심 변경
- 설정의 거래소 API 검증은 이제 "실패"만 보여주지 않고 원인별 조치 가이드를 함께 제공한다.
- Bitget 설정 패널에는 현재 공인 IP 표시와 새로고침 버튼이 추가되어 IP 화이트리스트 점검을 즉시 수행할 수 있다.
- Bybit/OKX/Bitget 검증 실패 문구는 어댑터 진단 상태(`last_error`, `last_auth_guidance`)를 기반으로 표시된다.

### 사용자 동선(대시보드 메뉴얼 기준)
1. 대시보드 우측 상단 설정 진입
2. 거래소 API 탭에서 해당 거래소 검증 실행
3. 실패 시 가이드 팝업의 원인/조치 순서대로 수정
4. 필요 시 Bitget 공인 IP를 복사해 거래소 IP 화이트리스트와 대조
5. 재검증 후 거래소 카드의 개별 시작 버튼으로 실행

### 운영 해석 규칙
- 현재 구조는 "선택한 거래소 개별 시작"이 기준이며, 검증 실패 거래소는 다른 거래소 시작을 직접 차단하지 않는다.
- 다중 거래소 미체결 이슈는 전략 조건뿐 아니라 인증/권한/IP 화이트리스트 계층을 우선 분리 진단한다.
