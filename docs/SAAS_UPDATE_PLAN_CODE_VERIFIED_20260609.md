...
## 3-2. SaaS 이전 AI 비용 제어 게이트 (2026-07-24 추가)

2026-07-21~23 실사용 점검에서 27,879회 모델 요청과 $9.08 비용이 확인됐다. 선택 거래소는 Binance였지만 활성 거래소 6개 로그가 함께 생성됐고, 거래 48건보다 심볼 분석 호출이 훨씬 많았다.

SaaS 전환을 기다리지 않고 클라이언트에 먼저 구현한다. 2026-07-24 후속 패치 기준 상태:

- `[완료]` 로컬 기술 신호 선판정 후 거래 후보·시장 이벤트만 모델 신규 호출
- `[완료]` 동일 시장상태 해시 15분 재사용
- `[완료]` 실제 주문 거래소와 `learning_enabled_exchanges` 분리
- `[완료]` 거래소·모델·전략·입력/출력 토큰 계측
- `[완료]` 일/월/거래소별 호출 한도와 로컬 신호 폴백
- `[부분 완료]` 구조화 출력 600→320 토큰 축소, 다심볼 배치는 A/B 대기
- `[완료]` Binance 실체결 Fee·진입/청산 order ID·모델/전략을 거래 원장에 저장
- `[남음]` 달러 예상비용 UI, 지원 내보내기 비밀값 제거, 장시간 실계좌 검증

SaaS에서는 위 클라이언트 게이트를 대체하지 않고 중앙 쿼터·공용 시장분석 캐시·테넌트별 원가 귀속을 추가한다.
