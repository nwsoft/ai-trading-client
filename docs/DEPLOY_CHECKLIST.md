...
### 2026-09-26 현재 인계 — 아래 과거 보류 문구보다 우선

고정한 세 구현(명시적 숫자 상태, 같은 주문 공유 원장 배분, 같은 주문 추가 체결/청산 교차)은 완료했다. 최종 소스 전체 4,242 passed / 9 skipped / 2 warnings / 3 subtests passed, 실제 컴파일러/검증기 연결 브라우저 Level 1~5·기존 전략 보완·복구 화면 PASS. 자세한 시험 경계는 TEST_STATUS 정본이다. Windows **빌드·설치 검증 진행 가능**, 일반 배포/실계좌 결과는 아직 미확인이다. 새로운 개발을 남겨 둔 상태로 테스터에게 떠넘긴다는 뜻이 아니다.

- [x] 공통 전략 보완/새 버전/승인/PAPER와 기존 원문·성과 보존, 복구 후 원장/비용/중복 검사 소스 회귀.
- [x] 매뉴얼·업데이트 내역·현행 완료 목록 동기화. PnL 초기화와 자동 LIVE 재개를 해결책으로 안내하지 않음.
- [ ] 사용자가 Windows에서 기존 절차로 빌드하고 후보 버전·파일 SHA-256을 기록.
- [ ] 같은 설치본에서 45/46→47 업그레이드 후 기존 전략·거래 원장·상태 보존, 새 등록/기존 보완/PAPER 시작/중지/재시작 확인.
- [ ] 연결된 테스트 계좌의 시작 차단 사유→인앱 복구→다시 위험 검사 및 실제 주문/청산/보호 상태 확인. 없는 근거를 0으로 만들거나 위험 차단을 해제하지 않음.
- [ ] 기관별 실제 시장국면/위험 알림 수신, 다기관 Windows 장시간/잠금/저장 증가 확인. 실제 전환이 없으면 미발송이 정상일 수 있으므로 관찰/확정/필터/전달을 구분.
- [ ] Drive 기능을 배포에서 제공하려면 운영자 API/OAuth 앱 설정 및 공개/제한 링크 실제 수집·승인 취소 확인. NoahAI 회원가입 방식은 변경하지 않음.

위 설치본 결과를 기록한 뒤 사용자가 릴리스한다. 고객 55개 원문/과거 35건을 운영자가 일일이 수정하는 것을 배포 조건으로 삼지 않는다. 원문 정의·공식 근거가 부족한 경우 앱의 구체적 보완/자료 안내를 확인한다. 아래 수치와 '미구현' 문장은 과거 시점 이력이다.

2026-09-26 문서 정합 정정: 현재 개발/근거/설치본/확장 범위 판정은 [47 실행 원장](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md)의 「현행 완료 판정과 잔여 분류」를 사용한다. 이 문서의 아래 과거 버전별 보류/통과 표시는 당시 이력이다. 문서 검사 PASS는 지정 문구/계약 검사의 통과이지 전체 기능·실계좌·배포 승인 증거가 아니다. 빌드/배포는 사용자가 수행하며, 기존 요구의 범위를 줄여 미완료 기능을 완료로 처리하지 않는다.
...
- 최신 후속: 3,704 passed / 8 skipped / 3 subtests passed. 고객 2.88GB 전체 사본/11기관 쓰기·청산·체결/유지관리 및 WebUI 한·영 다중 자료 표시 통과. TEST_STATUS 참조. 아래 수치는 이전 실행 이력.
- 부분청산/보조 기록 영속 재처리와 다중 파일/로컬 폴더 입력은 후속 구현됨. 최신 시험은 TEST_STATUS 참조.
- **전체 배포 승인 아님:** 증권 과거 lot 자동 귀속·공급자 비용/연속조회, Coinone LIVE 지원 개방, 디스크 전체 고갈/Windows 장시간·실제 공급자 조회/보호/알림 수신이 남음. Coinone 실조회는 현재 IP 거절이며 승인된 실제 주문 시험을 대체하지 않음.

- 추가 소스 시험: Python 3,600 passed / 8 skipped / 3 subtests passed. Coinone LIVE 미지원·내부 승인값 편집 제외, 잠금 실패 재처리/중복 방지, 현물 보유 대조, 원문 전략 빈 진입식 차단 포함.
- **미승인 게이트:** Coinone LIVE 실계좌 검증, 과거 현물 원장 대조, 전 저장 경로 경합/Windows 장시간, 실제 알림 수신. 미지원 상태와 실제 복구 완료를 구분해 배포 안내에 명시한다. 고객 원본을 수정하거나 LIVE를 시작하지 않음.

소스 3.9.1.46 / updater 3.9.146, 공개 v3.9.1.45 보존. 미배포 후보이며 아래 v45 항목은 이전 이력입니다. 시작 버튼→실제 워커→기관 시세→확정 국면→알림 수신과 Windows 설치본을 검증하기 전 전체 미수신 해결로 승인하지 않습니다. [시험과 제한](../reports/v39145-cross-venue-feedback-audit-20260923.md).

## v3.9.1.45 인계 체크리스트 — 2026-09-22

- 소스 제품 3.9.1.45 / updater 3.9.145. 공개 기반 v44 manifest·설치기·태그는 수정하지 않는다.
- 오늘 학습 지표·시장국면·알림 변경, 버전 메타데이터·매뉴얼 산출물을 함께 새 Windows 빌드에 포함한다.
- Windows v43/v44 업데이트 후 기존 기록 보존, 새 분석 RSI·MACD·추세 저장/표시, 기술점수 미산출 설명, 100/125/150% 배율을 확인한다.
- 실제 기관 응답·알림 수신·장시간 운용은 모의 회귀와 별도 게이트다. 설치기 빌드·릴리스는 사용자가 수행한다.

## 과거 v3.9.1.42 인계 체크리스트
...
- [ ] **배포 보류:** [DB 기반 PnL 피드백](V39141_PNL_GUARDRAIL_FEEDBACK.md)의 기존 미대조 기록 사용자 업데이트 처리·기관별 실제 어댑터/Windows·일별 계좌 대조를 검증한다. 새 근거 연결·기준값 영속화 회귀는 통과했으나 모든 실환경 검증 완료가 아니다. 특정 사용자 원장 복구는 배포의 필수 조건으로 묶지 않는다.

- [x] KPI 집계·인증 경로 경량화·DB 분리/외부 보관/복원 도구 소스와 격리 회귀.
- [x] 모바일 내 PC 상태·명시적 신규 제출 일시정지, 계정/기기 격리·중복/만료·PC 재개 회귀 및 300계정 격리 ASGI 시험.
- [x] 사용자 매뉴얼·설정 AI 설명·사용/운영 가이드 갱신. [KPI·원격 실행 계획](V39141_REMOTE_KPI_IMPLEMENTATION_PLAN.md).
- [ ] EC2 실제 DB 복사 리허설·최종 이관·전후 로그인 POST 측정·S3 실제 복원·독립 relay/캐시 규칙 적용.
- [ ] Windows 설치본에서 동의·재연결·명령 전달·기관별 보호/청산 유지·PC 재개 확인.
- [ ] 권한/개인정보·약관 확인 후 allowlist 20→100→300명, 24시간 운영 지표와 중단 기준 확인.
- 최신 소스 시험·화면·남은 게이트는 [통합 검증 보고](../reports/v39141-remote-kpi-verification.md)를 따릅니다.

- [x] 오늘·7일·30일 시장 트렌드, 시장 폭 원형·미니 추세선·방향/변동성 표시 소스 및 fixture 회귀.
- [x] 코인 현물/선물·KRW/USDT 구분, 주식 장기 일봉 순차 조회와 빈 상태·오류 상태 회귀.
- [x] 화면 근거 기반 AI 브리핑·관찰 후보 XAI, 지지/반대/무효화/누락 근거와 비주문 경계 회귀.
- [x] 시장 트렌드 변경 직후 기록: Python 2,639 passed / 8 skipped / 3 subtests, Node 43 passed, Web 63 modules, 격리 브라우저 오류 0건. KPI/원격 추가 후 최신 수치는 통합 검증 보고에 기록.
- [ ] Windows v41 새 설치본·v40→v41 업데이트·실기관 데이터·DPI·장시간 검증과 원격 게시.
- [v41 검증·배포 계획](V39141_MARKET_TREND_XAI_TEST_PLAN.md)

## v3.9.1.40 공개 기록

v3.9.1.40 · updater 3.9.140은 Windows stable/latest로 공개됐습니다. Strategy Studio·AlphaArena·설정 변경과 남은 외부 게이트는 [v40 검증 기록](V39140_STUDIO_ALPHA_SETTINGS_TEST_PLAN.md)에 보존합니다.
...
# v3.9.1.47 선택적 Drive 자료 연결 운영 설정

NoahAI 자체 회원가입/로그인은 변경하지 않는다. 공개 공유 폴더는 운영자 Google Cloud 프로젝트의 Drive API 키로 읽으며 사용자 Google 로그인을 요구하지 않는다. 특정 계정 제한 폴더만 Google의 기본 브라우저 승인 화면을 사용한다. Drive 읽기 전용 scope는 Google의 앱 검증/사용자 동의가 필요할 수 있다. 코드 시험 통과가 앱 등록·승인 완료는 아니다.

- 운영자: Google Cloud에서 Drive API 활성화, 공개 자료용 API 키의 API 제한/쿼터 설정, OAuth **데스크톱 앱** 등록·동의 화면·필요 검증을 수행한다. 실제 값은 문서/채팅에 기입하지 않는다.
- 배포 설정: `config/drive_client.json`에 Google에서 내려받은 `installed` 객체와 선택적 `public_api_key`를 둔다. Web 엔진 spec은 파일이 있을 때만 포함하며 사용자 access/refresh token이나 서비스 계정 개인키를 허용하지 않는다. 아직 이 파일/실제 운영 설정을 만들거나 Google Cloud를 변경하지 않았다.
- 개발/운영 대안: `NOAHAI_DRIVE_CLIENT_CONFIG`로 설정 파일 위치 지정. `NOAHAI_DRIVE_API_KEY`, `NOAHAI_DRIVE_OAUTH_CLIENT_ID`, `NOAHAI_DRIVE_OAUTH_CLIENT_SECRET` 환경값은 파일보다 우선한다. 일반 사용자에게 키/토큰 발급을 요구하지 않는다.
- 실제 수용 시험: 공개 타인 공유 폴더는 사용자 로그인 없이, 제한 폴더는 권한 있는 계정 승인 후 수집. 승인 거절·타 계정 권한 없음·토큰 만료·연결 해제·회원 전환·앱 재시작을 확인한다. 토큰은 계정별 메모리에만 있고 재시작 후 재연결한다. 수집 자료의 누락/축약 표시와 자동 승인·자동 LIVE 없음도 확인한다.
- 설정이 없으면 연결 불가 사유와 로컬 파일/폴더 입력을 제공한다. 설정 없는 설치본을 Google Drive 완전 지원으로 안내하지 않는다. 설치기 빌드·게시·실계정 시험은 아직 수행하지 않았다.
