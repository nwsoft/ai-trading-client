# 배포 체크리스트 (2026-07-26 · v3.9.0.2 기준)
...
- 대시보드 하단에 `v3.9.0.2 AI 커스텀·레퍼럴·거래 통계 고도화` 요약과 `업데이트·사용법` 버튼이 노출되는지 확인
...
- 사용자 매뉴얼 `업데이트` 최상단에 v3.9.0.2 변경·제한·배포 상태가 노출되는지 확인
- 대시보드 업데이트 카드에 `AI 커스텀·레퍼럴·거래 통계 고도화`가 표시되는지 확인
...
4) 태그 기반 배포 실행
  - 버전 업데이트 커밋 후 `git tag v3.9.0.2` / `git push origin v3.9.0.2`
  - 이후 버전도 동일 패턴
  - Windows 자동화 스크립트 사용 가능: `powershell -ExecutionPolicy Bypass -File scripts/release_tag_push.ps1 -Version 3.9.0.2 -Branch main -PushBranch`
