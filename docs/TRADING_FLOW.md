...
암호화폐 시작: 선택 기관 → resolve_crypto_execution_mode → LEARNING/PAPER/LIVE 별 시작 검사 → 기존 런타임. 전역 PAPER OFF만으로 LIVE로 판단하지 않는다. 공개 시세 LEARNING은 실제 주문 권한과 분리한다. 수동 분석에도 기관을 전달하고 타 기관 실패를 Binance 시세로 대체하지 않는다.

신규 거래의 전략 key/version은 진입→실제 TradeLog→조회와 증권 부분청산 lot까지 보존한다. 과거 미기록은 현재 전략으로 채우지 않는다. 보호 조회 실패는 빈 주문 목록이 아니며, 보호주문 접수는 보호 확인 완료가 아니다. 기관별 요청/조회 계약과 제한은 [보호 가이드](TP_SL_GUIDELINES.md), [검증 보고](../reports/v39145-cross-venue-feedback-audit-20260923.md)를 따른다.

## v3.9.1.42 원격 시작·재개 흐름

PC 별도 승인 → 웹 기관/모드 확인·비밀번호 재확인 → 명령 접수 → PC 권한·설정·위험 재검사 → 신규 진입 차단 상태에서 기존 워커 시작 → 실행 확인 후 진입 허용. 오류·만료 시 진입 차단을 유지하며 손실 기준이나 전략을 초기화하지 않습니다. 일시정지는 기존 포지션 보호·청산을 종료하지 않습니다. [검증](V39142_REMOTE_CONTROL_PLAN.md).

# 거래 흐름 기술 문서 (Technical Specification)

## v3.9.1.41 후보 · 원격 신규 제출 경계

공개판 v3.9.1.40. `EntryPause`는 Binance 두 진입 경로, Unified 6기관 주문 제출, 증권 4기관 BUY/PAPER BUY에 적용합니다. 증권 SELL과 NoahAI 소유 국내 현물 LONG 청산은 우회하여 보호·청산을 유지합니다. 일시정지 이전 in-flight 제출과 이미 거래소에 접수된 주문은 완료·체결될 수 있습니다. 원격 시작/재개/전량 청산은 없고 PC만 진입 재개를 허용합니다. 이 경계는 거래 권한·기존 위험 검사·PnL 근거를 대체하지 않습니다. [실행 계획](V39141_REMOTE_KPI_IMPLEMENTATION_PLAN.md).

이하 이전 버전 상태는 당시 기록입니다.

## v3.9.1.32 소스 후보 · 공개 v3.9.1.31

추가 보강: 시세 관측 실패 → 국면 `unknown` → 신규 진입 보류. 마지막 유효 국면을 정상/횡보로 바꾸지 않는다. 후보 없음 → 보유 관리 생략이 아니라 빈 후보와 함께 보유 평가·청산 서비스를 호출한다. 실제 현재가/소유권이 확인되지 않으면 임의 주문은 하지 않는다. LIVE 위험 근거 미확정 → 0원 추정 금지·신규 진입 보류·선택한 외부 채널로 확인 실패 알림. PAPER는 자신의 원장만 사용하고 LIVE 손실 알림에서 제외한다.
...
**작성일:** 2026년 4월 27일  
**최신 동기화:** 2026-09-12 · v3.9.1.28 소스 후보 (공개판 v3.9.1.27)  

v3.9.1.28은 LIVE 청산 뒤 정확한 주문 ID의 체결만 연결합니다. 거래소 실현 PnL과 비용 반영 순손익을 분리하고, 부분체결은 잔여 포지션을 유지합니다. 증권사 개별 fill은 완료 거래가 아니라 확인 체결로 저장하며 NoahAI BUY lot와 SELL 수량이 연결될 때만 주식/ETF 성과가 됩니다. 근거가 없는 구버전 값은 `대조 미확정`으로 남깁니다.
...
Strategy Studio의 v3.9.1.26 위험 입력 보완은 명시적 `%` 문장을 실행 위험계약으로 구조화합니다. 제작 경로는 `원문 그대로 구조화 / 질문으로 함께 완성 / 기본 NoahAI에 맡기기`로 분리됩니다. 질문 답변은 원본과 별도 근거로 누락값만 보완하고 기존 원문과 충돌하면 차단합니다. `AI에게 묻기`는 설명 전용이며 전략 진입·청산·승인·주문을 자동 수정하지 않습니다.

v3.9.1.27은 외부 AI 보조와 결정형 규칙 컴파일의 실패 경계를 분리합니다. 429가 발생해도 텍스트·Pine 컴파일은 진행하며, AI 답변은 미적용 검토 초안으로만 Strategy Studio에 전달됩니다. 사용자 확정 뒤 전체 규칙을 다시 컴파일하고 기존 저장·승인·검증·PAPER·LIVE 게이트를 그대로 적용합니다.

```text
원본 추출 → 결정론적 1차 컴파일 → 누락/미지원 질문
  → 사용자 직접 답변 → 원본과 별도 SHA-256 근거
  → 결정론적 재컴파일 → 충돌/미지원 실패 폐쇄
  → 버전 저장 → 승인 → 검증 → PAPER → 명시적 LIVE 적용
```

## v3.9.1.23 기관 공통 실행·통계 흐름

```text
venue_capabilities 정본 + Web UI inventory
  → 서비스/시장/통화/주문 단위 결정
  → LIVE | PAPER | LEARNING 실행 경계
  → 기관 어댑터와 주문/가상체결
  → LIVE trade_log | PAPER strategy_paper_outcomes | 학습 원장
  → account + service + source + mode + 기간 조회
  → 거래소 카드 · 운영 KPI · 거래 통계 · AI 리포트
```

- LIVE 청산 통계는 `trade_log`, PAPER 청산 통계는 PAPER 전용 원장을 사용한다. 외부 거래소 확인 체결은 별도 근거이며 두 원장을 서로 대체하지 않는다.
- 현재 포지션은 기간 통계와 분리해 최신 계좌/PAPER snapshot에서 읽는다. 조회 실패를 0개로 바꾸지 않는다.
- `통계 표시 기준 새로 시작`은 선택한 거래소·증권사 또는 전체 서비스 보기의 표시 기준시각만 저장한다. 과거 거래와 학습·가드레일·성과회복·전략 검증은 유지된다.
- Strategy Studio의 `PAPER 일시정지`는 같은 attempt를 보존하고 재개한다. `새 검증 시작`만 새 attempt를 만들며 이전 근거는 이력으로 남긴다. 최소 거래수나 활성 검증일수 미충족은 실패가 아니라 진행 중이다.
- 새 기관은 위 흐름 전체와 `EXCHANGE_SEPARATION_GUIDELINES.md`의 온보딩 행렬을 통과하기 전까지 메뉴·홍보·릴리스 노트에서 지원 완료로 표시하지 않는다.

v3.9.1.0 Gateway의 시작/중지 명령은 인증·명시 intent·멱등 command ID를 요구합니다. 인증된 Electron sidecar는 최초 명시 명령에서 기존 Python 엔진을 headless로 지연 생성하고 `Application Services → OMS/Guardrails → Adapter`를 사용합니다. 엔진 미연결·준비 실패는 `409`로 실패 폐쇄하며, 이 소스 연결은 `PAPER/Windows E2E` 외부 게이트 전에 배포 완료로 확대 해석하지 않습니다.
...
## v3.9.1.13 후보 선정 런타임 경계

- Binance·Upbit·Bithumb·Bybit·OKX·Bitget의 전체 티커는 거래소별 일괄 스냅샷을 우선합니다. 일괄 조회 미지원 때만 최대 8개 제한 병렬·단계별 10초 제한을 사용합니다.
- 상세 캔들·지표는 유동성 상위 30개로 제한하고, 같은 거래소의 UI·자동 요청은 single-flight로 합칩니다.
- 자동 재선정은 거래 사이클 밖의 거래소별 스레드에서 수행하고, 성공한 목록을 한 번에 교체합니다. 거래·포지션 감시와 TP/SL은 재선정을 기다리지 않습니다.
- 완료 시점 기준 120초를 넘긴 시세·선정 결과는 교체하지 않습니다. 이전 정상 후보가 없으면 `stale_unscored`로 신규 진입을 차단합니다.
- 시장국면은 5분 관찰, 2회 연속 확인, 최소 10분 유지 후 전환하며 후보는 국면 변화 또는 선정 후 3시간 경과 때 재평가합니다.
- 후보가 전부 `fallback_unscored`이면 즉시 한 번 복구한 뒤 60초부터 최대 15분까지 점진 재시도합니다. 같은 실패 목록은 기본 15분에 한 번만 원장에 기록하며, 정상 후보가 회복되면 재시도 단계가 초기화됩니다.
- OKX 파생 ticker의 `quoteVolume`이 비어 있으면 OKX 원문의 `volCcy24h`(기초자산 수량)×`last`를 USDT 거래대금으로 사용합니다. `vol24h`/CCXT `baseVolume`은 계약 수이므로 거래대금 계산에 사용하지 않으며, 올바른 수량이 실제로 없을 때만 미산출로 처리합니다.
- 주식·ETF는 증권사별 서비스와 유니버스를 5분 재사용합니다. 지수 이력 지원 시 KOSPI 5거래일 기준, 미지원 시 당일 지수 기준을 명시적 fallback으로 사용합니다.
- 실제 Windows 배포 후보 성능 목표는 캐시 2초, 단일 거래소 콜드 p95 10초, 6개 병렬 p95 15초입니다. 소스 테스트 통과만으로 이 목표를 충족했다고 판정하지 않습니다.

정본: `docs/COIN_SELECTION_GUIDE.md`
...
- 코인 선정(Evaluator)
  ...
  - 업비트/빗썸(현물): 현재 거래소의 KRW 페어만 추출 → `quoteVolume` 내림차순 정렬 → 상위 후보 평가.
  - Bybit/OKX/Bitget(선물): 현재 거래소 마켓에서 USDT 파생상품만 추출 → 24시간 티커의 거래대금·변동성으로 후보 평가.
  - Binance(선물): 거래 가능한 USDT 무기한 선물 전체의 24시간 티커를 거래대금 순으로 정렬한 뒤 캔들 확보와 종합점수를 평가. 심볼 목록의 앞 100개를 임의 후보로 사용하지 않음.
  - 상태 계약: 목표 충족 `scored`, 정상 후보 일부 `scored_partial`, 평가 후보 0개 `fallback_unscored`. 부분 후보는 그대로 사용하며 점수 없는 고정 목록으로 채우지 않음.
  - `fallback_unscored`는 진단용 참조 심볼이며 PAPER/LIVE 신규 진입을 차단. 기존 포지션 보호·청산은 계속함.
  - 코인 선정은 분석 후보를 줄이는 단계이며 주문이 아님. 정상 후보도 후속 신호·AI 커스텀·위험·주문 규격을 모두 통과해야 함.
...
- 운영 확인
  - 같은 미산출 고정 목록이 반복되면 시장에 기회가 없다는 판단이 아니라 데이터 평가 불가 상태이므로 API·티커·캔들·평가 로그를 점검한다.
  - 정상 점수 후보가 있지만 주문이 없으면 실시간 신호의 HOLD와 전략·가드레일 차단 사유를 확인한다.
  - 거래소별 후보·상태·실행 대상을 섞지 않으며 AI 커스텀도 거래소 미지원·데이터 실패·안전 차단을 우회하지 않는다.
