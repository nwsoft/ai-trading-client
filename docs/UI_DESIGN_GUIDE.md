...
> 2026-08-14 현재 React/Electron 소스 전환을 완료했다. 1080px 최소 확인에서 상단 매뉴얼·설정, 탭, 모달 Escape/backdrop 종료, 가로 overflow 0을 확인했으며 Windows 배율·다중 모니터는 외부 배포 게이트다.
...
- 상단의 기준 거래소와 활성 범위, 각 기능 화면의 자율주행 실행 상태, PAPER/LIVE 권한을 별도 필드로 표시한다. 공통 하단은 레거시 정본인 `NoahAI-AI 금융 동반자 | MODE ... | EXCHANGE ... | TIME ...` 형식을 유지한다.
