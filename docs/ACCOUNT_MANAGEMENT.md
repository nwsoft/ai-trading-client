...
- 포지션 진입·부분 청산·전량 청산 이벤트
- 학습 데이터 기록 성공/실패 이벤트
- 리포트 파일 저장 성공/실패 이벤트

운영 원칙:

- 클라이언트는 원문 로그 본문이 아니라 집계 가능한 KPI 이벤트만 보낸다.
- 관리자 화면에서 보이는 운영 KPI는 익명 이벤트 기반 최신 집계다.
- 외부 공개 KPI는 별도 코호트/정적 스냅샷 기준으로 해석해야 하며, 운영 KPI와 동일시하지 않는다.
...
v3.8.9.28 이후 추가로 누적되는 운영 KPI 예시:

- 평균 보유시간(`hold_seconds` 기반)
- 거래/응답 시간 계열 메타데이터
- AI 추론 완료/응답시간 계열 이벤트

v3.9.0.2에서는 주문 체결에 `quote_currency`를 기록해 KRW와 USDT 거래량을 분리합니다. 구형 이벤트는 자산군·거래소·심볼을 순서대로 확인해 호환 분류합니다.

평균 보유시간은 검증된 `trade_position_closed`만 사용합니다. 포지션 진입은 수집됐지만 아직 종료가 없으면 `0분`이나 `0%`가 아니라 `수집 대기/수집 관찰 중`으로 표시됩니다.
...
### 🔐 API 키 보안
- **현재 저장**: 거래소·증권사·AI API 키는 사용자별 로컬 설정에 저장
- **화면·로그**: 입력창을 마스킹하고 키 앞뒤 문자열을 로그에 기록하지 않음
- **전달 주의**: 설정 파일과 자동 백업을 지원 로그 압축에 포함하지 않음
