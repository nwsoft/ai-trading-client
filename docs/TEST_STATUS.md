## 2026-09-26 — v47 고정 잔여 개발 완료 · Windows 빌드 검증 인계

소스 후보 v3.9.1.47 / 공개 설치본 v3.9.1.46. 이 결과는 소스/로컬 시험이며 Windows 설치본·실계좌·일반 배포 완료가 아니다. 빌드와 게시 담당은 사용자다.

- 최종 코드 전체: `.venv/bin/python scripts/diagnose_v39145_feedback_contracts.py tests -q -rs --tb=short` → **4,242 passed / 9 skipped / 2 warnings / 3 subtests passed**, 86.82초. 네트워크 차단 시험이다. 제외: 실제 증권 API 6, Binance 통합 2, 비공개 고객 쓰기 fixture 1. 경고는 기존 Starlette/httpx 및 utcnow 사용 중단 예고다.
- 신규 `test_v39147_remaining_development.py` 76개 포함: 숫자 상태의 원문→IR→Level 1~5→새 버전 저장/승인/PAPER(백테스트 필수 아님), 재시작/8개 동시 호출/동일 봉 반복/계정·기관·종목·버전·모드 분리/실패 원자성. 11기관 CSV→RecordRecovery→같은 주문 공유 원장 및 추가 체결/부분청산 교차→손익·비용 보존/반복 불변. 주식/ETF × 4기관 실제 청산 경로의 상태 인계도 검사했다. EMA 조회 창 이동과 실제 원본 봉 변경을 구분하며 잘못된 함수 인수/함수 이름의 숫자 사용은 등록 시 거절한다.
- 중간 회귀에서 일반 기간식에도 숫자 상태 전용 원본 해시 필드를 붙인 변경이 기존 문맥 계약 시험 1개를 실패시켰다. 상태 프로그램이 있는 경우에만 추가하도록 수정했고 기존 시험은 변경하지 않았다. 이후 최종 전체 시험은 위와 같이 실패 0개다. 중간에 중단한 실행은 완료 근거에 포함하지 않는다.
- 연결 중 발견한 추가 결함 수정: 병행 PAPER의 기본 AI 후보 오귀속 방지, 증권 명시 청산의 완료 봉/숫자 상태 문맥 누락. 일반 기본 AI 경로의 의도된 fallback이나 LIVE 보호 조건은 해제하지 않았다.
- 실제 React 브라우저: `check-strategy-levels.mjs` 코인/증권 × Level 1~5 등록 10경로 PASS. `check-strategy-repair.mjs` 실제 Python 컴파일러/검증기 연결의 신규/기존 버전/초안/질의 답변 보완, 11기관 복구 화면 연결·실패 5상태 PASS. 저장/기관 API는 fixture다. `check-statement-recovery.mjs` 1440/700 화면의 열 매핑→검토→동의→저장, 검토 무효화 PASS. LIVE 요청 없음.
- TypeScript app/node 검사와 문서 정합 검사 PASS. 최종 코드 시험 이후 매뉴얼·현재 완료 목록을 갱신하고 문서 시험을 별도로 수행한다. 학습 기록은 선택 필드/후보 근거이며 숫자 상태 재생용 400봉 문맥을 통째로 추가 저장하지 않는다. 상태 DB는 최신 행만 갱신하고 연결을 닫는다.
- 고객 원본 읽기 재검사: 기존 58버전 중 준비 3/실행식 보완 55, 290 Level 투영, 원본 저장소 2개 불변. 이는 55개 원문 자동 복원 PASS가 아니다. 공통 인앱 보완→새 버전→승인/PAPER 경로의 구현 검증과 고객의 정의/자료 제공은 분리한다. 실제 과거 35건의 외부 근거까지 확보했다는 뜻도 아니다.
- 다음 단계: 같은 후보로 Windows 45/46→47 업그레이드·원장/전략 보존·재시작·실제 시작/복구/보호/알림 확인. Drive 공개/제한 폴더 기능은 운영자 Google 앱 설정 및 실제 권한 확인이 필요하다. 미확정 손익 초기화·자동 LIVE 시작·고객 원본 변경·설치기 빌드·배포 없음. 세 고정 항목의 미구현은 남지 않았지만 위 실환경 결과를 대신 PASS로 표시하지 않는다.

## 이전 실행 이력 — 같은 날 교차 진입/부분청산 복구 수정

소스 후보 v3.9.1.47 / 공개 설치본 v3.9.1.46. 일반 배포 승인이나 실제 계좌 검증 결과가 아니다.

실제 제한을 합성 DB에서 재현: 원본 `entry_allocations`가 있어도 중간에 다른 주문의 진입이 있으면 `exit_lot_allocation_required`로 거절했다. 수정 전 신규 회귀 실패 확인 후, 일괄적인 진입 선행 제약은 원본 배분 없는 비례 계산에만 적용하도록 수정했다. 각 lot의 진입 선행·정확한 수량·시각·비용 및 원자적 저장 검증은 유지한다.

- 신규 11기관 매개변수 시험: 다른 진입 주문 사이의 부분청산 → JSON 수량 근거가 포함된 CSV 검토/저장 → 실제 RecordRecovery → 2개 원장 손익 반영. 전체 수량·수수료 보존, 이미 처리한 근거 재사용 차단, 아직 진입하지 않은 lot에 청산을 배정한 근거 거절, 근거 없는 FIFO 금지, 자동 LIVE 없음. API 실계좌 시험이 아니다.
- 변경 후 전체 네트워크 차단 시험 **4,166 passed / 9 skipped / 2 warnings / 3 subtests passed**, 80.17초. 제외는 아래와 같은 실제 증권 API 6, Binance 통합 2, 비공개 고객 쓰기 fixture 1이다. 전체 실행 후 매뉴얼/문서만 갱신하고 문서 검사를 별도로 수행한다.
- 고객 기존 전략 읽기 검사는 58개 중 실행 준비 3/누락 실행식 55, 원본 2개 불변. 이 검사만으로 임의 영속 숫자 변수가 누락 원인이라고 결론 내릴 수 없다. 이 고객의 실제 미확정 거래를 이번 합성 근거로 복구한 것도 아니다.
- 남은 범위: 임의 영속 숫자 변수/대입, 동일 진입 주문을 여러 관리 원장이 공유하는 배분, 같은 진입 주문의 추가 체결과 자체 청산 교차는 완료하지 않았다. 재진입/부분청산 전체가 미지원이라는 이전 설명 대신 위 지원 범위를 따른다. 빌드·배포·실주문 없음.

## 2026-09-26 — v47 중첩 조건·공유 체결·현물/증권 탐색·Drive 승인 후속 검증 (이전 실행 이력)

최종 코드 변경 후 전체 네트워크 차단 회귀: `scripts/diagnose_v39145_feedback_contracts.py tests -q -rs` → **4,155 passed / 9 skipped / 2 warnings / 3 subtests passed**, 80.60초. 제외는 실제 증권 API 6, Binance 통합시험 2, 배포하지 않는 비공개 고객 쓰기 fixture 1이다. 기존 deprecation 경고 2개는 남는다. 아래 4,114 결과와 중첩/공유 배분/Drive 승인 미구현 설명은 후속 변경 이전 이력이다.

- 신규 계약 시험 42개: 중첩 temporal·latched 초기화/해제/만료/미래 참조 차단/성능 상한, Google PKCE·state·갱신·취소·연결 해제·설정 오류·HTTP 인증/명시 의도, 국내 현물 native API 합성 응답, 7 현물/증권 날짜 체크포인트, 11기관 공유 배분의 수량/비용 보존·중복 방지·전체 롤백, 실제 파일 가져오기→RecordRecovery→원장 저장을 검사했다. 공급자 응답은 fixture이며 실계좌 시험이 아니다.
- TypeScript app/node 검사 통과. 실제 React 브라우저/API fixture에서 코인·증권 1280/1440/1920px PAPER 동선, 자료 입력 800/1280/1440/1920px, Drive 연결/확인/해제 및 일회성 인증값 제거를 확인했다. 공식 내역 가져오기 1440/700px에서 열 대응→검토→명시 동의→저장과 검토 무효화·자동 LIVE 없음 재통과. 700px는 격리 컴포넌트 시험이다.
- 고객 전략은 읽기 전용 58개 중 실행 준비 3/보완 필요 55, Level 투영 290, 원본 저장소 2개 불변이다. 이 결과는 모든 기존 원문을 복원했다는 뜻이 아니다.
- 지원 범위: 완료 봉 최대 합성 300봉의 중첩 조건과 유한 boolean 상태, 현재 보유 없음이 확인되는 날짜별 과거 이력 탐색, 원본 lot 수량 또는 동일 가격 완전 청산 묶음의 공유 배분. 임의 영속 숫자 변수/대입, 재진입·부분청산이 교차하는 일반 배분, 동일 진입 주문을 여러 원장이 공유하는 경우는 개발 미완료로 남긴다. 근거 없는 체결·수수료·손익을 생성하지 않는다.
- Google 운영자 API/OAuth 등록·필요 앱 검증·실제 공개/제한 계정 수집, 제휴 증권사 실제 이력 계약, Windows 설치본·실제 기관 응답·메시지 수신 시험은 수행하지 않았다. 토큰은 메모리에서 갱신하며 앱 재시작 후 재연결한다. 소스 검증을 일반 배포 승인으로 바꾸지 않는다. 고객 원본 변경·실주문·설치기 빌드·배포 없음.

현재 범위 및 남은 개발은 [47 실행 원장](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md), Drive 운영자 설정은 [배포 체크리스트](DEPLOY_CHECKLIST.md)의 선택적 Drive 자료 연결 절을 따른다.

## 2026-09-26 — v47 기간 조건·복구 근거·Drive 수집 구현 후 검증 (이전 실행 이력)

최종 코드 변경(기간 조건의 참조 지표만 보존하는 compact context 포함) 후 전체 네트워크 차단 회귀: `scripts/diagnose_v39145_feedback_contracts.py tests -q -rs` → **4,114 passed / 9 skipped / 2 warnings / 3 subtests passed**, 80.88초. 제외는 실제 증권 API 6, Binance 명시적 통합시험 2, 배포되지 않은 비공개 고객 쓰기 fixture 1이다. 기존 deprecation 경고 2개는 남는다. 설치본·실계좌·실제 메시지 수신 통과를 뜻하지 않는다.

- 기간 조건 5종(`all_for`, `any_within`, `became_true`, `became_false`, `after`): 명시 시간봉·완료 캔들·누락 지표·반복 평가·원문→IR→저장→승인→백테스트 없는 PAPER를 검사했다. 중첩 기간 조건과 임의 영속 상태 변수는 미지원이다. 재생 평가의 시각 경계도 캔들 시작 시각 대신 실제/계산된 종료 시각으로 바로잡아 한 봉 지연과 미래 봉 포함을 구분했다.
- KIS 과거 주문 연속 조회/기간별 TR·Kiwoom 다음 페이지, Upbit/Bithumb의 연결된 주문 실제 부분체결 근거를 합성 API로 검사했다. 주문 ID 없는 국내 현물·증권 전체 과거 청산 탐색을 완료한 것은 아니다.
- 공식 내역에 명시된 `entry_order_id` 근거로 복수 진입·부분청산·수동 거래를 분리하는 경로를 검사했다. 근거 없는 FIFO 추정이나 하나의 공유 체결을 여러 원장에 수량 배분하는 기능은 제공하지 않는다.
- Drive 폴더 API 페이지·하위 폴더·다운로드·문서 내보내기·변경/누락 검출·기존 통합 분석 연결을 합성 공급자로 검사했다. 현재 수동 API key/token 연결이며 일반 Google 로그인/토큰 갱신 UX와 실제 Google 계정 시험은 남아 있다. 자격증명은 저장된 전략 근거에 포함하지 않는다.
- TypeScript app/node 검사 통과. React 브라우저에서 코인/증권 1280/1440/1920px의 선택형 백테스트/PAPER 동선, 자료 입력 800/1280/1440/1920px, Drive 일회성 인증값 전달/성공 후 제거, 파일 복구 1440/700px 컴포넌트 동선을 검사했다. API fixture 시험이며 Windows 설치본·Google 로그인·실제 거래 시험은 아니다. 700px는 복구 컴포넌트의 격리 시험으로 전체 앱 모바일 대응을 의미하지 않는다.
- 고객 기존 전략 읽기 전용 재검사: 58개 중 실행 준비 3/보완 필요 55, Level 투영 290, 원본 저장소 2개 불변. 이번 검사로 55개 원문을 모두 재컴파일·복원했다고 주장하지 않는다. 원본 고객 DB/전략 변경, 실주문, 설치기 빌드·배포는 하지 않았다.

현재 완료/미완료 분류는 [47 실행 원장](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md)을 따른다. 아래는 이전 실행 이력이며 위 구현 이전의 미지원 설명과 시험 수치를 현재 상태로 혼동하지 않는다.

## 2026-09-26 — v47 설계·문서 정합 감사 (배포 버전 변경 없음, 이전 이력)

후속 제품 결정 정정: Level 5 도입 및 향후 Level 6 이상 추가 가능, 요청된 Drive 수집의 47 범위 임의 제외 금지를 반영했다. 현재 코드의 승인/실행식 검사→PAPER와 별도 LIVE 검사를 다시 대조했다. 문서 정책·선택형 PAPER·Level 프로필 집중 **125 passed**, 문서 정합 검사 PASS. 백서의 현재 버전은 코드/manifest 대조 및 고의로 틀린 버전 주입 시험으로 확인했다. 아래 전체 4,075개는 이 후속 문서/검사 변경 이전 실행이며 최신 전체 재실행으로 주장하지 않는다. 이 후속은 엔진·Drive 기능 개발 완료가 아니다.

전체 오프라인 재실행: `scripts/diagnose_v39145_feedback_contracts.py tests -q -rs` → **4,075 passed / 9 skipped / 2 warnings / 3 subtests passed**, 79.14초. 제외: 실제 증권 API 6, Binance 명시적 통합시험 2, 별도 비공개 고객 쓰기 fixture 1. 지정 문서 정책 충돌 검사 6개를 추가했으며 선택형 PAPER·Level 1~5 관련 집중 122개 통과. 기존 deprecation 경고 2개는 남는다.

실제로 찾은 문서 충돌은 Level 5 추가/다섯 단계 금지, 선택형 과거 평가/백테스트 합격 후 PAPER 강제, 백서의 과거 공개 버전 현행 표기다. 현행 규범 문장을 수정하고 날짜별 이력은 보존했다. `doc_consistency_check.py`는 이제 지정 현행 절의 이 충돌을 검사하지만 전체 문서 의미·구현 완성도 인증은 아니다.

고객 전략 읽기 시험: 58개 중 준비 3/보완 55, 5 Level 투영 290, 원본 저장소 2개 불변. 이 실행은 원문 전체 재컴파일이나 기존 55개 복원 완료가 아니다. 거래 엔진·전략 규칙·회원 데이터·권한 변경, 실제 API/주문, 설치기 빌드·배포는 하지 않았다. 이번 변경은 문서와 개발용 검사다. 개발 미완료/근거 필요/실환경/확장/보장 한계는 [47 실행 원장](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md)의 현행 분류를 따른다.

## 2026-09-25 — v47 Level 1~5·기간 조회·고객 사본 재검증 (이전 실행 이력)

최신 자동 탐색/CSV 구현 후: **4,069 passed / 9 skipped / 3 subtests / 2 warnings**, 81.31초. 네트워크 차단 전체 회귀. 신규 40개는 11기관 파일 → 실제 원장 복구, 3기관 API 합성 전체 사이클, 비용/단위/시각/부분 주문/실패, 중복 충돌, 재시작, 인증·의도 확인 HTTP → 실제 bridge → API 키 없는 파일 복구(엔진 생성 없음)를 포함한다. TypeScript app/node 검사 통과. 브라우저 1440/700px 컴포넌트에서 열 대응 → 미리보기 → 본인 확인 → 저장, 설정 변경 시 검토 무효화, 자동 LIVE 없음 확인(API fixture). 기존 시작 보류 → 복구 → 진단/AI 동선도 재통과. 시각 형식·계약 단위 충돌·조회 상한 분할·원자적 저장 직후 중단/재개·불완전 파일의 API 복구 방해 방지도 포함한다. 자동 탐색은 OKX/Bybit/Bitget 선형 선물, 파일 복구는 11기관 공통이다. 파일은 사용자 검토 출처로 남으며 API 인증 원장과 구분한다. 아래의 파일 기능 미구현 표기는 이전 이력이다. 실제 계좌·공식 내역 포맷 표본·Windows 설치본 검증은 수행하지 않았다.

최신 공통 청산 시각 보강 후 전체 회귀: **4,029 passed / 9 skipped / 3 subtests / 2 warnings**, 79.37초 (`scripts/diagnose_v39145_feedback_contracts.py tests -q`, 네트워크 차단). 집중 복구 시험 **98 passed**. 타 10기관의 연결된 주문/저장 확정 체결 → 시각 복원 → 비용/손익 재검증, 귀속 충돌·부분 수량·잘못된 방향/종목·시각 역전·구형 비용 기본값·중단/재시작을 합성 공급자로 검사했다. API 응답은 fixture이므로 기관 실계좌 검증이 아니다. 사용자 원본은 변경하지 않았고 설치기 빌드/배포하지 않았다. 아래 수치는 이전 이력이다.

배포 판단: 알려진 Binance 복구 대상 누락과 공통 연결 주문 시각 복원은 소스 검증 완료. 최신 후속에서 3기관 선형 선물 자동 탐색과 11기관 파일 복구를 추가했으나 국내 현물·증권 일반 API 탐색과 혼합 구간 배분은 미지원이며, 고객별 실제 미확정 건수 0을 보장하지 않는다. 전체 피드백 해결판 일반 배포 완료로 판정하지 않는다. 제한 범위를 명시한 테스터용 후보와 모든 기능 완료판을 구분한다.

현재 소스 후보 v3.9.1.47 / 공개 v3.9.1.46. 아래 추가 시험은 설치본·실계좌 검증이 아니다.

최종 Python 전체 회귀: `.venv/bin/python scripts/diagnose_v39145_feedback_contracts.py tests -q` → **4,009 passed / 9 skipped / 3 subtests / 2 warnings**, 79.23초. 네트워크 연결 차단. 전체 통과는 아래 고객 원문의 미지원 범위나 실환경 검증을 완료로 바꾸지 않는다.

### 9/25 시작 직후 409 · 미청산 원장 경로

- 제보의 `현재 포지션 없음 · 과거 관리 원장 청산 대조 필요`를 코드와 대조. 수정 전 합성 DB에서 Binance/OKX/Bybit/Bitget 모두 위험 보류 → 복구 대상 0 → 보류 유지 재현. `RecordRecovery._prepare`의 `exit_time IS NOT NULL` 누락이 공통 원인이다. 고객의 최초 청산 저장 누락 원인은 별도 미확정이다.
- 소유권/주문 ID 있는 미청산 원장도 큐에 포함. 기존 진행 작업 scope_version=2 이관. Binance 전체 주문/체결/정산/안정 포지션 근거로 청산 시각/손익 복원. 현재 보유는 verified_open으로 분리해 원장 불변, API 실패는 포지션 0으로 취급하지 않음. 새 작업은 과거 미청산 조회 창/근거를 갱신. 40개 현재 보유 기록 조회는 위치 스냅샷 1회 공유 확인.
- 시작 거절 구조화 코드 → 제한된 백그라운드 복구 요청, 시작 보류 유지. 준비 실패/이미 진행 구분. 정지/일일 손실 한도/일반 API 장애에서 임의 복구·자동 재시작하지 않음.
- 전체 회귀 이후 강제 중단 재현 1개를 추가하고 미청산 전용 시험 **26 passed** 재실행. 청산 원장 커밋 직후 진행 기록 저장 전 중단 → 새 복구 객체로 이어서 실행 → 손익 -10.3 단 한 번 반영/행 불변 확인. 이 추가 시험은 위 전체 회귀 4,009개 수치와 별도다.
- 브라우저 `check-start-recovery.mjs`: 시작 버튼 → 원인 안내 → 인앱 복구 → 버전/기관/시각/요청 ID/사유 개수 진단 → 사용자 검토 후 AI 질문 PASS. 주문/실계좌는 fixture. 복구 후 추가 trading.start 명령 없음. 자동 복구 준비 실패 안내 PASS. 기존 오류 변환 115검사 PASS.
- TypeScript app 및 격리 Vite 빌드(76모듈) PASS, 큰 chunk 경고 유지. 설치기 빌드·배포·실주문·고객 현재 계좌 대조 없음.
- 경계: 타 기관의 청산 시각 없는 과거 원장 자동 탐색은 미지원으로 표시한다. 대상 누락은 고쳤지만 이들까지 자동 복구됐다는 뜻은 아니다. 현재 보유 검증은 Binance/OKX/Bybit/Bitget의 정규화된 포지션 계약으로 시험했다. 원본 고객 DB 수정 없음.

### 공통 셀프서비스 후속 — 고객별 수작업이 아닌 앱 내 경로

- 실제 발견: 미정의 이름 조건을 보완 답변으로 처리하도록 안내하면서 인터뷰는 `source_rewrite`로 분류해 입력란을 숨겼다. 또 일반 답변의 설명 접두어가 boolean 정의를 가려 직접 컴파일러 호출만 통과할 수 있었다. 명시적인 `answer_kind/answer_target` 계약으로 입력을 열고 정의식 그대로 전달하도록 수정. 중복 이름은 한 번만 질문하며 미지원 함수는 여전히 승인 불가다.
- 기존 버전 선택 → 보완 질문 → 답변 → 실제 Python 재컴파일/최종 검증 → 동일 전략 새 버전 저장: 코인/증권 브라우저 PASS. 저장 API는 fixture이며 실제 파이프라인 저장/재로드·이전 버전 보존·실행 평가 시험은 Python에서 별도로 수행했다. 외부 AI/API/실주문 없음.
- `check-strategy-repair.mjs`: **24경로 PASS** = 전략 8 + 11기관 복구 연결 확인/점검 + 연결 실패 5종. 복구 API는 fixture이며 실제 RecordRecovery→RiskManager 11기관 합성 응답 시험과 구분한다. 어느 UI 경로도 거래 시작 명령을 보내지 않음.
- `check-strategy-levels.mjs`: Level 1~5 × 코인/증권 **10경로 PASS**. TypeScript app·격리 Vite 빌드·매뉴얼 11섹션 추출·문서 정합 PASS. Vite 큰 chunk 경고 유지.
- 고객의 55개 전략/35개 기록 잔여 숫자 자체는 일반 배포 차단 조건이 아니다. 공통 경로의 작동·사용자 조치 가능성·미지원 한계가 기준이다. 현재 엔진 미지원 상태/기간 함수와 API 밖 공식 파일 가져오기는 제품 구현의 열린 항목이지 고객이 반복 버튼으로 해결할 항목이 아니다. 해당 기능까지 완료됐다고 주장하지 않는다.

### 실행 경로 후속 검증

- 명시적 복합식/이름 정의·부족한 정의의 사용자 확인 보완 → 저장/재로드 → 실제 비교 평가, 순환/미지원/NaN/잘못된 타입 거부 시험 추가. 단순 비교식의 기존 all/any 직렬화 형태 유지. 기존 테스트가 이 형식 변화로 1회 실패하여 호환 형식을 복원한 뒤 재검증했다.
- 11기관: 미대조 LIVE 손익 차단 → 합성 제공사 체결로 실제 복구 → 실제 RiskManager 재평가 통과. 확정 손실 -10.2 보존, 더 엄격한 손실 한도는 여전히 차단. 초기 증권 fixture가 매수 체결을 매도로 반환해 4건 실패했으며 실제 양쪽 주문 계약으로 fixture를 수정했다. 증권 실제 API 검증이 아니다.
- 고객 DB 전체 사본 AVAX 57909 캐시 근거 재생: 수량 6 / gross 1.709 / net 1.6463875, 최근 300 미확정 0, 원본·다른 거래 불변, 실 API·주문 없음. 전체 과거 35건과 이번 새 409는 해결 판정하지 않음.
- 고객 58버전 읽기 보존: 정상 3 / 보완 55 / 원본 두 파일 불변. **55개 원문 전체를 새 컴파일러로 재분석해도 실행 준비 완료는 0개**. 자연어 상태·기간·pivot·미지원 표현이 남는다. 합성 전략 성공을 이 55개 전체 복원으로 확대하지 않는다.
- 브라우저 오류 변환 **115검사 PASS**(11기관 합성 오류 + 실제 API 클라이언트의 fetch 대체/요청 ID 연결 + 계좌 새로고침 중복 안내), 전략 보완 **6경로**, Level1~5 × 코인/증권 **10경로 PASS**. 저장·계정 API는 fixture이며 원문 컴파일/검증은 실제 로컬 Python 사용.
- TypeScript app PASS, 격리 Vite 빌드 75모듈 PASS(큰 chunk 경고 유지). 설치기 빌드·배포 안 함. 전체 회귀 최신 수치는 아래 최종 실행 항목을 따른다.

### 동일 일자 후속: 409 안내·전략 보완 동선

- 브라우저 `check-gateway-errors.mjs`: **108개 오류 표시 검사 PASS**, 11기관 합성 문자열/객체·미분류/빈 상세·비밀값 비노출. 실제 고객 API/409 발생 원인 재현 아님.
- `check-strategy-repair.mjs`: 코인/증권 **6경로 PASS**, 실제 로컬 컴파일/검증기 + 저장 API fixture. 기존 원문 보완 포커스 이동·목록 안내·원문 보존 추가 확인. 55개 복합 전략 복원 시험 아님.
- Python 전략 보완/Web gateway/legacy parity **222 passed**, TypeScript app PASS. 인앱 매뉴얼 11섹션 재생성/문서 정합 PASS. 소스 후보만 수정, 설치기 빌드·배포·실주문 없음.
- 새 46 스크린샷의 실제 409 사유는 기관·명령·상세 로그가 없어 미확정. 지원 코드 보강을 엔진 차단 해결로 확대하지 않는다. 아래 전체 회귀 수치는 앞선 실행 결과이며 이번 후속 전체 재실행을 의미하지 않는다.

전체 완료/릴리스 승인이 아니다. 아래 시험과 미완료를 분리한다.

- 전체 네트워크 차단 Python 회귀 **3,883 passed / 9 skipped / 3 subtests passed**, 78.13초, 기존 경고 2개. 먼저 실행한 회귀에서는 예전 `resultView === Level 3` 문자열에 의존한 정적 검사 1건이 실패했다. 실제 3~5 공통 분기를 검사하도록 갱신하고 전체를 재실행했다. 시험 의미를 완화하거나 실패를 제외하지 않았다.
- 집중 회귀 **144 passed**: 5프로필 × 11기관의 정상 구형 규칙/IR/성과 보존과 PAPER 진행, 신규 연구 비용 계산, 누락 비용/비정상 숫자, 원문 질문 ID, 전체 과거 복구 큐·PAPER 제외·기존 범위/체크포인트, 7기관 페이지 커서·시간/봉 범위·워밍업, 11기관 기본/기간 선택 서비스 경로. 합성 시세·로컬 저장소이며 실제 주문 없음.
- 실제 React/Chrome + 로컬 Python 컴파일러/검증기: **Level 1~5 × 코인/증권 10경로** 신규 저장·기존 규칙 불변·날짜/종목/보유 가정 요청 PASS. 기존 선택형 PAPER/취소 **16경로**, 등록/명시적 기본 AI 전환/미완성 보관 **6경로** PASS. 계정/저장 API는 fixture다. 900px에서는 기존 body 최소 폭 1080px 때문에 전체 페이지가 넓다. 실제 앱 기준 1280px와 새 입력 패널 스크린샷을 확인했고 가로 넘침 없음. 모든 Windows 배율/모바일 UI 검증을 뜻하지 않는다.
- 실제 공개 HTTP 시세: 7코인 각 **500봉** 요청 시작/종료 일치. 초기 Bithumb 날짜 형식 거절, Bitget 1봉 누락을 관측해 KST offset-free 커서/종료 경계를 수정 후 재검증했다. API 키·실계좌·실주문은 사용하지 않았다. 다른 종목/모든 시간봉/상장 전 기간까지 보장하는 시험은 아니다.
- 고객 전략 사본: `scripts/verify_v39147_customer_strategy_compatibility.py data/260924_Teayu/custom_strategies`. **58버전 / 290 Level 투영 / 원본 2파일 SHA 불변 / 실행 준비 3 / 실행식 누락 55**. 원문과 기존 검증을 바꾸지 않았다. 55개 고객 전략이 실행 복원됐다고 선언할 수 없다.
- 고객 전체 DB 사본: **3,229,663,232 bytes, 기존 판단 3,895,846행**, 11기관 판단 1,100건·청산 20·체결 20 + 유지관리 9,300행/양보 7회. 최대 쓰기 점유 26.46ms/대기 26.84ms, 3.69초, 원본 불변·네트워크 차단·실주문 0. 실제 Windows 장시간 시험 아님.
- 실제 캐시 전체 사본 재생: AVAX 합산 청산의 해당 주문 수량 6, 순손익 1.6463875 USDT. 반복 저장/다른 거래/원본 SHA 불변. 최근 300건 미확정 0, 대조 정책 차단 해소. **전체 Binance 청산 1,103건 중 35건은 여전히 미확정**(legacy 29, 체결 미발견 5, 대조 대기 1). 최근 표본 통과를 전체 복구로 확대하지 않는다.
- Node 22.23.1 격리 환경 TypeScript app/node, Vite 75모듈 PASS(큰 chunk 경고 유지). 앱 매뉴얼 11섹션 재생성, 문서/버전 정합·설정/어시스턴트 217필드 소스 계약 PASS. 사용자 의존성·고객 원본·설치기·배포 manifest는 덮어쓰지 않았다.

열린 작업: 원문 복합 조건 컴파일/정의 확인, 전체 과거 근거·공식 내역 수입/증권 lot, 전체 증권 날짜 공급·실시간 청산 완전 재현, Drive 수집·자료 분할 완결, Windows 업그레이드/장시간·실제 계정 보호·메시지 수신. [배포 전 원장](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md)의 미완료를 PASS로 바꾸지 않는다.

## 2026-09-25 앞선 단계 — v47 선택형 과거 평가·기존 상태 보존 (이력)

- 최종 Python 전체 회귀 **3,780 passed / 9 skipped / 3 subtests passed**, 77.27초, 기존 경고 2개. 초기 전체 실행은 매뉴얼 정본/생성본 수정 시점이 겹쳐 일치 검사 1건이 실패했으며, 정본 재생성 후 전체를 다시 실행한 결과다. 제외를 PASS로 세지 않는다.
- 신규 `tests/test_v39147_optional_replay.py` 28개: 성과 수치 누락/비정상·표본 0·손실·낙폭, 구형/신규 기록의 재시작 보존·PAPER 진행·LIVE 자동 부여 금지, 원문 실행식 누락 차단. 7코인/4증권의 실제 ApplicationServices → 재생 계산 → 저장 → PAPER 등록을 합성 시세로 시험했다. 외부 API/실주문을 시험한 것은 아니다.
- 실제 React/Chrome `check-optional-replay.mjs` **16경로**: 코인/증권 × 900/1440 × 승인/과거 평가 미달/과거 완료/PAPER 미달. 취소 요청 0, 명시적 PAPER 요청/live_confirmation false, 선택형 백테스트 별도 요청. 저장 API fixture, 실제 설치본/거래 워커 시험 아님.
- 기존 `check-strategy-repair.mjs` **6경로** 재통과: 실제 Python 컴파일러/검증기와 브라우저 등록, 명시적 새 기본 AI 버전, 미완성 보관. 기존 원문 전체 자동 복원 완료가 아님.
- Node 22.23.1 격리 의존성에서 TypeScript app/node 및 Vite **75모듈** 빌드 PASS. 기존 대형 JS chunk 경고 유지. 설치기/배포 자산은 만들거나 게시하지 않음.
- 인앱 매뉴얼 정본 **11탭** 재생성 및 문서 정합 검사 PASS. Level 5/날짜 선택/Drive 전체 수집은 미구현 계획으로 표시. 고객 원본 전략·DB·설정 및 기존 운영 권한은 변경하지 않음.

공통 호환성 전환·복합 원문 실행 조건 구현·전체 과거 원장 복구·Windows/실계좌/실수신은 여전히 열린 게이트다. 이번 결과는 선택형 평가 수정에 대한 증거이며 47 전체 배포 승인이 아니다.

## 2026-09-24 저녁 — v3.9.1.47 후속 수정 검증

- 전체 Python 회귀 **3,752 passed / 9 skipped / 3 subtests passed**, 최종 75.30초. 비교식 분류/통계 표시 집중 시험도 **91 passed**, 10.03초. 9개 제외와 실제 환경 미검증을 통과로 계산하지 않는다.
- 최종 모니터 로그의 추정 PnL/TP·SL 원인 단정 제거 후 관련 **57 passed**, 1.38초. 무포지션과 청산 원인·확정 손익을 구별한다.
- 고객 캐시 원문 응답 + 전체 DB 사본 재현: `scripts/verify_v39147_cached_customer_recovery.py data/260924_Teayu/trading.db --trade-id 57909`. 합성 손익을 넣지 않음. AVAX 개별 주문 6개 복구, 최근 300건 미확정 0, 실제 ProfitabilityValidator의 대조 차단 해소. 반복 실행 불변, 대상 외 거래 행 불변, 원본 파일 SHA-256 불변, quick_check ok.
- 전체 과거 Binance 원장 35건은 여전히 미확정. 그중 6건의 실제 캐시 추가 점검: 5건 `position_cycle_ambiguous`, 1건 `exit_lot_allocation_required`. 추정 배분을 확정 손익으로 바꾸지 않았다. 나머지 29건은 legacy 상태다.
- 고객 원문 55개 재분석: 원문 행별 미지원/정의 질문을 55개 문서 모두 제공, 실행 준비 완료 **0개**. 신규 등록 경로 수정과 모든 기존 문서 전략의 원문 복원은 다른 완료 기준이다.
- WebUI 실제 React/Chrome: 코인·증권 신규 등록/명시적 기본 진입 전환/미완성 보관 6경로. `check-paper-connection.mjs`는 11기관 × pending/error/success × 900/1440px = 66개 API 없는 PAPER 화면을 검사해 통과. 모의 응답/API fixture이며 실계좌/실제 워커 주문 시험 아님.
- TypeScript app/node, Vite 75모듈 빌드 통과(큰 JS chunk 경고 유지). 앱 매뉴얼 11탭 재생성 및 문서 정합 검사 통과. Windows 설치본/실계좌/실제 Telegram 수신은 수행하지 않았다.
- 원장 쓰기: 영향 주문/종목 선택, 25행 writer 양보, 주문 소유권 인덱스, 확정값 불변 시 UPDATE 생략. 고객 사본 대조 986건/0.029초·관측 최대 점유 20.72ms. 장시간/다른 하드웨어 성능 보장 아님.
- `scripts/verify_v39146_customer_storage.py data/260924_Teayu/trading.db`를 **현재 47 코드**로 재실행: 원본 3,229,663,232바이트/AI 판단 3,895,846행의 전체 사본, 11기관 합성 판단 1,100회 + 청산 20건 + 체결 20건 + 유지관리 9,100행/양보 9회, 4.03초. 최대 writer 점유 27.67ms/대기 70.08ms, 영속 재처리 대기/검토 0, quick_check ok, 원본 크기/mtime 불변. 네트워크 차단·실주문 0. Windows 장시간·물리 장애 시험은 아님.

미해결 항목은 UPDATE_PLAN과 V39147_RUNTIME_BOUNDARY_TEST_PLAN을 따른다. 다음은 수정 전 조사 이력이다.

## 2026-09-24 오후 실제 고객 반례 — 이전 PASS는 전체 해결 증거 아님

`260924_Teayu` 읽기 전용 감사: DB quick_check ok, 기존 58개 전략 중 55개 차단, 그 55개 원문 재분석도 전부 진입식 없음. 실제 최근 300건 조회에서 AVAX 1건 미확정; 저장된 거래소 응답으로 6+6 진입/12 청산의 `position_cycle_ambiguous` 재현. 당일 UNI 복구 성공 기록은 별도로 확인. Binance 미정의 모니터 함수 호출 14회 및 v46 다기관 write admission timeout 226행. 복구 저널 OperationalError는 상세가 없어 정확한 SQL 원인 미확정. 별도 DB 복제본에서 기관 전체 대조 3.89초/986행 갱신 관측.

집중 회귀 95 passed / 1 skipped (13.99초). 새 고객 사례는 일반 시험으로 커버되지 않았다. 원본 DB/전략/설정·API·주문·제품 코드는 변경하지 않음. 상세와 다음 검증 경계는 reports/v39145-cross-venue-feedback-audit-20260923.md 최상단. v47 전체 해결/배포 승인 아님.
...
v47 재현: `.venv/bin/python -m pytest -q` (73.69초), `tests/test_v39147_strategy_repair.py` 13개, `webui/qa/check-strategy-repair.mjs` 6경로. 원문 컴파일·저장 검증·승인·재로딩, JSON 숫자 동등성/실제 수정 차단, 명시적 기본 진입과 원문 실행 구분, 미완성 승인 차단, 버전 상한 초과 근거 보존, 복구 대상/실행 모드 불명확, 정상 위험 보류의 워커 장애 중복 알림 억제를 검사했습니다. TypeScript와 Vite 75모듈 빌드 통과; 큰 chunk 경고는 남습니다. 11탭 매뉴얼 내보내기와 문서 정합 검사 통과. 실제 API·주문·메신저 전송 없음. 9개 제외를 PASS로 계산하지 않습니다.
...
이전 검증의 누락: v45 관찰 시험은 정상 후보를 공급하고 감지 함수를 직접 호출해 사용자 시작 검사·실제 워커 기동을 통과하지 않았다. v46에서도 시작 경계/관찰/전송 계약 시험을 전체 고객 E2E라고 표현하지 않는다. [최신 실제 환경 체크리스트](V39147_RUNTIME_BOUNDARY_TEST_PLAN.md)의 미완료 상태를 배포 판단에 반영한다.
