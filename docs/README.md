...
> 기준: 2026-07-30 · 현재 공개 v3.9.0.5 Fix Patch 1 / 재빌드 대상 Update Patch 1  
...
Windows 공식 배포물은 Windows 빌드 환경에서 생성합니다. 현재 공개된 v3.9.0.5 Fix Patch 1 EXE는 이전 공개 자산으로 보존하고, 이번 Update Patch 1 manifest는 `pending_windows_rebuild`입니다. 새 EXE의 크기·SHA·서명·설치·로그인·자동업데이트 E2E를 검증한 뒤 Update Patch 1로 배포합니다.
