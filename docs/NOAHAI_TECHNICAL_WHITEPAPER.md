...
- NoahAI Client는 핵심 제품 검증을 완료하고 무료·유료 Windows 서비스로 운영한다.
- 코인·국내 주식·ETF와 NoahAI Strategy Studio는 현재 제공 중이다.
- NoahAI Strategy Hub는 무료 공개 테스트 중이며 유료 Marketplace·결제·제작자 정산은 후속 단계다.
- 생활금융은 현금흐름·목표·보안 경고·세금 계산·금융상품 비교부터 클라이언트에서 단계적으로 제공한다. 상품 비교는 정보·시뮬레이션이며 실제 가입·대출·보험·금융기관 거래를 실행하지 않는다.
...
- 대시보드 사용자 메뉴얼 11개 정본을 문서형 화면과 전역 본문 검색으로 제공하며, 정본 재추출·UTF-8·기관 등록부·버전 경계를 자동 검사한다.
- v3.9.1.31은 메뉴얼 UI 소스 후보이고 거래 수량·신호·가드레일·원장은 공개 v3.9.1.30 계약을 유지한다. Windows 설치·업데이트·DPI 검증 전에는 공개 완료가 아니다.
- Coinone은 시세·PAPER·통계·Strategy Studio 범위이고 LIVE는 실계정 E2E 전까지 차단한다. AlphaArena도 현재 PAPER 전용이며 LIVE 시작을 실패 폐쇄한다.

### 2026-09-12 v3.9.1.27 기술·Marketplace 경계

- Strategy Studio는 원본→규칙 근거, 사용자 확인, 버전별 검증, PAPER/LIVE 분리, 기관·통화·주문 귀속을 유지한다.
- Strategy Assistant는 탭별 문맥 연속성, AI 비용·사용량 설명, 역할별 모델 라우팅을 보강했다. 공급자 계정·할당량·실계정·장시간 환경 검증은 공개 자산 게시와 별도다.
- daltrading의 무료 전략 Hub는 가격 0 상품 스냅샷·취득·계정별 영구 비독점 라이선스·재다운로드를 구현했다. 테스트 포인트 1:1 구매가치 전환, 유상/적립 포인트, 유료 주문·환불, 판매자 자율 가격, 제작자 수익·지급 원장과 관리자 콘솔은 후속 단계다.
- 유료 Marketplace의 서비스 경계는 인증·상품·결제 webhook·복식 포인트 원장·라이선스·정산·분쟁을 분리하고 idempotency key와 불변 가격/버전/패키지 해시를 저장하는 것이다. 10,000,000P 이상은 개별계약형으로 분리하며 자체 웹 Marketplace와 Windows 클라이언트가 현재 출시 범위다. 세부 정책은 `docs/STRATEGY_MARKETPLACE_POINTS_AND_LICENSE_PLAN.md`가 정본이다.

### 2026-07-26 v3.9.0.2 AI 커스텀·어시스턴트

- 영상·문서·Pine·텍스트에 명시된 적용 국면과 제외 문맥을 근거와 함께 추천한다.
- 소스의 적용 국면과 운용 시점의 실제 시장국면 판정을 분리한다.
- 초보자는 AI 커스텀 화면과 XAI 결과를 NoahAI 어시스턴트에 바로 질문할 수 있다.
- 대화형 변경은 현재값, 재질문, 허용 범위, 변경 미리보기, 사용자 확인을 거친 일부 설정으로 제한한다.
- 모든 설정·주문을 자유 대화로 실행하는 단계는 아니며 거래 작업은 별도 권한·시장/계좌 검증·감사 계층이 갖춰진 뒤 확장한다.
...
### API 키 보안
- **사용자별 로컬 저장**: 거래소·증권사·AI 키에 같은 로컬 설정 정책 적용
- **전송 범위 제한**: 키는 사용자가 선택한 거래소·증권사·AI Provider 인증 요청에만 사용
- **로그 제한**: 키의 앞뒤 문자열을 로그에 남기지 않고 설정·백업을 지원 로그에서 제외
- **권한 최소화**: "읽기" + "거래" 권한만 부여
...
- **v3.9.1.21 PAPER 정합**: 키움·신한·미래에셋·한국투자 공통 경로에서 주식/ETF 보유 포지션의 현재가 기준 KRW gross/net PnL과 추정 수수료·세금·슬리피지를 갱신한다. 일부 청산은 진입비용을 수량 비율로 배분하고, PAPER 성과 표본은 해당 증권사의 유효한 가상 청산에만 격리한다. 이 계산은 실제 증권사 청구액이 아니라 사용자 설정 가능한 비교용 추정 계약이다.
...
- **응답 속도**: 실시간 시장 변화 대응
- **시스템 안정성**: 릴리스·운영 로그로 관찰하며, 산정 기간과 모니터링 근거가 없는 가동률 수치는 대외 확정하지 않음
...
- **거래소 지원**: 6개 거래소 어댑터·분기 구조와 회귀 테스트 (근거: `trading/unified_trader.py`, `api/binance_client.py`)
- **의사결정 지원 사례**: 실전 환경 의사결정 지원 사례를 통해 운영 안정성, 재현성, 로그 완결성을 검증 중 (근거: `log_system/*`, `trading/recorder.py`)
- **안정성 검증**: Binance API 정책 변경 등 외부 환경 변화 즉시 대응 (근거: v3.8.9.9 릴리스)
- **재현성 검증**: 지원되는 경로의 입력·결과·설정·기준시각 저장 범위를 확대 중 (근거: `trading/recorder.py`, XAI 정책)
...
### 1. 검증된 실전 환경 실증 시스템
- **비교 원칙**: 경쟁 제품 전체를 계획 단계나 프로토타입으로 일반화하지 않고, 공개 기능과 동일 조건에서 비교한다.
- **NoahAI**: **2024년 11월부터 운영 로그를 축적해 온 실증 기반 시스템**
- **실증 지표** (근거: 릴리스 히스토리, 운영 로그):
  - 2024년 11월부터 약 1년 이상 실전 환경 검증
  - 6개 거래소 어댑터·분기 구조와 회귀 테스트 (근거: `trading/unified_trader.py`, `api/binance_client.py`)
  - 실전 환경 의사결정 지원 사례를 통해 운영 안정성, 재현성, 로그 완결성을 검증 중 (근거: `log_system/*`, `trading/recorder.py`, 운영 기간 2024.11~)
...
### 3. XAI (설명 가능한 AI) 철학
- **NoahAI**: **근거·설정·차단 사유·변경 전후를 확인할 수 있는 설명 가능성**
- **XAI 구현**:
  - 지원되는 의사결정·실행 경로의 로그 기록
  - 실시간 AI 판단 근거 설명
  - 자연어 대화를 통한 검증
  - 로컬 저장으로 외부 검증 가능 (로그 완결성)

### 4. 실시간 적응형 학습·환류 시스템
- **NoahAI**: **기록된 판단·결과를 학습·환류 후보로 연결**
- **학습 특징**:
  - 강화학습·피드백 구성요소 (근거: `trading/ai/ai_manager.py`; 운영 효과는 별도 검증)
  - 시장 변화 즉시 반영 (근거: `trading/analyzer.py` 동적 임계값)
  - 개인별 맞춤 정책 생성 가능
  - 동적 임계값 보정 후보와 실행 경계 분리

### 5. 다중 자산 판단 인프라
- **NoahAI**: **현재 암호화폐·주식·ETF에 공통 판단 구조를 적용하며, 부동산·일반 금융은 확장 설계 범위**
- **역할**:
