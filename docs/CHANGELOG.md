...
- 2026-08-11 23:34 제보 화면의 창 제목은 `Fix 2`였습니다. 현재 공개 `deploy/release-manifest.json`도 빌드 자산을 `Fix 2`, SHA-256 `c1edaf84410f...`로 식별하므로 이 화면은 Fix 3/4 설치 검증이 아니라 공개 Fix 2 실행 증거입니다. Fix 2의 UI 수정이 충분하지 않았다는 사실과 이후 소스가 아직 Windows 자산으로 배포되지 않았다는 사실을 분리합니다.
- CustomTkinter 5.2.2가 `CTkComboBox`·`CTkOptionMenu`마다 Windows native `tk.Menu`를 영구 생성하는 구조를 Windows 전용 지연 메뉴 어댑터로 교체했습니다. 선택창을 누를 때만 프로세스 전체에 최대 1개 HMENU를 만들고 선택·닫기·다른 선택창 열기 때 즉시 폐기합니다.
- 기존 탭 파괴 시 메뉴 회수와 설정창 단일 인스턴스는 2차 방어로 유지합니다. 이제 한 정리 경로가 누락되더라도 드롭다운 수만큼 native 메뉴가 누적되지 않습니다.
- Windows USER/GDI 로그에 `TK_MENU` 실측값을 추가했습니다. 설정 열기와 서비스 전환 전후에 native Tk 메뉴 수를 함께 남겨 원인 계층을 구분합니다.
- 지연 생성되는 기능·거래소 탭도 idle 시 정본 순서를 다시 적용합니다. `금융 인텔리전스`는 항상 거래소/증권사 탭보다 앞에 위치합니다.
- 상단 `거래소: BINANCE` 단일 표기를 다중 범위에서는 `거래소: 6곳 · 기준 BINANCE`로 변경했습니다. 이는 수집·분석 범위와 현재 기준 거래소를 구분하며 실행 상태를 뜻하지 않습니다.
- 하단 `상태: 전체정지`를 `자동매매: 정지 (0/6 실행)`으로 변경했습니다. 거래소 설정/API 활성 여부와 실제 거래 워커 실행 여부를 혼동하지 않게 했습니다.
- 실제 CustomTkinter 드롭다운 500개 생성 시 유휴 native Tk 메뉴 `0`, 전체 설정창 생성·숨김·재열기 뒤 `0`을 확인했습니다. 관련 집중 회귀 `41 passed`를 통과했으며 Windows EXE 100회 설정/서비스 왕복은 재빌드 뒤 외부 게이트입니다.
- 현재 공개 Windows 자산은 여전히 Fix 2입니다. Fix 4 Windows EXE를 빌드하고 GitHub release의 EXE와 manifest SHA를 함께 교체하기 전에는 Fix 3/4가 테스터에게 배포됐다고 보지 않습니다.
...
## 2026-08-11 - v3.9.0.8 AI Custom Update Fix 4 · AI 호출비용·암호화폐 주문 명령 안전
...
- Windows 새 EXE·실거래소 client-order ID 수용·타임아웃 직후 재시작·주문/포지션 조정 E2E는 `pending_windows_rebuild` 외부 게이트입니다.
