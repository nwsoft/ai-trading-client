...
2026-09-26 최종 개발 보강: STATE 초기값/UPDATE 조건부 숫자 대입을 원문·공통 IR·Level 1~5·PAPER/기관 실행/재생에 연결. 완료 봉당 한 번, 이전 값 동시 갱신, 계정·기관·종목·버전·모드 격리와 재시작 보존을 적용하며 무단 코드 실행은 허용하지 않는다. 같은 진입 주문을 공유하는 원장도 원본 ledger_allocations가 있으면 정확한 수량/비용으로 복구하며, 같은 주문의 추가 체결·부분청산 교차를 처리한다. 원본 없이는 귀속을 추측하지 않는다. 병행 PAPER가 조건 불일치 후 기본 AI 후보를 사용자 전략 성과로 기록하던 오류와 증권 명시 청산의 완료 봉/상태 전달 누락도 수정했다. 기존 자료 보존·새 버전 승인·자동 LIVE 금지 유지. 최신 결과는 TEST_STATUS, 설치본 빌드·배포는 사용자가 수행한다.

2026-09-26 복구 수정: 원본에 진입별 청산 수량이 명시된 경우 다른 진입과 부분청산이 교차해도 공통 복구 경로가 처리하도록 보강. 종전에는 완전한 귀속 근거가 있어도 진입/청산 순서의 일괄 제약 때문에 `exit_lot_allocation_required`로 거절했다. lot별 진입 선행·수량·시각·비용 검증, 원자적 저장·중복 방지는 유지한다. 근거가 없는 FIFO 추정이나 아직 진입하지 않은 lot의 청산을 허용하지 않는다. 과거 전체 복구·임의 영속 변수·실계좌 검증 완료를 의미하지 않는다.

2026-09-26 추가 구현: 완료 봉 기간식 중첩·유한 latched 이름 상태(초기 false, 해제 우선, N봉 만료), 합성 300봉 한도·평가 메모이제이션. 국내 현물/증권 누락 청산 날짜별 탐색·체크포인트·보유/오늘 이력 재확인. Upbit/Bithumb 네이티브 종료/부분취소 주문 실제 체결·최종 비용, Coinone 체결 커서, 증권 날짜별 주문 조회. 공유 체결은 원본 진입별 수량 또는 동일 가격 완전 청산 근거에서만 배분하고 수량·비용 합계/원자적 저장/중복 방지, 원문 한 번 저장·해시 참조. Drive 공개 폴더는 앱 API 키, 제한 자료는 기본 브라우저 PKCE 승인·계정별 메모리 토큰 갱신·연결 해제. NoahAI 회원 로그인 변경 없음. 운영자 Google 앱 등록/검증·실계정 시험 필요. 무기한 임의 상태 프로그램·근거 없는 배분·모든 기존 전략 복원 완료를 뜻하지 않으며 빌드·배포·실주문 없음.

같은 후속 점검에서 발견한 실제 불일치: 따라 만들기의 '과거 시세 재생 검사' 버튼이 PAPER 시작을 호출하고 있었다. PAPER 단계 이동과 백테스트 선택 버튼을 분리해 표시/명령을 일치시켰다. 과거재생 OHLCV에 종료 시각이 없을 때 시작 시각을 검사 경계로 쓰던 1봉 지연도 명시적 판단봉의 종료 시각으로 보강했다. 이후 봉은 여전히 제외한다.

2026-09-26 제품 결정 명확화 (배포 버전 변경 없음): Level 5를 도입하며 향후 Level 6 이상도 추가 가능함을 정본에 명시했다. 현행 1~5 구현을 영구 최대치로 설명하지 않는다. 요청된 Drive 자료 수집을 '확장' 명목으로 47에서 임의 제외하지 않도록 완료 원장을 정정했다. 백서의 현행 후보/공개 버전을 코드/manifest와 대조하는 검사를 추가했다. 이 변경은 Level 6 구현·Drive 수집 완성·미지원 전략/거래 복구 완료를 의미하지 않는다.

2026-09-26 문서·검사 정합 정정 (배포 버전 변경 없음): 마스터 플랜의 Level 5 도입/미도입 충돌과 AI 커스텀 계획·아키텍처의 과거재생 강제→PAPER 문장을 현행 코드의 선택형 평가 계약으로 정리. 일반 LIVE/제한운용의 실행식·검증·승인 조건을 구분. 개발 미완료/사용자 정의·근거/실환경/확장/보장 한계를 기존 47 원장 한 곳에서 분류하고, 지정 현행 절의 정책 충돌을 탐지하는 문서 검사·회귀시험을 추가했다. 거래 엔진·사용자 데이터·권한·버전 변경이나 남은 기능 완료 선언이 아니다.

최신 후속: 누락 청산 자동 탐색을 OKX·Bybit·Bitget 선형 선물에 연결. 89일 내 하루 구간/100건 상한 분할/주문 완료 수량/양방향 구분/계약 크기/전후 anchor와 이후 체결 대조. 모든 API 호출은 읽기 전용이며 작업·페이지를 DB에 체크포인트한다. 공식 체결 CSV/TSV는 11기관에서 미리보기/열 매핑/시간대/원본 해시/본인 계좌·완전 구간 확인/중복 방지/원장 귀속 검증 후 복구 가능. API 키 없이 파일 복구 경로 지원, 자동 LIVE 시작 없음. 파일 한도 8 MiB·20,000행/계정 100 MiB, 빈 값·수식·모호 숫자·불완전 비용/세금/구간은 적용하지 않는다. `pnl_source`로 사용자 제공 파일과 API 사이클 계산을 구분한다. 아래 미지원 문구는 이 구현 이전 이력이다. 국내 현물·증권의 API 무제한 탐색/역선물/혼합 보유 자동 배분 완료를 뜻하지 않는다.

2026-09-25 공통 복구 후속: 타 10기관에서 이미 연결된 청산 주문번호 + 저장된 확정 체결로 누락 청산 시각을 복원하고 기존 주문별 비용/손익 검증으로 연결. 주문 귀속 충돌·불완전 수량·시간 역전/미래 시각은 차단. 오래된 수수료 0 기본값은 비용 증거로 사용하지 않으며, 시각 복원 직후 중단돼도 손익 NULL 상태를 보존해 재시작 후 재검증한다. 쓰기 우선순위는 유지관리로 낮추고 거래 원장 기록을 우선한다. 주문번호 없는 타 기관 자동 탐색·공식 파일 가져오기는 미완료다. 전체 해결/일반 배포 승인으로 해석하지 않는다.

2026-09-25 미청산 원장 시작 차단 수정: 위험 검사는 미청산 기록을 읽지만 복구는 exit_time 있는 기록만 선택하던 대상 불일치를 수정. NoahAI 소유 미청산 원장도 점검하며 Binance는 실제 전체 체결/주문/정산/안정된 포지션 근거로 청산 시각·비용·손익을 복구한다. 실제 보유는 별도 확인 상태로 보존. 기존 진행 작업도 범위 버전을 이관하며 새 작업에서는 미청산 기록의 오래된 조회 창을 갱신한다. 시작 시 해당 불일치가 확인되면 제한된 백그라운드 복구를 요청하고 시작은 보류한다. 복구 후 자동 LIVE 재시작 없음. 화면 원인·인앱 복구·검토 후 Q&A 진단 복사/AI 질문 연결. 다른 기관의 청산 시각 없는 이력 자동 탐색은 현재 미지원으로 명시하며 대상 0건/전체 해결로 숨기지 않는다.

2026-09-25 공통 셀프서비스 보완: 기존 전략 버전을 선택해 보완 인터뷰로 연결. 정의되지 않은 조건을 답변 불가로 분류하던 오류와 사용자 답변에 붙은 설명 때문에 비교식 정의가 인식되지 않는 UI 연결을 수정. 같은 이름은 한 번만 묻고, 원문/이전 성과를 보존하여 같은 전략의 새 버전으로 저장. 복구 화면에서 거래를 시작하지 않고 기관 연결을 확인할 수 있음. 특정 고객 데이터 직접 변경 없음. 미지원 실행 함수·공식 내역 가져오기 구현 완료를 의미하지 않음.

2026-09-25 실행 후속: 명시적 ENTRY/EXIT의 AND/OR·괄호·이름 정의·연쇄 비교를 기존 실행식으로 변환하고 미정의 항목은 원문 보존 확인 답변으로 보완. 필드 간 이상/이하 경계를 유지하며 잘못된 수치 비교 통과 방지. 공통 거래소 시작 실패 이유를 보존하고 런타임 거절 감사/화면 요청 ID를 연결. 복구 상태에 이유별 요구 근거와 다음 조치 제공. 기존 저장 규칙·과거 승인·성과 소급 변경 없음. 고객 55개 원문 전체 복원·전체 과거 35건 복구·실계좌·설치본 완료 아님.

2026-09-25 오류 안내 보강: 객체 형태의 오류 코드와 복구/엔진 연결/종료 상태를 해석합니다. 미분류 409는 반복 재시도 대신 상태·최초 로그·지원 코드를 안내합니다. 계정 응답 객체를 그대로 출력하지 않습니다. 전략 보완 대상이 있으면 목록에 공통 안내를 표시하고 보완 버튼은 기존 내용을 불러와 입력 영역으로 이동합니다. 매뉴얼·사용자/AI 안내 동기화. 실제 차단 원인 해소·전체 기존 전략 복원·배포 완료를 뜻하지 않습니다.

2026-09-25 추가: 기존 1~4 설정을 그대로 둔 `research=5` 프로필·공통 IR·Web/레거시 표시·동일 거래 비용 민감도 비교. WebUI 모든 Level에서 UTC 기간·종목·최대 보유 봉 가정을 선택한다. 7코인 과거 캔들 페이지 조회(워밍업 포함 5,000봉), 증권은 공급 가능한 일봉 범위만 사용하며 부족한 기간 대체 없음. 공개 API에서 Bithumb KST 커서/Bitget 종료 경계 차이 수정. 새 기록 복구 큐의 45일 제한 제거, 기존 작업 체크포인트 보존. 원문 질의 중복 ID 수정. 연구 기능은 LIVE 권한·손실 한도·과거 전략 의미를 바꾸지 않는다. 고객 55개 불완전 원문의 자동 복원과 Windows/실계좌 게이트는 미완료다.

2026-09-25 후속: Web/기존 데스크톱 과거 재생의 성과 미달을 가드레일 위반으로 기록하지 않도록 분리하고, 표본 없음/부족/평가 완료/수치 확인 필요를 기록합니다. WebUI는 승인된 실행 가능 전략의 PAPER 시작을 기본 진행으로, 백테스트를 선택형으로 표시합니다. 기존 저장 상태·원문·IR·성과·실행 권한을 소급 변경하지 않습니다. 최대 12봉 등 재생 가정을 표시합니다. Level 5/날짜 지정/Drive 폴더 연결/모든 기존 전략 자동 복원은 아직 계획입니다. 시험 범위는 TEST_STATUS 최신 항목을 따릅니다.

저녁 후속 수정: 합산 진입 후 동일 가격의 단일 완전 청산은 정확한 진입 주문별 손익·비용으로 복구합니다. 전체 청산 손익을 한 거래에 복제하지 않으며 배분 근거를 보존합니다. 체결 저장의 전체 과거 대조를 대상 주문 중심/25행 유지관리로 줄이고 DB busy 재시도를 보강했습니다. Binance 비동기 모니터 시작·중복 방지·인계를 수정하고, PAPER 가상 포지션을 계좌 API와 분리했습니다. 전략 ENTRY/EXIT 문단·소수 비교식 및 미지원 원문 행별 질문을 보강했습니다. **기존의 모호한 전략 55개가 모두 실행 가능해진 것은 아닙니다.** 원문 정의/미지원 함수와 Windows·실계좌 검증은 별도 남아 있습니다. 최신 결과는 TEST_STATUS 최상단을 따르며 아래 시험 수치는 이전 시점입니다.
...
- 부분청산·분할 진입 일괄 청산·위험/증권 실행 품질·주문 명령 갱신 재처리 확대. 최초 청산 시각·전략 귀속·확정 주문 상태 보존. 진입 수수료 미기록은 0으로 인증하지 않음. 진단 정리의 과거 거래/위험 원장 삭제 제거.
- 신규 진입 공통 경계는 가용 공간 64MiB 미만/확인 실패 시 보류. 보호·청산 경로는 유지. 물리 장애나 디스크 전체 고갈의 무손실 보장은 아님.
- 전략 자료를 최대 40개·파일당 24MiB·합계 64MiB로 통합 입력. 로컬 폴더·XLSX/DOCX/CSV/TSV 추가, 자료별 포함 범위·중복·축약 표시 및 불완전 근거 실행 차단. Google Drive 폴더 자동 탐색은 미지원.
- 증권 진입/청산 양쪽 정확한 주문 근거 복구와 Coinone native 체결 페이지/주문 상태 정규화 보강. 증권 과거 lot 귀속 전체 복구 및 Coinone LIVE 개방 완료는 아님. 실제 Coinone 조회는 허용 IP 제한으로 거절됐으며 실주문 없음. 최신 시험/잔여 게이트는 TEST_STATUS/UPDATE_PLAN 참조.

### 2026-09-24 공통 저장·조회 후속 수정

- Recorder 쓰기 경로에 계정 DB별 프로세스 내 우선순위를 적용했다. 거래 기록을 판단/유지관리보다 우선하고 읽기는 직렬화하지 않는다. 유지관리 이관은 100행/시간 예산 단위로 양보하며 미처리 행을 건너뛰지 않는다. 반복 초기화 때 이미 완료한 수수료 통계 이관을 다시 합산하던 경로도 일회성 표식으로 제한했다.
- 영속 재처리를 거래소 체결 내역·주문 확인 결과·정확한 주문 ID가 있는 진입 기록으로 확대했다. 재처리 영수증을 본 기록과 같은 트랜잭션에 저장하고 미완료 LIVE 진입/청산은 위험 판단에서 누락하지 않는다. 디스크 전체 고갈과 모든 기록 경로의 복구를 보장하는 변경은 아니다.
- 이미 확인된 FILLED/종료 상태를 늦은 NEW 응답이 덮어쓰지 못하도록 상태와 원시 근거를 함께 보존한다. 이는 보호주문 접수를 보호 완료로 간주하는 변경이 아니다.
- Bybit/OKX/Bitget의 포지션 조회 실패와 실제 빈 계좌를 분리하고 관리 수량·방향을 확인한 뒤 미실현 손익을 사용한다. Binance의 과거 미청산 원장/실제 보유 없음도 확인 필요로 남긴다. 증권 4기관에는 명시적 조회 결과 계약을 연결해 화면·포트폴리오·청산 관찰에서 실패를 보유 0으로 표시하지 않는다.
- 유지관리 화면에 이번 실행의 DB 최대 점유/대기 시간을 표시한다. 활성 전략의 실행 조건 부족은 보관 버전 안내와 구분한다. 최신 시험/미완료 범위는 TEST_STATUS와 릴리스 검증 원장을 따른다. 아래 수치는 9월 23일 당시 시험 이력이다.

### Teayu 공유본 재현 후 추가 수정

- Coinone LIVE는 현재 기관 지원 계약상 미지원이다. 사용자가 승인할 수 없는 `coinone_live_e2e_verified`를 일반 설정에서 제외하고, LIVE 시작 버튼에 미지원 상태를 표시한다. 409의 실제 사유를 설명하며 PAPER·분석/학습 경로와 구분한다. 실계좌 검증을 완료한 것으로 꾸며 LIVE를 개방하지 않았다.
- AI 판단·정확한 진입 주문 ID가 있는 청산 기록의 SQLite 잠금 실패를 별도 로컬 재처리 DB에 보존한다. 백그라운드/앱 재시작 후 재시도하며 본 원장의 동일 트랜잭션 영수증으로 중복 적용을 막는다. 같은 종목 재진입/다른 기관 원장은 수정하지 않는다. 기존 청산과 충돌하면 보존·대조 필요로 표시한다. 대기 청산은 확인 전 신규 진입 위험 판단을 보류한다. 모든 저장 경로의 잠금 제거 또는 디스크 장애 복구 보장을 뜻하지 않는다.
- 국내 현물 관리 원장 수량과 현재 계좌 보유 수량을 대조한다. 보유하지 않은 과거 원장을 미실현 손실로 확정하지 않으며, 불일치는 위험 데이터 확인 필요로 보존한다. 과거 손익/일일 기준을 초기화하지 않는다.
- 원문 근거가 있는 사용자 수정 전략의 실행 진입식이 비었을 때 기본 진입으로 통과하던 경로를 차단한다. 원문을 검토해 실행 조건을 갖춘 새 버전을 검증해야 한다. 원문 없는 기존 기본 진입+위험값 전략은 별도 계약으로 유지한다.
- 고객 자료 읽기 전용 재현 포함 Python 전체 3,600 passed / 8 skipped / 3 subtests passed. 실제 계좌 응답·Windows 설치본·실제 알림 수신은 미검증이며 Coinone LIVE 지원 완료가 아니다. 상세 제한은 점검 보고서와 TEST_STATUS 참고.

### 학습 저장소 대용량 후속 구현

- 기존 미압축 학습 보관 파일을 스트리밍 gzip·압축해제 SHA-256 검증 후 대체하는 유지관리 경로 추가. 현재 JSON/저널/거래 원장/오늘 보관 파일 제외. 실패·변경 감지·충돌 시 원문 보존, 계정별 OS 잠금과 중단 임시파일 회수.
- 신규 XAI의 중첩 전략 규칙·출처 근거를 해시 참조로 공유하고 압축. 기존 큰 근거는 재개 가능한 배치 변환, 기존 이벤트/세그먼트 해시·조회 결과 유지. DB의 확보 페이지는 재사용하며 온라인 VACUUM은 하지 않음. 압축 XAI DB의 구버전 직접 열기는 지원하지 않음.
- 유지관리에 과거 파일 대기·처리 중 진행량·실제 확보 용량·8 GiB 경고 표시. 경고는 전체 저장소 자동 삭제 상한이 아니며 주문/PnL/학습 규칙·자동 LIVE 시작 변경 없음.
- Teayu 보관 파일 사본 1,133,898,244바이트 → 112,160,234바이트(원문의 9.89%), 해제 SHA-256 일치. 고객 원본 변경 없음. 모든 사용자 폴더가 같은 압축률이라는 뜻은 아님.

### 국내 현물 잔여 자산 평가와 거래 중단 후보 구분

- 업비트·빗썸·코인원 위험 평가에서 가격 조회 실패를 모두 0으로 합치던 경로를 분리. 성공적으로 갱신한 마켓 목록에 없는 자산만 관리 원장과 대조하며, 비관리 잔여 자산은 수량과 미평가 상태를 보존한다. 0원·상장폐지로 단정하거나 계좌 총평가액이 확인됐다고 표시하지 않는다.
- KRW 직거래가 없어도 같은 거래소의 BTC/USDT 등 지원 마켓과 KRW 환산 경로를 확인. 조회 실패·명시적 중단 마켓·관리 포지션 가격 누락·DB 실패는 계속 신규 진입 보류. Coinone CCXT의 active=None은 미지원으로 오인하지 않으며 실제 가격 근거로 평가하되 거래 가능 판정으로 사용하지 않음.
- 기존 일일 손실 기준값·확정 손실·원장은 유지. 기존 실행 중인 엔진의 다음 위험 점검에 적용하며 앱 업데이트로 거래를 시작하지 않는다. 후보 사전 필터는 명시적으로 비활성인 마켓과 현물 비KRW/파생 마켓을 제외한다.
- [재현·수정·검증 경계](../reports/v39145-cross-venue-feedback-audit-20260923.md). 고객 실제 API/Windows 설치본 검증은 별도이며 학습 저장소 50GB 문제의 수정 완료를 의미하지 않는다.

### 2026-09-23 — v45 이후 피드백 수정 (후속 패치 소스, 미배포)

사용자 확인 기준 v45는 공개 상태다. 오늘 변경은 v46 / updater 3.9.146으로 분리한다. 앱·빌드 메타데이터와 매뉴얼은 v46 후보이며 기존 공개 v45 manifest·설치기·해시는 보존한다. 설치기 빌드·게시·실메신저 검증은 수행하지 않았다.

- 기관별 LEARNING/PAPER 시작에 LIVE 조건이 잘못 적용되는 문제와 국내 현물 공개 시세 연결을 수정. 실제 LIVE 확인·Coinone LIVE 승인 조건은 유지.
- 수동 코인 분석의 선택 거래소 전달, factory 연결/실행 로그 모드, 국면 관찰/알림 제외·전송 진단을 보강. 수신 ON이 자동 실행을 뜻하지 않으며 기존 국면 판단 기준은 유지.
- 신규 LIVE 원장에 전략 key/version 저장. 증권·ETF 부분청산은 진입 lot의 전략 버전을 보존. 과거 미기록을 현재 전략으로 채우지 않음.
...
공개 v3.9.1.44 이후의 오늘 변경을 v45 / updater 3.9.145로 구분한다. 공개 v44 manifest·설치기·해시는 보존하며 같은 태그를 덮어쓰지 않는다.

- Web UI 코인·주식/ETF 어시스턴트와 스튜디오에 외부 AI 전략 상담 연결. 개념 공부와 맞춤 설계를 구분하고, 사용자 원문 조건 확인·한 번에 하나씩 되묻기·조건 미확인 시 초안 전달 보류를 추가. 예산/위험/기간/레버리지·시장 근거·XAI·실패 조건을 설명하며 유명 전략의 공개 원칙/해석/변형과 최신 순위 미조회 경계를 명시한다.
- 확인된 상담 초안은 사용자 검토 후 기존 자료 분석·컴파일·버전 승인·PAPER 경로를 사용한다. 빈 스튜디오로도 전달 가능하며 기존 원문은 보존한다. 자동 저장·승인·거래 실행, 보호 우회와 실시간 웹 검색 추가는 없다. 매뉴얼·사용 안내에 비용/개인정보/지원 범위와 사용 순서를 반영한다.

- 7개 거래소 공통 학습 저장 경로에서 분석 결과의 RSI·MACD·추세 등 구조화된 지표가 제외되던 문제를 수정. 새 기록은 판단 시점 값을 보존하고 JSON/SQLite 조회는 기존 중첩 지표도 읽는다. 원래 없던 과거 값은 추론 문장·현재 시세·중립값으로 복원하지 않는다.
- 코인 선정의 기술점수는 15분·1시간 RSI 근거가 없으면 미산출과 이유를 표시. 후보 선정의 추가 네트워크 호출을 되살리지 않으며 60점 기본값·부분 RSI의 50 대체도 제거. 기술점수와 종합점수는 별도이고 선정·매매 가중치를 변경하지 않는다.
- 아래 시장국면·알림 변경을 v45에 포함한다. 시작 경로와 반복 경로의 최소 유지시간 설정 해석을 일치시킨다. 기본 300초 관찰·2회 확인·600초 유지, 0초 유지는 일반 사용자 권장값이 아니다.

### 2026-09-22 - 시장국면 수신 모드·시세 입력 검증 (기존 v44 공개 자산에 미포함)

- 알림·리포트에 시장국면 PAPER/LIVE/학습·관찰 수신 선택 추가. 누락된 기존 설정은 모두 수신, 모드별 중복 억제 키 분리, 큐 전달 시에도 필터 재확인. LIVE 위험 경고와 다른 이벤트에는 적용하지 않음.
- Binance·공통 6거래소·4증권사 국면 알림에 실제 실행 모드 전달. 전략 실행 범위·후보 복구 최적화·국면 임계값은 유지.
- 빠른 코인 국면 계산의 누락/비정상 거래량을 0으로 사용하지 않고 unknown 처리. 실제 0 거래량은 보존. 완전 평탄 구간 RSI를 100이 아닌 중립 50으로 수정.
- 최종 감사 추가: 4증권사 공통 국면 입력에서 boolean 등락률을 거부하고 중복 날짜·누락 종가를 5거래일 근거로 사용하지 않음. 실패 이력은 유효한 지수/대표 종목 조회로 내려가며 모두 없으면 unknown. 코인 최소 유지시간의 명시적 0 설정을 보존하며 기본 600초는 유지.
- 알림의 비정상 JSON·빈 200 응답을 성공으로 처리하지 않고, 한 채널의 예기치 않은 오류가 다른 채널 전송을 막지 않도록 격리. 원문 비밀값 없이 오류 유형만 기록하며 제한 재시도·계정 격리는 유지.
- 8개 발송 유형, 전략별 market/symbol/both/none과 기관 시장 알림의 차이, 미수신 조건을 사용자 가이드·인앱 매뉴얼·설정 도움말에 반영.
- 소스 후보는 v45로 올렸으며 설치기 빌드·게시 전이다. 기존 공개 태그를 덮어쓰지 않음. [시험 및 검증 경계](../reports/market-regime-notification-audit-20260922.md).
...
- ai_decisions 정식 계약 버전·기관·모드·자산/종목 유형·시각·전략·판단·사유·실주문 컬럼과 인덱스, 과거 근거 충돌 시 unknown 유지, 재개 가능한 배치 이관.
- 신규 기관/모드 충돌은 저장 전 검증하여 원문·사유를 별도 보존하고 정상 학습에서 제외. 공유 Recorder 기관 변경과 학습 기관 덮어쓰기를 제거. 증권 청산 XAI에 실행 모드·종목 유형·주문 결과 연결, 응답 실패만으로 실주문 없음이라고 단정하지 않음.
- 완전히 같은 근거의 HOLD만 1분 구간으로 합산. 주문·원장 연결·차단·위험은 개별 기록 보존, 리포트 판단 수는 repeat_count 반영.
- 계정별 SQLite 증분 학습 이벤트, SHA-256 XAI 근거 중복 제거, 확인 후 원자적으로 이관하는 일별 gzip 세그먼트. 기존 JSON 원본 보존 및 재실행 중복 방지.
- 계정·기관별 학습 매니저 공유와 기관/종목 인덱스 조회. 정상 런타임의 매 100건 전체 JSON 재작성을 제거.
- Binance `Trader`, OKX·Bybit·Bitget·국내 현물 `UnifiedTrader`, 키움·신한·미래에셋·한국투자 `StockAnalysisService`가 공통 `LEARNING/PAPER/LIVE → 판단 → 차단/주문 → 체결/청산 → 원장 → XAI` 외곽 계약을 사용. 주식·ETF의 시장시간·수량·세금·ETF 지표와 현물·선물 규칙은 자산별 근거로 유지하며 미확인 주문/모드는 추정하지 않음.
- 증권의 `live_api`/`mock`을 각각 `LIVE`/`PAPER`로 정규화하고 자동거래·자동청산 XAI를 분석 조회와 통합. 증권 화면은 종목 유형·모드·판단 근거·사유·실제 주문 여부를 함께 표시.
- 메인/기관 로그 라우팅 중복 완화, 512 MiB 진단 예산, 영구 중요 이벤트 원장, 24/48/72시간 DEBUG 만료 지원(API), UI는 24시간 켜기/끄기.
- 닫힌 날짜형 진단 로그는 예산 80%부터 제한된 백그라운드 작업으로 압축·해시 검증 후 대체. 공간 확보 후 기록 재개, 쓰는 중인 파일은 제외. 보관 로그 용량·압축 오류·기록 충돌 수와 사용자가 필요한 조치를 한/영 유지관리에 표시.
- 설정 → 업데이트 → 유지관리 · 저장소: 용량·마지막 압축·DEBUG 만료·오류·백그라운드 최적화/재개. 거래/PAPER 원장 삭제·LIVE 시작·PnL 보호 우회 없음.
- [검증 보고](../reports/v39144-storage-verification.md), [완료/미완료 게이트](V39144_STORAGE_TEST_PLAN.md). 이번 작업에서 설치기 빌드·공개 배포는 실행하지 않음.

## 2026-09-20 - v3.9.1.43 Record Recovery Maintenance and Strategy UX Patch (소스 후보·배포 보류)

- 설정 → 업데이트 → 유지관리 → 거래 기록 점검·복구. 거래 통계에서도 같은 점검을 연다. 최근 45일 미확정 LIVE 기록을 최근 300건 제한 없이 점검한다.
- 계정별 직렬 작업, 초기 SQLite 백업, 작업별 변경 근거, 재시작/조회 예산 후 이어서 실행, 실패 쿨다운, 한국어/영어 상태 안내를 추가했다. 매 거래 주기 전체 복구는 실행하지 않는다.
- Binance 주문별 과거 날짜 조회, 이미 저장된 보호 주문 연결, 독립 진입 주문의 누락 수수료 복구를 보강했다. 표적 행만 재대조하여 fill 저장마다 전체 기록을 다시 훑지 않는다.
- 후속 수정: 청산/보호 주문 ID가 없어도 Binance 과거 주문·체결·정산 원장을 수집하고 포지션 수량 구간을 재구성해 실제 청산을 복구한다. 체결별 실현손익 교차 대조, 진입/청산 비용, 여러 청산 주문, 원장 저장 검증과 fill 귀속 중복 방지를 포함한다. 요청 구간 저장·포화 구간 분할·재점검 시 지연 자료 갱신·화면 내 자동 이어하기를 지원한다.
- 복구된 UTC 시각과 기존 지역/offset 시각이 섞여도 최근 성과 표본을 실제 시간순으로 선택하도록 정렬을 보강했다. 첫 거래/두 거래의 실현손익·순손익이 기존 거래 통계 조회에도 표시되는지 검사했다.
- 증권 LIVE 성과 튜닝이 미확정 표본을 제외한 일부 결과만으로 승률을 계산하지 않도록 보강했다.
- 고객 DB·로그 재검증에서 발견한 `save_exchange_trade_stats`의 저장 완료 후 `inserted_id` 미정의 예외/실패 반환을 수정했다. 11개 기관의 현재·이전 통계 스키마 22개 회귀시험을 추가했다. 이 수정과 과거 40건의 청산 주문 근거 복구는 별개다.
- **검증 경계:** 청산 ID 없는 Binance 복구의 API 응답 계약 시험은 통과했으나 고객 실계정 40건의 복구 결과는 아직 받지 않았다. 혼합 진입의 귀속 배분, 공식 파일 가져오기, 계좌 전체 일별 대조/조건부 재개, 증권사 과거 체결/세금/lot 어댑터는 미완료다. [구현·검증·남은 배포 조건](V39143_RECORD_RECOVERY_PLAN.md).

- 전략 스튜디오 난이도 설정을 AI 엔진/API의 실제 항목으로 바로 연결하고 포커스합니다.
- 설정·스튜디오 Level 1 (초보자)~Level 4 (실험실) 표기를 통일하고 저장 즉시 열린 코인/주식 스튜디오에 반영합니다.
- PnL 보호 정책은 유지합니다. 공유 DB의 과거 40건 주문 근거 누락은 미복구이며 이 패치를 손익 문제 완전 해결로 안내하지 않습니다.
- daltrading 영문 홈·내비게이션, NoahAI Labs 회사 설명·한국어 줄바꿈은 별도 웹 수정입니다.
- [진단·사용자 답변·검증 경계](V39143_FEEDBACK_REVIEW.md). 공개42 이후 변경이며 설치기 빌드·릴리스는 사용자가 수행합니다.
...
9월 19일 DB 확인 후 추가: 최근 300건 중 LIVE 미대조 40건의 청산 주문 ID 누락을 확인했습니다. 보호 주문의 진입 연결/실제 주문 ID 재대조, 현재 계좌와 과거 미청산 구분, 동일 API 연결의 일일 위험 기준값 영속화, 주문별 실제 fill 보존, 수수료 환급·누락값·순손익 기반 통계/성과 튜닝을 보강했습니다. 이전 PAPER 격리·일일 날짜 조회·원격 UI 수정도 유지합니다. Client 2,741 passed / 8 skipped / 3 subtests. **기존 미대조 기록의 안전한 업데이트 처리·실기관/Windows·일별 계좌 대조가 남아 배포 보류**. 특정 사용자 DB 복구는 필수 조건이 아닙니다. [조사 및 남은 작업](V39141_PNL_GUARDRAIL_FEEDBACK.md).

- Client 성공 학습·추론 60초 집계와 서버 지원 협상, 서버 5분 버킷·가중 평균·중복 방지. 인증/KPI DB 분리 도구와 성공 텔레메트리 30일 외부 보관·장기 요약·검증 복원. 거래/위험 원장 제외.
- 모바일 daltrading의 내 PC 상태 화면, 기기 토큰 해시·소유권·세션·CSRF 검증, 오프라인 및 명령 만료 표시.
- PC에서 명시적으로 허용한 기관별 신규 주문 제출 일시정지. 처리 중 주문·중복 요청·재시작·PC 재개 경계, 7개 거래소와 4개 증권사 공통 주문 경계 회귀. 이미 제출된 주문은 체결 가능하고 보호·청산은 유지.
- 원격 시작·재개·전량 청산·PnL 상세 공유는 미제공. 실제 운영 이관·Windows·외부 복원·300PC 장시간 증거는 별도 미완료.

현재 소스 후보 **v3.9.1.41** · updater **3.9.141**. 현재 공개 stable/latest는 **v3.9.1.40**이며 기존 공개 자산은 보존합니다.

- 시장 트렌드를 오늘·7일·30일 기간별 대시보드로 바꾸고, 시장 폭 원형 그래프·방향/변동성 막대·종목별 미니 추세선을 추가합니다. 원형은 상승·중립·하락 구성 비율에만 사용합니다.
- 코인 롱/숏 비율을 실제 공포탐욕지수처럼 표시하던 명칭을 제거했습니다. OI·청산 규모·BTC 도미넌스와 주식 수급·환율·금리·공시/뉴스는 정본 연결 전 추정하지 않습니다.
- 코인 7개 거래소의 현물/선물·KRW/USDT 캔들 범위를 분리하고, 주식 7일·30일은 선택 증권사의 일봉을 종목별 순차 조회합니다. 일봉 실패를 당일 값으로 대체하지 않습니다.
- 시장 트렌드 화면 근거를 AI 어시스턴트에 전달합니다. 로컬 XAI는 지지·반대·무효화·누락 데이터를 구분하며 관찰 후보를 개인화 추천이나 주문으로 승격하지 않습니다.
- 시장 트렌드 변경 직후 기록: Python **2,639 passed / 8 skipped / 3 subtests**, Node **43 passed**, Web **63 modules**, 격리 브라우저 오류 0건. 추가 KPI/원격 변경 후 최신 결과는 위 검증 보고를 따릅니다. Windows v41 설치본·실기관·업데이트 검증은 미완료입니다.
...
- [v40 검증 기록](V39140_STUDIO_ALPHA_SETTINGS_TEST_PLAN.md)
...
- 상세 자동·외부 검증 항목: [V39139_EXTERNAL_CLOSE_RECONCILIATION_TEST_PLAN.md](V39139_EXTERNAL_CLOSE_RECONCILIATION_TEST_PLAN.md)

- 전략 스튜디오 초보자 보완: Level 1 원문 규칙 풀이, 자료의 성과 발췌와 자체 검증의 구분, 실제 분석 문맥을 전달하는 결과 질문, 로컬 설명/외부 심층대화 구분, 접힌 항목 도움말 및 차트 따라 보기 매뉴얼을 추가했습니다. [사용법](STRATEGY_STUDIO_BEGINNER_GUIDE_V39139.md). 실행·승인·PAPER/LIVE 권한은 변경하지 않습니다.
- 초보자 보완 후 로컬 회귀 **2,616 passed / 8 skipped / 3 subtests**, 설명·차트 Node **33 passed**, Web **63 modules** build 통과. 코인·주식 설명/질문/재분석과 코인·주식·ETF 차트 격리 브라우저 오류 0. 실제 Provider·영상 수집·Windows·계정 검증을 대신하지 않습니다.
...
## 2026-09-18 - 제품·서비스 상태 및 공개 문서 정합

- NoahAI Client를 `검증 완료 후 무료·유료 서비스 중`으로 정본화했습니다.
- 코인·증권·ETF 제공 상태와 기관·계정·모드별 PAPER/LIVE 준비도를 분리했습니다.
- Strategy Studio는 현재 제공 중, Strategy Hub는 무료 공개 테스트 중, 유료 Marketplace·결제·정산은 후속 단계로 통일했습니다.
- 생활금융은 현금흐름·목표·보안 경고·세금 계산·금융상품 비교부터 단계적으로 제공 중인 서비스로 갱신했습니다.
- 금융상품 정보·시뮬레이션과 실제 가입·심사·실행의 책임 경계를 유지했습니다.
- README, 문서 인덱스, MASTER, UPDATE_PLAN, 개발·문서 정책, 생활금융, Strategy Studio/Hub, 대외 사업 정본을 같은 기준으로 동기화했습니다.
- GitHub v3.9.1.38 stable 릴리스의 설치기·blockmap·`latest.yml`·manifest 게시를 확인하고, 환경별 실계정·특수 DPI·장시간 무결함 검증은 별도로 유지했습니다.

## 2026-09-17 - v3.9.1.38 전략 과거재생·모드별 거래내역 배포 후보 및 공개

당시 소스 후보 버전: **v3.9.1.38** · updater **3.9.138**.

당시 공개 버전: **v3.9.1.37** stable/latest. 이후 v3.9.1.38 설치기·blockmap·`latest.yml`·manifest가 stable/latest로 게시됐습니다.

공개 v37 이후의 아래 변경은 v38 새 설치기·blockmap·`latest.yml`로만 배포하며 v37 자산을 교체하지 않습니다.

### 전략 스튜디오 과거재생 시각화

- 실제 검증 캔들·LONG/SHORT 진입/청산 타점·청산 기준 누적 수익률·거래 내역 연결을 추가합니다. 코인 및 주식/ETF 비용 계약을 유지합니다.
- 차트는 로컬 검증 근거이며 여권 등급·PAPER/LIVE 권한을 승격하지 않습니다. 상세 차트 스냅샷은 공유 패키지에서 제외합니다.
- 기본 전략 시연에서 발견한 수익률 축 정밀도 오류, 집중된 타점 이름 겹침, 과도한 가격 소수점을 수정했습니다. 저가 코인 표시는 0으로 뭉개지 않으며 선택 거래의 계산 원본을 따로 확인할 수 있습니다.
- 주식·ETF에도 관리형 `추세 따라가기` 현물 LONG 파생본을 500개 가상 일봉으로 실행해 네 증권사 × 두 자산의 8개 서비스 경로를 검사했습니다. 이는 실제 시장 성과나 실계정 연결 증거가 아닙니다.
- [설계·검증·배포 경계](STRATEGY_REPLAY_VISUALIZATION_20260917.md) · [주식/ETF 500봉 검증 증거](../reports/stock-500-preview-20260917/README.md)

### 기관별 LIVE/PAPER 거래내역

- 기관별 좌측 거래내역을 실제 PAPER/LIVE 모드에 따라 전환합니다. LIVE는 계정·기관별 저장된 종료 기록 최대 50건, PAPER는 기존 검증 원장을 별도 탭에서 조회합니다. 기록·검증 상태·실행 권한은 변경하지 않습니다.
- 거래내역은 LIVE/PAPER 모두 기본 접힘으로 시작하며 기관·실행 모드 전환 시 다시 접힙니다. 이력 탭 선택은 실제 운용 모드나 위쪽 포지션·통계의 모드를 변경하지 않습니다.
- 7개 코인 거래소와 4개 증권사(주식·ETF)의 공통 읽기 경로를 사용합니다. 기관 별칭을 제한 전에 필터링하고, 미확정 손익·통화 불명·외부/가져오기 기록·조회 실패를 확정 수익이나 0거래로 바꾸지 않습니다. 실제 기관 전체 체결 수집 완료를 뜻하지 않습니다.
- [데이터·표시 계약과 검증](SOURCE_TRADE_HISTORY_20260917.md) · [v3.9.1.38 검증·배포 원장](V39138_STRATEGY_REPLAY_LIVE_HISTORY_TEST_PLAN.md)

### 검증·배포 경계

- v3.9.1.38 버전 승격 후 전체 Python **2,572 passed / 8 skipped / 3 subtests**, 관련 집중 **70 passed**, Node **22.23.1** Web production **61 modules**, 11기관·과거재생 격리 브라우저 및 문서 정합 검사를 통과했습니다.
- 변경 파일 38개를 명시한 오프라인 prekey 릴리스 게이트도 통과했습니다. 실제 게시 전에는 Git 작업트리에서 동일 변경과 source revision·태그를 다시 증명해야 합니다.
- Windows x64/x86 새 빌드, v37→v38 업데이트, 실제 기관 계정과 키움 OCX, 장시간 PAPER는 미완료입니다.

## 2026-09-17 - v3.9.1.37 키움 조회 대기·실패 상태 수정 (공개 기록)

v3.9.1.37은 stable/latest로 공개됐습니다. 아래 후보·공개 v36 표기는 작업 당시 기록이며 현행 판단에는 사용하지 않습니다.

- 37부터 릴리스 태그를 실제 빌드 소스 커밋에 연결하도록 게시 전 검증과 명시적 `--target`을 추가합니다. 목록/Atom/Latest/업데이트 메타데이터의 게시 후 일치를 검사하며 기존 36 이하 태그는 보존합니다.

- 2026-09-17 추가: 업데이트가 옛 v3.9.0.10/latest.yml을 선택하던 prerelease 옵션을 제거하고 stable/latest·다운그레이드 차단을 적용했습니다. 오류 문구를 개선하고 실제 GitHubProvider 회귀를 추가했습니다. [원인과 검증](V39137_UPDATER_CHANNEL_INCIDENT.md).

당시 소스 후보 버전: **v3.9.1.37** · updater 3.9.137. 당시 공개 기반: v3.9.1.36 stable/latest.
...
- 키움 RPC timeout/EOF/응답 불일치 뒤 전용 호스트 자동 재시작과 중복 로그인을 차단하고 명시적 disconnect 뒤 수동 재연결만 허용합니다.
- 키움 수동 재연결 필요 상태의 같은 반복 알림을 억제하되 최초 오류와 호스트 진단 로그는 보존합니다.
- 네 증권사의 열린 주식/ETF PAPER 포지션과 비용·전략 버전 식별자를 계정별 원자적 저장소에 보존·복구합니다. LIVE/손상 행은 복구하지 않습니다.
...
현재 소스 후보 버전: **v3.9.1.35** · updater **3.9.135**. 현재 공개 버전: **v3.9.1.35** stable/latest. Windows 자동업데이트 자산과 macOS 수동 테스트 자산을 같은 릴리스에 게시했으며, 소스 수정·자동 테스트와 플랫폼별 실제 설치·연결 검증을 분리합니다.
...
- Windows·macOS 빌드 폴더와 manifest는 분리하되 같은 제품 버전의 GitHub Release에 함께 게시. v3.9.1.35는 Windows v3.9.1.34가 자동업데이트로 찾을 수 있도록 stable/latest로 전환했습니다. macOS 자산은 Developer ID 서명·공증 전 수동 테스트용입니다.
- 검증 정본: [3.9.1.35 검증 계획](V39135_STRATEGY_ACCOUNT_CONTINUITY_TEST_PLAN.md). 실제 Windows 연결 및 macOS 서명·공증은 별도 게이트입니다.

### v3.9.1.35 게시 후 확인된 자동 업데이트 수정 후보
...
- 키움 OpenAPI+를 전용 32비트 `NoahAIKiwoomHost.exe`로 빌드하고 x64 엔진과 분리한다. 설치기에는 두 실행 파일이 모두 들어가며 빌드·게시 단계에서 PE 비트수와 SHA-256을 확인한다.
- source fingerprint 범위를 키움 host spec·requirements·launcher 및 Windows build/publish workflow까지 확장해 해당 파일 변경 후 오래된 설치기를 게시하지 못하게 한다.
- 외부 게이트 미완료 상태의 `-AllowPendingExternalGates`는 GitHub prerelease만 생성한다. 안정판 `latest`와 기존 설치자의 자동 업데이트는 모든 검증 행이 `[x] VERIFIED`가 될 때까지 v3.9.1.33을 유지한다.
- 자정 직후 날짜가 전날로 넘어가던 PAPER 통계 테스트의 시간을 현재 시각으로 고정해 기능과 무관한 회귀 실패를 제거한다.
- 검증 원장: [v3.9.1.34 키움 x86·릴리즈 무결성 계획](V39134_KIWOOM_X86_RELEASE_INTEGRITY_TEST_PLAN.md).

## 2026-09-15 - v3.9.1.33 성과 근거·증권 학습·AI 응답 정합 (미배포 후보)

- 시세 누락 근본 원인 추가 수정: Coinone 거래용 캔들 역순, 통합 경로 실패 캐시/동시 요청 오판, 증권사 누락 등락률의 0 변환, 30분 변화 계산을 바로잡았습니다. 외부 장애와 코드 결함을 구분하며 관리 경로 유지가 청산 성공을 보장하지는 않습니다. [원인·공개 응답·검증 경계](V39133_MARKET_DATA_ROOT_CAUSES.md)

### 배포 후 사용자 피드백 수정 후보 (미배포)

- PAPER 통계에 LIVE 대조 조건을 적용하던 표시 오류, 증권 학습의 잘못된 JSON 조회, 자산 통합의 미확정 손익 사용을 수정한다.
- AI 애널리스트의 PAPER/LIVE 선택, 금융 인텔리전스 티커/프리셋 정합성, 미연결 공급자 상태, 공개 Project 준비 상태 재조회, 저장 원장 기반 AI 답변을 보강한다.
- 키움 전용 x86 호스트와 x64 엔진 간 IPC 및 Windows 패키징 검사를 추가한다. Windows 호스트 빌드·로그인 검증 전에는 해결 완료/재배포로 간주하지 않는다.
- 상세 판정·원장 대조·남은 게이트: [사용자 피드백 점검](V39132_USER_FEEDBACK_AUDIT.md). 공개 v3.9.1.32 설치본은 이 소스 수정과 별개이며 같은 태그를 덮어쓰지 않는다.

## 2026-09-15 - v3.9.1.32 알림·자동 업데이트·현물 후보·증권 실행 진단

### 추가 알림·증권 전수 점검 보강 (당시 소스 후보 기록)

- 7개 코인 거래소/4개 증권사의 시세 누락·조회 실패를 `unknown`으로 구분한다. 마지막 유효 국면을 장애로 덮어쓰거나 거짓 국면 변화 알림을 만들지 않는다. 신규 진입은 보류하고 기존 보유 관리·청산 경로를 유지한다.
- 알림 발송의 계정 세대 검사와 수신 설정 복사를 원자적으로 수행한다. 이전 계정 대기 메시지는 폐기하고 채널별 재시도 전에 재확인한다. 이미 시작한 네트워크 전송은 취소 보장이 아니다.
- 반복 방지 0초/재시도 0회를 보존하고, 부팅 직후 첫 알림이 억제되지 않도록 한다. 기관 스위치·초기화는 중앙 등록부의 11개 기관에서 생성하며 증권사 별칭을 정규화한다.
- 증권 워커 실행 중 오류·LIVE 위험/주문 가드레일 판정을 외부 이벤트에 연결한다. 임시 집계 손익 0원을 검증 근거로 사용하지 않는다. 증권 손익 근거가 미확정이면 신규 진입을 보류하고 위험 데이터 확인 실패를 알린다. 손실률 미제공 시 경고율을 창작하지 않으며 PAPER/LEARNING은 LIVE 알림에서 제외한다.
- 증권 후보가 없어도 실행 서비스의 보유 관리·청산 주기를 호출한다. 미래에셋 제휴 일봉 요청은 `candles`를 사용하며 `stock_list`와 분리한다.
- 사용자 원본 데이터·실계정 권한·공개 v31 설치 자산은 변경하지 않는다. Windows·실계정·실제 메신저 수신은 별도 배포 게이트다.
- 업데이트 알림 중복 방지 키를 계정+버전으로 분리한다. Discord 긴 메시지는 제한 내 요약과 전체 내용 안내를 표시하고 임의 멘션을 차단한다. Telegram 대화방 조회 실패는 빈 목록 성공으로 표시하지 않는다.
...
- 설정창 마운트 확인을 캐시 조회로 바꾸고 구독 해제, 단일 타이머·중복 요청 합치기·오류 재예약·절전 복귀 처리를 추가했습니다. 마지막 확인 시도·다음 확인 시각을 표시합니다. 설치는 사용자 확인과 안전 종료를 유지합니다.
- 공식 종목 마스터는 HTTPS·크기 제한·메모리 캐시로 조회하며 전일 값은 후보 순위에만 사용합니다. ETF와 ETN을 구분합니다.
- Coinone의 `fetch_tickers(symbols)`가 첫 종목만 반환하는 CCXT 차이를 어댑터에서 보정합니다. 전체 KRW 티커를 한 번 조회한 뒤 요청 후보를 필터링합니다.
- 공유 Teayu 데이터와 사용자 키는 수정·실행하지 않았습니다. Coinone LIVE 승인 게이트는 유지합니다.
- 메뉴얼 정본 재추출과 관련 사용법·설정·아키텍처·회귀 테스트를 동기화했습니다.
- 공개 v3.9.1.31 자산은 보존하며 새 Windows 빌드·주기 확인·기관 장중 PAPER는 별도 검증입니다.

[검증·배포 계약](V39132_RUNTIME_RECOVERY_TEST_PLAN.md)

## 2026-09-14 - v3.9.1.31 대시보드 메뉴얼 전 탭 문서형 가독성 패치

- 대시보드 메뉴얼 11개 탭 전체를 문서형 리치 레이아웃으로 전환했습니다. 각 탭은 `핵심 설명 → 역할/과정 도식 → 3개 핵심 카드 → 단계별 흐름 → 실제 사용 안내 → 주의사항 → 전체 사용 설명` 순서로 구성합니다.
- `NoahAI 소개`는 역할 도식·안전한 시작 순서·일반 자동매매 비교표·책임 경계를 제공합니다. 나머지 탭은 실거래 준비, 시작·설정, 금융 인텔리전스, 작동 원리, 자산별 사용, 증권/주식/ETF, AI 어시스턴트, 전략 스튜디오, AlphaArena, 업데이트의 실제 사용 흐름에 맞는 별도 요약과 도식을 제공합니다.
- 탭마다 포인트 색상을 달리하되 제목·본문·카드·단계·주의사항의 정보 위계와 글자 크기는 공통 규칙을 사용합니다. 좁은 화면에서는 영웅 영역·도식·단계 카드가 한 열로 전환되고, 긴 비교·주의 문구가 잘리지 않게 재배치됩니다.
- 메뉴얼 정본 `docs/USER_MANUAL_SECTIONS.json`은 삭제·축약하지 않았습니다. 필요한 전체 내용을 작은 글씨의 접힌 원문에 숨기지 않고 제목·본문·목록·체크리스트·주의문·표로 구조화해 기본 화면에 모두 표시합니다. 탭별 목차로 주요 장을 바로 이동할 수 있습니다.
- 전체 검색은 일치하는 탭만 순환하던 방식에서 모든 본문 일치 항목을 한 건씩 이동하는 방식으로 변경했습니다. `현재 결과/전체 결과 · 탭 이름`을 표시하고 실제 본문 검색어로 스크롤하며, 결과 지우기를 제공합니다.
- AI에게 묻기·설정/전략/거래소/증권 화면 이동 버튼은 기존 경로를 유지합니다. 이번 변경은 메뉴얼 표시와 이해도 개선이며 거래 수량·신호·가드레일·PAPER/LIVE·거래소·증권사 실행 코드는 변경하지 않았습니다.
- 11개 정본을 현행 기관 등록부와 다시 대조해 Coinone PAPER·LIVE 경계, 7개 코인 거래소, 4개 현재 지원 증권사, 확장 검토 증권사, Strategy Studio 소스 후보와 Windows 공개 버전을 바로잡았습니다. v3.9.1.31 상세 변경도 업데이트 탭 최상단에 추가했습니다.
- AlphaArena의 현재 계약을 소스와 다시 맞췄습니다. v3.9.1.31에서는 구조화된 판단과 PAPER 결과만 기록하며 Windows·주문 소유권·복구 외부 검증 전 LIVE 시작은 실패 폐쇄합니다. 과거의 실주문 가능 설명을 현재 기능처럼 보이지 않게 수정했습니다.
- 이후 배포는 정본 재추출 결과 일치, 11개 섹션·본문 보존, UTF-8 손상 없음, 현행 버전·기관 목록·최신 업데이트 순서를 자동 회귀로 확인합니다. 이 검사를 통과하지 못하면 소스 빌드 단계에서 중단합니다.
- 최종 소스 검증은 전체 Python `2,300 passed, 8 skipped`, Web production build `51 modules`, production 의존성 취약점 `0건`, 문서/버전 정합과 Web 표면 계약 `PASS`입니다. 로컬 빌드는 Node 20.11.0이므로 릴리스 요구 Node 22.12+ Windows 빌드·업데이트·DPI 확인은 [v3.9.1.31 검증·배포 계약](V39131_READABLE_MANUAL_RELEASE_TEST_PLAN.md)의 미완료 게이트로 유지합니다.

## 2026-09-14 - v3.9.1.30 통계 범위·전략 시간봉·검증 설명·증권 연결 정합 패치

- 거래소 PAPER 카드와 거래 통계는 같은 기관·통화·청산 원장 집계기를 사용합니다. 기관·기간을 먼저 필터링한 뒤 최대 100,000건을 적용하고, 카드는 `전체` 범위를 표시합니다. 상세 내역 50건 제한은 통계 합계에 영향을 주지 않습니다. 최대 범위 초과는 별도로 표시합니다.
- 통계 캐시는 파일 변경 시 무효화하며 원장·학습·위험 이력을 수정하지 않습니다. 전체 기관 최근 500행에서 한 기관을 추리던 카드와 해당 기관 전체 원장의 범위 차이를 제거했습니다. 최근 PAPER 손실이 존재할 수 있으며 버전 변경만으로 성과 하락을 단정하지 않습니다.
- Strategy Studio의 새 버전에 판단 시간봉을 저장합니다. 외부 원문의 시간봉은 사용자가 확인하고, 관리형 예제는 암호화폐 15m/주식 1d를 명시합니다. 시간봉 없는 구버전은 과거재생 시 확인을 요구하며 저장 원문이나 기존 승인을 소급 변경하지 않습니다.
- 과거 시세 재생 검사는 저장 시간봉·필요한 상위 지표봉·완성된 캔들만 사용합니다. 사용한 기관·시간봉·표본·UTC 시작/종료를 기록합니다. 증권사 분봉 공급 미지원 또는 서로 다른 판단/실행봉은 명확히 중단하며 다른 봉으로 대체하지 않습니다.
- 실시간 커스텀 전략도 선언 시간봉의 지표 문맥으로 평가하며 해당 시세가 없으면 진입 조건을 통과시키지 않습니다. Coinone 공개 캔들 공급을 연결했고 Bithumb 15m/4h는 공식 분봉 API로 직접 조회합니다. 2h 재구성은 정확한 1h 묶음만 사용하고 불완전 묶음은 제외합니다.
- 독립 사용자 진입 규칙과 NoahAI 기본 진입+사용자 위험·청산값의 검증 대상을 카드와 승인창에 표시하고 승인 원장에 기록합니다. 외부 수치·TradingView 백테스트·unsigned 패키지는 검증 완료로 자동 승격하지 않습니다.
- 따라 만들기 보완 버튼이 실제 입력 영역을 스크롤·포커스하며 다음 행동을 안내합니다. 생성 시각은 분 단위 별도 줄, PAPER 상태는 한 줄, v1은 이전 버전 변경점 제외, 허브 긴 설명은 펼치기로 제공합니다.
- 거래소·증권사 로그의 미연결 안내와 오류 메시지를 고정 슬롯으로 분리해 본문 행이 바뀌지 않습니다. 좁은 화면에서는 카드·로그를 세로 배치하고 실행 풀 안내가 다음 카드에 가리지 않도록 확인했습니다.
- API 인증 완료와 회원 거래 권한을 분리 표시합니다. 해외 레퍼럴 거래소 시작이 차단되면 일반 `상태 409` 대신 승인 필요·UID 확인 대기·거절·만료·승인 후 운영 활성화 대기를 구분하고 daltrading 확인 또는 관리자 승인 요청을 안내합니다. 시작 명령 직전 서버 정책 갱신과 정지 명령의 안전 예외는 유지합니다.
- 키움은 모든 Windows 호출 경로에서 기존 별도 COM 프로세스를 사용하고 유휴 중 Qt 이벤트를 처리합니다. RPC 시간초과 후 늦은 응답을 재사용하지 않으며, 주문 응답 미확정이면 새 주문을 차단합니다. KIS는 인증 성공/실패와 계좌·서버 설정 오류를 표시합니다.
- 검증은 전체 Python `2,296 passed, 8 skipped`, Node 22 production build 51 modules, 암호화폐·주식 화면 24개 조합을 통과했습니다. 세부 배포 상태는 [v3.9.1.30 테스트 계획](V39130_STRATEGY_VENUE_CONSISTENCY_TEST_PLAN.md)을 따릅니다. Windows 설치·업데이트/롤백·실계정 KIS/키움·장시간 PAPER는 환경별 게이트이며 소스 테스트로 대체하지 않습니다. 긴급 버그픽스와 기능 배포는 자동 회귀·빌드·fingerprint·SHA 검증을 통과한 산출물에 한해 운영자 `-AllowPendingExternalGates` 승인으로 게시할 수 있고, 남은 외부 게이트는 테스트 계획에 공개 상태로 유지합니다.

## 2026-09-12 - v3.9.1.29 공정 전략 생태계·회원별 운용 용량·코인원 온보딩 소스 후보

- AI Provider 점검을 모델 목록 조회가 아닌 선택 모델의 비민감 1회 실제 생성 호출로 바꿨습니다. 요청 모델과 Provider 실제 응답 모델·입출력 토큰·오류를 분리 표시하고 해당 호출을 AI 비용 원장에 기록합니다.
- 역할별 모델 카드에 `저장됨 · 실행값`과 `변경 대기 · 아직 미적용`을 표시합니다. 성공 응답의 usage는 설정 별칭이 아닌 Provider가 반환한 실제 모델 ID로 집계해 Usage 콘솔과 대조할 수 있게 했습니다.
- GPT-6 Astra 호출은 공식 reasoning 모델 계약에 맞춰 `max_completion_tokens`를 사용하고 지원되지 않는 `temperature`·`top_p`·logprobs 샘플링 옵션을 제거합니다. 모델 목록에 누락됐더라도 실제 호출 성공 여부를 우선 표시합니다.
- Strategy Studio의 전략 제작·로컬 분석·PAPER 검증·내보내기·공유 준비는 회원등급과 분리했습니다. 무료·유료가 같은 Level 1~4, 실행 계약과 최대 5개 전략별 PAPER 포지션 범위를 사용합니다.
- 실제 계정 운용은 집중운용 1개를 유지하고, 관리형 다중포지션은 국내 무료와 레퍼럴 확인 해외 무료가 거래소별 최대 3개, `pro_coin`·`premium`은 최대 5개입니다. 이는 목표 거래 수가 아니라 상한이며 계좌 위험·시장·성과·전략·가드레일이 더 작게 제한할 수 있습니다.
- 국내 현물 Upbit·Bithumb·Coinone은 레퍼럴 UID 없이 무료 경로로 분리하고, 해외 무료는 서버가 확인한 Binance·Bybit·OKX·Bitget만 실행 가능하게 유지했습니다. 유료 등급은 해당 라이선스 범위에서 제휴 귀속 없이 사용합니다.
- Coinone을 중앙 기관 등록부와 설정·시세·PAPER·통계·Strategy Studio 범위에 추가했습니다. CCXT 공통 기능과 Coinone V2.1 주문/대조 경로를 분리한 하이브리드 어댑터를 사용하며, 실제 계정 시장가·부분체결·수수료·재시작 대조 E2E 전에는 LIVE 시작과 주문을 이중으로 실패 폐쇄합니다.
- Upbit·Bithumb·Coinone의 로컬 PAPER와 공개 코인 선택·분석은 공개 KRW 시세와 가상 원장만 사용하므로 거래소 API 키 없이 실행할 수 있게 정합했습니다. LIVE와 실제 잔고·거래소 체결 동기화는 계속 인증을 요구하며, 세 현물 어댑터는 `BTC/KRW`·`KRW-BTC`·`BTCKRW` 입력을 같은 KRW 현물 심볼로 정규화합니다.
- Strategy Hub가 소비하는 기관 등록부도 같은 소스에서 재생성해 새 기관이 고정 카테고리나 별도 목록에서 누락되지 않게 했습니다. 전략 성과는 기관·상품·통화별로 분리하며 회원등급은 전략의 제작·검증 점수에 사용하지 않습니다.
- AI Provider·Web 설정 집중 회귀 `180 passed`, 전체 Python 회귀 `2,251 passed, 8 skipped`, Web production build `51 modules`와 Web 표면 정합 감사를 통과했습니다.
- 공개 v3.9.1.28 자산은 변경하지 않습니다. v3.9.1.29 Windows 빌드·업데이트/재시작과 Coinone 실계좌 E2E는 별도 배포 게이트입니다.

## 2026-09-12 - v3.9.1.28 체결-청산 원장 대조·순손익·통계 고정 레이아웃 공개판

- Strategy Studio의 실행 대상을 고정된 `Binance/전체 거래소` 표현에서 버전 관리 기관 등록부 기반의 `전체 호환 기관/개별 기관`으로 바꿨습니다. 현재 6개 코인 거래소와 4개 증권사를 상품·통화·SHORT·레버리지·주문 단위 capability와 함께 생성하며, 향후 기관은 온보딩 상태가 준비되기 전 LIVE 대상으로 노출되지 않습니다.
- 동일 등록부를 daltrading에 생성·검증 가능한 JSON으로 내보내고 허브의 자산군·상품 유형·기관·시장국면·증거 단계를 독립 동적 필터로 분리했습니다. Binance/Upbit/Bithumb 이름에 고정된 시장 카테고리는 제거했으며 기관별 PAPER/LIVE/체결 근거와 KRW·USDT 손익은 합산하지 않습니다.
- 새 `.noahstrategy`에는 개인 상세 거래 대신 정확한 전략 key·버전의 제한된 기관·통화별 집계 여권만 포함합니다. daltrading은 집계가 없는 공개본을 E0, 제작자 로컬 집계가 있는 공개본을 E1로 표시하고 두 단계 모두 서버 검증 랭킹에서 제외합니다. E2~E5는 서버 서명·계정 연동·기관 확인 근거가 있어야 하며 LIVE 준비를 통과하지 않은 기관은 E5를 받을 수 없습니다.
- 이미 E0로 공개한 동일 전략 key·버전은 같은 IR 해시의 새 패키지를 제출하면 새 카드를 만들거나 기존 다운로드 패키지를 바꾸지 않고 E1 근거만 append-only 스냅샷으로 갱신합니다. 실행 규칙 해시나 공개 범위가 달라지면 근거 갱신으로 우회하지 못하고 새 전략 버전을 요구합니다.
- 제출 1단계 미리보기에서 E0/E1 예정 상태와 제작자 로컬 PAPER/LIVE 집계를 보여 줍니다. 검증되지 않은 전략도 구조·권리 계약을 통과하면 낮은 진입장벽으로 공개할 수 있지만, `검증 전략`이나 성과 순위로 표시되지는 않습니다.
- 정확한 `exchange + symbol + exit_order_id`로만 NoahAI 청산과 기관 체결을 대조하고, 같은 종목의 수동 거래·재진입·다른 부분청산을 시간 범위로 흡수하던 경로를 제거했습니다.
- 거래소 실현 PnL(`gross_pnl`)과 진입·청산 수수료 및 증권 세금을 반영한 순손익(`net_pnl`)을 분리했습니다. 승패와 최대/최소 통계는 순손익을 사용합니다.
- 공급자가 실현 PnL을 제공하지 않거나 수수료 통화 변환 근거가 없거나 체결수량이 맞지 않으면 기존 값을 덮어쓰지 않고 `대조 미확정`으로 표시합니다.
- 주식 체결 동기화가 개별 체결을 가짜 완료 거래로 만들지 않게 하고, 4개 증권사의 NoahAI BUY lot→SELL/부분매도 경로에 수수료·세금을 연결했습니다.
- 거래 통계 메시지 영역과 표 스크롤 소유자를 고정하고 버튼을 `화면 다시 계산`/`거래소 체결 동기화`로 분리했습니다. 전체 거래소 동기화는 활성 대상 전부를 순차 실행합니다.
- 코인 정보·시장 트렌드·AI 학습·AI 리포트의 조건부 메시지도 전용 Grid 슬롯으로 옮겨 본문·헤더 위치 회귀를 차단했습니다.
- 운영 KPI·AI 애널리스트·AI 리포트는 같은 체결 대조 원장을 사용하며, 대조 완료가 없으면 손익·승률을 `0`으로 단정하지 않고 `대조 전` 또는 `계산 전`으로 표시합니다.
- 창 제목은 Electron 런타임 버전 `43.4.0`이 아니라 NoahAI 제품 버전 `v3.9.1.28`을 읽고, 사용자 화면에서 구현명 `Web UI`를 제거했습니다.
- 처음 사용자는 설정의 `처음 사용 · 빠른 시작`에서 자산군과 한 기관만 고르면 안전한 PAPER 시작값을 검토할 수 있습니다. 이 안내는 새 운용 모드나 설정 스키마를 만들지 않으며, 저장·API 확인·서비스 시작은 각각 사용자가 명시적으로 수행합니다.
- 전체 Python 회귀 `2,235 passed, 8 skipped`, v3.9.1.28 집중 회귀 `103 passed`, Web production build 51모듈을 통과했습니다. daltrading의 전략 허브·공개 초보자·설정 가이드 전체 회귀 `100 passed`를 통과했습니다.
- v3.9.1.28 Windows 설치기·blockmap·`latest.yml`과 manifest가 게시됐습니다. 실계정·장시간 환경 확인은 공개와 별도인 후속 운영 게이트입니다.

## 2026-09-11 - v3.9.1.27 Strategy Assistant 연속성·AI 비용·모델 라우팅 공개판

- OpenAI 기본 보호 Project와 공개 일반 질문용 Project 자격증명을 분리했습니다. 공개 경로는 기본 OFF이며, AI 어시스턴트 심층분석에서 사용자가 공개 질문임을 별도 확인한 1건만 공유용 키로 보냅니다.
- 공개 일반 질문에는 최근 대화·계좌·포지션·설정·전략·파일·차트 문맥을 붙이지 않습니다. 키 누락·설정 OFF·알 수 없는 범위는 기본 보호 경로로 돌아가며 보호 요청이 공유용 키를 쓰는 역방향 대체는 하지 않습니다. 응답에는 실제 사용 경로를 표시하고 캐시·사용량도 경로별로 분리합니다.
- `설정 → AI 엔진/API`에 실제 `AI 비용 관리 · 외부 호출 한도` 카드를 추가했습니다. 오늘·이번 달 사용량, 변경 대기 한도, 포함/제외 작업, UTC 갱신 기준을 한 화면에서 확인합니다.
- 비용 카드는 사용자 요청형 외부 AI 성공 응답의 실제 토큰과 기준일 공개 단가가 모두 있을 때만 오늘·이번 달 예상 비용을 계산합니다. 토큰 미제공·단가 미등록·구버전 호출은 `$0`으로 만들지 않고 `비용 미산출`로 분리하며, 자동매매 백그라운드 호출과 Provider 계정 전체 청구액은 포함하지 않는다고 명시했습니다.
- `나만의 AI 구성 · 작업별 모델` 요약을 추가해 빈번·저비용, 표준, 정밀·전략, 애널리스트, 어시스턴트, 전사 경로의 현재 Provider·모델을 한눈에 확인합니다. 실제 선택은 기존 역할별 설정을 사용하며 저장 전에는 실행에 반영되지 않습니다.
- DeepSeek의 확인되지 않은 `deepseek-v4.1-flash` 이름을 임의 등록하지 않고 공식 API 별칭 `deepseek-v4-flash`, `deepseek-v4-pro`, 실험 비전 모델 `deepseek-v4-flash-vision-exp`를 지원합니다. 정식 Flash 별칭은 최신 버전을 가리키며 실제 계정 모델은 사용자가 실행한 연결 점검 결과와 합칩니다.
- 일반 DeepSeek Flash를 비전 모델로 오인하지 않도록 차트 분석은 Provider뿐 아니라 선택 모델의 이미지 기능도 검사해 미지원 조합을 호출 전에 차단합니다.
- 기본 일 30회·월 500회와 설정 범위(일 1~1,000회·월 1~30,000회)를 사용자 화면·도움말·내장 매뉴얼에 일치시켰습니다. 이 한도는 회원·거래 제한이 아니며 거래 엔진과 로컬 전략 컴파일을 중단하지 않습니다.
- `interactive_ai_budget_exceeded` 로컬 비용 보호 429와 외부 Provider 자체의 속도·쿼터·결제 429를 구분해 안내합니다. 로컬 429는 Provider 호출 전에 발생하므로 Provider 장애로 표시하지 않습니다.
- 설정의 9개 탭을 단순 질문 문구가 아니라 타입이 제한된 `settings_section` 계약으로 Gateway·Application Service·AI 어시스턴트까지 전달합니다. 탭 전환 시 안내 질문·답변·변경 후보를 초기화하고 늦게 도착한 이전 요청은 폐기해 다른 탭의 상태로 답하지 않게 했습니다.
- OpenAI·DeepSeek·Kimi·Anthropic·Gemini의 지원 모델에 상태·기능·용도·장단점·가격 기준일을 표시하고 OpenAI 전사 경로도 별도 모델로 구분했습니다. 정적 호환 목록과 실제 계정에서 조회된 모델을 구분하며 연결 점검 전에는 사용 가능하다고 단정하지 않습니다.
- OpenAI API 데이터 공유와 무료 토큰은 계정별 외부 정책임을 명시했습니다. NoahAI가 공유 설정·무료량·실제 청구액을 읽거나 변경하지 않으며, 민감한 전략·차트·포지션·계좌 문맥에는 기본 보호 Project 키만 사용합니다.

- 전략 상태 변경의 두 번 누르기를 영향이 명시된 확인/취소 창으로 교체하고 버전별 저장 생성 시각을 표시했습니다.
- 외부 AI 429와 텍스트·Pine 결정형 컴파일을 분리해 5분 따라 만들기가 계속되며, AI 어시스턴트가 일/월 한도와 설정 위치를 정확히 설명합니다.
- 설명 수준을 실제 프롬프트·캐시에 반영하고 AI 답변은 Strategy Studio의 편집 가능한 미적용 초안으로 전달합니다. 사용자 확정 후에만 별도 근거로 재분석하며 저장·승인·PAPER·LIVE는 자동 실행하지 않습니다.
- 암호화폐와 주식·ETF 공통 경로에 적용하고 기존 레버리지 가드레일을 유지했습니다.
- OpenAI Project 경로 직접 회귀 `8 passed`, 관련 AI·설정·차트 집중 회귀 `56 passed`, 전체 Python 회귀 `2,208 passed, 8 skipped`, Node 22 Web production build `50 modules`, production 의존성 취약점 `0건`, 문서·사용자 노출 동기화와 prekey 릴리스 게이트를 통과했습니다. 기존 Starlette/httpx 폐기 예정 경고 1건은 남아 있습니다.
- 사용자 키·실계정·네트워크를 접촉하지 않는 prekey 릴리스 게이트도 통과했습니다. Windows 설치본과 실제 Provider·거래기관 환경은 배포 산출물 이후 별도 검증합니다.
- Windows 설치기·업데이트/재시작·실제 Provider 429·두 자산군 화면 E2E는 새 산출물의 외부 게이트입니다.

## 2026-09-10 - v3.9.1.26 전략 위험예산·질문형 사용자 확인 소스 후보

- v3.9.1.25 화면과 AI가 제시한 `거래당 계좌 손실 0.5%, 증거금 사용은 최대 10%` 문장을 원문에 추가해도 `position_size` 누락이 반복되던 원인을 수정했습니다. 기존 파서는 `포지션 크기/자산 N%`만 읽고 거래당 계좌 손실과 증거금 문장을 실행 위험계약으로 연결하지 못했습니다.
- 명시적 퍼센트가 있는 거래당 계좌 손실·1회 위험·risk per trade를 `risk_per_trade_percent`, 증거금/종목당 투자 비중을 `max_margin_usage_percent`와 실행 `position_size`로 구조화합니다. 단위 없는 숫자는 계속 실패 폐쇄합니다.
- 분석된 위험·비중·레버리지 값을 Strategy Studio 선택값과 동기화해 최종 버전 저장이 분석값을 이전 UI 기본값으로 덮어쓰지 않게 했습니다.
- 제작 방식을 `원문 그대로 구조화 / 질문으로 함께 완성 / 기본 NoahAI에 맡기기`로 분리했습니다. 이 선택은 현재 Level 1~4를 변경하지 않습니다.
- XAI 누락 카드와 5분 따라 만들기의 `질문으로 함께 완성`은 AI 답을 자동 적용하지 않고 사용자가 직접 확정한 누락 답변만 재분석합니다. `현재 선택값을 보완 근거로 추가`는 사용자가 이미 고른 위험·시장값만 원본과 별도 근거로 결합합니다.
- 보완 답변은 원본과 별도 SHA-256 근거로 기록하고 파일·Pine·PDF를 덮어쓰지 않습니다. 원문에 이미 선언된 진입·청산·방향·TP/SL·위험과 충돌하면 덮어쓰지 않고 실행 준비를 차단합니다.
- `NoahAI가 자동 판단`은 불완전한 독립 전략의 빈칸 채우기가 아닙니다. 기본 NoahAI 운용 또는 NoahAI 진입을 확인하는 confirm 전략으로 명시해 원문 독립 전략·PAPER 근거와 구분합니다.
- 암호화폐의 증거금 비중과 주식·ETF의 종목당 투자 비중은 같은 위험 의미를 공유하되 각 자산의 실행 단위를 보존합니다. Windows 설치본과 실제 화면 E2E는 별도 배포 게이트입니다.
- v3.9.1.26 전용 회귀 15건, 전체 Python 회귀 `2,179 passed, 8 skipped`, Web production build 50모듈, 문서/버전 정합과 Web UI 전체 표면 계약 `PASS`를 확인했습니다. 위험률 변경, 원문-보완 충돌, Web UI 문서 동기화 게이트도 회귀에 포함됩니다.

## 2026-09-09 - v3.9.1.25 전략 스튜디오 입력 보존·AI 문맥·따라 만들기 복구 소스 후보

- v3.9.1.24 사용자 피드백에서 확인된 `AI에게 묻기`의 서비스 문맥 오연결을 수정했습니다. 전략 검증 질문은 일반 블록체인/주식 운용 문맥이 아니라 `ai_custom` 정본으로 전달되어 차단 코드별 이유·수정 방법·입력 예시를 답합니다.
- 외부 AI 제공사의 성공 응답 본문이 비어 있거나 호출이 실패하면 빈 답변을 캐시에 저장하지 않고, 현재 앱 버전의 NoahAI 로컬 제품 지식으로 실패 안전 대체합니다.
- 전략 스튜디오는 처음 방문 뒤 서비스별로 계속 마운트되어 AI 어시스턴트·다른 탭을 왕복해도 입력 원문·선택 파일·분석 결과·가이드 단계가 유지됩니다. 블록체인과 주식/증권 초안은 서로 다른 인스턴스로 격리합니다.
- `기본 NoahAI에 맡기기`는 커스텀 전략 생성 없이 안내를 완료하고, 사용자가 전략 제작을 선택한 경우만 관리형 예제 또는 원문 가져오기로 이동합니다. 불완전한 자동 국면 문장을 전략처럼 분석해 안전 검증에서 막히던 모순을 제거했습니다.
- 5분 안내의 분석 단계 안에 실제 부족 조건·수정 질문·입력 예시·원문 편집·재분석·AI 질문을 표시합니다. 선택 파일은 브라우저 보안상 임의 수정하지 않고 편집 가능한 원본을 다시 선택하도록 안내합니다.
- `NoahAI 기본 진입 + 사용자 위험·청산` confirm 전략에 독립 진입조건 과거재생을 강제해 `실행 가능한 진입 조건이 없어`에서 안내가 반복되던 문제를 수정했습니다. 이 유형은 과거검증을 통과한 것으로 꾸미지 않고 `과거재생 비대상`을 명시한 뒤 PAPER에서 실제 NoahAI 후보와 함께 전진검증합니다. 자체 진입조건이 있는 전략만 기존 과거 시세 자동검증을 수행합니다.
- 관리형 `추세 따라가기`처럼 LONG/SHORT 진입을 `independent_entries`로 분리한 전략을 과거재생기가 단일 `executable_entry`만 검사해 진입조건 없음으로 오판하던 누락을 수정했습니다. 두 방향을 각각 같은 선언형 엔진으로 평가하며 한 방향만 일치할 때 거래하고, 동시에 일치하면 방향 충돌로 HOLD합니다.
- 주식·ETF 자동검증도 키움·신한·미래에셋·한국투자 각각의 선택 증권사 일봉을 사용하도록 회귀 범위를 확장했습니다. 전략 저장 범위와 검증 자산군이 다르면 시작 전에 차단하고, 국내 주식·ETF 현물의 신규 SHORT는 보유 수량 청산으로 추정하지 않으며 관리형 후보는 LONG·1배로 고정합니다.
- Level 1~4와 승인→자동검증→PAPER→명시적 LIVE 적용, 계좌 위험·하드 가드레일 계약은 변경하지 않습니다.
- Web production build 50 modules, 관련 집중 회귀 199건, 주식·ETF/증권사 집중 회귀 425건(6건 환경 스킵), 전체 Python 회귀 2,158건(8건 환경 스킵)을 통과했습니다. Windows 설치기·업데이트/재시작·사용자 DPI·AI 제공사 실패 E2E 전에는 배포 완료가 아닙니다.

## 2026-09-09 - v3.9.1.24 전략 스튜디오 가독성·전략 허브 접근 정합 소스 후보

- 이미 공개된 v3.9.1.23의 버전·업데이트 자산을 더 이상 같은 번호로 다시 만들지 않도록 후속 변경을 v3.9.1.24로 분리했습니다. v3.9.1.23 사용자는 새 버전 알림과 설치를 통해서만 이 묶음을 확정 적용합니다.
- 전략 스튜디오 모든 Level에 `처음 사용 · 5분 따라 만들기`를 추가했습니다. 별도 이지 모드 설정이나 Level 변경 없이 `기본 NoahAI에 맡기기 / 관리형 예제 / 내 전략 가져오기`에서 시작해 자산·거래 빈도·검토 위험을 한 문항씩 확인하고, 쉬운 설명 → 분석 → 최종 재검증·버전 저장 → 사용자 승인 → 자동검증 → PAPER까지 현재 필요한 동작 하나를 안내합니다. 안내 초기화는 저장 전략·PAPER·거래·학습 데이터를 삭제하지 않으며 예제 후보는 외부 LLM 자유생성이 아닌 관리형 선언 규칙을 사용합니다.
- `이 결과 AI에게 묻기`가 전략명만 전달하던 경로를 보강해 현재 입력 형식·실행 준비 상태·추천 시장국면·실제 누락 조건을 함께 전달합니다. AI 설명은 설정·규칙·승인·거래를 자동 변경하지 않습니다.
- 전략 스튜디오의 공개 `전략 둘러보기`와 회원 전용 `내 전략 라이선스`를 구분했습니다. daltrading 브라우저에 로그인되어 있지 않으면 로그인 화면으로 이동한 뒤 라이선스 화면으로 자동 복귀하며, 프로그램용 라이선스 API의 UTF-8 JSON `401` 계약은 유지합니다.
- daltrading 전략 제출을 1단계 서버 패키지 분석과 2단계 추출 범위·권리 확인으로 바꿨습니다. 자산·현물/선물·기관·시장국면·검증 대상을 패키지에서 추출해 수동 태그 누락·불일치를 차단하고, 2단계 오류는 24시간 분석 초안과 작성값을 보존합니다.
- Strategy Studio Level 3·4의 고급 실행 규칙 편집에서 확인 문구 없이 긴 빈 입력창처럼 보이던 체크박스를 전용 확인 카드로 수정했습니다. JSON을 직접 수정한 경우에만 「사용자 선언 규칙으로 저장」을 확인하며, 체크해도 서버 재검증·승인·자동검증·PAPER·LIVE·가드레일을 우회하지 않는다는 의미를 화면에 명시합니다.
- XAI 누락 조건과 `최종 규칙 재검증 실패`가 기술 코드 또는 상위 차단 문장만 보여주던 문제를 수정했습니다. 서버가 실제 세부 사유를 `무엇이 부족한지·왜 필요한지·고치는 방법·입력 예시`로 구조화해 분석 결과와 저장 버튼 바로 아래에 표시하며, 초보자는 해당 항목을 AI 어시스턴트에 그대로 질문할 수 있습니다. 기술 코드는 접어 기본 화면 복잡도를 줄였습니다.
- 화면 폭에 따라 일반 기능 탭과 거래소·증권사 탭이 한 줄에서 서로 밀리던 구조를 별도 탭 그룹으로 분리했습니다. 넓은 화면에서는 기관 탭을 우측에 고정하고 1500px 이하에서는 독립된 두 번째 줄로 배치해 해상도와 표시 배율이 달라도 순서·소속이 흔들리지 않습니다.
- PAPER 거래소 제어 카드의 `전략 스튜디오 실행 풀 없음` 안내가 다음 잔고 카드에 가려지던 높이 축소 문제를 수정했습니다. 빈 실행 풀은 황색 상태 행으로 전체 문구를 표시하고, 왼쪽 카드 열은 내용 높이를 보존한 채 화면이 낮을 때만 세로 스크롤합니다. 실행 풀이 없어도 기본 NoahAI PAPER 운용은 계속된다는 의미를 함께 표시합니다.
- 같은 strategy key의 한 PAPER 버전이 관찰 중이면 sibling 버전 시작이 기존 실행 권한을 조용히 교체하지 못하게 차단합니다. 기존 버전을 명시적으로 일시정지해야 하며 attempt·성과·활성 검증시간은 보존됩니다.
- 각 버전 카드에서 시장국면·역할·진입 방향·청산 책임·TP/SL·위험·레버리지·동시 포지션·적용 범위·IR 해시를 읽기 전용으로 다시 확인합니다.
- `검증 거래 내보내기`는 정확한 strategy key/version의 계정 로컬 PAPER 청산을 JSON으로 저장합니다. 종목·방향·시각·가격·보유시간·수량·Notional·레버리지·손익·비용·진입/청산 사유·실제 TP/SL·Smart Exit 근거를 기록된 범위에서 포함하고, 구형 미기록값은 0으로 추정하지 않습니다.
- 개인 세부 거래는 공유 `.noahstrategy`와 daltrading 자동 전송에서 분리합니다. 허브의 E0도 E2~E5 검증 랭킹과 분리하고 순위·0.0점 대신 성과 점수 없음·검증 시작 전으로 표시해 제작자 설명과 서버 검증 근거를 구분합니다.
- Electron 빌드·업데이트 경로의 `js-yaml`을 보안 수정 버전 4.3.2로 고정했습니다.

## 2026-09-08 - v3.9.1.23 통계 정본·PAPER 재개·비파괴 표시 기준 공개판

- 거래소·증권사 목록의 Python 정본을 `venue_capabilities.py`로 통합해 런타임과 통계가 같은 기관 집합을 사용하도록 했습니다. Web UI inventory와 목록이 어긋나면 자동 회귀가 실패하며, 미래 기관도 동일한 통계·PAPER·초기화 계약을 빠뜨린 채 공개되지 않도록 개발·아키텍처·배포 체크리스트를 고정했습니다.
- 운영 KPI와 거래 통계의 청산·승률·실현손익 정본을 `trade_log`의 LIVE 청산 원장으로 통일했습니다. `exchange_trade_stats`는 재생성 가능한 호환 캐시로만 남기며 화면의 금융 수치 정본으로 사용하지 않습니다.
- Binance LIVE 진입에서 전략 방식 `optimized/manual`이 `execution_mode`에 저장되던 원인을 수정했습니다. 신규 기록은 `live`로 저장하고, 기존 두 값은 레거시 LIVE 별칭으로만 읽습니다. PAPER·LEARNING은 LIVE 통계에서 제외됩니다.
- 거래소가 비어 있는 구형 청산은 Binance로 추정하지 않습니다. `레거시 출처 미확정` 건수로만 보존하고 현재 LIVE 대표 합계·승률·거래소 성과에서 제외합니다.
- 거래 통계 화면 한 곳에만 `오늘 / 7일 / 30일 / 전체 / 사용자 지정` 기간을 제공합니다. KRW·USDT는 환율 정책 없이 합산하지 않습니다.
- `통계 표시 기준 새로 시작`은 선택한 자산군·거래소/증권사의 기준시각만 저장합니다. 클릭 시각 이후 **청산된 거래**부터 기본 통계에 포함하며 원본 거래·체결·학습·Strategy Studio PAPER·위험 원장·열린 포지션은 삭제하거나 변경하지 않습니다. `전체 기록 복원`으로 기준을 해제합니다.
- 개발·배포 준비도 점검은 실제 사용자 폴더를 자동 선택하지 않습니다. dev/prekey는 오프라인 계약만 검사하고, 실증권 연결 점검은 승인된 QA 계정을 명시한 release 실행에서만 허용합니다.
- 운영 KPI는 오늘 청산 통계와 최신 계좌 조회의 현재 포지션을 구분합니다. 계좌 조회 전에는 저장 원장 참고 상태임을 표시하며, KRW와 USDT 손익을 한 숫자로 합치지 않습니다.
- 주식/ETF도 키움·신한·미래에셋·한국투자별 LIVE `trade_log` 청산에 동일한 기간 계약을 적용합니다. 증권사 매수·매도·미체결 요약 캐시는 별도 운영 스냅샷으로 표시합니다.
- 실행 기관이 모두 PAPER이면 메인 운영 KPI가 현재 가상 포지션과 오늘 PAPER 청산·손익·승률로 자동 전환됩니다. 거래 통계 탭은 `LIVE / PAPER`만 추가하며 기간·기관 필터는 공통으로 재사용합니다.
- PAPER 통계는 `strategy_paper_outcomes.jsonl` 가상 청산 정본을 읽고 거래소 확인 체결과 LIVE 표시 기준을 섞지 않습니다. 6개 코인 거래소와 4개 증권사를 기관별로 분리하고 KRW·USDT를 절대 합산하지 않습니다.
- 전략 PAPER 중지는 이제 실패나 새 시도가 아니라 일시정지입니다. 같은 버전 재개 시 기존 근거와 활성 검증시간을 이어가고, 일시정지 시간은 관찰일수에서 제외합니다. 별도 `새 검증 시작`은 두 번 확인한 뒤 이전 시도를 보관하고 새 attempt ID를 만듭니다.
- 7일 또는 최소 거래 수 미충족은 `in_progress`, 두 조건을 채운 뒤 가드레일 위반이 있으면 `failed`, 모두 충족하면 `passed`로 구분합니다. 구형 중지 저장본과 이전 `paper_validation_history`도 화면과 신규 패키지에서 보존 근거로 노출합니다.
- Windows 설치기·blockmap·`latest.yml`이 게시됐습니다. 배포 manifest의 v3.9.1.23 installer SHA-256은 `10183438bbb4b3316d340a916bc76c145b8214f739c43c24a7778731cd333f82`입니다. 기관별 장시간 실환경 대조는 운영 관찰 항목으로 계속 유지합니다.

## 2026-09-06 - v3.9.1.22 공통 투자금·성과회복·병행 PAPER 소스 후보

- 신규 계정은 `NoahAI 자동 위험관리(account_risk)`, v3.9.1.21 이하 공통 정책 미보유 계정은 `기존 거래소별 호환(legacy_venue)`, 사용자가 금액을 직접 지정한 경우는 `manual_notional`로 분리했습니다. 업데이트가 기존 LIVE 금액을 자동 확대하지 않습니다.
- 기존 계정의 `legacy_venue`가 Web 설정에서 빈 선택처럼 보이지 않도록 현재 상태·이유·자동 위험관리 전환 초안을 표시하며 실제 반영은 사용자의 설정 저장을 요구합니다.
- Strategy Studio Level 4의 위험·증거금·Notional·레버리지·동시 포지션 요청을 최종 주문 계산에 연결했습니다. 최종값은 계좌 마스터·전략 요청·시장·성과 중 가장 안전한 값이며 다운로드 전략은 계좌 한도를 높일 수 없습니다.
- Level 4에 전략 요청·계좌 정책·시장/성과 적용 전 허용값 비교를 추가하고, Binance·통합 거래소·증권 주문 직전 XAI에 예상손실·Notional·증거금·레버리지·수량·제한 사유를 기록합니다.
- 동시 포지션 계좌 상한은 기본 3, 전문가 범위 1~10으로 확장하되 전략 요청과 성과 제한이 더 낮으면 그 값을 사용합니다. Level 5나 하드 가드레일 해제는 추가하지 않았습니다.
- Binance·Upbit·Bithumb·Bybit·Bitget·OKX와 키움·신한·미래에셋·한국투자가 같은 계산 결과 계약을 사용하되, KRW 현물/주식은 1배·LONG/정수 주식, USDT 선물은 계약 크기·레버리지·증거금 상한을 적용합니다.
- 거래소 최소주문이 승인 위험 상한을 넘거나 잔고·현재가·SL 근거가 없으면 금액을 임의로 키우지 않고 차단합니다.
- Binance는 계좌 위험 수량을 다중 거래소 기회 승인 전에 확정해 구형 optimizer 수량이 새 Notional을 다시 20 USDT 등으로 축소하지 못하게 했고, 승인·정밀도 보정 후 최종 수량의 필요 증거금을 재검사합니다.
- 성과회복은 KRW·USDT 금액을 직접 비교하지 않고 거래별 순수익률로 승률·기대수익·Sharpe·MDD·창 안정률을 계산합니다. KPI 통과 즉시 위험배수 1.0으로 점프하지 않고 통과 폭에 따라 0.50~1.00에서 점진 복구합니다.
- AI 임계값 자동 보정은 사용자 저장 설정을 더 이상 덮어쓰지 않고, 거래소별 근거·만료시각을 가진 24시간 런타임 오버레이로만 적용합니다.
- 향후 거래소는 CCXT를 기본 전송 계층으로 사용하되 주문 단위·contractSize·포지션 모드·보호주문·체결대조·시간동기화·rate limit 기능 프로필과 native escape hatch 적합성 검사를 통과해야 합니다.
- LIVE 활성 전략과 PAPER 관찰 전략을 분리하고, 주문 API를 소유하지 않는 전략별 가상 포지션 엔진으로 LIVE 중 PAPER 전진검증을 선택적으로 병행합니다. 자동 LIVE 승격은 없습니다.
- 병행 PAPER는 LIVE 기관에만 붙고 LEARNING·전체 PAPER에서는 중복 실행하지 않습니다. Bybit·Bitget·OKX는 런타임 market의 `contractSize`로 가상 수량과 손익을 계산합니다.
- 전략 스튜디오가 전체 PAPER·LIVE 병행검증 ON·LIVE 병행검증 OFF 상태를 실제 설정으로 읽어, 즉시 집계와 대기 상태·독립 원장·주문 API 미소유·자동 LIVE 승격 금지를 화면과 시작 확인에 일관되게 표시합니다.
- daltrading 무료 허브는 권리 자기선언+패키지/IR 구조 검사 뒤 E0로 공개하고, 성과 근거는 별도 승격하며 복수 독립 권리·무결성 신고를 사후 안전 격리하는 낮은 진입장벽 모델로 정합했습니다.
- 인앱 매뉴얼·AI 지식·대외 비즈니스 정본·NoahAI Labs 검색/LLM 원본에 실제 시드 없는 PAPER 제작, TradingView와의 초기 병행, 검증 여권 범주와 공개/후속 경계를 동기화했습니다.
- 설정·블록체인·주식/ETF AI 어시스턴트에 투자금 전용 안내를 연결해 현재 고정/계좌 위험 모드, 목표 Notional, 성과회복 표본과 레버리지·증거금 차이를 저장 설정 기준으로 설명합니다.
- 전체 Python 회귀 `2,097 passed, 8 skipped`, 투자금·전략·기관 집중 회귀 `176 passed`, 증권 게이트 `169 passed, 6 skipped`, Web production build 49 modules와 npm audit 취약점 0건을 확인했습니다. daltrading 허브 전체 회귀는 `85 passed`, NoahAI Labs production build는 225 pages·lint 오류 0건, info/ip 통합 사이트 production build는 45 pages로 통과했습니다. Windows·실계정·장시간 E2E는 별도 미완료 게이트입니다.
- 자세한 구현·검증·미완료 외부 게이트는 [v3.9.1.22 검증 원장](archive/release/V39122_UNIFIED_SIZING_PARALLEL_PAPER_TEST_PLAN.md)을 따릅니다.

## 2026-09-04 - v3.9.1.21 PAPER 손익·비용·거래소 귀속 소스 후보

- 통합 5개 거래소 PAPER 활성 포지션의 현재가와 예상 비용 차감 후 미실현 PnL을 런타임·Web snapshot에 갱신합니다.
- v3.9.1.19 Binance 비용 0 행은 근거가 있을 때만 기본 계약 추정치로 표시하고 근거 부족 행은 비용·손익 미확정으로 승패 통계에서 제외합니다.
- Analyzer·Recorder 거래소 태그와 Web 로그 필터를 바로잡아 타 거래소 판단이 Binance 탭에 섞이지 않게 합니다.
- 성과회복 제한은 6개 거래소별 최신 완료 거래로 독립 판정하고 다음 체결 후 재평가합니다. 순손익의 비용을 이중 차감하지 않습니다.
- 키움·신한·미래에셋·한국투자 공통 주식/ETF PAPER 경로도 보유 중 현재가로 미실현 순손익을 갱신하고, KRW gross/net PnL·수수료·예상 세금·슬리피지를 청산 원장에 기록합니다.
- 증권 PAPER의 성과회복 표본을 해당 증권사 PAPER 원장으로 격리하고 실계좌 체결 조회를 금지했으며, 부분청산 비용 중복과 공유 Recorder의 증권사 로그 소유권 혼입을 방지합니다.
- PAPER 주식/ETF 비용은 실제 청구액이 아닌 설정 가능한 추정값입니다. 기본 프로필은 매수/매도 수수료 각 0.015%, 주식 매도세 0.20%, ETF 매도세 0%, 양방향 슬리피지 각 0.03%이며 포지션 생성 시 계약을 고정합니다.
- v3.9.1.20 공개 자산은 변경하지 않습니다. 새 Windows 산출물·업데이트·6개 거래소 장시간 PAPER E2E 전까지 v3.9.1.21은 소스 후보입니다.

## 2026-09-04 - v3.9.1.20 PAPER 생명주기·재시작·실행 정합성 공개판

- 중지한 PAPER 전략을 늦은 청산 결과나 시작 시 동기화가 재활성화하지 못하도록 실행 권한 맵과 상태 표시를 단일 방향으로 조정했습니다.
- Binance·Unified 열린 PAPER 포지션을 계정별로 원자 저장하고 재시작 복구하며, position ID 기반 멱등 청산 집계로 중복을 방지합니다.
- Binance PAPER의 gross PnL에서 왕복 추정 수수료와 슬리피지를 분리 차감해 net PnL·승률·검증 여권에 반영합니다.
- 다중 PAPER 관찰 전략은 공유 실행 슬롯을 순환하며, UI와 메뉴얼에 독립 계좌가 아닌 공정 순환 모델임을 표시합니다.
- UPBIT ticker 공용 캐시·호출 간격·429 backoff, 학습 append-only journal/checkpoint, Binance 일반+Algo TP/SL 감사 계약을 추가했습니다.
- Windows 설치기·blockmap·`latest.yml`이 2026-09-04 게시됐습니다. 게시 후 발견된 v3.9.1.21 수정은 포함하지 않습니다.

## 2026-09-03 - v3.9.1.19 전략 검증 여권·Binance 귀속·패키지 무결성 공개판

- UNIFIED 전략을 Binance 네이티브 경로가 실행할 때 PAPER 성과가 `binance_private`로 잘못 조회되어 화면에서 빠지던 범위 계약을 수정했습니다.
- 신규 원장은 `execution_scope`와 `strategy_scope`를 함께 기록하고, 구형 행은 고유 key/version 소유 저장소가 확인될 때만 복구합니다.
- 브라우저가 `.noahstrategy`를 재직렬화하지 않도록 서버 JSON을 그대로 저장하며, 기존 구형 파일은 원래 IR·전체 패키지 해시가 모두 재현될 때만 복원합니다.
- 실행 노드 없는 confirm 전략의 PAPER를 `NoahAI 기본 진입 + 사용자 위험·청산값`으로 표시해 원문 전체 전략 검증과 분리했습니다.
- 전략 허브·대외 사이트가 공유할 시장 포지셔닝과 검색/LLM 정본을 추가했습니다.

## 2026-09-03 - v3.9.1.18 OKX 후보 복구·전략 검증 근거 소스 후보

- 사용자 로그의 `코인 선택 데이터 저장 완료`가 약 1분마다 반복된 원장을 확인한 결과, 정상 재선정이 아니라 OKX `fallback_unscored` 10개가 매 복구 시도마다 새 DB 세션으로 저장된 상태였습니다.
- OKX 파생 ticker에서 CCXT `quoteVolume`이 비어도 원문 `volCcy24h`(기초자산 수량)×`last`로 USDT 거래대금을 계산합니다. `vol24h`/CCXT `baseVolume` 계약 수는 수량으로 사용하지 않아 단위 왜곡 없이 최소 유동성 필터를 적용합니다.
- 미산출 후보는 즉시 한 번 복구한 뒤 60·120·240초로 점진 재시도하고 최대 15분으로 제한합니다. 정상 점수 후보가 회복되면 재시도 단계가 초기화됩니다.
- 동일 거래소·상태·이유·심볼의 실패 원장은 최초 즉시 저장하고 이후 기본 15분에 한 번만 감사 기록을 남겨 DB·로그 폭증을 막습니다.
- 공개 v3.9.1.17 설치기와 GitHub 자산은 변경하지 않았습니다. 이 소스는 다음 불변 버전의 Windows 빌드·OKX 실제 ticker·PAPER 장시간 검증이 필요합니다.
- Strategy Studio 저장 버전에 Validation Lab 상세와 거래소·기준통화별 PAPER 성과 근거를 추가하고, 제작 전 현물/선물 호환성 안내를 표시했습니다.
- 한국어 자연어·Pine 대표 입력을 정답 코퍼스로 추가해 지원 의미 보존과 미지원 기능의 실패 폐쇄를 회귀 고정했습니다.

## 2026-09-01 - v3.9.1.17 전략 의미 보존·PAPER 격리·리포트 정산 패치

- PAPER 전진검증 후보가 불일치했을 때 기본 NoahAI 신호까지 전역 HOLD로 바꾸던 공통 실행 계약 오류를 수정했습니다. 관찰 후보는 불일치 시 기본 신호에 위임하고, 최종 적용 전략은 조건 미충족 시 기존처럼 실패 폐쇄합니다.
- 전략 상태를 `자료 읽기 → 문서/IR 완료 → 실행 규칙 완료 → PAPER 가능`으로 분리했습니다. 미완성 독립 전략은 승인 전부터 차단하며, 기존 미완성 버전은 삭제·자동 수정하지 않고 실행 풀에서 제외합니다.
- 독립 전략은 고정 LONG/SHORT 또는 분리된 `independent_entries.LONG/SHORT` 조건과 자체 TP/SL이 필요합니다. `confirm` 전략은 고정 TP/SL을 선언하지 않으면 NoahAI 스마트 청산을 명시적으로 상속합니다. 단위 없는 숫자는 추정하지 않고 `1%`처럼 단위가 명시된 구형 TP/SL 한 쌍만 percent-point 계약으로 승격합니다.
- confirm 전략의 원문 LONG/SHORT 방향을 보존해 반대 기본 신호를 승인하지 않습니다. 양방향은 두 조건을 분리하지 못하면 실행을 차단하고, 해석되지 않은 Pine 진입 별칭은 방향만 남겨 실행하지 않습니다.
- TP/SL·포지션 비중·레버리지·신호 임계값의 저장·PAPER·주문 범위를 통합하고, 범위 밖 사용자 값을 조용히 상한으로 보정하지 않습니다. 편집한 최종 규칙은 저장 직전에 서버에서 다시 검증합니다.
- Web AI 멘토를 8문항·2~3개 교육용 후보의 구조화 흐름에 연결했습니다. 후보는 입력 영역에만 불러오며 저장·승인·자동검증·PAPER·LIVE를 자동 실행하지 않습니다.
- 암호화폐 과거검증을 Upbit/Bithumb KRW 현물과 Binance/Bybit/OKX/Bitget USDT 선물로 분리하고 거래소·시장유형·시간봉·기준통화를 결과에 기록합니다.
- AI 자동 리포트 상세를 요약과 같은 NoahAI 청산 원장에 연결했습니다. 오늘·주간·월간·최근 1시간은 시작과 현재 시각을 모두 경계로 사용해 미래 시각 행을 제외하고, 100건 단위 페이지와 전체 건수·통화별 손익·수수료 체크섬으로 요약과 상세를 대조합니다.
- Teayu 원장을 읽기 전용으로 조사해 마지막 청산→PAPER 관찰 등록→25,201건 전역 차단의 순서를 확인했고 6개 거래소 공통 회귀로 고정했습니다.
- 소스 후보는 `pending_windows_rebuild`, `publish_ready=false`입니다. v3.9.1.17 Windows 설치기·업데이트·실계정·6개 거래소 PAPER 검증 전에는 공개 배포 완료가 아닙니다.

## 2026-08-29 - v3.9.1.16 국내 현물 주문규격·거래소 실행 계약 패치

- 업비트 시장가 매수가 최종 코인 수량을 그대로 CCXT에 넘겨 거래소의 KRW 총액 주문 계약과 충돌하던 실제 결함을 수정했습니다. 최종 정밀 수량과 참조가격으로 승인 범위의 KRW 총액을 계산해 제출합니다.
- 업비트 시장가 매도는 보유 코인 수량, 빗썸 현물 주문은 base 수량 계약을 유지해 한 거래소의 규격이 다른 거래소로 번지지 않게 했습니다.
- Upbit·Bithumb의 내부 SHORT 분석 토큰을 신규 공매도가 아니라 NoahAI 관리 LONG의 청산으로 고정했습니다. 관리 원장이 없으면 수동·에어드롭·외부 보유분을 매도하지 않습니다.
- 주문 전 가드 외에 포지션 원장 생성 단계에도 국내 현물 SHORT 차단을 추가해 향후 리팩터링이 앞단을 우회해도 잘못된 가상/실포지션을 만들지 못하게 했습니다.
- 10초 거래 루프마다 15분봉 시장국면을 다시 조회하고 동일 로그를 반복하던 경로를 거래소별 기본 5분 TTL single-flight로 통합했습니다. Upbit·Bithumb 대표 심볼은 `BTC/KRW`를 사용합니다.
- LEARNING은 체결이 아니며, PAPER도 Upbit·Bithumb에서는 LONG/청산만, 해외 USDT 선물에서는 LONG/SHORT를 사용하는 계약을 코드·인앱 매뉴얼·배포 원장에 맞췄습니다.
- 거래소 분리 정본을 파일명 기준에서 상품·방향·통화·주문수량·소유권·PAPER/LIVE 기준으로 전면 교체했습니다.
- 집중 회귀 결과는 `archive/release/V39116_VENUE_EXECUTION_CONTRACT_TEST_PLAN.md`와 `TEST_STATUS.md`에 기록합니다. Windows 6개 거래소 PAPER/승인된 최소 LIVE·설치·업데이트 E2E 전에는 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-28 - v3.9.1.15 주문규격 정합·Strategy Studio Level 4·Binance 사이클 복구

- v3.9.1.14 이하에서 위험배수 적용 뒤 유효한 Binance 최소 주문 `5.10 USDT`를 설정 목표 `20 USDT`와 다시 비교해 `최종 검증 실패`로 차단하던 오류를 수정했습니다. 설정 금액은 목표, 다중 거래소 승인 수량은 위험 상한, `MIN_NOTIONAL/minQty/stepSize`는 거래소 하드 규격으로 분리합니다.
- 승인 수량 상한 안에서 거래소 최소 규격을 만족하면 주문 검증을 통과합니다. 승인 상한이 최소 주문보다 작으면 위험을 임의로 늘리지 않고 명시적인 주문 가드레일 사유로 실패 폐쇄합니다.
- 다중 거래소 `SPLIT` 승인 수량에 이미 반영된 비율을 Binance 후단에서 다시 곱하던 이중 축소 가능성을 제거했습니다. Binance 심볼 규격의 camelCase·snake_case 호환을 함께 정리하고 규격 누락 시 설정값으로 추정 주문하지 않습니다.
- Strategy Studio의 실험실 프로필에 Level 4 전문가 운용을 추가했습니다. 안정형·표준형·적극형은 거래당 위험예산·최대 비중·레버리지 상한·국면 이탈 대응만 조절합니다.
- Level 4는 사용자 승인·LIVE 권한·일일 손실 중단·포지션/집중도 제한·거래소 주문 규격·TP/SL 보호·중복 주문 방지·포지션 대조·긴급 정지를 해제하지 못합니다. 해제 필드나 허용 범위 밖 값은 전략 저장 단계에서 거부합니다.
- 구형 Web UI가 저장한 `risk_budget` 표시는 새 전략 버전 저장 시 런타임 정본 `risk_model`로 변환합니다. 이미 저장·승인된 버전은 자동 변조하지 않으며 새 버전 저장→검토→PAPER 검증을 다시 거쳐야 합니다.
- 무료 Verified Strategy Hub에 제작자 공개 중지 요청, 요청 즉시 허브 노출·신규 다운로드 차단, 운영자 확정/반려와 감사 보존을 추가했습니다. daltrading 소스 회귀는 통과했으며 운영 URL 제출→승인→다운로드→중지→재다운로드 차단 E2E는 배포 게이트로 남습니다.
- Strategy Studio와 Hub의 다음 범위는 원문부터 체결·청산까지 단일 XAI trace, Pine/TradingView/PDF 의미 동등성, 전략 버전별 PAPER/LIVE 귀속 완결입니다. 성과 기반 과금과 블록체인 증명은 현재 릴리스 범위가 아닙니다.
- Marketplace 후속 계획을 Noah Point 구매, 전략 버전 영구·비독점 실행 라이선스, 제작자 수익 원장과 선택형 외부 코인 결제로 정본화했습니다. 일반 사용자 회원/팀 공유는 필수 범위에서 제외하고 PAPER E4도 향후 판매 가능한 검증 증거로 명시했습니다.
- daltrading 무료 베타의 첫 다운로드를 가격 0인 제공 스냅샷·취득 거래·계정별 영구 비독점 실행 라이선스로 원자 기록합니다. 반복 다운로드는 취득·라이선스를 중복 생성하지 않으며 「내 전략 라이선스」 화면과 API에서 취득 버전·해시·상태를 확인하고 다시 받을 수 있습니다.
- 제작자 공개 중지는 신규 취득을 즉시 막되 기존 정상 취득자의 재다운로드는 유지합니다. 권리 침해·취득 무효는 별도 라이선스 상태로 차단할 수 있으며, 패치 이전 무료 다운로드 감사 기록도 1회 호환 마이그레이션으로 라이선스 원장에 반영합니다.
- NoahAI 전략 스튜디오 실제 화면에 「내 전략 라이선스」 링크와 무료 취득·재다운로드 안내를 추가했습니다. 포인트 충전·차감·제작자 지급은 아직 활성화하지 않았고 운영자 화면은 Noah Point 단건 라이선스 준비 계약만 표시합니다.
- 주문·Level 4 집중 회귀 `26 passed`, NoahAI 전체 Python 회귀 `1,898 passed, 8 skipped`, daltrading 전체 회귀 `81 passed`, Web production build 49 modules와 문서 정합 검사를 통과했습니다.
- 위 항목은 현재 소스 후보입니다. Windows 엔진·설치기·blockmap·`latest.yml`, Binance PAPER 및 지정 소액 LIVE, Level 4 재시작·가드레일 E2E 전에는 사용자 배포 완료로 판정하지 않습니다.

- 후보용 상품·티커·백업 캐시를 현재 사용자 계정의 쓰기 가능한 캐시 폴더로 옮겨 설치 경로와 다른 계정에 의존하던 오류를 제거했습니다.
- `fallback_unscored`만 있는 목록은 성공 결과로 캐시하거나 재선정 완료로 처리하지 않고, 실패 캐시 무효화·60초 쿨다운·single-flight 재시도로 복구합니다.
- 후보가 미산출인 동안 시장국면·상세 분석을 매 사이클 반복하지 않고 기존 포지션 관리만 계속합니다.
- 설정/API 런타임 교체 시 후보 평가기의 Binance 클라이언트도 함께 최신 객체로 갱신합니다.
- 시작 직후 즉시 사이클과 첫 정기 사이클이 연속 실행되던 Binance worker 순서를 단일화했습니다.
- 한 LIVE 사이클의 상태형 포지션 스냅샷을 좀비 플래그 정리와 메모리 정합화가 공유합니다.
- 포지션 조회 오류·키 없음·연결 불가를 빈 계정으로 처리하지 않고 기존 포지션·플래그·모니터를 보존하며 신규 주문을 보류합니다.
- 실제 성공 응답으로 무포지션이 확인된 경우에만 오래된 플래그를 정리합니다.
- 제품 버전은 `3.9.1.15`, updater SemVer는 `3.9.115`입니다. Windows 설치본과 실계정 검증 전에는 배포 후보입니다.

## 2026-08-27 - v3.9.1.14 후보 선정 복구·증권 조회 연결 안전 종료 패치

- 제품 버전 `3.9.1.14`, updater SemVer `3.9.114`, 예정 설치 파일 `NoahAI-3.9.1.14-Setup.exe` 계약을 정렬했습니다.
- Teayu 원장에서 Binance만이 아니라 OKX와 일부 Bithumb도 동시 실행 시 `fallback_unscored`가 된 사실을 기준으로 거래소 공통 복구 경계를 추가했습니다.
- 후보 전체가 미산출이면 불필요한 상세 캔들·AI 분석과 신규 진입을 생략하고 기존 포지션 보호만 계속합니다. 3시간을 기다리지 않고 기본 60초 간격으로 거래소별 single-flight 재선정을 요청합니다.
- 일시적인 재선정 실패가 이전 정상 점수 후보를 덮지 않도록 Binance 네이티브와 통합 5개 거래소의 후보 교체를 보호했습니다.
- 6개 거래소의 30개 후보 스냅샷 점수 회귀와 재시도 쿨다운·거래소 격리 회귀를 추가했습니다.
- 거래 시작 전 증권사 탭의 잔고 조회만으로 만들어진 어댑터도 키움·신한·미래에셋·KIS 공통 종료 대상에 포함합니다. 키움 COM 자식 정리 외에 REST 증권사의 HTTP 세션·토큰 상태도 명시적으로 해제합니다.
- Binance·Bybit 서명 요청의 거래소 시간 오차를 자격증명 오류나 빈 계정 응답으로 오판하지 않습니다. 거래소 서버 시간 차이를 다시 측정해 한 번 재시도하고, 계속 실패하면 API 키 재발급이 아니라 Windows `시간 자동 설정 → 지금 동기화`를 안내합니다.
- KIS 접근토큰 발급을 프로세스 단위 single-flight로 합치고 유효 토큰을 재사용합니다. `EGW00133`은 키 오류가 아닌 1분 발급 제한으로 분류하며, 제한시간 안의 설정·계좌·worker 중복 발급을 차단합니다.
- 관찰·분석·학습 거래소 선택은 실행 범위 지정일 뿐 자동 시작이 아님을 카드와 매뉴얼에 명시했습니다. LEARNING은 거래소별 `분석·학습 시작`을 눌러야 로그와 분석 주기가 시작되고 실주문·가상 체결은 만들지 않습니다.
- `17:59:53→18:04:48`은 한 종목이 5분 걸린 기록이 아니라 같은 주기의 여러 종목 로그가 섞인 구간이었습니다. 근거 없는 전면 병렬화는 적용하지 않고 거래소 API 제한을 위한 3초 인터벌과 종목별 최신 캔들 분석을 유지합니다.
- 관련 집중 회귀와 연결·시간·LEARNING 회귀를 통과했습니다. 전체 회귀·Web build·문서 검사 결과는 `TEST_STATUS.md`에 갱신합니다.
- 현재 macOS 소스 작업공간에는 Windows 엔진을 새로 빌드할 수 없고 Git 작업트리도 없습니다. 새 Windows 엔진·설치기·blockmap·`latest.yml`, 1.13→1.14 업데이트와 실제 Windows 종료 E2E 전에는 배포 완료가 아닙니다.

## 2026-08-27 - v3.9.1.13 OKX 안전 종료·가독성·PAPER 검증 안내 패치

- 제품 버전 `3.9.1.13`, updater SemVer `3.9.113`, 설치 파일 `NoahAI-3.9.1.13-Setup.exe` 계약을 정렬했습니다.
- OKX를 포함한 CCXT 거래소 요청을 12초로 제한하고 분석 호출 뒤 중지 플래그를 재확인해 종료 중 새 주문 단계로 넘어가지 않게 했습니다.
- API 인증 배지를 거래소 제어 카드 안으로 이동하고, PAPER 이력을 접기/펼치기로 전환했습니다.
- 100%·112%·125% 화면 표시 프리셋으로 창과 Web UI 글자·로그·메뉴얼을 함께 확대합니다.
- 업데이트 가능 알림을 인앱과 사용자가 활성화한 Discord·Telegram에 비차단 전송합니다.
- LIVE에서는 AI 커스텀의 숨은 PAPER 거래나 검증 카운트가 진행되지 않음을 UI와 메뉴얼에 명시했습니다.
- 시장 후보 평가 실패 시 고정 안전 후보를 `0.00점`으로 오표시하던 경로를 `미산출·안전 폴백`으로 분리했습니다. 폴백 선정도 거래소·선정 상태와 함께 저장하며 코인 정보 화면은 현재 거래소의 최신 선정 세션만 읽습니다.
- v3.9.1.12 이하 거래소 미기록 선정 행은 임의로 Binance에 귀속하지 않고 보존만 하며, 정상 재선정 뒤 새 거래소별 세션으로 교체합니다.
- 6개 거래소는 전체 티커 일괄 스냅샷을 우선하고, 일괄 미지원 시 최대 8개 제한 병렬·단계별 10초 제한을 사용합니다. 상세 캔들·점수는 유동성 상위 30개만 계산합니다.
- 같은 거래소의 UI·자동 선정 요청을 single-flight로 합치고 자동 재선정은 거래·포지션 감시 밖에서 실행합니다. 완료된 목록만 원자 교체하며 120초를 넘긴 결과는 기존 정상 후보를 덮지 않습니다.
- 시장국면은 기본 5분 관찰, 2회 연속 확인, 최소 10분 유지 후 전환하고 안정 국면에서도 후보 선정 후 3시간이 지나면 백그라운드 재평가합니다.
- 주식·ETF는 증권사별 분석 서비스와 후보 목록을 기본 5분 재사용하고, 지원 증권사의 KOSPI 5거래일 수익률을 국면 정본으로 사용합니다.
- Teayu v3.9.1.13 원장에서 Binance뿐 아니라 OKX도 후보 점수 제한시간을 넘겨 안전 폴백된 것을 확인했습니다. 후보 점수 worker의 종목별 캔들·펀딩비·OI 네트워크 호출을 제거하고, 일괄 ticker와 사용 가능한 캐시 값만으로 제한시간 내 점수를 계산합니다.
- 거래 시작 전 잔고·종목 조회로 생성된 증권사 어댑터도 안전 종료 대상에 포함합니다. 키움 RPC가 진행 중이면 짧은 유예 뒤 NoahAI가 소유한 COM 자식만 정리하며 신한·미래에셋·KIS도 동일 수명주기 등록부를 사용합니다.
- 코인·종목 선택 정본, 사용자 가이드, 실행 흐름, 인앱 메뉴얼과 AI 어시스턴트 지식을 같은 기준으로 갱신했습니다.
- 후속 수정본 전체 Python 회귀 `1,866 passed, 8 skipped`, Web UI production build 49 modules, 문서·Web source parity 검사를 통과했습니다.
- v3.9.1.13 Windows 설치기는 이미 게시되었지만 위 Binance·OKX 점수 및 조회 전용 증권 어댑터 종료 수정은 게시 뒤 들어온 소스 변경입니다. 기존 자산을 같은 버전으로 덮어쓰지 않으며 후속 버전 Windows 빌드·업데이트·실기기 종료 검증 전에는 이 수정이 사용자에게 배포됐다고 간주하지 않습니다.

## 2026-08-27 - v3.9.1.12 전략 허브·메뉴얼·검증 신뢰 경계 패치

- 제품 버전 `3.9.1.12`, updater SemVer `3.9.112`와 설치 파일 `NoahAI-3.9.1.12-Setup.exe` 계약을 정렬했습니다.
- AI 커스텀에 무료 전략 허브, 제출·다운로드 안내, 전략 목록 상단 `내 전략 제출`과 버전별 `허브에 제출` 동선을 추가했습니다.
- 회원 전략은 자동 업로드하지 않고 `.noahstrategy`, 공개 설명, 시장·거래소·국면·위험, 원본·배포 권리를 직접 확인한 뒤 비공개 검토로 제출합니다.
- 로컬·다운로드·서버 검토 원본·DB 메타데이터 저장 위치, 검토 상태, 다운로드 후 승인·자동검증·PAPER 재검증을 메뉴얼과 AI 어시스턴트에서 같은 방식으로 설명합니다.
- 자기신고 성과를 랭킹에서 제외하고 해시, 미사용 구간, 워크포워드, 비용 스트레스, MDD, PAPER·실체결, 가드레일을 분리한 검증 여권과 백테스트의 한계를 명시했습니다.
- daltrading 무료 공개 베타 이후 제작자·팀·서버 서명, 약관·정산 E2E와 선택적 블록체인 해시 증명의 순서를 문서화했습니다.
- 공개 v3.9.1.11 자산은 보존합니다. 새 Windows 설치기·blockmap·`latest.yml`과 1.11→1.12 업데이트 E2E 전에는 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-26 - v3.9.1.11 안전 종료·PAPER 검증 정합 패치

- 6개 거래소 중지 신호를 먼저 전파하고 공통 45초 예산에서 worker·모니터 종료를 기다리도록 바꿔 Binance 5초 조기 실패를 제거했습니다.
- 거래 사이 고정 대기를 중지 이벤트 대기로 바꿔 종료 요청 뒤 새 분석 주기로 넘어가지 않게 했습니다.
- 구형 AI 커스텀 PAPER TP/SL `0.3/0.15`를 0.3%/0.15%로 호환 변환하고 PAPER/LIVE 모두 안전 범위 밖 값을 주문 전에 차단합니다.
- PAPER 전진검증이 첫 청산 전에도 관찰 일수를 표시하고, 정확한 전략 키·버전과 관찰 시작 이후 청산만 합산하도록 수정했습니다.
- LIVE 화면의 PAPER 원장을 과거 검증 이력으로 명확히 표시하고 현재 PAPER 포지션은 런타임 가상 상태를 사용합니다.
- 제품 버전 `3.9.1.11`, updater SemVer `3.9.111`. 공개 v3.9.1.10 자산은 보존하며 Windows 외부 게이트 전에는 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-25 - v3.9.1.10 PAPER 포지션·가상 통계·외부 알림 패치

- Web 설정의 집중 운용(`focus`)을 기존 Binance 실행 경로가 구형 `single`과 다르게 해석해 무시할 수 있던 문제와 PAPER가 `position_mode`를 보지 않던 문제를 공통 포지션 제한 정책으로 통합했습니다.
- 설정 저장 시 집중=1, 다중=기본 3과 6개 거래소별 override를 함께 동기화하며, 고급 상한 2를 명시한 경우에는 2를 유지합니다.
- PAPER 화면은 거래소의 실계정 포지션을 `PAPER`라고 오표시하지 않고 각 엔진의 런타임 가상 포지션을 표시합니다. 포지션과 가상 청산 이력에 내부 스크롤을 추가했습니다.
- 가상 포지션 헤더에 현재 활성 수와 집중/다중의 거래소별 상한을 함께 표시합니다. 상한을 낮추기 전에 열린 기존 포지션은 강제 청산하지 않고 `신규 진입 차단`으로 표시하며, 가상 거래내역은 종료 이력이라 `활성 수와 무관`함을 명시합니다.
- PAPER/LEARNING 포지션 상한은 과거 LIVE 수동·외부 포지션에 의해 줄어들지 않으며, 각 거래소의 현재 실행 모드를 별도로 판정합니다.
- 거래소별 실행 모드를 런타임이 반환해도 엄격한 Gateway DTO가 `execution_modes`를 미등록 필드로 거부해 `/api/v1/runtime/snapshot`이 500이 되던 실제 렌더 장애를 수정하고 Gateway 회귀를 추가했습니다.
- 가상 포지션은 계좌 네트워크 조회를 기다리지 않고 기존 5초 workspace 갱신에서 런타임 메모리만 읽습니다. 별도 프런트 폴링은 추가하지 않았습니다.
- PAPER 통계는 실거래 체결/DB 수치가 아니라 최근 PAPER 청산 원장의 청산 수·승률·순손익·수수료로 자동 전환합니다.
- append-only PAPER 원장을 5초마다 전체 읽던 경로를 끝부분 tail-read로 바꿔 장시간 운용 중 파일 크기에 따른 UI 지연을 차단했습니다.
- 100MiB 초과 합성 원장에서 최근 500건을 20회 연속 읽는 변이 회귀를 추가했습니다. 전체 파일 크기와 무관하게 회당 최대 128KiB, 20회 총 최대 2.5MiB만 읽도록 I/O 상한을 고정했습니다.
- Windows 장시간 검증용 모니터가 Electron과 PyInstaller one-file engine의 전체 프로세스 트리를 합산하고, 여섯 거래소의 런타임 실행 여부·PAPER 모드·가상 포지션 조회 상태와 런타임/workspace/설정/AI 커스텀 API P95를 함께 판정하도록 보강했습니다. 기존 초과 포지션 관찰과 깨끗한 계정의 신규 상한 엄격 검사를 분리합니다.
- 6개 거래소 압축 부하에서 Web read-only SQLite 연결이 `with` 블록 종료 뒤에도 닫히지 않아 `trading.db`/WAL/SHM 핸들이 누적되는 장시간 성능 결함을 발견했습니다. query·거래 이력 조회의 모든 연결을 명시적으로 닫아 200주기 디스크립터 증가를 181개에서 0개로 수정했습니다.
- 최종 720주기·5,328 HTTP 요청 회귀에서 실패 0, RSS 증가 5.51MB, 스레드·디스크립터 증가 0, 런타임/workspace/설정/AI 커스텀 P95가 모두 300ms 이내임을 확인했습니다.
- `적용 중` AI 커스텀 버전은 전역 PAPER 모드에서 자동 가상 실행됨을 표시해 `PAPER 시작 필요`라는 모순된 문구를 제거했습니다.
- AI 어시스턴트 일반 안내의 빠른 질문을 설정 정본과 런타임 근거 주제로 분기해 서로 다른 질문이 하나의 공통 상태 답변으로 반복되는 잔여 문제를 수정했습니다.
- 암호화폐·증권 PAPER 포지션 질문은 stale LIVE 저장소가 아니라 현재 가상 포지션과 최근 실행 사이클을 근거로 답합니다.
- 질문에 거래소·증권사를 명시하면 현재 탭보다 우선하며, 포지션을 말하지 않은 추세 질문은 최근 시장 신호로 분류합니다.
- 대시보드 인앱 매뉴얼의 상단 버전·자산별/증권 가이드·AI 커스텀·AI 어시스턴트 설명을 v3.9.1.10 계약으로 다시 맞춰, 어시스턴트가 설정을 저장할 수 있는 것처럼 보이던 과거 문구를 현행 읽기 전용 계약으로 수정했습니다.
- 설정에 `알림·리포트` 탭을 추가하고 Discord Webhook 저장·테스트, Telegram Bot Token 저장·Chat ID 자동 찾기·테스트를 단계형 UI로 연결했습니다. 연결 정보는 기존 API 키와 같은 write-only 계약을 사용합니다.
- 가드레일 중단·손실 경고·시장국면 변화·런타임 시작/중지/실패와 사용자가 요청한 AI 리포트 요약을 비동기 제한 큐로 발송합니다. timeout·재시도·중복 방지·계정 전환 격리·공식 HTTPS 대상 제한을 적용해 외부 메신저가 거래 루프를 지연시키거나 내부 주소로 요청하지 못하게 했습니다.
- 카카오톡·예약 리포트·PC 종료 후 서버 중계는 v3.9.1.10 제공 범위가 아니며, 실제 Discord/Telegram 계정과 Windows 설치본 검증을 배포 게이트에 추가했습니다.
- 기존 Windows release fingerprint가 Web Gateway·React 중심이라 Trader·통합 Trader·거래소 어댑터·포지션 정책·AI 커스텀 엔진 변경을 감지하지 못하던 배포 무결성 결함을 수정했습니다. 빌드·게시가 동일한 Python 계산기를 사용하고 거래 엔진부터 패키지 매뉴얼까지 같은 지문에 포함하므로, 엔진이 바뀐 뒤 만들어진 오래된 설치 후보는 게시할 수 없습니다.
- 2026-08-25 재검증에서 전체 Python 회귀 `1,789 passed, 8 skipped`, React/TypeScript production build, 문서·Web parity·설정/어시스턴트 소스 계약 감사를 통과했습니다. 로컬 실제 Binance PAPER 거래소 렌더에서 활성 `추세 따라가기 V1`의 자동 실행·재적용 불필요 안내, 다중 상한 3, 가상 포지션·가상 통계·활성과 무관한 종료 이력을 한 화면에서 확인했습니다. 추가 회귀로 V1 가상 진입 포지션의 전략 키·버전 보존과 TP/SL 청산 원장의 동일 버전 귀속도 고정했습니다.
- 제품 버전 `3.9.1.10`, updater SemVer `3.9.110`. 공개 v3.9.1.9 자산은 보존하며 Windows 재빌드·1.9→1.10 업데이트·6개 거래소 PAPER 실화면·실제 Discord/Telegram 발송 검증 전에는 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-24 - v3.9.1.9 PAPER 전진검증·근거형 AI 어시스턴트 패치

- 사용자 승인 전략은 과거 검증 구간이 없거나 미통과여도 LIVE와 분리된 PAPER 전용 실행 풀에서 새 근거를 모을 수 있게 했습니다.
- PAPER 최소 거래 수·관찰 일수의 진행 상태와 기본/AI 커스텀 가상 청산 이력을 실거래 통계와 분리했습니다.
- AI 어시스턴트 일반 안내를 서비스별로 분기하고 포지션 질문에 관리 포지션·TP/SL·전략 버전·최근 신호를 정본 근거로 제공합니다.
...
- 현물·선물 기능 계약과 기준통화를 공통 정의하고 현물 SELL을 신규 SHORT로 해석하지 않도록 고정했습니다.
- 거래소 확인 체결과 NoahAI 청산성과를 분리하고 기간 SQL 집계·통화별 손익·원장 연결 상태를 추가했습니다.
- Bithumb 종목별 미체결과 종목 필수 체결 이력, KIS ETF 공식 API, 키움 WebUI QAx/COM 별도 프로세스 경계를 수정했습니다.
- 다중 거래소 안전 종료를 일괄 신호·공통 대기로 바꾸고 Binance 중복 중지 호출을 제거했으며, Electron 제한시간과 worker 오류 전달을 보강했습니다.
- 현물의 계좌 전체 잔고와 NoahAI 관리 포지션을 분리해 수동·에어드롭·지원시장 외 자산을 자동매매 대상에서 제외하고 화면에 그 이유를 표시합니다.
- 전체 Python 회귀 `1,738 passed, 8 skipped`, React/TypeScript production build 49 modules, npm audit 0, 문서/버전 정합성 검사를 통과했습니다.
- 제품 버전 `3.9.1.8`, updater SemVer `3.9.108`. Windows 재빌드와 실계정/PAPER 검증 전에는 배포 완료가 아닙니다.

## 2026-08-23 - v3.9.1.7 장시간 성능·요청 격리 패치

- Web 네트워크 폴링을 완료 기반 순차 스케줄러로 변경하고 요청 시간 제한을 추가했습니다.
- 거래소 계좌 조회와 멤버십 확인을 single-flight로 통합해 동일 조회 중복 실행을 차단했습니다.
- 거래소 네트워크 호출이 설정·AI 커스텀 저장 잠금을 점유하지 않도록 분리했습니다.
- 운영 KPI 전체 집계와 최근 목록을 분리하고 상세 거래 통계를 SQLite 집계로 변경했습니다.
- 제품 버전 `3.9.1.7`, updater SemVer `3.9.107`. Windows 재빌드·업데이트·6개 거래소 24시간 PAPER soak 전에는 배포 완료가 아닙니다.

## 2026-08-23 - v3.9.1.6 학습 데이터·거래소 로그·런타임 안정화 패치

- 원본에서 제공하던 거래소별 독립 시작을 복구하도록 Web 런타임 명령의 전역 직렬 잠금을 제거하고 source별 워커 제어를 유지했습니다.
- 거래소 탭의 잔고 조회와 실행 버튼 상태를 분리했으며, 개별 실행과 설정 대상 전체 시작/실행 중 전체 정지를 모두 제공합니다. LIVE 일괄 시작은 실제 주문 경고와 대상 확인이 필수입니다.
- 로그인 후 계정별 메인/거래소 로그 sink를 다시 연결하고 현재 세션 LogStream을 영속 파일과 병합해 XAI 분석·전략·주문 로그를 실시간 표시합니다.
- 메인 대시보드 런타임 상태를 주기 갱신하고 죽은 워커의 오래된 실행 플래그를 제외해 `0/6 대기`, KPI 0, 운영 알림 불일치를 수정했습니다.
- AI 학습 화면은 최근 50개부터 열고 더보기마다 50개를 추가하며, 대용량 JSON은 최근 레코드만 tail로 읽고 현재 운영 파일의 전체 학습 수를 별도 계수·캐시합니다.
- 메인 로그는 최근 100줄, 거래소별 로그는 최근 200줄을 기준으로 1초마다 갱신하며 거래소·레벨·카테고리 필터와 숨김 옵션을 실제 데이터에 적용합니다.
- 상세 로그·로그 레벨 변경을 Trader·통합 Trader·Analyzer·connector가 공유하는 최신 런타임 설정에 반영합니다.
- 제품 버전 `3.9.1.6`, updater SemVer `3.9.106`. 전체 Python 회귀 `1,716 passed, 8 skipped`, React production build 48 modules, npm audit 0, 활성 소스·문서/버전·Web engine spec 감사를 통과했습니다. Windows 재빌드·v3.9.1.5→v3.9.1.6 업데이트·실제 다중 거래소 PAPER 검증 전에는 배포 완료가 아닙니다.

## 2026-08-22 - v3.9.1.5 Windows 거래소 API 인코딩·자격증명 검증 패치

- v3.9.1.4 Windows 연결 점검의 `cp949`·`🔧` 예외가 외부 API 요청 전 로컬 sidecar 초기화에서 발생함을 확인했습니다.
- Electron과 Python launcher의 UTF-8 실행 계약을 추가하고 설정 진단 출력이 제어 흐름을 바꾸지 않게 했습니다.
- 설정 정본 로더를 UTF-8 BOM·UTF-16·CP949 호환으로 통합하고, 유효한 기존 값과 API 자격증명을 보존한 채 검증 저장에서 UTF-8로 정규화합니다.
- 해독 불가·잘못된 JSON 설정 위의 저장과 잘못된 백업 복구를 차단하고, Web 설정 화면에 비밀값 없는 저장소 상태를 표시합니다.
- 6개 암호화폐 거래소 자격증명 저장·fresh reread·runtime refresh·account snapshot 라우팅을 회귀 검증했습니다.
- 연결 화면은 HTTP 200이 아니라 거래소별 `status=success`만 성공으로 판정합니다.
- 집중 회귀 `209 passed`, 전체 Python 회귀 `1,702 passed, 8 skipped`, React production build 47 modules, npm audit 취약점 0, 활성 소스·문서/버전·Web engine spec 감사를 통과했습니다.
- 제품 버전 `3.9.1.5`, updater SemVer `3.9.105`; Windows 설치본과 실제 외부 계정 점검 전 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-22 - v3.9.1.4 Windows 설정 잠금·업데이터 표시 패치

- 공개 v3.9.1.3의 설정 저장 실패 재현 제보를 근거로 해결 완료 판정을 철회하고 새 updater 버전 `3.9.104` 후보를 분리했습니다.
- `settings.json` 쓰기를 계정별 프로세스 간 잠금으로 직렬화하고 Windows 공유 위반을 재시도하며, 실패 원인을 `settings.write_failed`의 `code/stage/winerror`로 기록합니다.
- 원본 클라이언트의 제자리 쓰기와 달리 Web 패치의 `os.replace()`는 Windows 삭제 공유 권한을 추가로 요구해 기존 파일이 정상 쓰기 가능한 상태에서도 교체만 실패할 수 있었습니다. 교체 오류 5/32/33이 지속될 때만 완전한 임시 JSON과 원본 바이트를 보존한 제자리 쓰기·재검증으로 호환합니다.
- 단일 인스턴스 잠금에 실패한 Electron 프로세스가 startup handler와 sidecar를 시작하지 못하게 했습니다.
- 업데이트 확인 버전은 `latest.yml`을 정본으로 표시하고, 설정 하단 버튼의 축소·한글 글자 단위 줄바꿈을 막았습니다.
- 제품 버전 `3.9.1.4`, updater SemVer `3.9.104`; Windows 설치본 검증 전 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-21 - v3.9.1.3 설정 저장 재검증 패치

- 2026-08-22 전수 감사에서 정본 117개 최상위/646개 leaf를 직접 편집 132·전문가 JSON 261·전용 관리 171·보호 82로 분류하고 미분류 0, Web 활성 runtime 전체 writer 0, 사용자 명시 복구 경로 1개를 자동 게이트로 고정했습니다.
- Web의 잘못된 어시스턴트 프리셋(`beginner/advanced`)과 AI 커스텀 실험실 값(`laboratory`)을 런타임 정본 `saver/premium`, `lab`으로 마이그레이션합니다.
- 역할별 AI Provider/모델, 전사 상한, 음성, 자동업데이트 설정과 탭별·전체 기본값 불러오기, 최근 대화·저장 비용 프리셋을 사용하는 심층분석, 프로필별 AI 커스텀 Level 1~3을 연결했습니다.
- 최신 전체 회귀 `1,677 passed, 8 skipped`, 집중 회귀 `196 passed`, React build 47 modules, npm audit 0, 설정·AI 어시스턴트/전체 서비스/활성 소스/문서/sidecar 감사를 통과했습니다. Windows 설치본 E2E는 계속 PENDING입니다.
- 공개 v3.9.1.2에서 설정 저장 직후 값이 원복되고 API 연결확인이 실패하는 제보를 재접수해 저장 성공 판정을 강화했습니다.
- 설정 변경은 최신 정본 위에 경로 단위로 병합하고, 새 디스크 조회가 요청값과 일치해야 검증 영수증을 반환합니다. Web UI는 영수증이나 값이 불일치하면 사용자의 변경 대기를 유지합니다.
- Evaluator의 settings.json 직접 전체 쓰기와 자체 `.backup` 자동 복구를 제거했습니다. Windows 업데이트 뒤 남은 `NoahAIEngine.exe`도 새 sidecar 시작 전에 정리합니다.
- API 탭 전환 시 Provider 상태를 정규화하고, 입력 중인 자격증명은 저장·검증 후 연결 점검하며 다른 Provider의 모델을 교차 사용하지 않습니다.
- 기존 공통 AI 키를 당시 선택 Provider로 호환 로드하고 빈 `ai_credentials` 템플릿을 실제 저장 키로 오판하지 않도록 수정했습니다. Provider별 등록 여부, 네트워크 요청 전 중단, SDK 패키징 누락을 구분합니다.
- 제품 버전 `3.9.1.3`, updater SemVer `3.9.103`; Windows 설치본 검증 전 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-21 - v3.9.1.2 AI 커스텀 실행 범위·UX 수정 패치

- 전체 거래소·연결 증권사 표시값을 실행 엔진의 암호화폐·주식 자산군 범위로 정규화하고 v3.9.1.1 저장 별칭을 호환합니다.
- Level 1~3 XAI를 자료 범위/누락/위험, 핵심 규칙/엔진값/국면, 원문 추적/전체 IR/고급 편집으로 실제 분리했습니다.
- 초안 초기화, 소스 변경 재분석, 저장 잠금 사유, 패키지 기본 저장소, YouTube/PDF 처리 한도와 로컬 12단계 무과금 안내를 추가했습니다.
- Teayu 지원 데이터에서 확인된 설정 저장 후 503·값 되돌림을 수정했습니다. 디스크 저장 성공 뒤 런타임 반영 실패는 더 이상 저장 실패 503으로 표시하지 않고 재시작 안내로 분리하며, 실행 중 Trader·통합 Trader·옵티마이저의 오래된 설정 참조가 새 설정을 다시 덮지 않도록 같은 정본으로 동기화합니다.
- 백그라운드 AI 임계값 저장을 최신 설정 위의 경로 단위 병합으로 바꾸고 Analyzer·AlphaArena 설정 참조도 함께 갱신합니다. Web AI 어시스턴트의 무동작 차트분석 버튼은 실제 이미지 업로드·OCR·비전 AI·근거/위험/참고 플랜 화면으로 연결했습니다.
- 제품 버전 `3.9.1.2`를 updater SemVer `3.9.102`로 매핑했습니다. 새 Windows 자산과 v3.9.1.1→v3.9.1.2 실제 업데이트 전 상태는 `pending_windows_rebuild`, `publish_ready=false`입니다.

## 2026-08-21 - v3.9.1.1 설정 저장 패치

- 사용자 제공 설정 파일 조건에서 일반 설정과 write-only 거래소/AI 자격증명 변경이 저장되지 않고 503 안내가 뒤따르던 경로를 수정했습니다.
- 설정 저장은 원자 교체와 Windows 파일 점유 재시도를 사용하며 Web 화면의 후속 새로고침 실패를 파일 저장 실패와 분리했습니다. 엔진 내부 런타임 갱신 예외와 오래된 설정 재저장 경계는 위 v3.9.1.2 항목에서 추가로 완료했습니다.
- 제품 버전 `3.9.1.1`을 updater SemVer `3.9.101`로 매핑하고 package, lockfile, Windows 메타데이터, 설치 파일명, 대시보드 표시, manifest를 동기화했습니다.
- 기존 공개 v3.9.1.0 설치자는 새 GitHub Release의 `latest.yml`을 통해 패치를 받을 수 있습니다. 단, 새 Windows 자산과 v3.9.1.0→v3.9.1.1 실제 업데이트 게이트 전에는 게시하지 않습니다.

## 2026-08-20 - v3.9.1.0 Windows 최종 로컬 후보 재빌드

- Windows PowerShell 5.1에서 동작하지 않던 상대 경로 계산과 npm lockfile `packages[""]` JSON 처리를 호환 방식으로 수정하고, 빌드·게시 정확한 PowerShell 명령을 `docs/BUILD_GUIDE.md`에 정리했습니다.
- Windows/Linux Electron 기본 `File/Edit/View/Window` 메뉴를 제거하고, 안전 종료 후 PyInstaller one-file engine 자식 프로세스가 남지 않도록 Windows engine 프로세스 트리 정리를 추가했습니다.
- 설치본에서만 로그인 후 Gateway 오류와 1:1 화면 오류가 함께 발생하던 원인은 Web engine 패키지에서 `config/web_ui_feature_inventory.json`이 누락되어 `/api/v1/features`가 500을 반환한 것이었습니다. sidecar spec에 원장을 포함하고 packaged smoke가 `platform`, `session`, `features`, `runtime/snapshot` 인증·CORS 계약을 모두 검사하도록 확대했습니다.
- 공식 전체 빌드는 Web 집중 `120 passed`, 전체 `1,652 passed, 8 skipped`, production build 47 modules, npm audit 0, 활성 소스 381개와 engine TOC·health 감사를 통과했습니다.
- 수정 Setup SHA-256은 `0354c712e7d5be7bf7ef87e0d9901cb9a8f301267d82cb24a31190d6096abc5e`입니다. 격리 설치 후 내장 engine에서 health와 로그인 후 bootstrap API 4개를 모두 재검증했습니다.
- 사용자 설치본 확인 후 `v3.9.1.0` 정식 GitHub Release를 공개했습니다. Setup, `latest.yml`, blockmap, manifest 4개 자산의 원격 크기와 SHA-256 digest가 로컬 산출물과 일치합니다.

## 2026-08-19 - v3.9.1.0 Windows 최종 소스 재빌드 후보

- 이전 생성 산출물을 정리한 뒤 공식 Windows 전체 빌드로 Electron NSIS 설치기와 UI-neutral engine을 다시 생성했습니다. PowerShell 5.1에서 동작하지 않던 `[IO.Path]::GetRelativePath()` 사용을 호환 가능한 상대 경로 계산으로 교체했습니다.
- 깨끗한 빌드에서 manifest가 아직 없는 상태를 정상적으로 허용하도록 문서 동기화 테스트를 수정했습니다. 집중 회귀 `101 passed`, 전체 회귀 `1,620 passed, 8 skipped`, React production build, npm audit 0, 활성 소스·문서·engine TOC·health 감사를 통과했습니다.
- 설치기 `NoahAI-3.9.1.0-Setup.exe` SHA-256은 `af1f35e123901166f66d37a24dfb64953b5eec4efcbfe479d66f6ad568a82446`, engine SHA-256은 `59269707a1e01f7609800e0b1ef11f0216a56d019b653912542c214293838c44`입니다.
- 격리 설치·로그인 페이지 실행·engine 기동·silent 제거를 통과했습니다. 코드서명, 외부 자동업데이트·롤백·실계정·다중 모니터·24~72시간 PAPER와 UI 실행 원장 OPEN 항목은 남아 있어 `built_windows_unverified`, `publish_ready=false`를 유지합니다.

## 2026-08-16 - v3.9.1.0 Web UI 전체 서비스 소스 계약·데이터 경계 재감사

- 블록체인만이 아니라 주식/증권, 자산 통합, 생활금융, AI애널리스트, 메뉴얼, 설정을 하나의 정본 계약으로 다시 감사했습니다. 제품 서비스 5개·상위 기능 35개·거래소/증권사 10개·생활금융 내부 7개·설정 8개·매뉴얼 11개·AI애널리스트 카드 6개를 자동 검사하는 `scripts/audit_web_ui_full_parity_contract.py`를 추가했습니다.
- Gateway가 서비스와 무관한 기능 ID를 받아 공통 데이터로 우회하지 못하도록 서비스별 기능 소유권을 검증하고 `workspace_feature_service_mismatch`로 실패 폐쇄합니다.
- 생활금융 화면이 거래 workspace를 재사용하던 경로를 제거하고 계정별 `LifeFinanceManager` 정본 데이터와 분석을 사용하도록 연결했습니다.
- AI애널리스트가 기본 블록체인/Binance 문맥으로 떨어지던 경로를 제거하고 계정 전체 암호화폐·주식 거래, AI 판단·분석·위험·시나리오를 함께 집계합니다.
- AI 어시스턴트 심층분석의 과거 임시 `overview/report` 기능 ID를 블록체인 로그·주식 로그·자산 통합 인사이트·AI애널리스트 워크스페이스 정본 ID로 교체했습니다.
- 공통 `AI에게 묻기`가 자산 통합·생활금융·설정에서 블록체인 어시스턴트로 이동하던 교차 서비스 라우팅을 제거했습니다. 생활금융은 내부 7번째 `AI 어시스턴트` 탭과 전용 생활금융 snapshot을 사용하고, 자산 통합은 자산 문맥을 유지한 AI애널리스트 어시스턴트로 연결하며, 설정은 현재 서비스 문맥을 보존합니다.
- 현재 제품 기능 35개는 모두 전용 Web 화면과 정본 Application Service에 소스 연결됐으며 상태는 `source_connected_parity_open` 32개, `source_connected_external_e2e_open` 3개입니다. 이는 시각·동작 1:1 완료가 아니며 레거시 나란히 비교와 Windows 설치본 E2E 전에는 배포하지 않습니다.
- 집중 회귀 `147 passed`, 전체 회귀 `1,620 passed, 8 skipped`, React production build, 전체 범위 소스 계약 감사, 활성 소스 379개 감사, 설정 계약 감사를 통과했습니다.

## 2026-08-15 - v3.9.1.0 Web UI Internal Integration Candidate · 기존 화면 동등성 재작업

- 공유된 CTk/WebUI 좌우 비교를 정본으로 다시 감사해, 실시간 로그에 임의로 추가했던 `XAI 판단·체결 감사` 중복 탭을 제거했습니다. XAI·체결 근거는 AI 리포트 `실행 품질`과 하단 감사 원장에서 확인합니다.
- 주식/ETF에서 합쳐 보이던 AI 허브를 철회하고 `AI 학습 / AI 리포트 / AI 어시스턴트 / AI 커스텀`을 각각 독립 탭으로 복원했습니다. 시장 트렌드는 기존 5개 영역과 블록체인/증권별 명칭을 복원했습니다.
- 회전 로그를 최신 파일부터 읽은 뒤 배열 끝을 잘라 과거 2025 로그가 선택되던 오류를 수정했습니다. 모든 후보 줄을 실제 시각으로 정렬·중복 제거한 뒤 최신 항목을 반환합니다.
- 실제 로그인 Electron 재검증에서 기본 로그와 거래소 source 로그가 최신 데이터를 가지고도 첫 행에 머무르는 문제를 추가로 확인했습니다. 두 로그 콘솔 모두 렌더 뒤 마지막 행으로 자동 이동하며 2026-08-15 최신 로그가 보이는 것을 확인했습니다.
- 이전 Windows 설치 후보는 이번 parity 소스보다 오래되어 게시할 수 없습니다. manifest를 `pending_windows_rebuild`로 되돌리고 빌드 source fingerprint가 없거나 달라지면 게시 스크립트가 실패하도록 고정했습니다.
- 오후 기능 재감사에서 증권 화면에 Binance 상태·로그·코인 후보가 섞인 원인을 전역 `selected_source`와 공통 저장소 재사용으로 확인했습니다. 블록체인/증권의 선택·활성·실행 source와 로그를 서비스별로 분리하고, 증권 후보 저장소가 없을 때 암호화폐 후보를 주식으로 표시하지 않도록 실패 폐쇄했습니다.
- AI 어시스턴트의 기존 자주 묻는 질문을 블록체인/증권/애널리스트별로 복원하고 초보자·일반·고급 설명 수준과 로컬 무과금 안내/명시 외부 심층분석을 구분했습니다. 비용이 생길 수 있는 선택은 `심층분석 (외부 AI·비용)`으로 명확히 표시합니다.
- Web UI의 축약·재작성 매뉴얼을 제거했습니다. 빌드 시 `ui/widgets/user_manual_widget.py`에서 실제 `show_manual()`이 여는 11개 탭과 본문 전체를 `docs/USER_MANUAL_SECTIONS.json`으로 추출하고, Gateway와 Web UI가 그 정본만 읽습니다. 정의만 있고 열리지 않는 2개 메서드를 탭으로 세던 기존 테스트도 수정했습니다. 화면 크기·검색·딥링크 동등성은 계속 OPEN입니다.
- 이번 변경의 최신 집중 검증은 `61 passed`, React production build 43 modules, JS `329.07 kB`(gzip `98.09 kB`) PASS입니다. 저장소 전체 회귀 `1,536 passed, 8 skipped`와 npm/활성 소스/engine 감사 수치는 직전 전체 검증 기준선이며 이번 변경 뒤 전체 회귀는 별도 재실행 대상입니다. 전체 탭/설정/매뉴얼 렌더 대조, 실제 계좌·주문 및 Windows 설치·업데이트·롤백 E2E 전에는 공개하지 않습니다.

- 정오 사용자 실화면 재감사에서 코인 정보와 BINANCE 동시 선택, 공통 차트/원시 DB 화면 오연결, 로그 전체 창 세로 확장, 거래소·AI 어시스턴트·AI 커스텀·설정·매뉴얼 흐름 불일치를 확인했습니다. 오전의 일부 화면 PASS 표현을 철회하고 모두 동등성 원장에 OPEN/RECHECK로 되돌렸습니다.
- Electron 개발/Dock과 Windows 패키지에 NoahAI 제품 아이콘을 지정하고 로그인 카드 높이·버튼 아래 여백, 도움말 탭·본문 글자 크기를 기존 화면 기준으로 재조정했습니다.
- 대시보드를 고정 viewport로 바꾸고 실시간 로그·우측 운영 열이 각자 내부 스크롤을 사용하도록 수정했습니다.
- 기능 탭과 거래소·증권사 source 탭을 상호배타 상태로 변경했습니다. 코인·종목 정보는 심볼 조회·AI 평가 점수표, 거래소·증권사는 제어·잔고·포지션·거래 통계·해당 로그 전용 화면을 사용하며 개발용 원시 DB 표가 제품 화면을 대신하지 않습니다.
- AI 어시스턴트는 기존 대화창·FAQ·복사/TXT·전송·음성·설정관리·차트분석 구조로, AI 커스텀은 제품 철학·멘토/처음 사용법 차이·안전 순서·원본→XAI→버전 관리 흐름으로 다시 배치했습니다.
- 로그인 정보 저장은 renderer/localStorage가 아니라 Electron OS 암호화 저장소와 권한 제한 파일을 사용하도록 연결했습니다. 저장 실패가 로그인 성공 자체를 실패로 바꾸지 않으며, 저장 해제 시 암호문 파일을 제거합니다.
- 로그인과 로그인 전 도움말은 실제 레거시와 macOS Electron에서 다시 나란히 대조했습니다. 최신 검증은 React production build 49 modules, 집중 회귀 `12 passed`, 저장소 전체 `1,529 passed, 8 skipped`, npm audit `0 vulnerabilities`입니다.
- 로그인된 macOS 대시보드의 기본 로그·BINANCE source·주식/증권 내비게이션은 실제 렌더로 재검증했습니다. 설정/매뉴얼과 나머지 기능 탭 전수 대조, Windows 설치본과 실제 계정 E2E 전에는 `publish_ready=false`를 유지합니다.

- “기존 기능 소스 이전 완료”가 UI/UX와 사용자 흐름까지 완료했다는 뜻으로 읽힌 문제를 정정했습니다. 당시 완료 범위는 API/Application Service/React 컴포넌트 연결이었으며 기존 `AITrading.exe`/`python3 main.py` 화면 동등성은 미완료였습니다.
- 로그인 450×708, 실제 로고, 로그인 전 별도 도움말 3탭과 로그인 뒤 `실시간 거래 로그` 기본 화면을 복원했습니다.
- 실시간 로그는 기존 좌측 로그·필터·도구와 우측 KPI·거래 현황·Quick Actions·운영 알림·통합 잔고 구성으로 복원했습니다.
- 거래소 범위를 6개 암호화폐와 4개 증권사로 분리하고 내부 통합 source 탭과 비활성 AlphaArena를 사용자 메뉴에서 숨겼습니다.
- 설정은 세로형 구성을 유지하면서 기존 8개 설정 영역, 영역별 `AI에게 묻기`, write-only 연결 정보, 변경분 저장을 연결했습니다. 매뉴얼은 11개 기능 탭·검색·AI 질문으로 갱신했습니다.
- 하단 AI 실행 기록을 실제 계정 감사 원장에 연결하고 종료 버튼을 안전 종료 handshake에 연결했습니다.
- macOS Electron 실제 렌더로 로그인·도움말·기본 로그·설정·매뉴얼을 확인했고 집중 회귀 `41 passed`, React production build 48 modules를 통과했습니다. 전체 기능 탭의 기존 화면 대조와 Windows 설치본 E2E는 아직 남아 있어 `publish_ready=false`입니다.
- 세부 판정은 `docs/WEB_UI_LEGACY_PARITY_MATRIX_v3.9.1.0.md`를 따릅니다.

## 2026-08-14 - v3.9.1.0 Web UI Internal Integration Candidate · 기능 소스 연결, 외부 검증 진행 전

- 최초 Windows x64 NSIS 후보에서 packaged engine의 상대 import 오류와 콘솔 없는 실행 시 Uvicorn formatter 오류를 발견해 수정하고 build-time health smoke를 추가한 뒤 후보를 교체했습니다. 현재 `NoahAI-3.9.1.0-Setup.exe` SHA-256은 `a0ecd5bee659cc7bf84bab849493046f8bd870fba09a803cf9c8518d17c0c4bb`, UI-neutral engine SHA-256은 `933905e7fddbae4897091cb4e14c5b080fd70554f4f52ac3ba2f7d2d2ff1673c`입니다. 현재 상태는 `built_windows_unverified`, `publish_ready=false`이며 Authenticode는 미서명입니다.
- Windows PowerShell의 manifest UTF-8 BOM, Electron의 4단계 제품 버전과 SemVer 매핑, Electron main 진입점, PyInstaller TOC 실제 작업 경로·제외 목록 오탐을 바로잡았습니다. 기본 회귀에서 실제 Binance 자격증명을 읽던 TP/SL 통합 테스트는 `INTEGRATION_TEST=1` 명시 실행으로 격리했습니다.

- 남아 있던 `read_first` 12개를 전용 Web 수직 기능으로 이전했습니다. 블록체인/주식 금융 인텔리전스, 자산 인사이트·배분·리스크·성과, 생활금융 분석·상품·세금, AI 요약·금융 인텔리전스 허브가 공통 placeholder가 아닌 전용 Application Service·엄격 DTO·Gateway·React 화면을 사용합니다.
- AlphaArena는 Web UI에서 상태·시작·정지·이벤트를 제공하되 v3.9.1.0 내부 후보는 PAPER만 허용합니다. PAPER 판단은 위험 게이트를 통과해도 거래소 주문을 제출하지 않으며 LIVE는 외부 소유권·복구·장시간 검증 전 fail-closed입니다.
- 자산 분석은 KRW/USDT를 임의 합산하지 않고 snapshot 시각·부분 실패·데이터 부족을 표시합니다. 상품 비교는 신청하지 않으며 세금 계산은 참고용으로만 반환합니다. 금융 인텔리전스는 사용자가 명시한 시장 갱신만 수행하고 직접 거래 신호로 쓰지 않습니다.
- 당시 기능 원장은 `source_complete`라는 과도한 명칭을 사용했습니다. 2026-08-15 재감사에서 이 완료 표현을 폐기하고 현재 43개를 `source_connected_parity_open` 35, `source_connected_external_e2e_open` 4, `legacy_parity_in_progress` 2, `integrated_in_ai_custom` 1, `server_later` 1로 다시 판정했습니다.
- 실브라우저 1920×1080 기준에서 상단 매뉴얼/설정 노출, 전용 화면 렌더, 가로 overflow 0, 콘솔 오류 0을 확인했습니다. 동기화 도중 실제 발생한 `*_Conflict.tsx` 이름 충돌은 Python뿐 아니라 TypeScript/JavaScript/JSON까지 빌드 전 활성 소스 감사에서 차단합니다.

- Web runtime의 `from main import NoahAIClient` 경로를 제거하고 UI-neutral `HeadlessTradingRuntime`을 추가했습니다. Electron sidecar가 `main.py`, `ui.*`, `tkinter`, `customtkinter`를 거래 엔진 생성 과정에서 불러오지 않습니다.
- Web sidecar 전용 PyInstaller spec을 독립 작성하고 레거시 spec 상속을 제거했습니다. Windows 빌드는 `Analysis-00.toc`에서 레거시 UI 모듈이 하나라도 발견되면 실패합니다. PyQt5/QAx는 키움 broker 경계에만 유지합니다.
- Electron 종료·업데이트는 Gateway의 안전 종료 endpoint가 신규 명령 차단, 거래 worker 정지, Recorder/log flush를 완료한 뒤에만 sidecar 종료와 `quitAndInstall`을 허용합니다. 실패·timeout이면 설치를 중단합니다.
- Windows 공개 이름을 설치기 `NoahAI-3.9.1.0-Setup.exe`, 사용자 실행 파일 `NoahAI.exe`, 내부 sidecar `NoahAIEngine.exe`로 고정했습니다. 기존 `AITrading.exe`는 v3.9.0.10 레거시 자산으로만 보존합니다.
- Web 조회 경로의 자산 데이터 정규화를 `ui` 패키지 밖으로 옮기고 감사 JSON 내보내기와 기능별 데이터 설명·표시를 추가했습니다.
- 이전의 근거 없는 “기능 전환 완료” 판정은 철회한 상태를 유지합니다. 소스/API 연결은 완료 판정이 아니며 `legacy_visual_parity`, `legacy_flow_parity`, 설치본 외부 검증이 남아 있으므로 공개 배포 상태는 차단됩니다.
- 현재 소스 검증은 전체 Python `1,517 passed, 6 skipped`, React production build 48 modules, npm audit `0 vulnerabilities`, 문서·Python/Web 활성 소스 감사 PASS입니다. 이 수치는 Windows 설치·자동업데이트·실브로커·PAPER soak 증거가 아닙니다.

- React/Vite 기본 UI에 5개 서비스, 설정, AI Custom, AI 어시스턴트/애널리스트, 자산 통합, 생활금융, 로그, 실행 제어, 차트, 매뉴얼과 업데이터 UI를 연결했습니다. 1080px에서 매뉴얼·설정 버튼이 숨지 않고 가로 넘침이 없도록 반응형 헤더를 보정했습니다.
- 설정은 템플릿 일반 필드 94/116과 증권사 안전 중첩 필드를 편집하며, 모든 거래소·증권사·AI 자격증명은 write-only로 처리합니다. 저장은 revision/diff와 적용 범위를 반환하고 설정 모달은 Escape·backdrop으로 정리됩니다.
- 6개 암호화폐 거래소의 spot/futures 캔들과 4개 증권사의 일봉 계약을 공통 차트에 연결했습니다. 진입·청산·XAI 마커, 명시적 재시도, 비활성/미연결 fail-closed를 적용했습니다.
- AI Custom은 text/Pine/TradingView/YouTube/URL/PDF/이미지/영상 원본, 제한 IR·XAI, 버전 diff/rollback, 승인·적용·해제·삭제, `.noahstrategy` import/export, 과거 최소검증, 암호화폐·주식 PAPER 결과의 전략 key/version 자동 귀속을 제공합니다.
- AI 도움말은 무과금 로컬 정본을 사용하고, 심층분석은 사용자가 버튼으로 명시 실행한 질문만 Provider를 호출합니다. 일/월 예산, 캐시, 토큰, 예상 비용을 보여줍니다.
- `web_platform/launcher.py --gateway-only`, 독립 `noahai_web_engine.spec`, `scripts/build_web_ui_windows.ps1`, Electron 명시 다운로드·설치/재시작 UI를 추가했습니다.
- 최신 검증 수치는 `docs/TEST_STATUS.md`를 정본으로 사용합니다. 과거 `1,507 passed` 기록은 당시 소스 결과이며 현재 Windows 산출물 증거가 아닙니다.
- 기능 동등성 완료와 Windows 배포 완료를 별도 게이트로 관리합니다. 현재 후보는 외부 게이트 전 `built_windows_unverified`를 유지합니다.

### 초기 수직 통합 기록

- 동일 계정 정본을 사용하는 Application Services를 추가해 설정 snapshot/revision/diff, 허용 필드 저장, 거래소·AI write-only 자격증명 갱신을 Web UI에 연결했습니다. API 키·secret·token은 renderer 응답과 감사 로그에서 반환하지 않습니다.
- 로그인 전 계정 데이터 병렬 조회를 제거하고 daltrading 인증 성공 뒤에만 계정별 설정·전략·원장을 엽니다. 비밀번호는 저장하지 않고 토큰은 계정 파일에만 보존하며 응답·감사 로그에 노출하지 않습니다.
- AI 커스텀 프라이빗 전략의 새 버전 생성, XAI/IR 조회, 승인·적용·해제, 활성 삭제 차단, 비활성 버전/전체 전략 2단계 삭제를 연결했습니다. 사용자 수기 PAPER/PnL 입력을 제거하고 Binance 공개 캔들 500개와 비용으로 엔진이 PnL·MDD를 직접 계산하며 과거재생 단독 적용은 차단합니다.
- 외부 Provider를 호출하지 않는 초보자·일반·고급 AI 제품 가이드와 인앱 핵심 매뉴얼을 추가했습니다. 설정·AI 커스텀·백테스트/PAPER·webhook 질문은 토큰 비용 없이 버전 정본으로 답합니다.
- Electron은 임의 loopback port와 실행별 token, `NOAHAI_ENABLE_WEB_RUNTIME=1`로 Python sidecar를 기동합니다. 거래 엔진은 인증 뒤 최초 명시 시작 명령에서 지연 생성되고 암호화폐는 기존 Binance/Unified 경로를 사용합니다.
- 대시보드에 있던 증권 자동매매 스레드를 UI-neutral `StockRuntimeController`로 분리했습니다. PAPER 우선과 전역·증권사·어댑터 LIVE AND 게이트를 유지하고 소유권 증거 없는 계좌 전체 청산을 차단합니다.
- 이 항목의 `19 passed`/기본 수직 통합은 중간 기록이다. 최종 판정은 `WEB_UI_MIGRATION_STATUS_v3.9.1.0.md`와 기능 inventory를 따른다.

### Stage 0 기준선

- 기존 Python 거래·AI 엔진과 v3.9.0.10 운영 UI를 변경하지 않고 Web UI 전환용 기능 inventory, 엄격한 DTO/event 계약, localhost Gateway를 추가했습니다.
- Gateway는 강한 실행별 token, Origin 제한, URL에 token을 두지 않는 WebSocket 최초 메시지 인증을 사용합니다. 플랫폼·기능·런타임 snapshot·Binance 공개 캔들 GET만 제공하며 설정·전략·주문 명령은 제공하지 않습니다.
- React/Vite UI에 서비스 내비게이션, 읽기 전용 안전 게이트, 엔진 snapshot, Lightweight Charts 기반 BTCUSDT 캔들·거래량 POC를 구현했습니다.
- Electron 43.4.0 셸은 `nodeIntegration=false`, `contextIsolation=true`, `sandbox=true`, 외부 navigation/window 차단을 적용했습니다. Node 22.12 이상에서 build/install하며 npm audit 0건을 확인했습니다.
- Web 플랫폼 집중 Python 회귀 `8 passed`, 전체 회귀 `1,483 passed, 6 skipped`, TypeScript/Vite production build와 브라우저 실렌더·서비스/주기 전환·콘솔 오류 0건을 통과했습니다. 실제 레거시 runtime adapter, Windows bundle·자동업데이트·롤백·다중 모니터는 후속 게이트입니다.
- v3.9.0.10 EXE SHA `6aaa66787666...`를 이전 자산으로 보존했으며 v3.9.1.0은 Windows 자산 생성 전 `pending_windows_rebuild`입니다.
...
- 상세 원인·변경 계약·재빌드 후 시험 절차는 `docs/archive/history/INCIDENT_260812_V3909_UI_SETTINGS_ROOT_CAUSE.md`를 정본으로 사용합니다.
