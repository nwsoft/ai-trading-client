  ...
  return <section className="legacy-log-workspace">
    <article className="legacy-log-card">
      <div className="legacy-log-filters">
        <label><input type="checkbox" checked={simpleOnly} onChange={(event) => { setSimpleOnly(event.target.checked); if (event.target.checked) setAnalysisOnly(false); }} />거래 시그널만</label>
      ...
      <div className="legacy-log-console" ref={logConsoleRef}>
        ...
        {!logs && !error && <div className="empty-state">실시간 로그를 불러오는 중입니다.</div>}
        {logs && !filteredLines.length && <div className="empty-state">현재 필터에 표시할 로그가 없습니다.</div>}
      </div>
      <div className="legacy-log-toolbar">
        <button type="button" onClick={() => setLogHelpOpen(true)}>로그도움말</button>
        <button type="button" onClick={() => { clearMarkerRef.current = visibleLines.at(-1)?.message ?? logs?.lines.at(-1)?.message ?? ""; setVisibleLines([]); }}>로그지우기</button>
        <button type="button" onClick={() => { clearMarkerRef.current = ""; refresh(); }}>새로고침</button>
        <span />
        <label>로그 레벨:<select value={logLevel} onChange={(event) => setLogLevel(event.target.value)}><option>ALL</option><option>INFO</option><option>WARNING</option><option>ERROR</option></select></label>
        <label>{sourceNoun}:<select value={exchange} onChange={(event) => setExchange(event.target.value)}><option>ALL</option>{allowedSources.map((item) => <option key={item}>{item.toUpperCase()}</option>)}</select></label>
        <label>카테고리:<select value={category} onChange={(event) => setCategory(event.target.value)}><option>ALL</option><option>거래</option><option>분석</option><option>학습</option><option>시스템</option></select></label>
      </div>
    </article>
    <aside className="legacy-operations-column">
      <section><h3>운영 KPI · 오늘 · {statisticsMode.toUpperCase()}</h3><div className="legacy-kpi-grid"><div className="legacy-kpi-card"><span>{statisticsMode === "paper" ? "가상 포지션" : "현재 포지션"}</span><b>{openPositionCount}</b></div><div className="legacy-kpi-card"><span>{statisticsMode === "paper" ? "가상 청산" : "오늘 청산"}</span><b>{trading?.closed_count ?? 0}</b></div><div className="legacy-kpi-card"><span>{statisticsMode === "paper" ? "가상 손익" : "대조 완료 손익"}</span><b className={confirmedMetricsAvailable ? (pnl < 0 ? "negative" : "positive") : ""}>{pnlLabel}</b></div><div className="legacy-kpi-card"><span>{statisticsMode === "paper" ? "가상 승률" : "대조 완료 승률"}</span><b>{winRateLabel}</b></div></div></section>
      <section><h3>거래 현황</h3><div className="legacy-operation-box"><b>{statisticsMode === "paper" ? "현재 가상 원장 + 오늘 PAPER 청산" : "현재 계좌 + 오늘 LIVE 청산"}</b><p>• {statisticsMode === "paper" ? "현재 가상 포지션" : "현재 계좌 포지션"}: {openPositionCount}{statisticsMode === "live" ? accountPositionsAvailable ? "" : requestedAccountSources.length ? " (일부 조회 실패 · 저장 원장 참고)" : " (계좌 조회 전 · 저장 원장 참고)" : ["temporarily_unavailable", "partial"].includes(String(workspace?.paper_positions_status ?? "")) ? " (일부 런타임 조회 지연)" : ""}<br />• 오늘 {statisticsMode === "paper" ? "가상 " : ""}청산 수: {trading?.closed_count ?? 0}{statisticsMode === "live" ? ` · 체결 대조 완료 ${Number(trading?.reconciled_closed_count ?? 0)}건 · 미확정 ${Number(trading?.unresolved_closed_count ?? 0)}건` : ""}<br />• 오늘 {statisticsMode === "paper" ? "가상 " : "체결 대조 완료 "}순손익: {pnlLabel}<br />• 자동 거래 상태: {runningCount ? "실행 중" : `대기 (${runningCount}/${enabledCount} ${sourceNoun})`}{mixedExecutionModes ? <><br />• 혼합 운용 중: 메인 KPI는 LIVE 기준이며 PAPER 상세는 거래 통계 탭에서 확인</> : null}</p></div></section>
      <section><h3>{sourceNoun} 일괄 실행</h3><div className="legacy-quick-actions"><button type="button" disabled={batchBusy || !startableSources.length} onClick={() => void commandAll("start")}>{batchBusy ? "처리 중…" : "설정 대상 전체 시작"}</button><button type="button" disabled={batchBusy || !stoppableSources.length} onClick={() => void commandAll("stop")}>실행 중 전체 정지</button></div>{batchMessage && <div className="legacy-operation-box"><p>{batchMessage}</p></div>}</section>
      <section><h3>Quick Actions</h3><div className="legacy-quick-actions"><button type="button" onClick={() => onAskAssistant("현재 설정과 거래 기록을 기준으로 AI 최적화 진단을 해줘. 변경은 하지 말고 근거와 위험을 설명해줘.")}>AI 최적화 진단</button><button type="button" onClick={() => onAskAssistant("AI 최적화 적용 후보를 보여줘. 자동 적용하지 말고 현재값, 제안값, 근거와 위험을 비교해줘.")}>AI 최적화 적용</button></div></section>
      <section><h3>실시간 운영 알림</h3><strong className={`legacy-alert-badge level-${alertLevel}`}>{alertLevel}</strong><div className="legacy-operation-box"><p>{alertText}</p></div></section>
      <section><h3>통합 잔고 요약</h3><div className="legacy-operation-box"><p>• {statisticsMode === "paper" ? "가상" : "체결 대조 완료"} 순손익: {pnlLabel}<br />• 계좌 잔고는 자산 통합에서 명시적으로 새로고침합니다.</p></div></section>
    </aside>
...
export function LifeFinance({
  ...
  const summaryPanel = <article className="panel finance-summary"><div className="panel-heading"><div><span className="eyebrow">PERSONAL FINANCE</span><h2>{view === "personal_finance.cashflow" ? "현금흐름 분석" : "생활금융 대시보드"}</h2></div><button className="secondary-button" type="button" onClick={refresh}>새로고침</button></div>{error && <div className="inline-notice error-text">{error}</div>}{message && <div className="inline-notice">{message}</div>}<div className="metric-grid life-finance-kpi-grid"><div><span>누적 수입</span><strong>{Number(cumulative.total_income ?? 0).toLocaleString()}원</strong></div><div><span>누적 지출</span><strong>{Number(cumulative.total_expenses ?? 0).toLocaleString()}원</strong></div><div><span>순자산 흐름</span><strong>{Number(cumulative.net_position ?? 0).toLocaleString()}원</strong></div><div><span>기록 거래</span><strong>{data?.transactions.length ?? 0}건</strong></div></div></article>;
  const transactionPanel = <article className="panel finance-editor life-finance-entry-panel"><div className="panel-heading"><div><span className="eyebrow">TRANSACTIONS</span><h2>거래</h2></div></div><div className="command-row life-finance-toolbar"><button type="button" className="primary-button" onClick={() => setTx({ ...tx, type: "지출" })}>+ 지출 추가</button><button type="button" className="primary-button" onClick={() => setTx({ ...tx, type: "수입" })}>+ 수입 추가</button><button type="button" className="secondary-button" onClick={refresh}>새로고침</button></div><div className="form-grid life-finance-form"><label>날짜<input type="date" value={tx.date} onChange={(event) => setTx({ ...tx, date: event.target.value })} /></label><label>구분<select value={tx.type} onChange={(event) => setTx({ ...tx, type: event.target.value })}><option>지출</option><option>수입</option></select></label><label>금액<input type="number" min="1" value={tx.amount} onChange={(event) => setTx({ ...tx, amount: event.target.value })} /></label><label>결제 수단<input value={tx.method} onChange={(event) => setTx({ ...tx, method: event.target.value })} /></label></div><label className="life-finance-description">설명<input value={tx.description} maxLength={500} onChange={(event) => setTx({ ...tx, description: event.target.value })} /></label><div className="command-row life-finance-save-row"><button className="primary-button" disabled={!tx.amount || !tx.description.trim()} onClick={addTransaction} type="button">저장</button></div><div className="compact-list life-finance-records">{data?.transactions.slice(0, 30).map((item) => <div key={String(item.id)}><span>{String(item.date)} · {String(item.type)}</span><strong>{Number(item.amount ?? 0).toLocaleString()}원</strong><small>{String(item.description ?? "")}</small><button className="text-button danger-text" onClick={() => deleteTransaction(String(item.id))} type="button">삭제</button></div>)}{!data?.transactions.length && <div className="empty-state">거래 내역이 없습니다.</div>}</div></article>;
  const goalPanel = <article className="panel goal-panel life-finance-entry-panel"><div className="panel-heading"><div><span className="eyebrow">GOALS</span><h2>{view === "personal_finance.goals" ? "생활금융 목표" : "목표"}</h2></div><span className="count-badge">{goals.length}개</span></div><div className="form-grid life-finance-form"><label>목표명<input value={goal.name} onChange={(event) => setGoal({ ...goal, name: event.target.value })} /></label><label>목표 금액<input type="number" min="1" value={goal.target_amount} onChange={(event) => setGoal({ ...goal, target_amount: event.target.value })} /></label><label>마감 기한<input type="date" value={goal.deadline} onChange={(event) => setGoal({ ...goal, deadline: event.target.value })} /></label><label>우선순위<select value={goal.priority} onChange={(event) => setGoal({ ...goal, priority: event.target.value })}><option>높음</option><option>중간</option><option>낮음</option></select></label></div><div className="command-row life-finance-save-row"><button className="secondary-button" disabled={!goal.name.trim() || !goal.target_amount} onClick={addGoal} type="button">+ 목표 추가</button></div><div className="goal-list life-finance-records">{goals.map((goalItem) => <div key={String(goalItem.id)}><div><strong>{String(goalItem.name)}</strong><span>{Number(goalItem.progress_rate ?? 0).toFixed(1)}%</span></div><progress max={100} value={Number(goalItem.progress_rate ?? 0)} /><small>{Number(goalItem.current_amount ?? 0).toLocaleString()} / {Number(goalItem.target_amount ?? 0).toLocaleString()}원</small><div className="command-row"><button className="text-button" onClick={() => addGoalSavings(String(goalItem.id))} type="button">+ 저축</button><button className="text-button danger-text" onClick={() => deleteGoal(String(goalItem.id))} type="button">삭제</button></div></div>)}{!goals.length && <div className="empty-state">활성 목표가 없습니다. 새로운 목표를 추가해보세요.</div>}</div></article>;
  const dashboardPanel = <section className="life-dashboard-stack">
    <article className="panel legacy-life-monthly">
      <div className="panel-heading"><div><h2>{String(thisMonth.date_str ?? "이번 달")} 월간 재무 요약</h2></div><button className="secondary-button" type="button" onClick={refresh}>새로고침</button></div>
      ...
      <div className="legacy-life-metrics">
        <div><span>수입</span><strong>{Number(thisMonth.total_income ?? 0).toLocaleString()}원</strong></div>
        <div><span>지출</span><strong>{Number(thisMonth.total_expense ?? 0).toLocaleString()}원</strong></div>
        <div><span>저축</span><strong>{Number(thisMonth.net_savings ?? 0).toLocaleString()}원</strong></div>
        <div><span>저축률</span><strong>{Number(thisMonth.savings_rate ?? 0).toFixed(1)}%</strong></div>
        <div><span>누적 수입</span><strong>{Number(cumulative.total_income ?? 0).toLocaleString()}원</strong></div>
        <div><span>순 자산</span><strong>{Number(cumulative.net_position ?? 0).toLocaleString()}원</strong></div>
      </div>
    </article>
    <article className="panel legacy-life-comparison">
      <div className="panel-heading"><div><h2>이전 달과 비교</h2></div></div>
      <div className="legacy-life-comparison-grid">
        <div><span>수입 변화</span><strong className={Number(comparison.income_change ?? 0) < 0 ? "negative" : "positive"}>{Number(comparison.income_change ?? 0) >= 0 ? "+" : ""}{Number(comparison.income_change ?? 0).toLocaleString()}원</strong><small>지난달 대비 수입</small></div>
        <div><span>지출 변화</span><strong className={Number(comparison.expense_change ?? 0) > 0 ? "negative" : "positive"}>{Number(comparison.expense_change ?? 0) >= 0 ? "+" : ""}{Number(comparison.expense_change ?? 0).toLocaleString()}원</strong><small>지난달 대비 지출</small></div>
        <div><span>저축 변화</span><strong className={Number(comparison.savings_change ?? 0) < 0 ? "negative" : "positive"}>{Number(comparison.savings_change ?? 0) >= 0 ? "+" : ""}{Number(comparison.savings_change ?? 0).toLocaleString()}원</strong><small>지난달 대비 순저축</small></div>
      </div>
    </article>
    {dashboardGoals.length > 0 && <article className="panel"><div className="panel-heading"><div><h2>목표 진행 ({Number(data?.summary.goals?.active_count ?? dashboardGoals.length)}개 진행 중)</h2></div></div><div className="goal-list">{dashboardGoals.map((goalItem) => <div key={String(goalItem.id)}><div><strong>{String(goalItem.name)}</strong><span>{Number(goalItem.progress_rate ?? 0).toFixed(1)}%</span></div><progress max={100} value={Number(goalItem.progress_rate ?? 0)} /><small>{Number(goalItem.current_amount ?? 0).toLocaleString()}원 / {Number(goalItem.target_amount ?? 0).toLocaleString()}원 · 남은 금액 {Number(goalItem.remaining_amount ?? 0).toLocaleString()}원</small></div>)}</div></article>}
    {dashboardAlerts.length > 0 && <article className="panel"><div className="panel-heading"><div><h2>금융 알림</h2></div></div><div className="alert-list">{dashboardAlerts.slice(0, 5).map((alert, index) => <section className={`alert-card ${String(alert.level ?? "info")}`} key={`${String(alert.title)}-${index}`}><strong>{String(alert.title ?? "알림")}</strong><p>{String(alert.message ?? "")}</p><small>{String(alert.action_hint ?? "")}</small></section>)}</div></article>}
  </section>;
  ...
  return <section className="life-finance-service">
    <article className="panel life-finance-header"><h1>생활금융 관리</h1><div className="command-row"><button type="button" onClick={() => setInnerTab("대시보드")}>대시보드</button><button type="button" onClick={() => { setInnerTab("거래"); setTx({ ...tx, type: "지출" }); }}>지출 추가</button><button type="button" onClick={() => { setInnerTab("거래"); setTx({ ...tx, type: "수입" }); }}>수입 추가</button><button type="button" onClick={() => setInnerTab("목표")}>목표 관리</button><button type="button" onClick={() => setInnerTab("금융상품")}>금융상품</button></div></article>
    <nav className="life-finance-inner-tabs" aria-label="생활금융 서비스 내부 기능">{tabs.map((tab) => <button className={tab === innerTab ? "active" : ""} key={tab} type="button" onClick={() => setInnerTab(tab)}>{tab}</button>)}</nav>
