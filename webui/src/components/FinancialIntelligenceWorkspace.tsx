...
function LineChart({ points = [], series, title = "가격 흐름", valueSuffix = "" }: { points?: Array<Record<string, any>>; series?: ChartSeries[]; title?: string; valueSuffix?: string }) {
  ...
  return <div className="intelligence-chart-shell" aria-label={title}>
    ...
    </svg> : <span>{t("조회하면 검증 가능한 시계열이 여기에 표시됩니다.")}</span>}</div>
  </div>;
}
...
function ScreenerResult({ payload }: { payload: Record<string, any> }) {
  ...
  return <div className="intelligence-result-view"><section className="intelligence-result-section"><header><h3>{t("조건 통과 후보")}</h3><span>{t("가격 조건을 통과한 ")}{rows.length}{t("개 자산")}</span></header>{rows.length ? <>
    <div className="candidate-rank-list">{rows.map((row, index) => <article key={`${row.symbol}-${index}`}><b>{index + 1}</b><div><strong>{row.symbol}</strong><span>{row.asset_type} · {row.status === "ok" ? "시세 정상" : displayValue(row.status)}</span></div><div><span>{t("현재가")}</span><strong>{displayValue(row.price)}</strong></div><div><span>{t("1개월")}</span><strong className={Number(row.returns?.["1M"]) >= 0 ? "positive" : "negative"}>{periodValue(row.returns?.["1M"])}</strong></div><div><span>{t("연환산 변동성")}</span><strong>{periodValue(row.volatility_annualized)}</strong></div><small>{sourceName(row)}</small></article>)}</div>
  </> : <div className="honest-empty-state"><b>{t("조건을 통과한 후보가 없습니다.")}</b><span>{t("입력 종목, 최소 가격, 공개 시세 연결 상태를 확인하세요.")}</span></div>}</section>{errors.length > 0 && <section className="intelligence-result-section warning-section"><header><h3>{t("수집하지 못한 종목")}</h3><span>{errors.length}{t("건")}</span></header>{errors.map((row, index) => <p key={index}>{row.symbol ?? t("종목")}: {row.error ?? "시세 수집 실패"}</p>)}</section>}</div>;
}
...
function TechnicalResult({ payload }: { payload: Record<string, any> }) {
  ...
  return <div className="intelligence-result-view"><div className="technical-chart-grid"><LineChart title={`${payload["종목"] ?? "종목"} 가격·이동평균`} series={[{ label: "종가", values: prices, color: "#3b82f6" }, { label: "MA20", values: movingAverage(prices, 20), color: "#22c55e" }, { label: "MA50", values: movingAverage(prices, 50), color: "#f59e0b" }]} /><LineChart title={t("RSI(14) 흐름")} valueSuffix="" series={[{ label: "RSI", values: rsiSeries(prices), color: "#a78bfa" }]} /></div><section className="intelligence-result-section"><header><h3>{t("지표 판독")}</h3><span>{payload.count ?? prices.length}{t("개 시세 표본")}</span></header><div className="indicator-metric-grid">{metrics.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><div className="signal-strip"><span>RSI <b>{signals.rsi ?? "-"}</b></span><span>MACD <b>{signals.macd ?? "-"}</b></span><span>{t("추세 ")}<b>{signals.trend ?? "-"}</b></span><span className={payload.conflict ? "negative" : "positive"}>{t("신호 ")}{payload.conflict ? "충돌 있음" : "정합"}</span></div></section></div>;
}
...
function BacktestResult({ payload }: { payload: Record<string, any> }) {
  ...
  return <div className="intelligence-result-view"><LineChart title={t("검증 자산 곡선")} series={[{ label: "평가액", values: curve, color: "#22c55e" }]} /><section className="intelligence-result-section"><header><h3>{t("검증 결과")}</h3><span>{t("실제 주문을 만들지 않는 과거 시세 계산")}</span></header><div className="indicator-metric-grid"><div><span>{t("최종 평가액")}</span><strong>{displayValue(payload.final_equity)}</strong></div><div><span>{t("총 수익률")}</span><strong>{periodValue(Number(payload.total_return) * 100)}</strong></div><div><span>{t("거래 수")}</span><strong>{displayValue(metrics.total_trades ?? payload.trades?.length ?? 0)}</strong></div><div><span>{t("승률")}</span><strong>{periodValue(Number(metrics.win_rate ?? 0) * (Number(metrics.win_rate ?? 0) <= 1 ? 100 : 1))}</strong></div><div><span>{t("순손익")}</span><strong>{displayValue(metrics.net_pnl ?? Number(payload.final_equity ?? 0) - Number(payload.initial_cash ?? 0))}</strong></div><div><span>{t("최대 낙폭")}</span><strong>{displayValue(metrics.max_drawdown ?? metrics.max_drawdown_amount)}</strong></div><div><span>{t("수수료율")}</span><strong>{periodValue(Number(assumptions.fee_rate ?? 0) * 100)}</strong></div><div><span>{t("슬리피지")}</span><strong>{displayValue(assumptions.slippage_bps)} bps</strong></div></div></section></div>;
}
...
function ResultView({ action, payload, marketPoints }: { action: Action; payload: Record<string, any>; marketPoints: Array<Record<string, any>> }) {
  ...
  if (payload.status === "configuration_required") return <div className="connection-required-state"><span>{t("준비 중")}</span><h3>{payload.message ?? "운영 데이터 공급자 연결이 필요합니다."}</h3><p>{t("이 화면은 임의 뉴스나 일정을 만들지 않습니다. 운영 관리자가 금융 인텔리전스의 RSS·일정·공시 공급자를 연결하면 같은 탭에서 자동으로 조회됩니다.")}</p><small>{t("일반 사용자 설정 화면에서 연결하는 항목은 아니며, 현재 단계에서는 조회 UI만 준비되어 있고 실제 주문과 연결되지 않습니다.")}</small></div>;
...
function Summary({ snapshot }: { snapshot: Record<string, any> | null }) {
  ...
  return <section className="data-workspace ai-summary-workspace"><article className="panel"><div className="panel-heading"><div><span className="eyebrow">{t("근거 기반 요약")}</span><h2>{t("AI 요약 리포트")}</h2></div></div><p className="workspace-copy">{t("AI 판단·분석·위험·실행 근거를 분리해 현재 계정의 기록만 요약합니다. 내부 테이블명이나 원시 JSON은 사용자 화면에 표시하지 않습니다.")}</p><div className="metric-grid human-summary-metrics"><div><span>{t("AI 판단 근거")}</span><strong>{Number(summary.decision_count ?? 0).toLocaleString()}{t("건")}</strong></div><div><span>{t("분석 기록")}</span><strong>{Number(summary.analysis_count ?? 0).toLocaleString()}{t("건")}</strong></div><div><span>{t("위험 기록")}</span><strong>{Number(summary.risk_count ?? 0).toLocaleString()}{t("건")}</strong></div><div><span>{t("데이터 상태")}</span><strong>{summary.data_status === "available" ? "근거 있음" : "표본 부족"}</strong></div></div><div className="human-evidence-sections">{groups.map(([title, rows]) => <article className="human-evidence-card" key={title}><header><h3>{title}</h3><span>{rows.length}{t("건")}</span></header>{rows.length ? <div className="human-evidence-list">{rows.slice(0, 3).map((row, index) => <CompactEvidence key={index} row={row} />)}</div> : <div className="honest-empty-state"><span>{t("저장된 ")}{title}{t(" 기록이 없습니다.")}</span></div>}</article>)}</div></article></section>;
}
...
export function FinancialIntelligenceWorkspace({ client, service, summaryOnly = false, onAskAssistant }: { client: GatewayClient; service: Service; summaryOnly?: boolean; onAskAssistant?: (question: string) => void }) {
  ...
  return <section className="data-workspace legacy-intelligence-workspace">
    <article className="panel intelligence-header"><h2>{t("NoahAI 금융 인텔리전스")}</h2><p>{t("종목이나 시장을 선택하면 앱이 공개 시세와 연결된 운영 데이터를 불러옵니다. JSON 파일을 만들거나 붙여 넣을 필요가 없습니다.")}</p><small>{t("일부 뉴스·공시·기관 데이터는 운영 공급자 연결 전까지 ‘데이터 연결 필요’로 표시되며 임의 값을 만들지 않습니다.")}</small>{service === "blockchain" && <strong>{t("코인 정보는 내 계좌·선택 코인의 운용 상태를 보는 화면이고, 코인 탐색은 여러 코인을 조건으로 비교해 후보를 찾는 시장 검색 화면입니다.")}</strong>}</article>
    <nav className="intelligence-tabs" aria-label={t("금융 인텔리전스 기능")}>{tabs.map((item) => <button type="button" key={item.label} className={item.label === tab.label ? "active" : ""} onClick={() => setActive(item.label)}>{item.label}{results[item.action] && <i aria-label={t("조회 결과 보존됨")} />}</button>)}</nav>
    <article className="panel intelligence-action-panel"><div className="panel-heading"><div><h2>{tab.title}</h2></div><button className="secondary-button" type="button" onClick={() => onAskAssistant?.(assistantPrompt)}>{t("AI에게 사용법 묻기")}</button></div><p className="workspace-copy">{tab.description}</p><p className="feature-guide">{t("처음 사용: ")}{tab.guide}</p><span className={`state-pill ${error ? "danger" : resultUnavailable ? "warn" : result ? "ok" : "warn"}`}>{busy ? "수집·계산 중" : error ? t("확인 필요") : resultUnavailable ? "데이터 연결·수집 필요" : result ? t("완료") : "조회 전"}</span>
      <div className="intelligence-controls">
        {tab.action === "market" && <><select aria-label={t("시장 프리셋")} value={fields.market_preset} onChange={(e) => update("market_preset", e.target.value)}>{[...Object.keys(MARKET_PRESETS), "직접 입력"].map((name) => <option key={name}>{name}</option>)}</select><input aria-label={t("조회 종목 (수정 시 직접 입력)")} value={fields.market_preset === "직접 입력" ? fields.symbols : (MARKET_PRESETS[fields.market_preset] ?? []).map((item) => item.symbol).join(",")} onChange={(e) => update("symbols", e.target.value)} placeholder={t("예: 삼성전자 005930.KS, Apple AAPL, BTC")} /><select aria-label={t("자산 유형")} disabled={fields.market_preset !== "직접 입력"} value={fields.asset_type} onChange={(e) => update("asset_type", e.target.value)}><option value="stock">stock</option><option value="etf">etf</option><option value="index">index</option><option value="fx">fx</option><option value="commodity">commodity</option><option value="crypto">crypto</option></select>{fields.market_preset !== "직접 입력" && <small>{t("프리셋은 종목별 자산 유형을 사용합니다. 종목을 수정하면 직접 입력으로 전환됩니다.")}</small>}</>}
        {tab.action === "fundamental" && <><select value={fields.provider} onChange={(e) => update("provider", e.target.value)}><option value={"DART"}>DART</option><option value={"SEC"}>SEC</option></select><input value={fields.code} onChange={(e) => update("code", e.target.value)} placeholder={t("DART 기업코드 또는 SEC CIK")} /><input value={fields.year} onChange={(e) => update("year", e.target.value)} aria-label={t("사업연도")} /></>}
        {tab.action === "valuation" && <><label>{t("기준 잉여현금흐름")}<input value={fields.base_fcf} onChange={(e) => update("base_fcf", e.target.value)} /></label><label>{t("발행주식 수")}<input value={fields.shares} onChange={(e) => update("shares", e.target.value)} /></label><label>{t("순부채")}<input value={fields.net_debt} onChange={(e) => update("net_debt", e.target.value)} /></label></>}
        {tab.action === "screener" && <><input value={fields.symbols} onChange={(e) => update("symbols", e.target.value)} aria-label={t("관심 종목")} /><label>{t("최소 가격")}<input value={fields.min_price} onChange={(e) => update("min_price", e.target.value)} /></label></>}
        {tab.action === "technical" && <input value={fields.symbol} onChange={(e) => update("symbol", e.target.value)} aria-label={t("종목")} />}
        {tab.action === "backtest" && <><label>{t("종목")}<input value={fields.symbol} onChange={(e) => update("symbol", e.target.value)} /></label><label>{t("빠른 평균")}<input value={fields.fast} onChange={(e) => update("fast", e.target.value)} /></label><label>{t("느린 평균")}<input value={fields.slow} onChange={(e) => update("slow", e.target.value)} /></label></>}
        <button className="primary-button" type="button" onClick={() => void run()} disabled={busy}>{busy ? "실행 중…" : tab.button}</button>
      </div>{error && <div className="inline-notice error-text">{error}</div>}<div className="legacy-intelligence-output">{resultPayload ? <ResultView action={tab.action} payload={resultPayload} marketPoints={marketPoints} /> : <div className="legacy-intelligence-placeholder">{t("조회 버튼을 누르면 결과가 여기에 표시됩니다. 다른 탭으로 이동해도 이번 실행 중에는 조회 결과가 유지됩니다.")}</div>}</div><div className={`legacy-intelligence-result-status ${error ? "error-text" : ""}`}>{busy ? "공개 시세와 연결된 운영 데이터를 조회하고 있습니다." : error || (resultUnavailable ? "분석 결과 없음 · 공급자 연결 상태와 수집 오류를 확인하세요." : result ? "조회가 완료되었습니다. 기준 시각과 출처를 확인하세요." : "조회 전")}</div>
    </article>
  </section>;
}
