  ...
  return <>
    <article className="panel portfolio-section legacy-portfolio-summary">
      ...
      {allocations.length ? <div className="legacy-portfolio-allocation-list">{allocations.map(([currency, row]) => <div key={currency}>
        ...
        <span>{t("최대 단일 비중 ")}{format(Number(row.max_weight) * 100)}%</span>
        <span className={`state-pill ${row.concentration === "balanced" ? "ok" : "warn"}`}>{t("집중도 ")}{concentrationLabel(row)}</span>
      </div>)}</div> : <div className="honest-empty-state"><b>{t("현재 계좌 잔고가 아직 수신되지 않았습니다.")}</b><span>{t("블록체인 또는 주식/증권 계좌의 API를 연결한 뒤 실시간 계좌 새로고침을 실행하세요.")}</span></div>}
    </article>
    <article className="panel portfolio-section legacy-portfolio-summary">
      <div className="panel-heading"><div><h2>{LEGACY_PORTFOLIO_SECTION_ORDER[2]}</h2></div></div>
      <div className="legacy-portfolio-risk-list">
        <div><span>{t("집중도")}</span><strong>{allocations.length ? `${format(maxConcentration, 0)}/100` : "N/A"}</strong></div>
        <div><span>{t("자산군 상관계수")}</span><strong>{correlation?.status === "ok" ? Number(correlation.value ?? 0).toFixed(2) : "N/A"}</strong></div>
        <div><span>{mode === "paper" ? "PAPER 가상 순손익" : "LIVE 체결 대조 완료 순손익"}</span><strong>{pnl}</strong></div>
      </div>
      <small className="workspace-copy">{t("상세 손실 시나리오와 대응 방향은 ‘리스크 브리핑’ 탭에서 확인합니다.")}</small>
    </article>
    <article className="panel portfolio-section legacy-portfolio-summary">
      <div className="panel-heading"><div><h2>{LEGACY_PORTFOLIO_SECTION_ORDER[3]}</h2></div><span className="state-pill ok">{t("판단 보조 전용")}</span></div>
      ...
      <small className="workspace-copy">{t("사용자의 승인 없이 주문 설정을 바꾸거나 거래를 실행하지 않습니다.")}</small>
    </article>
  </>;
}
  ...
  return <>
    <article className="panel portfolio-section">
      ...
      {allocations.length ? <div className="card-grid">{allocations.map(([currency, row]) => <section className="insight-card" key={currency}>
        <header><strong>{currency}</strong><span className={`state-pill ${row.concentration === "balanced" ? "ok" : "warn"}`}>{t("집중도 ")}{concentrationLabel(row)}</span></header>
        <dl><dt>{t("평가 합계")}</dt><dd>{currencyAmount(row.total_value, currency)}</dd><dt>{t("최대 단일 비중")}</dt><dd>{format(Number(row.max_weight) * 100)}%</dd><dt>HHI</dt><dd>{format(row.hhi, 4)}</dd></dl>
      ...
      </section>)}</div> : <div className="honest-empty-state"><b>{t("현재 계좌 잔고가 아직 수신되지 않았습니다.")}</b><span>{t("블록체인 또는 주식/증권 계좌의 API를 연결한 뒤 실시간 계좌 새로고침을 실행하세요.")}</span></div>}
    </article>
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("포트폴리오 집중도 (HHI)")}</h2></div></div>
      {allocations.length ? <div className="portfolio-hhi-list">{allocations.map(([currency, row]) => <div key={currency}><div><strong>{currency}</strong><span>{t("집중도 ")}{format(concentrationScore(row), 1)}/100 ({concentrationLabel(row)})</span></div><progress max={100} value={concentrationScore(row)} /></div>)}</div> : <div className="empty-state">{t("비중을 계산할 계좌 데이터가 없습니다.")}</div>}
    </article>
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("자산군 상관계수 히트맵")}</h2></div></div>
      {correlation?.status === "ok" ? <div className="correlation-grid"><span>{t("암호화폐")}</span><b>+1.00</b><b>{Number(correlation.value ?? 0).toFixed(2)}</b><span>{t("주식")}</span><b>{Number(correlation.value ?? 0).toFixed(2)}</b><b>+1.00</b></div> : <div className="honest-empty-state"><b>{t("표본 데이터 부족")}</b><span>{t("암호화폐와 주식의 같은 날짜 수익률이 충분히 쌓이면 자동 계산됩니다.")}</span></div>}
    </article>
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("리밸런싱 제안")}</h2></div></div>
      {!allocations.length ? <div className="empty-state">{t("계좌 데이터를 확인한 뒤 제안을 계산합니다.")}</div> : allocations.map(([currency, row]) => <p className="portfolio-guidance" key={currency}><strong>{currency}</strong> · {String(row.guidance ?? "현재 통화군 내 비중을 유지 관찰하세요.")}</p>)}
      <p className="workspace-copy">{t("제안은 정보 제공용이며 주문으로 자동 적용되지 않습니다. KRW와 USDT는 환율 기준시각 없이 합산하지 않습니다.")}</p>
    </article>
  </>;
}
...
function RiskSections({ allocations, performances, correlation }: { allocations: Array<[string, Row]>; performances: Array<[string, Row]>; correlation: Row }) {
  ...
  return <>
    <div className="portfolio-risk-cards">
      <section><span>{t("집중도")}</span><strong>{maxConcentration ? `${format(maxConcentration, 0)}/100` : "N/A"}</strong><small>{concentration}</small></section>
      <section><span>{t("상관계수")}</span><strong>{correlationText}</strong><small>{correlation?.status === "ok" ? "계산 완료" : "데이터 없음"}</small></section>
      <section><span>{t("실현 손익")}</span><strong>{pnlRows.length ? pnlRows.map(([currency, pnl]) => currencyAmount(pnl, currency)).join(" · ") : "N/A"}</strong><small>{t("가져오기 거래 제외")}</small></section>
    </div>
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("시나리오별 손실액 추정")}</h2></div></div>
      {!allocations.length ? <div className="honest-empty-state"><b>{t("현재 잔고가 수신되지 않아 손실액을 계산하지 않습니다.")}</b><span>{t("통화가 다른 자산은 환율 없이 하나의 손실액으로 합산하지 않습니다.")}</span></div> : <div className="portfolio-scenario-table">
        <div><b>{t("통화")}</b><b>{t("하락 -5%")}</b><b>{t("하락 -10%")}</b><b>{t("하락 -20%")}</b><b>{t("하락 -30%")}</b><b>{t("하락 -50%")}</b></div>
    ...
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("리스크 경고 및 대응 방향")}</h2></div></div>
      ...
      <strong className="portfolio-action-title">{t("즉시 권장 액션")}</strong>
      <p className="portfolio-guidance">{t("→ 계좌 데이터의 기준시각과 통화를 확인하고, 집중도가 높은 통화군의 단일 자산 비중부터 점검하세요.")}</p>
    </article>
  </>;
}
...
function PerformanceSections({ performances, recentTrades }: { performances: Array<[string, Row]>; recentTrades: Row[] }) {
  return <>
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("성과·위험 요약")}</h2></div></div>
      {performances.length ? <div className="card-grid">{performances.map(([currency, row]) => <section className="insight-card" key={currency}><header><strong>{currency}</strong><span>{Number(row.trades ?? 0)}{t("거래")}</span></header><dl><dt>{t("순손익")}</dt><dd>{currencyAmount(row.net_pnl, currency)}</dd><dt>{t("승률")}</dt><dd>{format(Number(row.win_rate) * 100)}%</dd><dt>Profit Factor</dt><dd>{row.profit_factor == null ? "손실 표본 없음" : format(row.profit_factor)}</dd><dt>{t("최대 누적 손실폭")}</dt><dd>{currencyAmount(row.max_drawdown_amount, currency)}</dd></dl></section>)}</div> : <div className="honest-empty-state"><b>{t("선택 원장의 계산 가능한 종료 거래 표본이 없습니다.")}</b><span>{t("거래 이력이나 성과 자체가 없다는 뜻은 아닙니다. 과거 LIVE 기록과 대조 상태를 아래에서 확인하세요.")}</span></div>}
    </article>
    <article className="panel portfolio-section">
      <div className="panel-heading"><div><h2>{t("최근 종료 거래")}</h2></div><span className="count-badge">{recentTrades.length}{t("건")}</span></div>
      {recentTrades.length ? <div className="portfolio-recent-trades"><div><b>{t("종목")}</b><b>{t("자산군")}</b><b>{t("손익")}</b><b>{t("종료 시각")}</b></div>{recentTrades.slice(0, 30).map((row, index) => <div key={`${row.symbol}-${row.exit_time}-${index}`}><strong>{String(row.symbol ?? "-")}</strong><span>{String(row.asset_type ?? row.asset_class ?? "-")}</span><span>{currencyAmount(row.pnl, String(row.currency ?? "USDT"))}</span><span>{String(row.exit_time ?? row.timestamp ?? "-")}</span></div>)}</div> : <div className="empty-state">{t("표시할 종료 거래가 없습니다.")}</div>}
    </article>
  </>;
}
...
export function PortfolioWorkspace({ client, featureId, enabledSources, credentialStatus }: { client: GatewayClient; featureId: string; enabledSources: string[]; credentialStatus: Record<string, boolean> }) {
  ...
  return <section className="data-workspace portfolio-workspace">
    <article className="panel portfolio-hero"><div className="panel-heading"><div><h1>{title}</h1><p>{t("실제 계좌 잔고와 종료 거래 기록을 분리해 계산합니다. 통화가 다른 자산은 환율 근거 없이 합산하지 않습니다.")}</p></div><button className="secondary-button" type="button" disabled={busy} onClick={refreshAccounts}>{busy ? "처리 중…" : "실시간 계좌 새로고침"}</button></div>{error && <div className="inline-notice error-text">{error}</div>}</article>
    ...
    {featureId.endsWith("insights") && <>
      ...
      <article className="panel portfolio-section legacy-portfolio-summary">
        ...
        <div className="legacy-portfolio-snapshot-row">
          ...
          <button className="primary-button" type="button" disabled={busy} onClick={saveSnapshot}>{t("현재 상태 저장")}</button>
        </div>
      </article>
