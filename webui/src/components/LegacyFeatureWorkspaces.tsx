...
export function LegacyAILearningWorkspace({ client, service, source, sources }: { client: GatewayClient; service: "blockchain" | "stock"; source: string; sources: string[] }) {
  ...
  return <section className="legacy-ai-learning">
    ...
    {service === "stock" && <p className="workspace-copy">{t("증권사별 분석 기록을 같은 DB에서 기관 식별자로 분리 조회합니다. 파일이 같아도 다른 증권사의 기록을 섞지 않습니다. 현재 국내 주식·국내 상장 ETF 분석이며, 미국 시장 시세 조회는 해외주식 자동매매·학습 지원을 의미하지 않습니다. 과거 기록에 없는 신호·지표는 추정하지 않습니다.")}</p>}
    ...
    <article className="legacy-learning-data"><header><div><h2>{t("AI 학습 데이터")}</h2><small>{t("최근 ")}{rows.length}{t("개 · 저장 근거: ")}{String(snapshot?.learning?.source ?? "확인 중...")}</small></div><button type="button" disabled={loading} onClick={refresh}>{t("새로고침")}</button></header><div className={`legacy-workspace-feedback${error ? " error-text" : ""}`} role="status" aria-live="polite">{error}</div><div className="legacy-learning-table"><div className="legacy-learning-row header"><span>{t("시간")}</span><span>{service === "stock" ? t("종목") : t("코인")}</span><span>{t("시그널")}</span><span>{t("신뢰도")}</span><span>RSI</span><span>MACD</span><span>{t("트렌드")}</span><span>{t("추론")}</span></div><div className="legacy-learning-body">{[...rows].reverse().map((row, index) => <div className="legacy-learning-row" key={String(row.id ?? `${recordTime(row)}-${index}`)}><span>{recordTime(row).slice(11) || "—"}</span><b>{String(row.symbol ?? row.coin ?? row.ticker ?? "—")}</b><strong className={recordSignal(row) === "LONG" ? "positive" : recordSignal(row) === "SHORT" ? "negative" : "warning"}>{recordSignal(row)}</strong><span>{row.confidence == null && row.score == null ? '—' : numberText(row.confidence ?? row.score, 2)}</span><span>{row.rsi == null && row.RSI == null ? '—' : numberText(row.rsi ?? row.RSI, 4)}</span><span>{row.macd == null && row.MACD == null ? '—' : numberText(row.macd ?? row.MACD, 4)}</span><span>{String(row.trend ?? row.market_trend ?? '—')}</span><span title={String(row.reasoning ?? row.reason ?? row.inference ?? row.analysis ?? "")}>{String(row.reasoning ?? row.reason ?? row.inference ?? row.analysis ?? "—")}</span></div>)}{!loading && !rows.length && <div className="empty-state">{t("저장된 AI 학습 데이터가 없습니다.")}</div>}{hasMore && <button className="legacy-learning-more" type="button" disabled={loading} onClick={() => void loadMore()}>{loading ? "불러오는 중…" : `이전 데이터 ${LEARNING_PAGE_SIZE}개 더보기`}</button>}</div></div></article>
