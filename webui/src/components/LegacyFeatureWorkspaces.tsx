...
function xaiEvidenceText(row: Record<string, any>) {
  const mode = String(row.execution_mode ?? "unknown").toUpperCase();
  const instrument = String(row.instrument_type ?? "unknown").toLowerCase() === "etf" ? "ETF"
    : String(row.instrument_type ?? "unknown").toLowerCase() === "stock" ? localized("주식", "Stock") : localized("자산 미확정", "Instrument unconfirmed");
  const order = row.actual_order === true || row.actual_order === 1 ? localized("실제 주문 있음", "Actual order placed")
    : row.actual_order === false || row.actual_order === 0 ? localized("실제 주문 없음", "No actual order") : localized("주문 여부 미확정", "Order status unconfirmed");
  const reason = String(row.reasoning ?? row.reason ?? row.reason_code ?? row.inference ?? row.analysis ?? localized("근거 미기록", "Evidence not recorded"));
  return `${mode} · ${instrument} · ${order} · ${reason}`;
}
...
export function LegacyAILearningWorkspace({ client, service, source, sources }: { client: GatewayClient; service: "blockchain" | "stock"; source: string; sources: string[] }) {
  ...
  return <section className="legacy-ai-learning">
    <header className="legacy-learning-summary"><div><b>{t("오늘: ")}{todayCount}{t("개")}</b><b>{t("주간: ")}{weeklyCount}{t("개")}</b><b>{t("신호: LONG(")}{signals.LONG}) SHORT({signals.SHORT}) HOLD({signals.HOLD})</b><small>{summaryScope}</small></div><strong className={error || snapshot?.learning?.status === "unavailable" ? "negative" : "positive"}>{error || snapshot?.learning?.status === "unavailable" ? "조회 실패" : rows.length ? "분석 기록 있음" : "분석 기록 대기"}</strong></header>
    {service === "stock" && <p className="workspace-copy">{t("NoahAI XAI는 주식·ETF도 판단과 실행을 분리해 보여줍니다. LEARNING은 분석 판단, PAPER는 가상 체결, LIVE는 증권사 실제 주문이며 각 기록에 종목 유형·판단 근거·차단 사유·실제 주문 여부를 함께 남깁니다. 증권사별 기록은 기관 식별자로 분리하며 과거에 없던 근거는 추정하지 않습니다. 현재 국내 주식·국내 상장 ETF 범위이고, 미국 시세 조회만으로 해외주식 자동매매를 의미하지 않습니다.")}</p>}
    <label className="legacy-learning-source">{service === "stock" ? t("증권사") : t("거래소")}: <select value={selectedSource} disabled={!sourceOptions.length} onChange={(event) => setSelectedSource(event.target.value)}>{!sourceOptions.length && <option value="">{t("설정에서 선택 필요")}</option>}{sourceOptions.map((item) => <option value={item} key={item}>{item}</option>)}</select></label>
    <article className="legacy-learning-status"><h2>{t("AI 학습 모니터 (")}{service === "stock" ? "주식/ETF" : t("암호화폐")})</h2><div><span>{t("현재 상태:")}</span><b>{latest ? `최근: ${String(latest.symbol ?? latest.coin ?? "전체")} ${recordSignal(latest)} (신뢰도 ${numberText(latest.confidence ?? latest.score, 2)})` : "대기 중"}</b></div><progress max={100} value={rows.length ? 100 : 0} /><small className="workspace-copy">{t("RSI·MACD·추세는 해당 판단 시점에 저장한 값입니다. 기록 없음은 0이나 중립을 뜻하지 않습니다. 과거 누락값은 현재 시세나 추론 문장으로 채우지 않으며, 지표를 만들지 않는 판단에는 값이 없을 수 있습니다.")}</small></article>
    <article className="legacy-learning-data"><header><div><h2>{service === "stock" ? t("AI 판단·학습 근거") : t("AI 학습 데이터")}</h2><small>{t("최근 ")}{rows.length}{t("개 · 저장 근거: ")}{String(snapshot?.learning?.source ?? "확인 중...")}</small></div><button type="button" disabled={loading} onClick={refresh}>{t("새로고침")}</button></header><div className={`legacy-workspace-feedback${error ? " error-text" : ""}`} role="status" aria-live="polite">{error}</div><div className="legacy-learning-table"><div className="legacy-learning-row header"><span>{t("시간")}</span><span>{service === "stock" ? t("종목") : t("코인")}</span><span>{t("시그널")}</span><span>{t("신뢰도")}</span><span>RSI</span><span>MACD</span><span>{t("트렌드")}</span><span>{service === "stock" ? t("XAI 실행 근거") : t("추론")}</span></div><div className="legacy-learning-body">{[...rows].reverse().map((row, index) => <div className="legacy-learning-row" key={String(row.id ?? `${recordTime(row)}-${index}`)}><span>{recordTime(row).slice(11) || "—"}</span><b>{String(row.symbol ?? row.coin ?? row.ticker ?? "—")}</b><strong className={recordSignal(row) === "LONG" ? "positive" : recordSignal(row) === "SHORT" ? "negative" : "warning"}>{recordSignal(row)}</strong><span>{row.confidence == null && row.score == null ? '—' : numberText(row.confidence ?? row.score, 2)}</span><span>{row.rsi == null ? t('기록 없음') : metricText(row.rsi, 4)}</span><span>{row.macd == null ? t('기록 없음') : metricText(row.macd, 4)}</span><span>{String(row.trend ?? t('기록 없음'))}</span><span title={service === "stock" ? xaiEvidenceText(row) : String(row.reasoning ?? row.reason ?? row.inference ?? row.analysis ?? "")}>{service === "stock" ? xaiEvidenceText(row) : String(row.reasoning ?? row.reason ?? row.inference ?? row.analysis ?? "—")}</span></div>)}{!loading && !rows.length && <div className="empty-state">{t("저장된 AI 학습 데이터가 없습니다.")}</div>}{hasMore && <button className="legacy-learning-more" type="button" disabled={loading} onClick={() => void loadMore()}>{loading ? "불러오는 중…" : `이전 데이터 ${LEARNING_PAGE_SIZE}개 더보기`}</button>}</div></div></article>
    <article className="legacy-learning-stats"><h2>{t("AI 학습 통계")}</h2><div><span>{service === "stock" ? "선택 증권사 분석 기록 수" : "운영 파일 학습 수"}: <b>{totalCountLabel}</b></span><span>{t("평균 신뢰도: ")}<b>{totalCount ? numberText(averageConfidence, 2) : "—"}</b></span><span>{t("마지막 업데이트: ")}<b>{latest ? recordTime(latest).slice(11) : "N/A"}</b></span><span>{t("LONG 신호: ")}<b>{signals.LONG}</b></span><span>{t("SHORT 신호: ")}<b>{signals.SHORT}</b></span><span>{t("HOLD 신호: ")}<b>{signals.HOLD}</b></span></div></article>
  </section>;
}
...
export function LegacyAIReportWorkspace({ client, service, source: _source, sources, onAskAssistant }: { client: GatewayClient; service: "blockchain" | "stock"; source: string; sources: string[]; onAskAssistant?: (question: string) => void }) {
  ...
  return <section className="legacy-ai-report">
    <header><h1>{t("AI 자동 리포트")}</h1><p>{t("AI가 실제 거래 데이터를 분석하여 생성하는 리포트입니다.")}</p></header>
    ...
    {activeTab === "실시간" ? <div className="legacy-report-tab-body realtime">
      <article className="legacy-report-summary-card"><h2>{t("실시간 거래 분석 (최근 1시간)")}</h2>{error && <div className="inline-notice error-text">{error}</div>}<div className="legacy-report-summary">{periodClosed ? <><p><b>{t("분석 시간:")}</b> {recordTime({ timestamp: new Date().toISOString() })}</p><p><b>{t("청산 완료:")}</b> {periodClosed}{t("건 · ")}<b>{t("체결 대조:")}</b> {periodReconciled}{t("건 · ")}<b>{t("미확정:")}</b> {periodUnresolved}{t("건")}</p><p><b>{t("확정 승률:")}</b> {periodReconciled ? `${numberText(periodWinRate, 2)}%` : "계산 전"}</p><p><b>{t("통화별 확정 순손익:")}</b> {periodReconciled ? pnlText : "계산 전"} · <b>{t("통화별 Fee:")}</b> {realtimeFees}</p><p>{t("환율 정책 없이 KRW·USDT는 합산하지 않습니다.")}</p></> : <><p>{t("최근 1시간 청산 완료 거래가 없습니다.")}</p><p>{t("거래소 확인 체결 ")}{confirmedExecutions}{t("건과 포지션 청산은 서로 다른 단위입니다.")}</p><p>{t("시장 상황과 실행 상태를 계속 모니터링하세요.")}</p></>}</div></article>
      <article className="legacy-report-detail-card realtime-analysis"><h2>{t("현재 거래 상황 분석")}</h2><div className="legacy-report-summary">{periodClosed ? <><p><b>{t("확정 수익:")}</b> {periodWins}{t("건 · ")}<b>{t("확정 손실:")}</b> {realtimeLosses}{t("건")}</p><p><b>{t("대조 상태:")}</b>{t(" 완료 ")}{periodReconciled}{t("건 · 미확정 ")}{periodUnresolved}{t("건입니다.")}</p><p><b>{t("원장 연결:")}</b>{t(" 청산 ")}{periodClosed}{t("건 중 거래소 체결과 연결 ")}{Number(periodMetric?.linked_closed_count ?? 0)}{t("건, 미연결 ")}{Number(periodMetric?.unlinked_closed_count ?? 0)}{t("건입니다.")}</p><p><b>{t("주의사항:")}</b>{t(" 부분체결·진입·청산 때문에 체결 건수와 청산 건수는 1:1이 아닙니다.")}</p></> : <p>{t("분석할 청산 거래 데이터가 없습니다.")}</p>}</div></article>
      <article className="legacy-report-detail-card realtime-transfer"><h2>{t("AI 어시스턴트 연동")}</h2><p>{periodClosed ? "통화별 실시간 성과와 원장 연결 상태를 전달합니다. 자동 적용이나 주문은 실행하지 않습니다." : "현재 청산 데이터가 없어 원인 점검 요청만 전달합니다."}</p><button type="button" onClick={() => onAskAssistant?.(realtimePrompt)}>{t("AI 어시스턴트에 전달")}</button></article>
    </div> : <div className="legacy-report-tab-body">
      <article className={`legacy-report-summary-card ${activeTab === "실행 품질" ? "quality" : ""}`}><div className="legacy-report-card-title"><div><span className="report-eyebrow">{t("근거 기반 요약")}</span><h2>{periodTitle}</h2></div>{activeTab === "실행 품질" && <button type="button" onClick={refresh} disabled={loading}>{loading ? "갱신 중…" : "메트릭 갱신"}</button>}</div>{error && <div className="inline-notice error-text">{error}</div>}{activeTab !== "실행 품질" ? <div className="legacy-report-summary professional"><div className="report-summary-metrics">{reportMetrics.map((metric) => <div className={`tone-${metric.tone}`} key={metric.label}><span>{metric.label}</span><strong>{metric.value}</strong></div>)}</div><div className="report-summary-copy"><section><b>{t("거래 상태")}</b><p>{snapshot?.report_periods?.execution_history_available === false ? "이 증권/거래소의 외부 체결 이력은 아직 공통 원장으로 확인할 수 없습니다." : confirmedExecutions ? `선택 기간의 거래소 확인 체결 ${confirmedExecutions}건입니다.` : "선택 기간에 거래소가 확인한 체결이 없습니다."}</p></section><section><b>{t("성과 계산")}</b><p>{periodClosed ? `NoahAI 청산 완료 ${periodClosed}건 중 수익 거래 ${periodWins}건입니다.` : confirmedExecutions ? "체결은 있으나 청산 완료 포지션이 없어 승률과 손익은 계산하지 않습니다." : "청산 완료 표본이 없어 승률과 손익을 계산하지 않습니다."}</p></section><section><b>{t("체결 규모")}</b><p>{executedNotionalText}{t(" · 통화가 다른 금액은 임의 합산하지 않습니다.")}</p></section><section><b>{t("원장 연결")}</b><p>{t("청산 ")}{periodClosed}{t("건 중 체결 연결 ")}{Number(periodMetric?.linked_closed_count ?? 0)}{t("건 · 미연결 ")}{Number(periodMetric?.unlinked_closed_count ?? 0)}{t("건 · 레거시 출처 미확정 ")}{Number(periodMetric?.legacy_unknown_count ?? 0)}{t("건")}</p></section><section><b>{t("정산 대조")}</b><p>{periodMetric?.ledger_reconciled === false ? "요약과 상세 원장 체크섬이 일치하지 않습니다. 이 리포트를 확정 정산으로 사용하지 마세요." : `기간 상세 원장 ${Number(periodMetric?.detail_total_count ?? periodClosed)}건과 통화별 손익·수수료를 대조했습니다.`}</p></section><section><b>{t("다음 확인")}</b><p>{periodMetric?.ledger_reconciled === false ? "거래소 필터·기간·원장 손상 여부를 확인하고 새로고침하세요." : "상세 거래의 청산 사유와 체결 연결 상태를 확인하세요."}</p></section></div></div> : <div className="legacy-report-summary quality-copy"><section><b>{t("표시 범위")}</b><p>{t("최근 사이클의 quality_score, 슬리피지, 지연과 이상 감지 결과를 표시합니다.")}</p></section><section><b>{t("안전 경계")}</b><p>{t("메트릭 갱신은 저장된 실행 기록만 다시 조회하며 주문이나 설정 변경을 실행하지 않습니다.")}</p></section></div>}</article>
      <article className="legacy-report-detail-card"><h2>{detailTitle}</h2><div className="legacy-report-list">{reportRows.map((row, index) => { const closedTrade = activeTab !== "실행 품질" && row.pnl !== undefined && row.exit_time; return <article key={String(row.id ?? `${activeTab}-${index}`)}><header><b>{activeTab === "실행 품질" ? `${String(row.source || reportSource || "전체").toUpperCase()} · ${String(row.engine ?? "실행 엔진")}` : `${String(row.exchange || reportSource || "전체").toUpperCase()} · ${String(row.symbol ?? row.coin ?? row.event ?? "전체")}`}</b><span>{recordTime(row)}</span></header><p>{activeTab === "실행 품질" ? String(row.daily_briefing ?? row.message ?? (Array.isArray(row.anomalies) && row.anomalies.length ? `이상 감지: ${row.anomalies.join(", ")}` : "최근 사이클 실행 품질을 확인했습니다.")) : closedTrade ? `${String(row.side || "포지션").toUpperCase()} 청산 · 실현손익 ${Number(row.pnl) > 0 ? "+" : ""}${numberText(row.pnl, String(row.currency) === "KRW" ? 0 : 6)} ${String(row.currency || "")} (${Number(row.pnl_percent) > 0 ? "+" : ""}${numberText(row.pnl_percent, 3)}%) · 수수료 ${numberText(row.fees, String(row.currency) === "KRW" ? 0 : 6)} ${String(row.currency || "")} · 진입가 ${numberText(row.entry_price, 8)}${row.exit_price !== undefined ? ` → 청산가 ${numberText(row.exit_price, 8)}` : ""}` : String(row.summary ?? row.reason ?? row.analysis ?? row.details ?? row.status ?? "저장된 상세 설명이 없습니다.")}</p>{activeTab === "실행 품질" && <dl className="quality-metric-grid"><div><dt>{t("품질 점수")}</dt><dd>{metricText(row.quality_score, 2)}</dd></div><div><dt>{t("주문 시도")}</dt><dd>{metricText(row.attempted_orders ?? row.order_count, 0, "건")}</dd></div><div><dt>{t("실패")}</dt><dd>{metricText(row.failed_orders ?? (row.success === false ? 1 : row.success === true ? 0 : undefined), 0, "건")}</dd></div><div><dt>{t("평균 지연")}</dt><dd>{metricText(row.avg_latency_ms ?? row.latency_ms ?? row.latency, 2, "ms")}</dd></div><div><dt>{t("슬리피지")}</dt><dd>{metricText(row.avg_slippage_bps ?? row.slippage_bps ?? row.slippage ?? row.slippage_pct, 2, "bps")}</dd></div><div><dt>{t("상태")}</dt><dd>{Array.isArray(row.anomalies) && row.anomalies.length ? `주의 ${row.anomalies.length}건` : String(row.status ?? row.result ?? "정상")}</dd></div></dl>}</article>; })}{!loading && !reportRows.length && <div className="empty-state report-empty">{activeTab === "실행 품질" ? (snapshot?.execution_quality?.message ?? "최근 사이클 실행 품질 기록이 없습니다.") : periodClosed ? `요약에는 청산 ${periodClosed}건이 있으나 상세 원장 행을 불러오지 못했습니다. 새로고침 후에도 같으면 데이터 원장을 점검하세요.` : confirmedExecutions ? "거래소 확인 체결은 있지만 아직 청산 완료된 NoahAI 포지션이 없습니다." : "선택 기간에 청산 완료된 NoahAI 거래가 없습니다."}</div>}</div>{activeTab !== "실행 품질" && Number(periodMetric?.detail_total_count ?? 0) > REPORT_PAGE_SIZE && <div className="legacy-report-pagination"><button type="button" disabled={loading || reportPage === 0} onClick={() => setReportPage((page) => Math.max(0, page - 1))}>{t("이전 ")}{REPORT_PAGE_SIZE}{t("건")}</button><span>{Number(periodMetric?.detail_offset ?? 0) + 1}–{Number(periodMetric?.detail_offset ?? 0) + reportRows.length} / {Number(periodMetric?.detail_total_count ?? 0)}{t("건")}</span><button type="button" disabled={loading || !periodMetric?.detail_has_more} onClick={() => setReportPage((page) => page + 1)}>{t("다음 ")}{REPORT_PAGE_SIZE}{t("건")}</button></div>}</article>
    </div>}
    <div className="report-notification-status" role="status" aria-live="polite">{notificationMessage}</div>
    <footer><label>{service === "stock" ? t("증권사") : t("거래소")}: <select value={reportSource} onChange={(event) => setReportSource(event.target.value)}><option value="">{t("전체")}</option>{sourceOptions.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><button type="button" onClick={refresh} disabled={loading}>{t("전체 새로고침")}</button><button type="button" onClick={() => void sendCurrentReport()} disabled={loading}>{t("연동 채널로 보내기")}</button><button className="legacy-realtime-report-button" type="button" onClick={runRealtimeAnalysis} disabled={loading}>{t("실시간 분석")}</button></footer>
  </section>;
}
...
function MarketSparkline({ values, tone }: { values: number[]; tone: "positive" | "negative" | "neutral" }) {
  ...
  if (points.length < 2) return <div className="market-spark-empty">{t("기간 시계열 미제공")}</div>;
  const minimum = Math.min(...points);
  const maximum = Math.max(...points);
  const range = maximum - minimum || 1;
  const polyline = points.map((value, index) => `${(index / Math.max(1, points.length - 1)) * 100},${34 - ((value - minimum) / range) * 30}`).join(" ");
  return <svg className={`market-sparkline ${tone}`} viewBox="0 0 100 38" preserveAspectRatio="none" role="img" aria-label={t("선택 기간 가격 흐름")}>
...
function MarketBreadthDonut({ positive, neutral, negative }: { positive: number; neutral: number; negative: number }) {
  ...
  return <div className="market-breadth-visual">
    ...
    <div><b>{t("시장 폭")}</b><span><i className="up" />{t("상승 ")}{positive}</span><span><i className="flat" />{t("중립 ")}{neutral}</span><span><i className="down" />{t("하락 ")}{negative}</span></div>
  </div>;
}
...
export function MarketTrendWorkspace({ client, service, source, onAskAssistant }: { client: GatewayClient; service: "blockchain" | "stock"; source: string; onAskAssistant?: (question: string) => void }) {
  ...
  return <section className="legacy-market-trend">
    <header className="legacy-market-trend-title"><div><span>{t("시장 트렌드 인사이트")}</span><small>{service === "stock" ? "국내 주식·ETF 표본" : `${(source || "binance").toUpperCase()} 공개 시세`} · {periodLabel}</small></div><div className="market-period-tabs" role="group" aria-label={t("시장 트렌드 기간")}>{(["today", "week", "month"] as MarketTrendPeriod[]).map((item) => <button className={period === item ? "active" : ""} type="button" key={item} onClick={() => setPeriod(item)}>{item === "today" ? t("오늘") : item === "week" ? t("7일") : t("30일")}</button>)}</div><button type="button" onClick={refresh} disabled={loading}>{loading ? "수집 중…" : t("새로고침")}</button></header>
    ...
    <div className="market-trend-kpis">
      <article><span>{t("시장 방향 · ")}{periodLabel}</span><strong className={direction === "상승" ? "positive" : direction === "하락" ? "negative" : ""}>{direction} {validRows.length ? `${averageChange >= 0 ? "+" : ""}${averageChange.toFixed(2)}%` : ""}</strong><div className="market-direction-scale"><i style={{ left: `${Math.max(2, Math.min(98, 50 + averageChange * 5))}%` }} /></div><small>{t("표본 ")}{validRows.length}{t("개 동일가중 평균")}</small></article>
      <article><MarketBreadthDonut positive={positiveBreadth} neutral={neutralBreadth} negative={negativeBreadth} /><small>{t("원형은 비율에만 사용합니다.")}</small></article>
      <article><span>{service === "stock" ? "지수 · 거래량" : "파생 심리 · 거래량"}</span><strong>{service === "stock" ? indexText || "지수 연결 중" : fundingRate == null ? "해당 거래소 미연동" : `펀딩 ${(fundingRate * 100).toFixed(4)}%`}</strong><div className="market-metric-lines"><span>{service === "stock" ? `표본 거래량 ${Number.isFinite(averageVolumeChange) ? `${averageVolumeChange >= 0 ? "+" : ""}${averageVolumeChange.toFixed(1)}%` : "기간 비교 미제공"}` : `롱/숏 ${longShortRatio == null ? "미제공" : longShortRatio.toFixed(2)}`}</span><span>{t("평균 거래량 변화 ")}{Number.isFinite(averageVolumeChange) ? `${averageVolumeChange >= 0 ? "+" : ""}${averageVolumeChange.toFixed(1)}%` : "미제공"}</span></div><small>{service === "blockchain" && "공포·탐욕 지수가 아니라 선택 거래소 파생 데이터입니다."}</small></article>
      <article><span>{t("평균 일중 변동폭")}</span><strong>{Number.isFinite(averageVolatility) ? `${averageVolatility.toFixed(2)}%` : "기간 데이터 미제공"}</strong><div className="market-volatility-bar"><i style={{ width: `${Math.min(100, Math.max(0, averageVolatility * 10))}%` }} /></div><small>{t("고가–저가 ÷ 시가의 기간 평균 · 위험 참고값")}</small></article>
    </div>
    <section className="market-asset-board">
      <header><div><h3>{service === "stock" ? "주식·ETF 표본 흐름" : "대표 코인 흐름"}</h3><p>{t("가격 방향·거래량·변동성을 같은 기간으로 맞춰 봅니다.")}</p></div><span>{status}</span></header>
      <div className="market-asset-grid">{marketRows.map((row) => { const tone = !Number.isFinite(row.change) ? "neutral" : row.change > .25 ? "positive" : row.change < -.25 ? "negative" : "neutral"; return <article key={row.symbol} className={tone}><header><div><b>{row.name ?? row.symbol}</b><small>{row.symbol}</small></div><strong>{Number.isFinite(row.change) ? `${row.change >= 0 ? "+" : ""}${row.change.toFixed(2)}%` : "기간 데이터 없음"}</strong></header><MarketSparkline values={row.spark} tone={tone} /><footer><span>{t("거래량 ")}{Number.isFinite(row.volumeChange) ? `${row.volumeChange >= 0 ? "+" : ""}${row.volumeChange.toFixed(1)}%` : row.volumeValue != null ? numberText(row.volumeValue, 0) : "미제공"}</span><span>{t("변동폭 ")}{Number.isFinite(row.volatility) ? `${row.volatility.toFixed(2)}%` : "미제공"}</span></footer></article>; })}{!marketRows.length && <div className="empty-state">{status}{t(" · 공개 시세가 확보되면 그래프를 표시합니다.")}</div>}</div>
    </section>
    <div className="market-trend-detail-grid">
      ...
      <article><h3>{t("추가 확인이 필요한 필수 데이터")}</h3><p>{service === "stock" ? "외국인·기관 수급 · 원/달러 환율 · 금리 · 공시/뉴스는 현재 이 카드의 정본 데이터가 아닙니다. 연결 전에는 AI가 추정하지 않습니다." : "실제 공포탐욕지수 · 미결제약정 · 청산 규모 · BTC 도미넌스는 현재 이 카드의 정본 데이터가 아닙니다. 롱/숏 비율을 공포탐욕으로 바꾸어 부르지 않습니다."}</p></article>
      <article><h3>{t("내 실행과 분리")}</h3><p>{t("저장된 종료 거래 ")}{closed}{t("건 · 실현손익 ")}{numberText(pnl, service === "stock" ? 0 : 4)} {service === "stock" ? "KRW" : "USDT"}<br />{service === "stock" ? "오늘: 네이버 금융 공개 시세(계좌·주문과 분리)" : "선택 거래소 공개 시세(계좌·주문과 분리)"}<br />{t("공개 시장 표본은 계좌 잔고·미실현손익·주문 신호가 아닙니다.")}</p></article>
      <article className="market-xai-card"><h3>{t("AI 질문 · XAI 관찰 후보")}</h3><p>{t("화면의 기간·기관·수집시각·표본을 함께 전달합니다. 답변은 근거·반대 근거·무효화 조건·누락 데이터를 나누며 주문을 실행하지 않습니다.")}</p><div><button type="button" disabled={!onAskAssistant || !validRows.length} onClick={() => askMarketAssistant("brief")}>{t("이 시장 브리핑 AI에게 묻기")}</button><button type="button" disabled={!onAskAssistant || !validRows.length} onClick={() => askMarketAssistant("candidates")}>{t("관찰 후보 XAI 비교")}</button></div></article>
    </div>
  </section>;
}
...
export function AssetInfoWorkspace({ client, service, source, enabledSources = [] }: { client: GatewayClient; service: "blockchain" | "stock"; source: string; enabledSources?: string[] }) {
  ...
  if (service === "stock") {
    ...
    return <section className="legacy-stock-info-workspace">
      <header className="legacy-stock-info-title">{t("종목/ETF 검색 & 분석 (")}{stockMode === "all" ? t("통합") : stockMode === "stock" ? t("주식만") : t("ETF만")})</header>
      <div className="legacy-stock-search">
        <label>{t("종목코드 검색")}</label>
        <input value={query} placeholder={t("예: 005930, 069500")} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void runStockAnalysis()} />
        <button type="button" onClick={() => void runStockAnalysis()} disabled={loading}>{loading ? "분석 중…" : t("검색")}</button>
        <label className="legacy-stock-mode">{t("보기 모드")}<select value={stockMode} onChange={(event) => setStockMode(event.target.value as "all" | "stock" | "etf")}><option value="all">{t("통합")}</option><option value="stock">{t("주식만")}</option><option value="etf">{t("ETF만")}</option></select></label>
      </div>
      <div className="legacy-stock-quick">
        <b>{t("⭐ 즐겨찾기")}</b>
        {stockProfile.favorites.length ? stockProfile.favorites.slice(0, 6).map((symbol) => <button key={`favorite-${symbol}`} type="button" onClick={() => void runStockAnalysis(symbol)}>{symbol}</button>) : <span>{t("없음")}</span>}
        <i aria-hidden="true" />
        <b>{t("최근검색")}</b>
        {stockProfile.recent_codes.length ? stockProfile.recent_codes.slice(0, 6).map((symbol) => <button key={`recent-${symbol}`} type="button" onClick={() => void runStockAnalysis(symbol)}>{symbol}</button>) : <span>{t("없음")}</span>}
      </div>
      <div className="legacy-stock-suggestions"><b>{t("자동완성")}</b>{stockSuggestions.length ? stockSuggestions.map((item) => <button key={`${String(item.broker)}:${String(item.code)}`} type="button" onClick={() => void runStockAnalysis(String(item.code))}>{String(item.code)} {String(item.name ?? item.code).slice(0, 8)} ({item.is_etf ? "ETF" : t("주식")})</button>) : <span>{t("추천 없음")}</span>}</div>
      <article className="legacy-stock-note"><h3>{t("NoahAI 증권 사용 방식")}</h3><p>{t("증권 탭은 종목 검색·분석·설명 중심으로 동작합니다.")}<br />{t("조절 요청과 판단 해석은 AI 어시스턴트를 사용하세요.")}</p></article>
      ...
      <article className="legacy-stock-analysis">
        <div className="legacy-section-title compact">{t("분석 결과")}</div>
        {stockAnalyses.length ? <div className="legacy-stock-analysis-list">{stockAnalyses.map((stockAnalysis) => {
          ...
          return <div className="legacy-stock-analysis-row" key={`${String(stockAnalysis.source)}:${symbol}`}>
            <div className="legacy-stock-analysis-title">
              <div><b>{stockAnalysis.name ? `${stockAnalysis.name} (${symbol})` : symbol} - {stockAnalysis.is_etf ? "ETF" : t("주식")}</b><em>{String(stockAnalysis.source ?? source).toUpperCase()}</em></div>
            ...
            <div className="legacy-stock-market-line"><span>{t("현재가 ")}<strong>{numberText(stockAnalysis.close, 0)}{t("원")}</strong></span><span className={stockAnalysis.change >= 0 ? "positive" : "negative"}>{t("등락률 ")}{stockAnalysis.change >= 0 ? "+" : ""}{numberText(stockAnalysis.change, 2)}%</span><span>{t("거래량 ")}{stockAnalysis.volume == null || !Number.isFinite(Number(stockAnalysis.volume)) ? "미제공" : numberText(stockAnalysis.volume, 0)}</span></div>
            <div className="legacy-stock-score-line"><b>{t("AI 판단 점수 ")}{numberText(stockAnalysis.score, 1)}</b><span>{String(stockAnalysis.reasoning ?? `${String(stockAnalysis.source ?? source).toUpperCase()} 증권사 분석`)}</span></div>
          </div>;
        })}</div> : <div className="empty-state">{t("종목코드를 검색하면 활성 증권사의 기존 StockAnalysisService가 시세·지표·AI 점수를 분석합니다.")}</div>}
      </article>
      <article className="legacy-stock-activity"><div className="legacy-section-title compact">{t("내 보유·최근 체결 종목")}</div>{recentTrades.length ? recentTrades.slice(0, 8).map((row, index) => <div className="legacy-stock-activity-row" key={String(row.id ?? index)}><b>{String(row.symbol ?? row.code ?? "—")}</b><span>{String(row.side ?? row.type ?? "—")}</span><span>{String(row.exit_time ?? row.created_at ?? "")}</span></div>) : <div className="empty-state">{t("저장된 주식/ETF 체결 기록이 없습니다.")}</div>}</article>
      <div className="legacy-stock-preview-header"><span>{t("브로커")}</span><span>{t("종목")}</span><span>{t("구분")}</span><span>{t("현재가")}</span><span>{t("등락률")}</span><span>{t("거래량")}</span><span>{t("비고")}</span></div>
      <article className="legacy-stock-preview"><div className="legacy-section-title compact">{t("시장 종목 미리보기 · 검색용 (")}{stockMode === "all" ? t("통합") : stockMode === "stock" ? t("주식만") : t("ETF만")})</div><div className="empty-state">{t("종목을 검색하면 확인된 시세가 이 화면에 표시됩니다. 코인 후보 데이터는 사용하지 않습니다.")}</div></article>
      <div className="legacy-coin-actions"><button className="legacy-refresh-button" type="button" onClick={refresh} disabled={loading}>{t("종목 정보 새로고침")}</button></div>
    </section>;
  }

  return <section className="legacy-coin-workspace">
    <header className="legacy-coin-heading">{t("선택된 거래 코인 정보 (AI 평가 결과)")}</header>
    <div className="legacy-coin-search">
      <label>{t("심볼 직접 분석")}</label>
      <input value={query} placeholder={t("예: BTCUSDT, ETHUSDT")} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void runCoinAnalysis()} />
      <button type="button" onClick={() => void runCoinAnalysis()} disabled={loading}>{loading ? "분석 중…" : t("분석")}</button>
      <span>{t("NoahAI가 실시간 분석합니다")}</span>
    </div>
    <div className="legacy-coin-notices" role="status" aria-live="polite">
      {(error || actionMessage) && <div className={`inline-notice ${error ? "error-text" : ""}`}>{error || actionMessage}</div>}
      {fallbackSelection && <div className="inline-notice warning-text">{t("후보 데이터를 평가하지 못해 고정 참조 심볼을 표시합니다. 점수는 0이 아니라 미산출이며, 이 목록의 PAPER·LIVE 신규 주문은 차단됩니다. 기존 포지션 보호는 계속되며 연결·티커·캔들 데이터 회복 후 다시 선정하세요.")}</div>}
      {partialSelection && <div className="inline-notice warning-text">{t("목표 수량은 채우지 못했지만 정상적으로 점수를 산출한 후보만 부분 선정했습니다. 고정 목록으로 바꾸지 않으며, 각 후보는 후속 실시간 신호·전략·가드레일을 다시 통과해야 합니다.")}</div>}
      <div className="inline-notice">{t("코인 선정은 분석 후보를 좁히는 단계입니다. 정상 선정되어도 시장·전략 조건이 맞지 않으면 실시간 판단은 HOLD로 남으며 주문을 보장하지 않습니다.")}</div>
      <div className="inline-notice">{t("기술점수 미산출: 후보 선정은 일괄 시세를 우선 사용합니다. 선정 시점의 15분·1시간 RSI 근거가 없으면 점수를 만들지 않습니다. 실행 중 분석에서 저장된 RSI·MACD·추세는 AI 학습에서 확인하세요. 종합점수는 기술점수와 별도로 계산됩니다.")}</div>
    </div>
    <article className="legacy-coin-analysis-result">
      <div className="legacy-section-title compact">{t("코인 분석 결과")}</div>
      <div className="legacy-coin-analysis-body">
        {coinAnalysis && <div className="legacy-coin-analysis-card live" key={String(coinAnalysis.symbol)}><b>{String(coinAnalysis.symbol)}{t(" 코인 분석")}</b><span>{t("현재가 ")}{numberText(coinAnalysis.current_price, 4)}<br />{t("신호 ")}<strong className={String(coinAnalysis.signal).toUpperCase() === "LONG" ? "positive" : String(coinAnalysis.signal).toUpperCase() === "SHORT" ? "negative" : "warning"}>{String(coinAnalysis.signal ?? "HOLD")}</strong>{t(" · 신뢰도 ")}{numberText(Number(coinAnalysis.confidence ?? 0) * 100, 1)}%</span><span>{t("트렌드 ")}{String(coinAnalysis.trend ?? "N/A")}{t(" · 변동성 ")}{numberText(coinAnalysis.volatility, 2)}%{coinAnalysis.reasoning ? <><br />{String(coinAnalysis.reasoning).slice(0, 220)}</> : null}</span></div>}
        {!coinAnalysis && analyzedRows.map((row, index) => <div className="legacy-coin-analysis-card" key={String(row.id ?? `${row.symbol}-${index}`)}><b>{String(row.symbol ?? row.coin ?? "—")}</b><span>{t("AI 종합점수 ")}{numberText(row.overall_score)}</span><span>{t("기술 ")}{numberText(row.technical_score)}{t(" · 트렌드 ")}{numberText(row.trend_score)}{t(" · 리스크 ")}{numberText(row.risk_score)}</span></div>)}
        {appliedQuery && !coinAnalysis && !analyzedRows.length && !loading && !error && <div className="empty-state">{t("실시간 분석 결과를 가져올 수 없습니다. 거래소 연결 상태와 심볼을 확인하세요.")}</div>}
      </div>
    </article>
    <div className="legacy-coin-table-header">
        <span>{t("코인")}</span><span>{t("AI종합점수")}</span><span>{t("변동성점수")}</span><span>{t("거래량점수")}</span><span>{t("기술점수")}</span><span>{t("트렌드점수")}</span><span>{t("리스크점수")}</span>
    </div>
    <article className="legacy-coin-result"><div className="legacy-coin-table-body">
        {rows.map((row, index) => {
          ...
          return <div className="legacy-coin-row" key={String(row.id ?? `${row.symbol}-${index}`)}>
            ...
            <span>{metricText(row.volatility_score)}</span><span>{metricText(row.volume_score)}</span><span title={t("기술점수는 선정 시점의 15분·1시간 RSI 근거가 있을 때만 표시합니다. 종합점수와 별도이며 누락값을 추정하지 않습니다.")}>{row.technical_score == null ? t("미산출") : metricText(row.technical_score)}</span><span>{metricText(row.trend_score)}</span><span>{metricText(row.risk_score)}</span>
          </div>;
        })}
        {!loading && !rows.length && <div className="empty-state legacy-selection-empty"><span>{t("선정된 코인이 없습니다.")}</span><span>{t("• 아래 '코인 선정 실행' 버튼을 누르거나")}</span><span>{t("• 자동매매를 시작하면 자동으로 코인이 선정됩니다.")}</span></div>}
        {loading && <div className="empty-state">{t("코인 평가 결과를 불러오는 중입니다.")}</div>}
      </div></article>
    <div className="legacy-coin-actions"><button className="legacy-select-button" type="button" onClick={runCoinSelection} disabled={loading}>{t("코인 선정 실행")}</button><button className="legacy-refresh-button" type="button" onClick={refresh} disabled={loading}>{t("코인 정보 새로고침")}</button></div>
  </section>;
}
  ...
  return <section className="legacy-exchange-workspace">
    <div className="legacy-exchange-left">
      ...
      <article className="legacy-exchange-card exchange-statistics">
        <header className="source-card-header"><div className="source-card-title"><span className="source-card-mark statistics" aria-hidden="true">↗</span><div><small>{liveMode ? "VERIFIED PERFORMANCE" : paperMode ? "PAPER PERFORMANCE" : "LEARNING STATUS"}</small><h3>{liveMode ? t("실거래 통계 · 오늘") : paperMode ? t("가상 거래 통계 · 전체") : "학습 실행 상태"}</h3></div></div><div className="source-stat-context"><strong>{sourceLabel}</strong>{paperMode ? <><span>{t("전체 가상 청산 ")}<b>{paperClosedCount}{t("건")}</b></span>{paperStatistics?.window_limited && <span>{t("기관별 최대 ")}{Number(paperStatistics.recent_window_limit)}{t("건 · 전체 원장 초과")}</span>}</> : liveMode ? service === "stock" ? <span>{t("오늘 청산 기준")}</span> : <><span>{t("오늘 거래소 확인 체결 ")}<b>{executionCountLabel(sourceExecutionCount, sourceStatistics?.execution_history_status)}</b></span><span>{t("오늘 NoahAI 청산 ")}<b>{sourceClosedCount}{t("건")}</b></span></> : <span>{t("신규 주문·가상 체결 없음")}</span>}</div></header>
        {paperMode ? <><div className="legacy-trade-kpis"><div><span>{t("가상 청산")}</span><b>{paperClosedCount}{t("건")}</b></div><div><span>{t("가상 승률")}</span><b className="positive">{paperWinRate == null ? t("미확정") : `${numberText(paperWinRate, 2)}%`}</b></div><div><span>{t("가상 순손익 (")}{sourceCurrency})</span><b className={paperPnl < 0 ? "negative" : "positive"}>{paperPnl >= 0 ? "+" : ""}{numberText(paperPnl, service === "stock" ? 0 : 4)}</b></div><div><span>{t("가상 수수료 (")}{sourceCurrency})</span><b>{numberText(paperFees, service === "stock" ? 0 : 4)}</b></div></div>{paperUnverifiedCount > 0 && <p className="spot-holding-scope">{t("구버전 손익 미확정 ")}{paperUnverifiedCount}{t("건은 승률·손익에서 제외했습니다.")}</p>}</> : liveMode ? service === "stock" ? <div className="legacy-trade-kpis"><div><span>{t("총 거래")}</span><b>{Number(stockStatistics?.total_trades ?? 0)}{t("건")}</b></div><div><span>{t("오늘 체결")}</span><b>{Number(stockStatistics?.today_count ?? 0)}{t("건")}</b></div><div><span>{t("실현손익")}</span><b className={Number(stockStatistics?.realized_pnl ?? 0) < 0 ? "negative" : "positive"}>{Number(stockStatistics?.realized_pnl ?? 0) > 0 ? "+" : ""}{numberText(stockStatistics?.realized_pnl ?? 0, 0)}{t("원")}</b></div><div><span>{t("미체결")}</span><b>{Number(stockStatistics?.open_orders_count ?? openOrders.length)}{t("건")}</b></div></div> : <div className="legacy-trade-kpis"><div><span>{localized("NoahAI 완료 거래", "NoahAI completed trades")}</span><b>{sourceClosedCount}{t("건")}</b></div><div><span>{t("청산 승률")}</span><b className="positive">{numberText(sourceWinRate, 2)}%</b></div><div><span>{t("청산 순손익")}</span><b className={sourcePnl < 0 ? "negative" : "positive"}>{sourcePnl >= 0 ? "+" : ""}{numberText(sourcePnl, 4)}</b></div><div><span>{t("청산 수수료")}</span><b>{numberText(accumulatedFees, 4)}</b></div></div> : <div className="empty-state">{running ? "LEARNING은 분석·전략·가드레일 판단만 기록 중이며 실제 주문과 가상 체결 통계는 만들지 않습니다." : "LEARNING은 자동으로 시작되지 않습니다. 위의 분석·학습 시작 버튼을 누르면 시세·코인 선정·전략·가드레일 판단만 기록하고 신규 주문은 차단합니다."}</div>}
        {liveMode && <ExecutionEvidenceNote status={sourceStatistics?.execution_history_status} />}
      </article>
      ...
      <article className={`legacy-exchange-card exchange-statistics paper-history-card ${paperHistoryOpen ? "expanded" : "collapsed"}`}><header className="source-card-header"><div className="source-card-title"><span className="source-card-mark statistics" aria-hidden="true">P</span><div><small>PAPER HISTORY</small><h3>{paperMode ? "현재 PAPER 거래내역" : t("과거 PAPER 검증 이력")}</h3></div></div><div className="paper-history-header-actions"><div className="source-stat-context"><strong>{t("PAPER 기록")}</strong><span>{liveMode ? "LIVE와 분리" : "청산 완료"} · {Number(workspace?.paper_trades?.length ?? 0)}{t("건 · 활성 수와 무관")}</span></div><button className="paper-history-toggle" type="button" aria-expanded={paperHistoryOpen} onClick={() => setPaperHistoryOpen((value) => !value)}>{paperHistoryOpen ? t("접기") : t("펼치기")}</button></div></header>{paperHistoryOpen && <>{liveMode && <p className="spot-holding-scope">{t("과거 PAPER 가상 청산 기록입니다. 현재 LIVE 거래나 가상체결 발생을 뜻하지 않습니다.")}</p>}<div className="legacy-paper-history">{[...(workspace?.paper_trades ?? [])].reverse().slice(0, 10).map((row, index) => { const verified = String(row.calculation_status ?? "valid") === "valid"; return <div className="legacy-position-row" key={String(row.event_id ?? index)}><div><b>{String(row.symbol ?? "—")}</b><span>{String(row.exchange ?? source).toUpperCase()}</span></div><div><span>{row.strategy_key && row.version_id ? t("전략 스튜디오") : "기본 전략"}</span><small>{String(row.closed_at ?? "")}</small></div><div><span>{t("가상 실현손익")}</span>{verified ? <strong className={Number(row.net_pnl ?? 0) < 0 ? "negative" : "positive"}>{Number(row.net_pnl ?? 0) > 0 ? "+" : ""}{numberText(row.net_pnl, service === "stock" ? 0 : 4)} {String(row.quote_currency ?? sourceCurrency)}</strong> : <strong>{t("과거 손익 미확정")}</strong>}</div></div>; })}{!workspace?.paper_trades?.length && <div className="empty-state">{t("이 거래소에서 완료된 PAPER 가상 청산이 아직 없습니다.")}</div>}</div></>}</article>
      </SourceTradeHistory>
    </div>
