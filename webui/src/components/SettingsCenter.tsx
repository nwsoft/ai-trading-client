  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <section className="settings-center" role="dialog" aria-modal="true" aria-labelledby="settings-center-title">
        ...
        <div className="settings-live-warning"><span>{t("v3.9.1.45 · LIVE는 별도 권한입니다 · PAPER OFF + 주문 대상/증권 LIVE + API 준비 + 가드레일")}</span><button type="button" onClick={() => onAskAssistant("실거래 전 필수 준비, PAPER와 LIVE의 차이, API 권한과 주문 가드레일을 현재 설정 기준으로 설명해줘.", activeSection)}>{t("실거래 필수 안내")}</button></div>
        ...
        <div className="settings-body">
          <main className="settings-fields">
            ...
            {activeSection === "general" && snapshot && <section className="settings-contract-status">
              ...
              <p>{t("앱 v3.9.1.45 · 설정 스키마 ")}{snapshot.schema_version}{t(" · 현재 모드 ")}{diagnostics?.trading?.paper_trading ? "PAPER" : "LIVE 확인 필요"}{t(" · 계정 설정 ")}{snapshot.account_scope}</p>
            ...
            {activeSection === "update" && <section className="settings-update-panel">
              ...
              <UpdateCenter client={client} accountScope={snapshot?.account_scope ?? ""} detailed currentVersion={platform?.release_version ? `v${platform.release_version}` : "v3.9.1.45"} />
