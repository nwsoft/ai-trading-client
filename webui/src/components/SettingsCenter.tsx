...
const QUICK_START_SOURCES: Record<QuickStartAsset, Array<{ id: string; label: string; detail: string }>> = {
  crypto: [
    ...
    { id: "coinone", label: "코인원", detail: "KRW 현물 · PAPER/LIVE" },
...
const SETTING_OPTION_LABELS: Record<string, string> = {
  ...
  coinone: "코인원 (Coinone) · KRW 현물 · PAPER/LIVE",
  bybit: "바이비트 (Bybit) · 선물",
  okx: "OKX · 선물",
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <section className="settings-center" role="dialog" aria-modal="true" aria-labelledby="settings-center-title">
        ...
        <div className="settings-live-warning"><span>{t("v3.9.1.46 · LIVE는 별도 권한입니다 · PAPER OFF + 주문 대상/증권 LIVE + API 준비 + 가드레일")}</span><button type="button" onClick={() => onAskAssistant("실거래 전 필수 준비, PAPER와 LIVE의 차이, API 권한과 주문 가드레일을 현재 설정 기준으로 설명해줘.", activeSection)}>{t("실거래 필수 안내")}</button></div>
        ...
        <div className="settings-body">
          <main className="settings-fields">
            ...
            {activeSection === "general" && snapshot && <section className="settings-contract-status">
              ...
              <p>{t("앱 v3.9.1.46 · 설정 스키마 ")}{snapshot.schema_version}{t(" · 현재 모드 ")}{diagnostics?.trading?.paper_trading ? "PAPER" : "LIVE 확인 필요"}{t(" · 계정 설정 ")}{snapshot.account_scope}</p>
            ...
            {activeSection === "notifications" && <section className="notification-settings-panel">
              ...
              <p>{t("시장국면 알림은 실행 중인 기관의 변화가 확정될 때 전송합니다. 알림 ON은 분석 시작이 아닙니다. PAPER·LIVE 외에 학습·관찰 수신과 기관별 스위치도 확인하고 변경 내용을 저장하세요. 국면 유지·확정 대기·필터 차단·전송 실패는 해당 기관 로그에서 구분할 수 있습니다.")}</p>
            ...
            {activeSection === "update" && <section className="settings-update-panel">
              ...
              <UpdateCenter client={client} accountScope={snapshot?.account_scope ?? ""} detailed currentVersion={platform?.release_version ? `v${platform.release_version}` : "v3.9.1.46"} />
