    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <section className="settings-center" role="dialog" aria-modal="true" aria-labelledby="settings-center-title">
        <header className="settings-window-header">
          <div><h2 id="settings-center-title">⚙ 설정</h2><p>처음이면 빠른 시작에서 PAPER와 한 기관만 고르세요. 기존 설정은 그대로 두고 싶다면 탭별 설명을 따라 필요한 항목만 변경할 수 있습니다.</p></div>
        ...
        <div className="settings-live-warning"><span>v3.9.1.38 · LIVE는 별도 권한입니다 · PAPER OFF + 주문 대상/증권 LIVE + API 준비 + 가드레일</span><button type="button" onClick={() => onAskAssistant("실거래 전 필수 준비, PAPER와 LIVE의 차이, API 권한과 주문 가드레일을 현재 설정 기준으로 설명해줘.", activeSection)}>실거래 필수 안내</button></div>
        ...
        <nav className="settings-section-tabs" aria-label="설정 영역">
          ...
          }} type="button">{section.label}</button>)}
        </nav>
        <div className="settings-body">
          <main className="settings-fields">
            <div className="settings-tab-commandbar">
              <div><strong>{activeSection === "general" ? "1. 운용 모드 · 변경 후 저장을 눌러야 적용됩니다." : activeSectionInfo.label}</strong><p>{activeSectionInfo.description}</p></div>
              <div className="settings-command-actions">
                <button className="secondary-button" type="button" onClick={() => setShowTechnicalFields((value) => !value)}>{showTechnicalFields ? "기본 설정만 보기" : "고급 설정 보기"}</button>
                <button className="secondary-button" type="button" disabled={busy} onClick={() => stageDefaults("section")}>이 탭 기본값 불러오기</button>
                <button className="primary-button" type="button" onClick={() => setGuideOpen((value) => !value)}>✦ AI에게 묻기 · 설정 도우미</button>
              </div>
              <div className="settings-command-save-row">
                <span>{sectionChangedCount ? `${sectionChangedCount}개 변경 대기` : "이 탭의 변경 내용 없음"}</span>
                <button className="settings-save-button" type="button" disabled={!sectionChangedCount || busy || Boolean(sectionInvalid.length)} onClick={() => void save("section")}>▣ 현재 설정 저장</button>
              </div>
            </div>
            {guideOpen && <section className="settings-guide-panel" aria-label={`${activeSectionInfo.label} AI 설정 도우미`}>
              <header><div><strong>{activeSectionInfo.label} · AI 설정 도우미</strong><p>질문 답변과 안전 추천안을 제공합니다. 비밀값·API 키·LIVE 권한은 자동 입력하거나 저장하지 않습니다.</p></div><button type="button" onClick={() => setGuideOpen(false)}>도우미 닫기</button></header>
              <div className="settings-guide-controls">
                <label><span>사용 수준</span><select value={guideProfile} onChange={(event) => { setGuideProfile(event.target.value as SettingsGuideProfile); setGuideProposal({}); }}><option value="beginner">처음 사용 · 안전 우선</option><option value="balanced">일반 사용 · 균형</option><option value="advanced">숙련 사용자 · 상세 설명</option></select></label>
                <label className="settings-guide-question"><span>무엇이 궁금한가요?</span><input value={guideQuestion} onChange={(event) => setGuideQuestion(event.target.value)} placeholder="예: 이 설정을 PAPER에서 안전하게 시작하려면 무엇을 확인해야 하나요?" onKeyDown={(event) => { if (event.key === "Enter") void askSettingsGuide(); }} /></label>
                <button type="button" disabled={guideBusy} onClick={askSettingsGuide}>{guideBusy ? "답변 준비 중…" : "AI 답변 받기"}</button>
              </div>
              {guideAnswer && <div className="settings-guide-answer"><strong>설정 안내 · {activeSectionInfo.label} 문맥 고정</strong><pre>{guideAnswer}</pre><button type="button" onClick={() => onAskAssistant(`${guideQuestion || activeSectionInfo.label}에 대해 방금 본 설정 안내를 이어서 질문하고 싶습니다. 현재 저장값의 목적과 영향만 설명하고 설정을 직접 저장하거나 거래를 실행하지 마.`, activeSection)}>대화형 AI에서 이어서 묻기</button></div>}
              <div className="settings-guide-proposal-actions"><button type="button" onClick={createSettingsGuideProposal}>안전 추천안 만들기</button><span>추천안은 현재 탭의 변경 대기에만 반영되며 저장은 별도 승인입니다.</span></div>
              {Object.keys(guideProposal).length > 0 && <div className="settings-guide-proposal"><strong>변경 미리보기</strong>{Object.entries(guideProposal).map(([path, value]) => { const field = snapshot?.fields.find((item) => item.path === path); return <div key={path}><span>{field?.label || path}</span><code>{JSON.stringify(draft[path])} → {JSON.stringify(value)}</code><small>{field?.risk === "critical" ? "중요 설정 · 저장 시 추가 확인" : field?.help}</small></div>; })}<button type="button" onClick={stageSettingsGuideProposal}>변경 대기에 반영</button></div>}
            </section>}
            {activeSection === "general" && quickStartOpen && <section className="settings-quick-start" aria-label="처음 사용 빠른 시작">
              <header><div><strong>처음 사용 · 빠른 시작</strong><p>외부 AI 호출 없이 한 질문씩 안전한 PAPER 시작값을 고릅니다. 새 운용 모드가 아니며 Level·전략·기존 고급 설정을 없애지 않습니다.</p></div><button type="button" onClick={() => setQuickStartOpen(false)}>닫기</button></header>
              ...
              {quickStartStep === 1 && <div className="settings-quick-start-question">
                <strong>어떤 자산을 먼저 가상으로 연습할까요?</strong>
                <p>나중에 다른 자산을 추가할 수 있습니다. 여기서는 첫 화면을 단순하게 만들기 위해 하나만 고릅니다.</p>
                <div><button type="button" onClick={() => { setQuickStartAsset("crypto"); setQuickStartSource(""); setQuickStartStep(2); }}><b>코인</b><span>현물 또는 선물 거래소</span></button><button type="button" onClick={() => { setQuickStartAsset("stock"); setQuickStartSource(""); setQuickStartStep(2); }}><b>주식·ETF</b><span>국내 증권사</span></button></div>
              </div>}
              {quickStartStep === 2 && quickStartAsset && <div className="settings-quick-start-question">
                <strong>먼저 확인할 {quickStartAsset === "crypto" ? "거래소" : "증권사"} 한 곳을 고르세요.</strong>
                <p>이 선택은 관찰·분석 범위입니다. 실제 주문 권한은 열지 않습니다.</p>
                ...
                <button className="back" type="button" onClick={() => setQuickStartStep(1)}>이전 질문</button>
              </div>}
              {quickStartStep === 3 && quickStartAsset && quickStartSource && <div className="settings-quick-start-review">
                <strong>이렇게 시작합니다</strong>
                <div><span><b>운용</b>PAPER · 실주문 없음</span><span><b>대상</b>{QUICK_START_SOURCES[quickStartAsset].find((item) => item.id === quickStartSource)?.label}</span><span><b>투자금</b>NoahAI 자동 위험관리</span><span><b>보호</b>LIVE 대상 해제 · 증권 주문 가드 유지</span></div>
                <p>API 연결은 시세·계좌 조회에 필요할 수 있지만 외부 AI 키는 필수가 아닙니다. 이 단계는 비밀키를 입력하거나 거래를 시작하지 않습니다.</p>
                <footer><button type="button" onClick={() => setQuickStartStep(2)}>이전</button><button className="primary" type="button" onClick={stageQuickStart}>안전 시작안을 변경 대기에 넣기</button></footer>
              </div>}
              {quickStartStep === 4 && <div className="settings-quick-start-done">
                <strong>빠른 시작 선택을 확인했습니다.</strong>
                <p>아직 적용되지 않았습니다. 화면 아래의 <b>전체 설정 저장</b>을 누르면 중요 설정 확인창이 한 번 더 표시됩니다. 저장 후 거래소 API 탭에서 선택 기관의 연결을 점검하고, 대시보드에서 해당 서비스를 직접 시작하세요.</p>
                <div><button type="button" onClick={() => { setQuickStartOpen(false); setActiveSection("exchange_api"); setCredentialProvider(quickStartAsset === "stock" ? `stock:${quickStartSource === "koreaInvestment" ? "kis" : quickStartSource.toLowerCase()}` : quickStartSource); }}>다음: API 연결 확인</button><button type="button" onClick={() => onAskAssistant(`처음 사용 빠른 시작에서 ${quickStartAsset === "crypto" ? "코인" : "주식·ETF"} ${quickStartSource} PAPER를 선택했습니다. 지금 변경 대기와 저장 후 API 연결, 대시보드 시작 순서를 초보자에게 3단계로 설명해줘. 설정을 저장하거나 거래를 실행하지 마.`, "general")}>AI에게 다음 순서 묻기</button></div>
              </div>}
            </section>}
            {activeSection === "general" && snapshot && <section className="settings-contract-status">
              <strong>설정 정리 상태</strong>
              <p>앱 v3.9.1.38 · 설정 스키마 {snapshot.schema_version} · 현재 모드 {diagnostics?.trading?.paper_trading ? "PAPER" : "LIVE 확인 필요"} · 계정 설정 {snapshot.account_scope}</p>
            ...
            {activeSection === "general" && <section className="settings-autopilot-guide">
              <header><div><strong>기본 자율운행 · 시작 전에 3가지만 확인하세요</strong><p>전략 스튜디오 전략을 만들지 않아도 기본 자동매매가 작동합니다. 거래 이력이 0건이어도 20번을 기다리지 않고 조건이 맞는 첫 판단부터 PAPER 또는 허용된 LIVE 주문 후보를 처리합니다.</p></div><span>거래 데이터 0건부터 실행 가능</span></header>
              <div className="settings-autopilot-flow"><b>시작</b><i>→</i><span>코인 자동 선정</span><i>→</i><span>시장·기술 분석</span><i>→</i><span>AI 연결 시 보강</span><i>→</i><span>위험 검사</span><i>→</i><b>주문·기록</b></div>
              <div className="settings-autopilot-responsibilities"><div><strong>사용자가 한 번 확인</strong><p>① PAPER/LIVE ② 분석·주문 거래소 ③ API 연결과 주문 권한</p></div><div><strong>앱이 실행 중 자동 처리</strong><p>후보 선정, 신호 판단, 포지션 크기, 익절·손절, 위험 차단, 결과 기록</p></div></div>
              <footer><button type="button" onClick={onOpenManual}>매뉴얼에서 자세히 보기</button><button type="button" onClick={() => onAskAssistant("전략 스튜디오 전략을 사용하지 않는 기본 자율운행이 현재 내 PAPER/LIVE, 거래소 선택, API 연결 상태에서 어떻게 작동하는지 초보자 기준으로 설명해줘. 거래 이력 0건부터 시작되는 흐름과 사용자가 확인할 3가지만 먼저 알려줘.", activeSection)}>AI에게 현재 상태 설명받기</button></footer>
            </section>}
            {(activeSection === "exchange_api" || activeSection === "ai_engine" || activeSection === "alpha") && <div className="credential-box settings-credential-panel">
              <strong>연결 자격증명</strong>
              <p>저장된 비밀값은 보안상 화면에 다시 보내지 않습니다. 대신 Provider와 필드별 저장 여부를 표시합니다.</p>
              ...
              <small>입력값 보기는 현재 입력 중인 값만 보여 줍니다. 이미 저장된 비밀값은 다시 표시하지 않습니다.</small>
              {OFFICIAL_API_PORTALS[credentialProvider] && <a className="official-api-link" href={OFFICIAL_API_PORTALS[credentialProvider].url} target="_blank" rel="noreferrer">↗ {OFFICIAL_API_PORTALS[credentialProvider].label}에서 발급·권한 확인</a>}
            </div>}
            {activeSection === "ai_engine" && <section className="settings-ai-budget-panel" aria-label="AI 비용 관리">
              <header><div><strong>AI 비용 관리 · 외부 호출 한도</strong><p>회원·거래 제한이 아니라 API 비용 폭주와 반복 실행을 막는 사용자 설정입니다. 카드에는 현재 저장된 한도가 표시되며, 아래 입력값을 바꾼 경우 `현재 설정 저장`을 눌러야 반영됩니다.</p></div><button type="button" onClick={() => client.assistantStatus().then((next) => setAssistantStatus(next?.budget ?? next)).catch(() => setAssistantStatus(null))}>사용량 새로고침</button></header>
              <div className="settings-ai-budget-meters">
                <div className={assistantStatus?.daily_exhausted ? "exhausted" : ""}><span>오늘 외부 호출 · 현재 저장 한도</span><strong>{assistantStatus?.daily_used ?? "—"} / {assistantStatus?.daily_limit ?? 30}회</strong><small>설정 범위 1~1,000회</small></div>
                <div className={assistantStatus?.monthly_exhausted ? "exhausted" : ""}><span>이번 달 외부 호출 · 현재 저장 한도</span><strong>{assistantStatus?.monthly_used ?? "—"} / {assistantStatus?.monthly_limit ?? 500}회</strong><small>설정 범위 1~30,000회</small></div>
              </div>
              <div className="settings-ai-cost-estimate">
                <div><span>오늘 정가 기준 예상 비용</span><strong>{Number(assistantStatus?.priced_calls?.today ?? 0) > 0 ? `약 $${Number(assistantStatus?.estimated_cost_usd?.today ?? 0).toFixed(6)}` : "산정 근거 없음"}</strong></div>
                <div><span>이번 달 정가 기준 예상 비용</span><strong>{Number(assistantStatus?.priced_calls?.month ?? 0) > 0 ? `약 $${Number(assistantStatus?.estimated_cost_usd?.month ?? 0).toFixed(6)}` : "산정 근거 없음"}</strong></div>
                <small>AI 어시스턴트·전략 스튜디오·차트 분석·전사처럼 사용자가 직접 실행한 외부 AI 호출의 로컬 정가 추정치입니다. 무료 토큰·데이터 공유 혜택·캐시·프로모션·세금이 반영된 실제 청구액은 Provider 콘솔이 정본입니다. 자동매매 백그라운드 AI와 Provider 계정 전체 청구액은 포함하지 않습니다. 구버전·실패 요청·토큰 미제공·단가 미등록 모델은 0원으로 만들지 않고 비용 미산출로 분리합니다. 미완료 시도 오늘 {assistantStatus?.incomplete_attempts?.today ?? 0}회 · 비용 미산출 완료 오늘 {assistantStatus?.cost_unavailable_calls?.today ?? 0}회</small>
              </div>
              {assistantStatus?.usage_by_model && Object.keys(assistantStatus.usage_by_model).length > 0 && <div className="settings-ai-actual-usage">
                <strong>최근 로컬 원장 · 실제 응답 모델별 사용량</strong>
                {Object.entries(assistantStatus.usage_by_model).slice(-6).reverse().map(([modelName, raw]) => {
                  ...
                  return <span key={modelName}><b>{modelName}</b><small>{Number(row.calls || 0)}회 · {Number(row.total_tokens || 0).toLocaleString()} 토큰</small></span>;
                })}
                <small>성공 응답의 Provider 반환 모델을 기록합니다. Provider 콘솔의 Organization·Project·기간 필터와 함께 대조하세요.</small>
              </div>}
              <div className="settings-ai-budget-boundary"><div><strong>한도에 포함</strong><span>심층분석 · 차트/이미지 해석 · 영상/음성 전사 · 외부 AI 전략 보조</span></div><div><strong>한도 후에도 계속</strong><span>일반 안내 · 텍스트/Pine 로컬 규칙 분석 · 5분 따라 만들기 · 거래 엔진</span></div></div>
              <div className="settings-ai-budget-error"><strong>상태 429를 구분하세요</strong><span><code>interactive_ai_budget_exceeded</code>는 NoahAI가 Provider 요청 전에 막은 로컬 비용 보호입니다. 제공사 자체 429는 해당 Provider의 속도·쿼터·결제 제한이며 별도 문제입니다.</span><small>Provider 전송을 시도한 요청은 응답 실패 여부와 관계없이 반복 폭주 방지를 위해 1회로 집계합니다. 현재 일일 집계는 UTC 00:00에 갱신됩니다(한국시간 09:00). 월간 집계도 UTC 달력 기준입니다.</small></div>
            </section>}
            {activeSection === "ai_engine" && <section className="settings-ai-routing-panel" aria-label="나만의 AI 구성">
              <header><div><strong>나만의 AI 구성 · 작업별 모델</strong><p>한 모델을 모든 작업에 강제하지 않고 비용·속도·정밀도에 따라 역할별 Provider와 모델을 선택합니다. 아래 표는 현재 화면 선택값이며 저장 전에는 실행에 반영되지 않습니다.</p></div></header>
              <div className="settings-ai-route-grid">
                ...
                <div className="fixed"><span>영상·음성 전사</span><strong>OPENAI · {SETTING_OPTION_LABELS[String(draft["ai_custom_transcription.model"] || "")] ?? String(draft["ai_custom_transcription.model"] || "gpt-4o-mini-transcribe")}</strong><small>YouTube 공개 자막이 없을 때만 음성 전사 사용</small></div>
              </div>
              <div className="settings-ai-model-notice"><strong>DeepSeek 4.1 표기에 관하여</strong><span>공식 API 모델 ID는 현재 <code>deepseek-v4-flash</code>입니다. 이 별칭이 DeepSeek 서버에서 최신 Flash 버전으로 자동 갱신됩니다. 존재가 확인되지 않은 <code>deepseek-v4.1-flash</code> 문자열은 추가하지 않습니다.</span><small>실제 계정의 목록과 선택 모델 호출 가능 여부는 아래 실제 점검에서 따로 확인합니다. 비전 실험형은 deepseek-v4-flash-vision-exp로 별도 선택할 수 있습니다.</small></div>
              <div className="settings-ai-model-catalog">
                <header><div><strong>{visibleModelProvider.toUpperCase()} 모델 비교</strong><span>호환 목록 {visibleModelDetails.length}개 · 모델 {snapshot?.model_catalog_meta?.model_as_of ?? "기준일 확인 필요"} · 가격 {snapshot?.model_catalog_meta?.price_as_of ?? "기준일 확인 필요"}</span></div>{snapshot?.model_catalog_meta?.pricing_urls?.[visibleModelProvider] && <a href={snapshot.model_catalog_meta.pricing_urls[visibleModelProvider]} target="_blank" rel="noreferrer">공식 가격표 열기</a>}</header>
                <div className="settings-ai-model-grid">{visibleModelDetails.map((model) => {
                  ...
                  return <article key={`${visibleModelProvider}:${model.model}`}>
                    ...
                    <p><b>강점</b>{model.strength}</p><p><b>주의</b>{model.limitation}</p>
                ...
                <footer>이 목록은 NoahAI가 연결할 수 있는 정적 호환 목록입니다. 공급자의 모든 모델을 뜻하지 않으며, 실제 호출 가능 여부는 사용자 계정의 모델 목록과 기능 점검 결과가 우선합니다.</footer>
              </div>
              <div className="settings-openai-sharing-notice">
                <strong>OpenAI 공개 질문용 Project 분리 · 기본 OFF</strong>
                ...
                <span>OpenAI API 데이터는 기본적으로 모델 학습에 사용되지 않지만 사용자가 조직 설정에서 명시적으로 공유에 동의하면 모델 개선에 사용될 수 있습니다. 무료 제공량·대상 모델·기간은 계정별로 다르며 NoahAI가 확인하거나 변경하지 않습니다.</span>
                <span>이제 `OpenAI · 기본 보호 Project`와 `OpenAI · 공개 일반 질문용 Project` 키를 분리할 수 있습니다. 공개 경로는 위 설정을 켜고 AI 어시스턴트에서 사용자가 `공개 일반 질문`을 직접 선택한 1건에만 사용합니다. 전략·Pine·문서·차트·포지션·계좌·설정값·최근 대화는 항상 기본 보호 경로에 남습니다.</span>
                <span>현재 작업별 Provider가 DeepSeek·Kimi·Claude·Gemini이면 기존 배치는 그대로 유지됩니다. 공개 일반 질문을 명시한 경우에만 별도 OpenAI 모델과 키를 사용하며 거래 판단·전략 생성 경로를 자동 변경하지 않습니다.</span>
                <span>공유용 키가 없거나 설정이 꺼져 있으면 기본 보호 경로로 안전하게 돌아갑니다. NoahAI는 OpenAI 조직의 실제 데이터 공유 활성화 여부나 무료 제공량을 API 키만으로 확인하지 못하므로 조직 콘솔을 직접 확인해야 합니다. `store=false`는 요청 저장을 줄이는 별도 옵션이며 모델 학습 동의나 기본 악용 방지 보관 정책을 대신하지 않습니다.</span>
                <div><a href="https://developers.openai.com/api/docs/guides/your-data" target="_blank" rel="noreferrer">OpenAI 공식 데이터 정책</a><a href="https://platform.openai.com/settings/organization/data-controls/sharing" target="_blank" rel="noreferrer">내 조직 데이터 공유 설정 확인</a></div>
            ...
            {activeSection === "ai_engine" && <section className="settings-parity-panel">
              ...
              {providerCheck && <div className={providerCheck.model_callable ? "diagnostic-result ready" : "diagnostic-result error-text"}>
                ...
                {Array.isArray(providerCheck.models) && providerCheck.models.length > 0 && <small>Provider가 반환한 모델 예시: {providerCheck.models.slice(0, 8).join(", ")}</small>}
                {providerCheck.model_callable && <small>진단 사용량: 입력 {Number(providerCheck.probe_usage?.input_tokens || 0)} · 출력 {Number(providerCheck.probe_usage?.output_tokens || 0)} · 전체 {Number(providerCheck.probe_usage?.total_tokens || 0)} 토큰 · 외부 호출 1회 기록</small>}
            ...
            {activeSection === "notifications" && <section className="notification-settings-panel">
              ...
              <div className="notification-channel-grid">
                <article className={snapshot?.credential_status?.["notification:discord"] ? "ready" : ""}>
                  <div className="notification-channel-title"><div><span>가장 쉬움</span><h3>Discord 웹훅</h3></div><b>{snapshot?.credential_status?.["notification:discord"] ? "저장됨" : "연결 필요"}</b></div>
                  <ol><li>Discord 채널의 설정을 엽니다.</li><li>연동 → 웹후크 → 새 웹후크 → URL 복사를 누릅니다.</li><li>아래에 붙여 넣고 저장한 뒤 테스트합니다.</li></ol>
                  <label><span>웹훅 URL · 저장 후 다시 표시하지 않음</span><input type={showCredentialDraft ? "text" : "password"} autoComplete="off" placeholder={snapshot?.credential_field_status?.["notification:discord"]?.webhook_url ? "•••••••• 저장됨 · 변경할 때만 입력" : "https://discord.com/api/webhooks/…"} value={credentialDrafts["notification:discord"]?.webhook_url ?? ""} onChange={(event) => setCredentialDrafts((current) => ({ ...current, "notification:discord": { ...(current["notification:discord"] ?? {}), webhook_url: event.target.value } }))} /></label>
                  <div className="notification-actions"><button type="button" disabled={busy} onClick={() => void saveNotificationCredential("notification:discord")}>1. 연결 저장</button><button type="button" disabled={busy || !snapshot?.credential_status?.["notification:discord"]} onClick={() => void runNotificationTest("discord")}>2. 테스트 보내기</button><button className="danger-button" type="button" disabled={busy || !snapshot?.credential_status?.["notification:discord"]} onClick={() => void saveNotificationCredential("notification:discord", true)}>삭제</button></div>
                  <a href="https://support.discord.com/hc/ko/articles/228383668" target="_blank" rel="noreferrer">↗ Discord 웹훅 안내</a>
                </article>
                <article className={snapshot?.credential_status?.["notification:telegram"] ? "ready" : ""}>
                  <div className="notification-channel-title"><div><span>개인 알림 권장</span><h3>Telegram 봇</h3></div><b>{snapshot?.credential_status?.["notification:telegram"] ? "저장됨" : "연결 필요"}</b></div>
                  <ol><li>Telegram의 @BotFather에서 /newbot으로 봇 토큰을 발급합니다.</li><li>토큰을 저장하고 만든 봇 대화에서 Start 또는 /start를 보냅니다.</li><li>대화방 자동 찾기 → 받을 곳 선택 → 저장 → 테스트 순서로 진행합니다.</li></ol>
                  <label><span>봇 토큰 · 저장 후 다시 표시하지 않음</span><input type={showCredentialDraft ? "text" : "password"} autoComplete="off" placeholder={snapshot?.credential_field_status?.["notification:telegram"]?.bot_token ? "•••••••• 저장됨 · 변경할 때만 입력" : "123456789:AA…"} value={credentialDrafts["notification:telegram"]?.bot_token ?? ""} onChange={(event) => setCredentialDrafts((current) => ({ ...current, "notification:telegram": { ...(current["notification:telegram"] ?? {}), bot_token: event.target.value } }))} /></label>
                  <label><span>받을 대화방 ID · 자동 찾기 권장</span><input type="text" inputMode="numeric" autoComplete="off" placeholder={snapshot?.credential_field_status?.["notification:telegram"]?.chat_id ? "저장됨 · 바꿀 때 자동 찾기" : "대화방 자동 찾기로 선택"} value={credentialDrafts["notification:telegram"]?.chat_id ?? ""} onChange={(event) => setCredentialDrafts((current) => ({ ...current, "notification:telegram": { ...(current["notification:telegram"] ?? {}), chat_id: event.target.value } }))} /></label>
                  <div className="notification-actions telegram"><button type="button" disabled={busy} onClick={() => void saveNotificationCredential("notification:telegram")}>1. 토큰/대화방 저장</button><button type="button" disabled={busy || !(snapshot?.credential_field_status?.["notification:telegram"]?.bot_token || credentialDrafts["notification:telegram"]?.bot_token)} onClick={() => void discoverTelegram()}>2. 대화방 자동 찾기</button><button type="button" disabled={busy || !snapshot?.credential_status?.["notification:telegram"]} onClick={() => void runNotificationTest("telegram")}>3. 테스트 보내기</button><button className="danger-button" type="button" disabled={busy || !(snapshot?.credential_field_status?.["notification:telegram"]?.bot_token || snapshot?.credential_field_status?.["notification:telegram"]?.chat_id)} onClick={() => void saveNotificationCredential("notification:telegram", true)}>삭제</button></div>
                  {telegramChats.length > 0 && <div className="telegram-chat-list"><strong>받을 대화방 선택</strong>{telegramChats.map((chat) => <button type="button" key={chat.chat_id} onClick={() => setCredentialDrafts((current) => ({ ...current, "notification:telegram": { ...(current["notification:telegram"] ?? {}), chat_id: chat.chat_id } }))}><span>{chat.label}</span><small>{chat.type} · {chat.chat_id}</small></button>)}</div>}
                  <a href="https://core.telegram.org/bots/tutorial" target="_blank" rel="noreferrer">↗ Telegram 공식 봇 안내</a>
                </article>
              </div>
              <div className="notification-final-steps"><strong>마지막 확인</strong><span>① 연결 정보를 저장·테스트합니다. ② 아래에서 외부 알림과 사용할 채널을 ON으로 바꿉니다. ③ 받을 이벤트와 반복 방지 시간을 확인한 뒤 이 탭 저장을 누릅니다.</span><button type="button" onClick={() => setShowCredentialDraft((value) => !value)}>{showCredentialDraft ? "현재 입력값 숨기기" : "현재 입력값 보기"}</button></div>
              {notificationResult?.last_test?.ok && <div className="diagnostic-result ready"><b>최근 테스트 성공</b><span>{String(notificationResult.last_test.channel).toUpperCase()} 메시지 전송을 확인했습니다.</span></div>}
            </section>}
            {activeSection === "advanced" && <section className="settings-legacy-guide advanced-guide">
              <h3>5. 자동매매 안전 규칙 (선택)</h3>
              <p>생성형 AI가 수익 규칙을 스스로 만드는 기능이 아닙니다. 전략이 만든 거래 후보를 수익성·집중도·시장 상태·슬리피지 같은 규칙으로 다시 검사하는 안전 계층입니다.</p>
              <p>일반 사용자는 개별 숫자를 조절할 필요 없이 safe(권장)를 유지하고 PAPER에서 먼저 확인하세요. 세부 숫자는 ‘고급 설정 보기’를 눌렀을 때만 표시됩니다.</p>
              <strong>프리셋 빠른 전환</strong>
              <div className="settings-preset-row"><button type="button" onClick={() => applyAdvancedPreset("dev")}>전체 OFF (개발·진단)</button><button className="safe" type="button" onClick={() => applyAdvancedPreset("safe")}>safe (신규 권장)</button><button className="aggressive" type="button" onClick={() => applyAdvancedPreset("aggressive")}>aggressive (숙련자)</button></div>
              <small>전체 OFF는 개발·진단 전용이고 aggressive는 충분한 검증 자료가 있는 숙련자용입니다. 프리셋도 저장 버튼을 누르기 전에는 적용되지 않습니다.</small>
            </section>}
            {activeSection === "alpha" && <section className="settings-legacy-guide alpha-guide">
              <h3>6. AlphaArena 실험실</h3>
              <p>숙련자용 독립 실험 모드 · 기본 OFF. Binance USDT 선물 전용이며 표준 자동매매의 수익성·포트폴리오·전략 합의 계층과 공유되지 않습니다.</p>
              <div className="alpha-capital-summary"><span>초기 자금 기준</span><strong>{SETTING_OPTION_LABELS[String(draft["alpha_arena.initial_capital_benchmark"] ?? "10000")] ?? String(draft["alpha_arena.initial_capital_benchmark"] ?? "10000")}</strong><small>LLM 비교·판단용 벤치마크이며 실제 주문 잔액이나 계좌 손실 한도가 아닙니다.</small></div>
              <strong>현재 고정 가드레일과 기본값</strong>
              <span>판단 주기 최소 30초 · 레버리지 10~20배 제한 · 진입마다 TP/SL 필수 · 심볼별 쿨다운 · 최대 동시 포지션 · 틱당 위험 상한</span>
              <div className="alpha-warning-card"><strong>주의사항</strong><ul><li>기본 OFF인 독립 실험 모드이며 표준 자동매매와 동시에 켜기 전에 별도 검증이 필요합니다.</li><li>기존 TP/SL뿐 아니라 AlphaArena 자체 TP/SL·주문 가드도 모두 통과해야 합니다.</li><li>LLM이 TP/SL을 지정하지 않거나 위험·쿨다운·포지션 한도를 넘으면 주문 후보가 차단됩니다.</li><li>Windows 설치본과 실계정 장시간 E2E 전에는 LIVE 완료로 판정하지 않습니다.</li></ul></div>
            </section>}
            {activeSection === "exchange_api" && <section className="settings-parity-panel">
              <header><div><strong>거래소·증권 계정 연결 점검</strong><p>선택한 대상의 실제 계정 snapshot을 조회합니다. 주문은 제출하지 않습니다.</p></div><button className="primary-button" type="button" disabled={busy} onClick={runAccountConnectionCheck}>저장된 키로 실제 연결 점검</button></header>
              <div className="settings-assistant-tools"><button type="button" onClick={() => onAskAssistant(`처음 연결하는 사용자입니다. ${credentialProvider.toUpperCase()} API 발급, 읽기·주문 권한 분리, IP 허용목록, PAPER/LIVE 확인, 연결 점검을 3단계로 안내해줘. 출금 권한은 금지하고 값을 자동 저장하지 마.`, activeSection)}>초보자 연결 3단계</button><button type="button" onClick={() => onAskAssistant(`현재 ${credentialProvider.toUpperCase()} 등록 상태를 기준으로 연결 실패 원인과 최소 권한을 점검해줘. 비밀값은 표시하지 말고 주문도 실행하지 마.`, activeSection)}>거래소·증권 설정 도우미</button></div>
              {supportSummary && <div className="support-summary"><pre>{supportSummary}</pre><div><button type="button" onClick={() => navigator.clipboard?.writeText(supportSummary)}>지원 요약 복사</button><button type="button" onClick={() => downloadText("NoahAI-연결점검.txt", supportSummary)}>지원 요약 파일 저장</button></div></div>}
            </section>}
            {(activeSection === "advanced" || activeSection === "alpha") && <section className="settings-scope-warning">
              ...
              <p>표시값은 계정 설정 정본이며 저장 시 diff/revision 검증을 거칩니다. 중요 설정은 즉시 주문을 만들지 않지만, 실행 중 워커의 다음 평가부터 영향을 줄 수 있습니다.</p>
            ...
            {activeSection === "advanced" && String(draft["position_sizing_policy.mode"] ?? "") === "legacy_venue" && <section className="settings-parity-panel">
              <header><div><strong>기존 투자금 계산을 그대로 보존 중입니다</strong><p>업데이트가 기존 LIVE 주문금액을 자동으로 키우지 않도록 거래소별 이전 계산을 유지했습니다. 오류가 아니며, 선택 전까지 기존 방식으로 동작합니다.</p></div><button type="button" onClick={() => updateDraftField("position_sizing_policy.mode", "account_risk")}>자동 위험관리 불러오기</button></header>
              <small>버튼은 설정 초안만 변경합니다. 계좌 평가금·손절거리 기반의 NoahAI 자동 위험관리를 사용하려면 아래 값을 검토한 뒤 저장해야 하며, 저장만으로 주문이 즉시 생성되지는 않습니다.</small>
            </section>}
            {activeSection === "system" && <section className="settings-parity-panel">
              <header><div><strong>AI·런타임 준비 상태</strong><p>네트워크 호출 없이 로컬 설정과 현재 런타임 연결을 진단합니다.</p></div><button type="button" disabled={busy} onClick={refreshDiagnostics}>상태 새로고침</button></header>
              <div className="system-status-grid"><div><span>AI 자동거래</span><strong>{diagnostics?.trading?.paper_trading ? "PAPER" : diagnostics?.trading?.live_ready ? "LIVE 준비" : "정지·차단"}</strong></div><div><span>백엔드 AI 신호</span><strong>{diagnostics?.ai?.configured ? "키 등록됨" : "AI 키 필요"}</strong></div><div><span>실시간 최적화</span><strong>{draft.dynamic_thresholds_enabled ? "자동 보정" : "수동·꺼짐"}</strong></div><div><span>리스크 관리</span><strong>{draft["advanced_trading_layers.profitability_validation.enabled"] ? "가드 활성" : "설정 확인"}</strong></div></div>
              <div className="system-optimization-guide"><h3>코인 시장상태 자동 보정 (규칙 기반)</h3><p>코인 변동성을 저변동·보통·고변동으로 분류하고 RSI·모멘텀 진입 기준을 정해진 범위 안에서 바꿉니다. 생성형 AI가 임의로 학습하거나 전략을 다시 쓰는 기능이 아니며 주식 매매에는 적용되지 않습니다.</p><button type="button" onClick={stageDynamicThresholdDefaults}>코인 자동 보정 권장값 불러오기</button><small>일반 사용자는 자동 모드를 유지하면 됩니다. 세부 기준은 고급 설정 보기에서만 표시되며, 저장 버튼을 눌러야 적용됩니다.</small></div>
              <div className="system-warning-card"><strong>사용자가 직접 확인할 항목</strong><p>PAPER/LIVE, 분석·주문 거래소, API 주문 권한은 사용자가 선택해야 합니다. 신호 기준·슬리피지·쿨다운 같은 기술값은 권장 프리셋을 유지하고 PAPER에서 먼저 검증하세요.</p></div>
            </section>}
            {activeSection === "update" && <section className="settings-update-panel">
              ...
              <UpdateCenter client={client} accountScope={snapshot?.account_scope ?? ""} detailed currentVersion={platform?.release_version ? `v${platform.release_version}` : "v3.9.1.38"} />
            ...
            {busy && !snapshot ? <div className="empty-state">설정 정본을 불러오는 중입니다.</div> : activeFields.map((field) => (
              <label className={`setting-row risk-${field.risk} presentation-${field.presentation}`} key={field.path}>
                <div><strong>{field.label}</strong><p>{field.help}</p>{showTechnicalFields && <code>{field.path}</code>}</div>
                {field.kind === "boolean" ? (
                  ...
                  <select value={String(draft[field.path] ?? "")} onChange={(event) => updateDraftField(field.path, event.target.value)}>
                    {!String(draft[field.path] ?? "") && <option value="" disabled>모델을 선택하세요</option>}
                  ...
                  : <span className="protected-structure-value">고급 구조 · 현재값 유지</span>
                ) : field.kind === "text" ? (
            ...
            {snapshot?.coverage && <details className="settings-coverage"><summary>설정 정본 범위</summary><span>{snapshot.coverage.editable_top_level}/{snapshot.coverage.template_top_level}개 최상위 항목 편집 · 계정 {snapshot.account_scope}</span><p>비밀값과 런타임 전용 값은 보호 경로에서 관리됩니다.</p></details>}
          </main>
        </div>
        <footer>
          <div><button className="primary-button" type="button" disabled={!totalPendingCount || busy || Boolean(changeState.invalid.length)} onClick={() => void saveAllPending(false)}>{busy ? "저장 중…" : "▣ 전체 설정 저장"}</button><button className="danger-button" type="button" onClick={cancelSettings}>× 취소</button><button className="secondary-button" type="button" disabled={busy} onClick={() => stageDefaults("all")}>전체 기본값 불러오기</button><button className="secondary-button" type="button" disabled={busy} onClick={openBackups}>백업에서 복구</button><button className="secondary-button settings-close-action" type="button" disabled={busy} onClick={requestClose}>닫기</button></div>
        ...
        {backupOpen && <div className="settings-backup-layer" role="dialog" aria-modal="true" aria-label="설정 백업 복구">
          <section>
            <header><div><strong>설정 백업 복구</strong><p>최근 계정 설정 백업만 표시합니다. 파일 경로와 비밀값은 노출하지 않습니다.</p></div><button className="icon-button" type="button" onClick={() => setBackupOpen(false)} aria-label="백업 창 닫기">×</button></header>
            <div className="settings-backup-list">{backups.map((backup) => <button type="button" disabled={busy || Boolean(totalPendingCount)} onClick={() => restoreBackup(backup)} key={backup.name}><span>{new Date(backup.created_at).toLocaleString()}</span><small>{Math.max(1, Math.round(backup.size / 1024))}KB · 복구</small></button>)}{!backups.length && <div className="empty-state">복구할 설정 백업이 없습니다.</div>}</div>
            {totalPendingCount > 0 && <p className="error-text">현재 변경과 연결 정보 입력을 저장하거나 취소한 뒤 복구하세요.</p>}
          </section>
        </div>}
        {closePromptOpen && <div className="settings-backup-layer settings-close-prompt" role="dialog" aria-modal="true" aria-label="설정 닫기 확인">
          <section><header><div><strong>변경한 설정을 저장할까요?</strong><p>저장하지 않은 설정 변경이 있습니다. 일반 설정 {changedCount}개와 연결 정보 {pendingCredentialCount}건을 저장·폐기하거나 계속 편집하세요.</p></div></header><div className="settings-close-prompt-actions"><button className="primary-button" type="button" disabled={busy || Boolean(changeState.invalid.length)} onClick={() => void saveAndClose()}>저장 후 닫기</button><button className="danger-button" type="button" disabled={busy} onClick={() => { setClosePromptOpen(false); onClose(); }}>저장하지 않고 닫기</button><button className="secondary-button" type="button" onClick={() => setClosePromptOpen(false)}>계속 편집</button></div></section>
        </div>}
      </section>
    </div>
  );
