  ...
  return <section className="data-workspace legacy-alpha-arena">
    {showGuide && <div className="modal-backdrop alpha-guide-backdrop" role="presentation">
      <section className="alpha-guide-modal" role="dialog" aria-modal="true" aria-labelledby="alpha-guide-title">
        <h2 id="alpha-guide-title">{t("Alpha Arena 모드 안내")}</h2>
        <p>{t("AlphaArena는 기본 OFF인 숙련자용 Binance USDT 선물 독립 실험 모드입니다.")}</p>
        <p>{t("시장 데이터와 직전 결과 → DeepSeek V4 Flash 판단 → 구조화 파서 → AlphaArena 자체 게이트 → PAPER 결과 기록 순서로 동작합니다.")}</p>
        <strong>{t("기본 가드레일")}</strong>
        <ul><li>{t("60초 판단 주기(최소 30초), BTC/ETH/SOL/XRP/DOGE/BNB")}</li><li>{t("레버리지 10~20배 제한, 진입마다 TP와 SL 필수")}</li><li>{t("심볼 쿨다운 30초, 최대 동시 포지션 6개")}</li><li>{t("틱당 모델 제시 위험 합계 상한 1,500 USDT")}</li></ul>
        <p>{t("표준 자동매매의 수익성·포트폴리오·전략 합의 계층과 기존 TP/SL 보험·워치독을 공유하지 않습니다. 실주문은 차단됩니다. PAPER 결과는 AI 판단·조건 점검 기록이며 가상 체결·손익·여권 검증 결과가 아닙니다. 수익률 검증은 전략 스튜디오를 이용하세요. 화면 이동 후에도 실행은 계속되며 상단 상태에서 정지할 수 있습니다.")}</p>
        <button className="primary-button" type="button" onClick={() => setShowGuide(false)}>{t("확인")}</button>
      </section>
    </div>}
    <article className="panel arena-control-panel"><h2>{t("AlphaArena · Binance 전용")}</h2><div className="arena-engine-control"><span>{t("엔진: ")}{String(data?.engine || "확인 중")}</span><b>{t("PAPER 판단 실험 · 실주문 차단")}</b><button className={data?.running ? "danger-button" : "success-button"} type="button" disabled={busy || (!data?.running && (!data?.available || data?.paper_trading === false))} onClick={() => void control(data?.running ? "stop" : "start")}>{data?.running ? "■정지" : "▶시작"}</button></div></article>
    <p className="inline-notice">{t("가상 체결·청산·PnL은 계산하지 않습니다. 설정 변경 시 실험은 정지하며, 저장한 설정으로 다시 시작해야 합니다. 다른 거래소·증권사 탭은 이 실험의 대상 선택이 아닙니다.")}</p>
    {data?.paper_trading === false && <div className="inline-notice error-text">{t("현재 앱 설정은 LIVE입니다. AlphaArena는 PAPER에서만 시작할 수 있습니다.")}</div>}
    {error && <div className="inline-notice error-text">{error}</div>}
    {!data?.available && <div className="inline-notice">{t("설정 → AlphaArena에서 기능을 활성화하고 전용 DeepSeek 키를 저장·점검하세요. Binance 연결과 PAPER 설정도 필요합니다.")}</div>}
    <article className="panel arena-tab-panel"><nav className="intelligence-tabs" aria-label={t("AlphaArena 기록")}>{ARENA_TABS.map((name) => <button key={name} className={activeTab === name ? "active" : ""} type="button" onClick={() => setActiveTab(name)}>{name}</button>)}</nav><pre className="legacy-intelligence-output arena-output">{filtered.length ? formatRows(filtered) : emptyCopy}</pre></article>
    <details className="panel arena-guide"><summary>{t("Alpha Arena 모드 안내")}</summary><p>{t("AlphaArena는 기본 OFF인 숙련자용 Binance USDT 선물 독립 실험 모드입니다.")}</p><p>{t("시장 데이터와 직전 결과 → DeepSeek V4 Flash 판단 → 구조화 파서 → AlphaArena 자체 게이트 → PAPER 결과 기록 순서로 동작합니다.")}</p><p>{t("60초 판단 주기(최소 30초), 레버리지 10~20배 제한, 진입마다 TP·SL 필수, 심볼 쿨다운 30초, 최대 동시 포지션 6개, 틱당 위험 합계 상한 1,500 USDT가 기본 가드레일입니다.")}</p><p>{t("표준 자율주행의 수익성·포트폴리오·전략 합의 계층과 기존 TP/SL 보험·워치독을 공유하지 않습니다. 실주문은 차단됩니다. PAPER 결과는 AI 판단·조건 점검 기록이며 가상 체결·손익·여권 검증 결과가 아닙니다. 수익률 검증은 전략 스튜디오를 이용하세요. 화면 이동 후에도 실행은 계속되며 상단 상태에서 정지할 수 있습니다.")}</p></details>
  </section>;
}
