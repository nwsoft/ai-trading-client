...
export function LifeFinanceAdvanced({ client, featureId }: { client: GatewayClient; featureId: string }) {
  ...
  if (featureId.endsWith("products")) {
    return <section className="data-workspace">
      <article className="panel legacy-product-workspace">
        <div className="panel-heading"><div><span className="eyebrow">PRODUCT COMPARISON</span><h2>금융상품 비교</h2></div></div>
        <p className="workspace-copy">기존 클라이언트와 같은 대출·보험·예적금 조건을 각각 비교합니다. 정보 제공 전용이며 가입 신청이나 수익 보장을 하지 않습니다.</p>
        ...
        <section className="legacy-product-profile">
          ...
          <div className="form-grid legacy-product-profile-fields">
            <label>신용도<select value={profile.credit} onChange={(event) => setProfile({ ...profile, credit: event.target.value })}><option>좋음 (750~900)</option><option>보통 (650~750)</option><option>낮음 (~650)</option></select></label>
            <label>위험도<select value={profile.risk} onChange={(event) => setProfile({ ...profile, risk: event.target.value })}><option>회피형</option><option>보수형</option><option>공격형</option></select></label>
          </div>
        </section>
        <section className="legacy-product-conditions">
          ...
          <div className="legacy-product-condition-grid">
            <div className="form-grid advanced-form">
              <label>대출 금액(만원)<input type="number" min="1" value={loan.amount_wan} onChange={(event) => setLoan({ ...loan, amount_wan: event.target.value })} /></label>
              <label>대출 기간(개월)<input type="number" min="1" max="600" value={loan.term_months} onChange={(event) => setLoan({ ...loan, term_months: event.target.value })} /></label>
              <label>보험 월 예산(원)<input type="number" min="1" value={insurance.monthly_budget} onChange={(event) => setInsurance({ ...insurance, monthly_budget: event.target.value })} /></label>
              <label>보험 종류<select value={insurance.category} onChange={(event) => setInsurance({ ...insurance, category: event.target.value })}><option>전체</option><option>종합</option><option>건강</option><option>가족</option></select></label>
              <label>원금(만원)<input type="number" min="1" value={savings.principal_wan} onChange={(event) => setSavings({ ...savings, principal_wan: event.target.value })} /></label>
              <label>기간(개월)<input type="number" min="1" max="600" value={savings.term_months} onChange={(event) => setSavings({ ...savings, term_months: event.target.value })} /></label>
            </div>
            <div className="command-row legacy-product-actions"><button type="button" disabled={busy} onClick={() => runComparison("loan")}>대출 비교</button><button type="button" disabled={busy} onClick={() => runComparison("insurance")}>보험 비교</button><button type="button" disabled={busy} onClick={() => runComparison("savings")}>예적금 비교</button><button type="button" disabled={busy} onClick={() => runComparison()}>전체 비교</button></div>
          </div>
        </section>
        <small className="workspace-copy">신용도는 원본과 동일하게 대출·예적금 비교의 우대/가산 금리 안내에 반영됩니다. 위험도는 AI 상담 문맥에만 사용하며 상품 가입이나 신청은 실행하지 않습니다.</small>
        <div className="legacy-product-catalog"><span>{Object.entries(data?.counts ?? {}).map(([name, count]) => `${PRODUCT_LABELS[name as ProductKind] ?? name} ${Number(count)}개`).join(" · ") || "상품 정보 확인 중..."}</span><button className="secondary-button" type="button" onClick={load}>상품 정보 다시 읽기</button></div>
      </article>
  ...
  if (featureId.endsWith("security")) {
    ...
    return <section className="data-workspace"><article className="panel life-security-panel"><div className="panel-heading"><div><span className="eyebrow">SAFETY</span><h2>금융 보안 경고 센터</h2><p>보이스피싱·스미싱·이상거래·약탈적 대출 패턴을 자동 감지합니다. 모든 결과는 참고용이며 오탐이 있을 수 있습니다.</p></div></div><p className="workspace-copy">생활금융 거래·목표 정본에서 이상 패턴과 주의 항목을 확인합니다.</p>{error && <div className="inline-notice error-text">{error}</div>}<div className="alert-list">{alerts.map((alert, index) => <section className={`alert-card ${String(alert.level ?? "info")}`} key={`${alert.title}-${index}`}><strong>{String(alert.title)}</strong><p>{String(alert.message)}</p><small>{String(alert.action_hint)}</small></section>)}{!alerts.length && <div className="empty-state">현재 기록에서 생성된 생활금융 경고가 없습니다.</div>}</div></article></section>;
  }

  if (featureId.endsWith("tax")) {
    return <section className="data-workspace"><article className="panel"><div className="panel-heading"><div><span className="eyebrow">TAX REFERENCE</span><h2>세금 계산 & 절세 시뮬레이터</h2></div></div><p className="workspace-copy">결과는 참고용이며 공식 신고·세무 자문이 아닙니다. 신고 전에는 반드시 최신 법령과 국세청 자료를 확인하세요.</p>{error && <div className="inline-notice error-text">{error}</div>}<div className="form-grid advanced-form"><label>계산 유형<select value={tax.calculation} onChange={(event) => { setTax({ ...tax, calculation: event.target.value }); setTaxResult(null); }}><option value="year_end">연말정산</option><option value="financial_income">금융소득 종합과세</option><option value="investment">금융투자소득</option><option value="saving_accounts">절세 계좌 비교</option><option value="optimization">절세 최적화</option></select></label>{(TAX_INPUTS[tax.calculation] ?? []).map(([key, label, step]) => <label key={key}>{label}<input type="number" min={key.includes("profit") ? undefined : "0"} step={step ?? "1"} value={tax[key] ?? "0"} onChange={(event) => setTax({ ...tax, [key]: event.target.value })} /></label>)}{tax.calculation === "saving_accounts" && <label>ISA 유형<select value={tax.isa_type} onChange={(event) => setTax({ ...tax, isa_type: event.target.value })}><option value="general">일반형</option><option value="preferential">서민형·농어민형</option></select></label>}</div><div className="command-row"><button className="primary-button" type="button" disabled={busy} onClick={calculateTax}>{busy ? "계산 중…" : "세금 계산하기"}</button></div>{taxResult && <TaxResult response={taxResult} />}</article></section>;
  }
...
function ProductResult({ kind, response }: { kind: ProductKind; response: Record<string, any> }) {
  ...
  return <article className="panel legacy-product-result"><div className="panel-heading"><div><span className="eyebrow">COMPARISON RESULT</span><h2>{PRODUCT_LABELS[kind]}</h2></div><span className="count-badge">{rows.length}개</span></div><p className="workspace-copy">{String(result.summary ?? "비교 결과")}</p>{rows.length ? <div className="legacy-product-table"><div className="legacy-product-row header"><span>순위</span><span>상품명</span><span>제공사</span>{fields.map((field) => <span key={field}>{PRODUCT_FIELD_LABELS[field]}</span>)}</div>{rows.map((row, index) => <div className={`legacy-product-row${index === 0 ? " best" : ""}`} key={`${kind}-${String(row.name)}-${index}`}><span>{index + 1}</span><strong>{String(row.name ?? "상품")}</strong><span>{String(row.provider ?? "제공사 확인 필요")}</span>{fields.map((field) => <span key={field}>{displayValue(field, row[field])}</span>)}</div>)}</div> : <div className="empty-state">조건에 맞는 비교 결과가 없습니다.</div>}</article>;
}
...
function FinanceAnalysis({ data, error, chartOnly }: { data: Record<string, any> | null; error: string; chartOnly: boolean }) {
  ...
  if (chartOnly) return <section className="data-workspace"><article className="panel"><div className="panel-heading"><div><span className="eyebrow">12 MONTHS</span><h2>고급 차트</h2></div></div><p className="workspace-copy">저장된 기록의 월별 저축·지출 흐름과 12개월 예측을 표시합니다.</p>{error && <div className="inline-notice error-text">{error}</div>}<div className="trend-grid">{monthlySavings.map(([month, saving], index) => <div key={month}><span>{month}</span><strong>{money(saving)}원</strong><small>지출 {money(spendingMap[month])}원 · 예측 {money(projection[index])}원</small></div>)}</div>{!monthlySavings.length && <div className="empty-state">차트를 만들 생활금융 기록이 없습니다.</div>}</article></section>;
  return <section className="data-workspace"><article className="panel"><div className="panel-heading"><div><span className="eyebrow">PERSONAL FINANCE ANALYTICS</span><h2>재무 분석</h2></div></div><p className="workspace-copy">저장된 생활금융 기록만 계산합니다. 표본이 없으면 0원 기준이며 데모 수치를 만들지 않습니다.</p>{error && <div className="inline-notice error-text">{error}</div>}<div className="metric-grid"><div><span>이번 달 수입</span><strong>{money(basis.monthly_income)}원</strong></div><div><span>이번 달 지출</span><strong>{money(basis.monthly_expenses)}원</strong></div><div><span>12개월 예상 누적</span><strong>{money(projection.at(-1))}원</strong></div><div><span>경고</span><strong>{Number(data?.alerts?.length ?? 0)}건</strong></div></div></article><article className="panel"><div className="panel-heading"><div><span className="eyebrow">CATEGORY</span><h2>지출 카테고리 분석</h2></div></div><div className="card-grid">{Object.entries(data?.category_stats ?? {}).map(([category, row]) => <section className="insight-card" key={category}><header><strong>{category}</strong><span>{Number((row as any).count ?? 0)}건</span></header><dl><dt>합계</dt><dd>{money((row as any).total)}원</dd><dt>평균</dt><dd>{money((row as any).avg)}원</dd></dl></section>)}</div></article></section>;
}
