...
export function StrategyStudio({ client, service, source = "", onAskAssistant, onOpenSettings, assistantDraft, onAssistantDraftConsumed }: { client: GatewayClient; service: "blockchain" | "stock"; source?: string; onAskAssistant?: (question: string) => void; onOpenSettings?: () => void; assistantDraft?: StrategyAssistantDraft | null; onAssistantDraftConsumed?: () => void }) {
  ...
  return <section className="legacy-strategy-workspace">
    <header className="legacy-strategy-header">
      ...
      {showLevelGuide && <article className="legacy-level-guide" aria-label={t("전략 스튜디오 Level 1부터 Level 4 비교")}><header><div><h3>{t("Level 1~4 이용 범위와 예시")}</h3><p>{t("앱에 포함된 로컬 안내이므로 열어보는 데 AI API 비용이 들지 않습니다.")}</p></div><button type="button" onClick={onOpenSettings}>{t("사용 난이도 설정")}</button></header><div>{LEVEL_GUIDE.map((item) => <section className={featureViewLevel === item.level ? "current" : ""} key={item.level}><span>Level {item.level}</span><h4>{item.profile}</h4><p><b>{t("할 수 있는 것")}</b>{item.canDo}</p><p><b>{t("예시")}</b>{item.scenario}</p>{featureViewLevel === item.level && <small>{t("현재 설정에서 기본으로 열리는 단계")}</small>}</section>)}</div><footer>{t("TradingView·Pine 사용자는 보통 Level 2에서 실제 적용값을 먼저 확인한 뒤, 직접 규칙을 수정할 때만 Level 3으로 올리는 것이 안전합니다. Level 4는 하드 가드레일 해제 기능이 아닙니다.")}</footer></article>}
