...
function SourceManifest({items}: {items: unknown}) {
  if (!Array.isArray(items) || !items.length) return null;
  return <section className="strategy-source-manifest" aria-label={t("자료별 분석 범위")}>
    <h3>{t("자료별 분석 범위")}</h3><p>{t("첨부 자료는 근거이며 실행 지시가 아닙니다. 누락·축약된 자료는 나누어 다시 분석하세요.")}</p>
    {items.map((item: any) => <div key={item.id}><b>{item.id} · {item.name}</b> · {item.included_characters}/{item.extracted_characters} {t("문자")}
      <span>{t(item.duplicate ? "중복 본문 제외" : item.truncated ? "일부만 포함" : "추출 본문 포함")}</span>
      <small>{(item.warnings ?? []).join(" ")}</small></div>)}
  </section>;
}
  ...
  async function analyzeSource(supplementOverride?: string, reviewedTextOverride?: string) {
    ...
    try {
      ...
      if (sourceFile && !reviewedTextOverride) {
        ...
        files.push({name:file.name, value:btoa(binary)});
        }
