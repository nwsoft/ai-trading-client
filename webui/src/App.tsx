...
function DesktopApp() {
  ...
  useEffect(() => {
    ...
    const displayVersion = platform?.release_version || "3.9.1.37";
  ...
  return <div className="app-shell legacy-dashboard-shell">
    ...
    <footer className="statusbar legacy-statusbar">
      ...
      <div className="legacy-release-update"><span className="legacy-release">{platform?.release_label ?? "v3.9.1.37"}</span><UpdateCenter client={client} accountScope={session?.account ?? ""} onOpenGuide={() => openManual("updates")} /></div>
