...
SETTINGS_TABS: Dict[str, Dict[str, Any]] = {
    ...
    "알림·리포트": {
        "aliases": ("알림", "리포트 전송", "discord", "디스코드", "telegram", "텔레그램", "웹훅", "가드레일 메시지", "원격", "모바일", "내 pc"),
        "summary": "Discord·Telegram으로 선택한 알림을 보냅니다. 별도의 원격 관리 베타는 PC 이름 입력 → 상태 공유 동의 → daltrading 로그인 → 내 NoahAI에서 상태 확인 순서입니다. 모바일 신규 주문 제출 일시정지는 PC에서 별도 허용·저장해야 합니다. 원격 시작·재개·전량 청산은 제공하지 않으며 재개는 PC에서만 합니다. 이미 제출된 주문은 체결될 수 있고 보호·청산은 유지됩니다. 상태 공유 OFF는 정지된 진입을 재개하지 않습니다. 원격 상태는 60초 전송, 3분 미수신 시 오프라인이며 실시간 원격 데스크톱이 아닙니다. 서버 베타 활성화가 필요합니다.",
    },
    "고급 매매 계층": {
...
def _safe_current_summary(settings: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- 원격 공유·허용 상태는 계정별 별도 파일에 저장하므로 이 일반 설정 요약만 보고 연결 완료라고 답하지 않습니다. 설정 → 알림·리포트 → 원격 관리의 상태·마지막 전송과 모바일 수신 시각을 확인하세요. API 키·전략 원문·손익 상세는 전송하지 않습니다.",
