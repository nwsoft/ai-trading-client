...
def build_ai_custom_knowledge(
    ...
    *,
    ...
    if _topic(message, "ai멘토", "ai 멘토", "멘토 인터뷰", "처음 사용법", "처음사용법"):
        ...
        body = (
            "Fix 4는 Windows CTk 드롭다운의 native 메뉴를 클릭 중 프로세스 전체 최대 1개로 제한하고, 금융 인텔리전스를 거래소 탭 앞에 고정하며, 다중 거래소 범위와 실제 자동매매 워커 상태를 분리 표시합니다. "
            "창 제목이 Fix 2이면 Fix 3/4가 설치된 것이 아니며 설정 → 업데이트의 현재/배포 SHA가 같아야 최신 설치로 판정합니다. "
