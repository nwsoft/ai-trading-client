    ...
    *,
    ...
    if _topic(message, "ai멘토", "ai 멘토", "멘토 인터뷰", "처음 사용법", "처음사용법"):
        ...
        body = (
            ...
            "비용을 더 줄이려면 사용하지 않는 관찰·학습 거래소를 끄고 빈번 호출 역할을 저비용 모델로 두세요. Provider 청구액은 모델 단가·토큰·다른 앱/API 키 사용까지 포함할 수 있어 NoahAI 원장과 차이가 날 수 있습니다."
        )
    elif _topic(message, "전략 삭제", "프라이빗 삭제", "수정본", "전략 수정", "범위 변경", "시장상황 변경", "국면 변경"):
        body = (
            "AI 커스텀 → 내 프라이빗 전략 버전에서 대상 행의 ‘수정본 만들기’를 누르세요. 저장된 적용 범위·시장상황·국면 기준·우선순위·위험예산·전략 역할·고급 규칙을 편집기에 불러옵니다. "
            "값을 바꾼 뒤 ‘검토 및 전략 버전 저장’을 누르면 승인된 기존 버전을 덮어쓰지 않고 같은 전략의 다음 버전으로 저장됩니다. 새 버전은 XAI 확인·사용자 승인·자동검증·PAPER를 다시 거쳐야 합니다.\n"
    ...
    elif _topic(message, "버전", "업데이트", "3.9.0.10", "3.9.0.9", "3.9.0.8", "새 기능", "추가", "변경", "고도화"):
        body = (
            ...
            "v3.9.0.10 Windows 새 실행 파일과 실제 TradingView/webhook·장시간 PAPER/LIVE·서버 팀 공유는 별도 검증 대상이며, "
