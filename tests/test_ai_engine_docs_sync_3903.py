...
def test_release_metadata_and_installed_manual_are_scoped_separately():
    ...
    assert "v3.9.0.8 AI Custom Update Fix 2" in _read("README.md")
    ...
    assert "v3.9.0.8 Fix 3 최신 업데이트" in manual
    ...
    assert 'RELEASE_HIGHLIGHT = "AI 커스텀 P1~P3 · 자동업데이트 상태 머신·Windows UI 재사용 안정화"' in _read("config/app_version.py")
