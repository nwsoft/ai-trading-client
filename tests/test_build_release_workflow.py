...
def test_release_upload_requires_remote_digest_match():
    ...
    assert "ConfirmExternalGates is required" in script
    assert "Assert-ParityLedgerComplete" in script
    assert "Get-PatchPlanPath" in script
    assert "Assert-PatchPlanComplete" in script
    assert "V*_TEST_PLAN.md" in script
    assert "Multiple patch test plans found" in script
    assert "patch test plan is incomplete" in script
    assert "checked row without VERIFIED evidence" in script
    assert "AllowPendingExternalGates" in script
    ...
    assert "windows_stable_external_gates_pending" in script
...
def test_external_gate_override_publishes_prerelease_not_stable():
    script = (ROOT / "scripts" / "publish_web_ui_windows_release.ps1").read_text(encoding="utf-8")

    assert 'build_status = "windows_external_gates_pending"' in script
    assert 'channel = "prerelease"' in script
    assert '$manifest.publish_ready = $false' in script
    assert 'gh release create $tag --repo $Repo --target $releaseTarget --draft' in script
    assert '--draft=false --prerelease --latest=false' in script
...
def test_explicit_operator_override_can_publish_pending_candidate_to_stable_channel():
    script = (ROOT / "scripts" / "publish_web_ui_windows_release.ps1").read_text(encoding="utf-8")
    release_wrapper = (ROOT / "scripts" / "release_windows.ps1").read_text(encoding="utf-8")
    one_shot = (ROOT / "scripts" / "build_and_release_windows.ps1").read_text(encoding="utf-8")

    for source in (script, release_wrapper, one_shot):
        assert "PublishStableWithPendingExternalGates" in source
    assert "incomplete external gates cannot be published as stable" not in script
    assert "Explicit opt-in test publication" in release_wrapper
    assert "Normal user distribution remains visible" in release_wrapper
    assert 'build_status = "windows_stable_external_gates_pending"' in script
    assert '$publishAsPrerelease = $externalGatesPending -and -not $PublishStableWithPendingExternalGates' in script
...
def test_github_workflow_builds_candidate_before_explicit_release():
    ...
    assert "publish_release && !inputs.confirm_external_gates" in workflow
    assert "publish_web_ui_windows_release.ps1 -ConfirmExternalGates" in workflow
